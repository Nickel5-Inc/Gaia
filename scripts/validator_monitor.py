...
class ValidatorMonitor:
    def __init__(self):
        ...
        logger.info(
            "Initialized ValidatorMonitor with freeze threshold: %d seconds",
    ...
    def check_log_activity(self):
        ...
        try:
            ...
            if time_since_update > self.freeze_threshold:
                logger.warning(
                    "Potential freeze detected - no log updates for %.1f seconds",
                    time_since_update,
                )
                return True
    ...
    def _log_resource_warning(self, resource_type, details):
        ...
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        warning_file = (
            f"{self.dump_dir}/resource_warning_{resource_type}_{timestamp}.txt"
        )
