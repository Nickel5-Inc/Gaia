...
class RuntimeWeightTracer:
        ...
        try:
            ...
            if weights is None:
                ...
                try:
                    ...
                    weights_summary = (
                        f"List type: {type(weights)}, Length: {len(weights)}"
                    )
            else:
                weights_summary = (
                    f"Type: {type(weights).__name__}, Value: {str(weights)[:50]}"
                )
    ...
    def patch_fiber_functions(self):
        ...
        try:
            ...
            print(
                f"✅ [RUNTIME_TRACER] Fiber library loaded from: {fiber_weights.__file__}"
            )

            # Store original functions
            self.original_functions["set_node_weights"] = fiber_weights.set_node_weights
            self.original_functions["normalize_and_quantize"] = (
                fiber_weights._normalize_and_quantize_weights
            )
            self.original_functions["send_weights_to_chain"] = (
                fiber_weights._send_weights_to_chain
            )

            # Create traced version of set_node_weights
            def traced_set_node_weights(
                substrate,
                keypair,
                node_ids,
                node_weights,
                ...
                **kwargs,
                ...
                tracer.log_weights(
                    ...
                    node_weights,
                    {
                        ...
                        "wait_for_finalization": kwargs.get(
            ...
            def traced_normalize_and_quantize(node_ids, node_weights):
                ...
                # Call original function
                formatted_ids, formatted_weights = tracer.original_functions[
            ...
            def traced_send_weights_to_chain(*args, **kwargs):
                ...
                node_weights = (
                    args[3] if len(args) > 3 else kwargs.get("node_weights", [])
                )
                ...
                wait_for_inclusion = (
                    args[6]
                    if len(args) > 6
                    else kwargs.get("wait_for_inclusion", False)
                )
                wait_for_finalization = (
                    args[7]
                    if len(args) > 7
                    else kwargs.get("wait_for_finalization", False)
                )
                ...
                result = tracer.original_functions["send_weights_to_chain"](
                    *args, **kwargs
                )
    ...
    def remove_patches(self):
        ...
        try:
            ...
            if "set_node_weights" in self.original_functions:
                fiber_weights.set_node_weights = self.original_functions[
                    "set_node_weights"
                ]
            if "normalize_and_quantize" in self.original_functions:
                fiber_weights._normalize_and_quantize_weights = self.original_functions[
                    "normalize_and_quantize"
                ]
            if "send_weights_to_chain" in self.original_functions:
                fiber_weights._send_weights_to_chain = self.original_functions[
                    "send_weights_to_chain"
                ]
...
def test_tracing():
    ...
    try:
        ...
        print(
            f"✅ [RUNTIME_TRACER] _normalize_and_quantize_weights: {fiber_weights._normalize_and_quantize_weights}"
        )
        print(
            f"✅ [RUNTIME_TRACER] _send_weights_to_chain: {fiber_weights._send_weights_to_chain}"
        )
