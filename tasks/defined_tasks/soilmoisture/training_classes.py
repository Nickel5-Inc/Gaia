...
class SoilMoistureDataset(Dataset):
    ...
    def _align_data(self) -> List[Dict]:
        aligned_samples = []
        regions = [d for d in os.listdir(self.monthly_root) if os.path.isdir(os.path.join(self.monthly_root, d))]
        
        for region in regions:
            monthly_files = sorted(glob.glob(os.path.join(self.monthly_root, region, '*.tif')))
            three_day_files = sorted(glob.glob(os.path.join(self.three_day_root, region, '*.tif')))
            
            for i in range(len(three_day_files) - 1):
                current_file = three_day_files[i]
                future_file = three_day_files[i + 1]
                
                current_date = datetime.strptime(os.path.basename(current_file).split('_')[-1].split('.')[0], '%Y-%m-%d')
                future_date = datetime.strptime(os.path.basename(future_file).split('_')[-1].split('.')[0], '%Y-%m-%d')
                
                if (future_date - current_date).days != 3:
                    continue
                
                monthly_date = current_date.replace(day=1)
                monthly_file = next((f for f in monthly_files if monthly_date.strftime('%Y-%m') in f), None)
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            sentinel_ndvi = (sentinel_ndvi - sentinel_ndvi.min()) / (sentinel_ndvi.max() - sentinel_ndvi.min() + 1e-8)
            ...
            era5 = F.pad(era5, (0, target_size[1] - era5.shape[2], 0, target_size[0] - era5.shape[1]))
            future_smap = F.pad(future_smap, (0, target_size[1] - future_smap.shape[2], 0, target_size[0] - future_smap.shape[1]))
            ...
            print(f"Error loading sample {index} from region {sample['region']}, date {sample['date']}")
...
class SoilMoistureDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if not hasattr(self, 'full_dataset'):
            self.full_dataset = SoilMoistureDataset(self.monthly_root, self.three_day_root, patch_size=self.patch_size)
        ...
        train_indices = [idx for region in train_regions for idx, _ in samples_by_region[region]]
        val_indices = [idx for region in val_regions for idx, _ in samples_by_region[region]]
        test_indices = [idx for region in test_regions for idx, _ in samples_by_region[region]]
    ...
    def print_dataset_regions(self):
        def get_regions(dataset):
            return set([self.full_dataset.samples[idx]['region'] for idx in dataset.indices])
        ...
        print("Overlap between train and test:", train_regions.intersection(test_regions))
