    ...
    try:
        ...
        with tqdm(total=total_size, unit='B', unit_scale=True, desc="Downloading") as pbar:
            process = subprocess.Popen(
                ['wget', '--progress=dot', f'--http-user={EARTHDATA_USERNAME}',
                 f'--http-password={EARTHDATA_PASSWORD}', '--no-check-certificate',
                 '--auth-no-challenge', '-O', str(cache_file), url],
...
def process_smap_data(filepath, bbox, target_shape=(220, 220)):
    ...
    with xr.open_dataset(filepath, group='Geophysical_Data') as ds:
        surface_sm = ds['sm_surface'].sel(
            lat=slice(bbox[1], bbox[3]), 
            lon=slice(bbox[0], bbox[2])
        ).values
        rootzone_sm = ds['sm_rootzone'].sel(
            lat=slice(bbox[1], bbox[3]), 
            lon=slice(bbox[0], bbox[2])
        ).values
        ...
        return {
            'surface_sm': surface_resampled,
            'rootzone_sm': rootzone_resampled
        }
        ...
        with tempfile.NamedTemporaryFile(suffix='.h5') as temp_file:
            ...
            with xr.open_dataset(temp_file.name, group='Geophysical_Data') as ds:
                ...
                for i, region in enumerate(regions):
                    ...
                    y_idx_start = int((smap_lat_range[1] - top) * smap_y_size / 
                                    (smap_lat_range[1] - smap_lat_range[0]))
                    y_idx_end = int((smap_lat_range[1] - bottom) * smap_y_size / 
                                  (smap_lat_range[1] - smap_lat_range[0]))
                    x_idx_start = int((left - smap_lon_range[0]) * smap_x_size / 
                                    (smap_lon_range[1] - smap_lon_range[0]))
                    x_idx_end = int((right - smap_lon_range[0]) * smap_x_size / 
                                  (smap_lon_range[1] - smap_lon_range[0]))
...
def get_smap_data_for_sentinel_bounds(filepath, sentinel_bounds, sentinel_crs):
    ...
    with xr.open_dataset(filepath, group='Geophysical_Data') as ds:
        ...
        if sentinel_crs != 'EPSG:4326':
            transformer = Transformer.from_crs(sentinel_crs, 'EPSG:4326', always_xy=True)
...
def test_smap_download():
    ...
    test_bounds = (-51.401355453052076, -27.0156800074561, 
                  -50.401355453052076, -26.0156800074561)
    test_crs = 'EPSG:4326'
    ...
    if smap_data:
        ...
        print("\nData ranges:")
        print(f"Surface data range: {np.nanmin(smap_data['surface_sm']):.3f} to {np.nanmax(smap_data['surface_sm']):.3f}")
        print(f"Rootzone data range: {np.nanmin(smap_data['rootzone_sm']):.3f} to {np.nanmax(smap_data['rootzone_sm']):.3f}")
    else:
