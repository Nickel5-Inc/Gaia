...
def upgrade() -> None:
    ...
    columns.extend([
        ...
        sa.Column('status', sa.VARCHAR(50), server_default=sa.text("'pending'"), nullable=True),
    ])
    ...
    op.create_table(
        'score_table_new',
        *columns,
        sa.UniqueConstraint('task_name', 'task_id', name='uq_score_table_new_task_name_task_id'),
    )
    ...
    # Recreate indexes
    op.create_index('idx_score_created_at_on_score_table', 'score_table', ['created_at'])
    op.create_index('idx_score_task_name_created_at_desc_on_score_table', 'score_table', 
    ...
    """))
    if result.scalar() > 0:
        op.drop_constraint('uq_weather_miner_responses_run_miner', 'weather_miner_responses')
    
    # Convert miner_uid from Integer to Integer with FK (it's already Integer)
    op.create_foreign_key(
        'fk_weather_miner_responses_miner_uid',
        'weather_miner_responses', 'node_table',
        ['miner_uid'], ['uid'],
        ondelete='CASCADE'
    )
    ...
    op.create_foreign_key(
        ...
        'weather_miner_scores', 'node_table',
        ['miner_uid'], ['uid'],
        ondelete='CASCADE'
    )
    ...
    # Add new column
    op.add_column('soil_moisture_predictions', 
    ...
    op.alter_column('soil_moisture_predictions', 'miner_uid_new', 
                    new_column_name='miner_uid', nullable=False)
    
    # Add foreign key
    op.create_foreign_key(
        'fk_soil_moisture_predictions_miner_uid',
        'soil_moisture_predictions', 'node_table',
        ['miner_uid'], ['uid'],
        ondelete='CASCADE'
    )
    ...
    # Add new column
    op.add_column('soil_moisture_history',
                  sa.Column('miner_uid_new', sa.Integer(), nullable=True))
    ...
    op.alter_column('soil_moisture_history', 'miner_uid_new',
                    new_column_name='miner_uid', nullable=False)
    
    # Add foreign key
    op.create_foreign_key(
        'fk_soil_moisture_history_miner_uid',
        'soil_moisture_history', 'node_table',
        ['miner_uid'], ['uid'],
        ondelete='CASCADE'
    )
    ...
    # Add new column
    op.add_column('geomagnetic_predictions',
                  sa.Column('miner_uid_new', sa.Integer(), nullable=True))
    ...
    op.alter_column('geomagnetic_predictions', 'miner_uid_new',
                    new_column_name='miner_uid', nullable=False)
    
    # Add foreign key
    op.create_foreign_key(
        'fk_geomagnetic_predictions_miner_uid',
        'geomagnetic_predictions', 'node_table',
        ['miner_uid'], ['uid'],
        ondelete='CASCADE'
    )
    ...
    # Add new column
    op.add_column('geomagnetic_history',
                  sa.Column('miner_uid_new', sa.Integer(), nullable=True))
    ...
    op.alter_column('geomagnetic_history', 'miner_uid_new',
                    new_column_name='miner_uid', nullable=False)
    
    # Add foreign key
    op.create_foreign_key(
        'fk_geomagnetic_history_miner_uid',
        'geomagnetic_history', 'node_table',
        ['miner_uid'], ['uid'],
        ondelete='CASCADE'
    )
    ...
    # Add new column
    op.add_column('miner_performance_stats',
                  sa.Column('miner_uid_new', sa.Integer(), nullable=True))
    ...
    op.alter_column('miner_performance_stats', 'miner_uid_new',
                    new_column_name='miner_uid', nullable=False)
    
    # Add foreign key
    op.create_foreign_key(
        'fk_miner_performance_stats_miner_uid',
        'miner_performance_stats', 'node_table',
        ['miner_uid'], ['uid'],
        ondelete='CASCADE'
    )
...
def downgrade() -> None:
    ...
    # Weather tables
    op.drop_constraint('fk_weather_miner_responses_miner_uid', 'weather_miner_responses')
    op.drop_constraint('fk_weather_miner_scores_miner_uid', 'weather_miner_scores')
    
    # Soil moisture tables
    op.drop_constraint('fk_soil_moisture_predictions_miner_uid', 'soil_moisture_predictions')
    op.drop_constraint('fk_soil_moisture_history_miner_uid', 'soil_moisture_history')
    
    # Geomagnetic tables
    op.drop_constraint('fk_geomagnetic_predictions_miner_uid', 'geomagnetic_predictions')
    op.drop_constraint('fk_geomagnetic_history_miner_uid', 'geomagnetic_history')
    
    # Miner performance stats
    op.drop_constraint('fk_miner_performance_stats_miner_uid', 'miner_performance_stats')
    ...
    for table in tables_to_convert:
        ...
        op.alter_column(table, 'miner_uid_text', 
                       new_column_name='miner_uid', nullable=False)
