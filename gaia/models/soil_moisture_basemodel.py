...
class SoilModel(pl.LightningModule, PyTorchModelHubMixin):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            print(f"Surface - Predictions: [{predictions[:, 0].min():.3f}, {predictions[:, 0].max():.3f}]")
            print(f"Surface - Targets: [{target[:, 0].min():.3f}, {target[:, 0].max():.3f}]")
            print(f"Rootzone - Predictions: [{predictions[:, 1].min():.3f}, {predictions[:, 1].max():.3f}]")
            print(f"Rootzone - Targets: [{target[:, 1].min():.3f}, {target[:, 1].max():.3f}]")
        ...
        target_min, target_max = target_reshaped.min(dim=0)[0], target_reshaped.max(dim=0)[0]  
    ...
    def on_test_epoch_end(self):
        ...
        weighted_metrics = {
            ...
            'test_final_rmse_surface': torch.sqrt(F.mse_loss(all_preds[:, 0], all_targets[:, 0])),
            'test_final_rmse_rootzone': torch.sqrt(F.mse_loss(all_preds[:, 1], all_targets[:, 1])),
        }
        pred_reshaped = all_preds.reshape(-1, 2)
        target_reshaped = all_targets.reshape(-1, 2)
        pred_min, pred_max = pred_reshaped.min(dim=0)[0], pred_reshaped.max(dim=0)[0]
        target_min, target_max = target_reshaped.min(dim=0)[0], target_reshaped.max(dim=0)[0]
        pred_scaled = (pred_reshaped - pred_min.unsqueeze(0)) / (pred_max - pred_min).unsqueeze(0)
        target_scaled = (target_reshaped - target_min.unsqueeze(0)) / (target_max - target_min).unsqueeze(0)
        ...
        weighted_metrics.update({
            ...
            'test_final_r2_surface': r2_surface_metric(pred_scaled[:, 0], target_scaled[:, 0]),
            'test_final_r2_rootzone': r2_rootzone_metric(pred_scaled[:, 1], target_scaled[:, 1])
        })

        pred_normalized = (all_preds - all_preds.min()) / (all_preds.max() - all_preds.min())
        target_normalized = (all_targets - all_targets.min()) / (all_targets.max() - all_targets.min())
        weighted_metrics['test_final_ssim'] = ssim(pred_normalized, target_normalized, data_range=1.0)
        ...
        for layer in ['surface', 'rootzone']:
            ...
            print(f"Predictions: min={final_stats[f'predictions_{layer}']['min']:.3f}, "
            ...
            print(f"Targets: min={final_stats[f'targets_{layer}']['min']:.3f}, "
