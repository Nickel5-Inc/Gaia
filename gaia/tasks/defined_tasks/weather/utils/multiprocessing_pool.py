...
class MemoryAwareProcessPool:
    ...
    def __init__(
        ...
        try:
            ...
            def cleanup_process_pool_caches():
                ...
                logger.debug(
                    f"[MemoryAwareProcessPool] Performed cleanup, collected {collected} objects"
                )

            register_thread_cleanup(
                "memory_aware_process_pool", cleanup_process_pool_caches
            )
            logger.debug(
                "[MemoryAwareProcessPool] Registered for global memory cleanup"
            )
        except Exception as e:
            logger.debug(f"[MemoryAwareProcessPool] Failed to register cleanup: {e}")

        logger.info(
            f"MemoryAwareProcessPool initialized: max_workers={self.max_workers}, "
    ...
    def _adjust_workers(self) -> bool:
        ...
        if current_memory > self.memory_threshold_mb:
            ...
            if self.current_workers > self.min_workers:
                ...
                logger.warning(
                    f"High memory pressure ({current_memory:.1f}MB) - "
            ...
            if self.current_workers < self.max_workers:
                ...
                logger.info(
                    f"Low memory pressure ({current_memory:.1f}MB) - "
    ...
    def _create_executor(self) -> ProcessPoolExecutor:
        ...
        logger.debug(
            f"Creating ProcessPoolExecutor with {self.current_workers} workers"
        )
    ...
    async def execute(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.enable_monitoring:
            ...
            if current_memory > self.memory_threshold_mb * 1.2:  # 20% over threshold
                logger.error(
                    f"Memory too high ({current_memory:.1f}MB) - falling back to async execution"
                )
        ...
        try:
            ...
            logger.debug(
                f"Process pool execution completed in {duration:.2f}s using {self.current_workers} workers"
            )
            return result
...
def get_process_pool() -> MemoryAwareProcessPool:
    ...
    if _global_process_pool is None:
        ...
        _global_process_pool = MemoryAwareProcessPool(
            max_workers=max_workers, memory_threshold_mb=memory_threshold
        )
...
def compute_climatology_interpolation_mp(
    ...
    try:
        ...
        # Create meshgrids
        source_lat_mesh, source_lon_mesh = np.meshgrid(
            source_lat, source_lon, indexing="ij"
        )
        target_lat_mesh, target_lon_mesh = np.meshgrid(
            target_lat, target_lon, indexing="ij"
        )

        # Flatten for interpolation
        source_points = np.column_stack(
            (source_lat_mesh.ravel(), source_lon_mesh.ravel())
        )
        target_points = np.column_stack(
            (target_lat_mesh.ravel(), target_lon_mesh.ravel())
        )
        ...
        if climatology_data.ndim == 3:  # (time, lat, lon)
            ...
            for t in range(climatology_data.shape[0]):
                ...
                if np.sum(valid_mask) < 4:  # Need at least 4 points for interpolation
                    logger.warning(
                        f"MP: Not enough valid points for {var_name} at time {t}"
                    )
                    result[t] = np.nan
                    continue
            ...
            if np.sum(valid_mask) < 4:
                ...
                return np.full(
                    (target_lat.size, target_lon.size), np.nan, dtype=np.float32
                )
        ...
        logger.debug(
            f"MP: Successfully computed climatology interpolation for {var_name}"
        )
...
def compute_statistical_metrics_mp(
    ...
    try:
        ...
        if forecast.shape != truth.shape:
            logger.error(
                f"MP: Shape mismatch - forecast: {forecast.shape}, truth: {truth.shape}"
            )
        ...
        if "acc" in metrics and climatology_data is not None:
            ...
            if climatology.shape == truth.shape:
                ...
                if weights is not None:
                    ...
                    cov = np.average(
                        forecast_anom_centered * truth_anom_centered, weights=weights
                    )
                    var_forecast = np.average(
                        forecast_anom_centered**2, weights=weights
                    )
...
def process_coordinate_transformation_mp(
    ...
    try:
        ...
        if transformation_type == "interpolation":
            ...
            if any(
                coord is None
                for coord in [source_lat, source_lon, target_lat, target_lon]
            ):
