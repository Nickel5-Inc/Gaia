...
def extract_and_reference_static_fields(
    ds: xr.Dataset, static_file_path: str = "static_fields.nc"
) -> xr.Dataset:
    """
...
def reconstruct_full_dataset(
    dynamic_ds: xr.Dataset, static_file_path: str = None
) -> xr.Dataset:
    """
...
def combine_forecast_steps(
    file_paths: List[str], output_path: str, static_file_path: str = "static_fields.nc"
) -> str:
    """
...
def create_synthetic_variant(
    ds: xr.Dataset, perturbation_scale: float = 0.05, seed: int = None
) -> xr.Dataset:
    """
    ...
    for var in ds.data_vars:
        ...
        if config["type"] == "additive":
            noise = (
                np.random.normal(0, 1, noise_shape)
                * config["scale"]
                * perturbation_scale
            )
...
def generate_ensemble_members(
    ...
    static_file_path: str = "static_fields.nc",
) -> List[str]:
    """
    ...
    for i in range(num_members):
        perturbed = create_synthetic_variant(
            base_ds, perturbation_scale=0.05 + (i * 0.01), seed=i
        )
...
def calculate_ensemble_stats(
    ...
    return {"mean_path": output_path, "num_members": len(members)}
