...
warnings.filterwarnings(
    "ignore", message="numpy.core.numeric is deprecated", category=DeprecationWarning
)

DODS_BASE_URL = "http://nomads.ncep.noaa.gov/dods/gfs_0p25/gfs"
AURORA_PRESSURE_LEVELS_HPA = [
    50,
    100,
    150,
    200,
...
async def fetch_gfs_data(
    ...
    def _sync_fetch_and_process(
        ...
        if target_surface_vars is not None:
            surface_vars_to_fetch = [
                var for var in target_surface_vars if var in GFS_SURFACE_VARS
            ]
        ...
        if target_atmos_vars is not None:
            atmos_vars_to_fetch = [
                var for var in target_atmos_vars if var in GFS_ATMOS_VARS
            ]
            logger.info(
                f"Fetching targeted atmospheric variables: {atmos_vars_to_fetch}"
            )
        ...
        if target_pressure_levels_hpa is not None:
            valid_target_levels = [
                lvl
                for lvl in target_pressure_levels_hpa
                if lvl in aurora_pressure_levels_const
            ]
            if not valid_target_levels:
                logger.warning(
                    f"Target pressure levels {target_pressure_levels_hpa} have no overlap with available GFS pressure levels {aurora_pressure_levels_const} via configured indices. Atmospheric data might be empty."
                )
                ...
                for level_hpa in valid_target_levels:
                    try:
                        idx_in_const_list = aurora_pressure_levels_const.index(
                            level_hpa
                        )
                        level_indices_to_fetch.append(
                            aurora_level_indices_const[idx_in_const_list]
                        )
                        actual_pressure_levels_to_fetch_hpa.append(level_hpa)
                    except ValueError:
                        logger.warning(
                            f"Level {level_hpa} unexpectedly not found in aurora_pressure_levels_const."
                        )
                logger.info(
                    f"Fetching targeted pressure levels (hPa): {actual_pressure_levels_to_fetch_hpa} using indices: {level_indices_to_fetch}"
                )
        else:
            logger.info(
                f"Fetching default pressure levels (hPa): {actual_pressure_levels_to_fetch_hpa} using indices: {level_indices_to_fetch}"
            )
        ...
        try:
            with warnings.catch_warnings():
                ...
                for vt in valid_times:
                    ...
                    if abs(vt_np - actual_time) > np.timedelta64(3, "h"):
                        logger.warning(
                            f"Requested time {vt} has large difference from closest dataset time {actual_time} at index {closest_idx}"
                        )
                    else:
                        logger.debug(
                            f"Requested time {vt} matches dataset time {actual_time} at index {closest_idx}"
                        )

                time_indices = sorted(list(set(time_indices)))
                logger.info(f"Selected time indices: {time_indices}")
                logger.info(
                    f"Loading surface variables: {surface_vars_to_fetch} at selected times."
                )
                if surface_vars_to_fetch:
                    surface_ds = (
                        full_ds[surface_vars_to_fetch].isel(time=time_indices).load()
                    )
                ...
                if atmos_vars_to_fetch and level_indices_to_fetch:
                    for var in atmos_vars_to_fetch:
                        logger.info(
                            f"Loading atmospheric variable: {var} at selected times and levels."
                        )
                        if var in full_ds:
                            if "lev" in full_ds[var].coords:
                                var_ds = (
                                    full_ds[[var]]
                                    .isel(time=time_indices, lev=level_indices_to_fetch)
                                    .load()
                                )
                                logger.debug(
                                    f"Loaded {var}, shape: {var_ds[var].shape}"
                                )
                                atmos_ds_list.append(var_ds)
                            elif not level_indices_to_fetch:
                                logger.warning(
                                    f"Atmospheric variable {var} does not have 'lev' coordinate, but levels were specified. Check GFS_ATMOS_VARS."
                                )
                            else:
                                var_ds = full_ds[[var]].isel(time=time_indices).load()
                                logger.debug(
                                    f"Loaded {var} (all levels as none specific requested/applicable), shape: {var_ds[var].shape}"
                                )
                                atmos_ds_list.append(var_ds)
                        else:
                            logger.warning(
                                f"Atmospheric variable {var} not found in dataset."
                            )
                elif not atmos_vars_to_fetch:
                    logger.info("No atmospheric variables targeted for fetching.")
                elif not level_indices_to_fetch:
                    logger.info(
                        "No pressure levels targeted for atmospheric variables."
                    )
            ...
            logger.error(
                f"Error during synchronous OPeNDAP fetch/process: {e}", exc_info=True
            )
    ...
    try:
        result_dataset = await asyncio.to_thread(
            _sync_fetch_and_process,
...
def process_opendap_dataset(ds: xr.Dataset) -> xr.Dataset:
    ...
    if "z_height" in new_ds:
        ...
        logger.warning(
            "Geopotential Height (hgtprs/z_height) not found, cannot calculate Geopotential (z)."
        )
    ...
    for var_name, expected_units in default_units.items():
        if var_name in new_ds:
            ...
            if not current_units:
                logger.debug(
                    f"Assigning default units '{expected_units}' to {var_name}."
                )
                ...
                logger.warning(
                    f"Unexpected units for {var_name}. Expected '{expected_units}', found '{current_units}'. No conversion applied."
                )
    ...
    if "lon" in new_ds.coords:
        if new_ds.lon.values.min() < -1.0:
            ...
            new_ds = new_ds.assign_coords(
                lon=(((new_ds.lon + 180) % 360) - 180 + 360) % 360
            )  # Careful conversion
...
def get_consecutive_lead_hours(
    first_lead: int, last_lead: int, interval: int = 6
) -> List[int]:
    """
...
def _get_gfs_cycle_url(target_time: datetime) -> Optional[str]:
    ...
    if target_time.hour % 6 != 0 or target_time.minute != 0 or target_time.second != 0:
        logger.error(
            f"Target time {target_time} is not a valid GFS cycle init time (00, 06, 12, 18 UTC). Cannot fetch analysis."
        )
        return None
...
async def fetch_gfs_analysis_data(
    ...
    for t in target_times:
        if t.hour % 6 == 0 and t.minute == 0 and t.second == 0:
            ...
            logger.warning(
                f"Skipping invalid target time {t}: Not a GFS cycle hour (00, 06, 12, 18 UTC)."
            )
    ...
    if cache_file_to_use:
        try:
            ...
            if is_pickle:
                ...
                for engine in engines_to_try:
                    try:
                        ...
                        logger.debug(
                            f"Successfully loaded cache using '{engine}' engine"
                        )
                        break
                    except Exception as engine_err:
                        logger.debug(
                            f"Failed to load with '{engine}' engine: {engine_err}"
                        )
                        continue
            ...
            if "time" in ds_cached.coords:
                ...
                if all(
                    t_np_ns in ds_cached.time.values for t_np_ns in target_times_np_ns
                ):
                    logger.info("GFS Analysis cache hit is valid.")

                    if progress_callback:
                        file_size = (
                            cache_file_to_use.stat().st_size
                    ...
                    logger.warning(
                        "Cached GFS analysis file missing requested times. Re-fetching."
                    )
                ...
                logger.warning(
                    "Cached GFS analysis file missing 'time' coordinate. Re-fetching."
                )
                if hasattr(ds_cached, "close"):
                    ds_cached.close()
                cache_file_to_use.unlink()
            ...
            logger.warning(
                f"Failed load/validate GFS analysis cache {cache_file_to_use}: {e}. Re-fetching."
            )
            if cache_file_to_use.exists():
                try:
                    cache_file_to_use.unlink()
    ...
    logger.info(
        f"GFS Analysis cache miss for times: {time_strings[0]}...{time_strings[-1]}. Fetching from NOMADS."
    )
    ...
    def _sync_fetch_and_process_analysis():
        ...
        with warnings.catch_warnings():
            ...
            for idx, target_time in enumerate(target_times):
                ...
                try:
                    ...
                    if not base_url:
                        logger.error(
                            f"Could not get URL for GFS cycle {target_time}. Skipping."
                        )
                        continue

                    logger.info(
                        f"Opening GFS cycle {target_time.strftime('%Y-%m-%d %H:%M:%S')} from {base_url} for analysis (T+0h)..."
                    )
                    full_ds = xr.open_dataset(base_url, decode_times=False, chunks={})
                    logger.info(
                        f"Successfully opened GFS dataset with decode_times=False. Raw time variable attributes: {full_ds.time.attrs}"
                    )
                    logger.info(
                        f"Raw time variable sample values: {full_ds.time.values[:5] if len(full_ds.time.values) > 5 else full_ds.time.values}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Manually decoded time with cftime (sample numpy array): {decoded_time_values_np_array[:5] if len(decoded_time_values_np_array) > 5 else decoded_time_values_np_array}"
                        )
                        # Replace the time coordinate in full_ds
                        full_ds = full_ds.assign_coords(
                            time=decoded_time_values_np_array
                        )
                        time_coordinate_decoded_successfully = True
                        logger.info(
                            f"Time coordinate in full_ds updated. Sample from full_ds.time.values: {full_ds.time.values[:5] if len(full_ds.time.values) > 5 else full_ds.time.values}"
                        )
                    except Exception as e_manual_decode:
                        logger.error(
                            f"Failed to manually decode time with cftime: {e_manual_decode}",
                            exc_info=True,
                        )
                        logger.warning(
                            "Proceeding with original undecoded (numerical) time coordinate for selection."
                        )

                    analysis_target_dt_np = np.datetime64(
                        target_time.replace(tzinfo=None)
                    )
                    analysis_slice = None  # Initialize to ensure it's defined for logging in case of error
                    time_difference = np.timedelta64(
                        24 * 3600, "s"
                    )  # Default to a large diff
                    ...
                    if time_coordinate_decoded_successfully:
                        ...
                        if len(unique_times) < len(time_values):
                            logger.warning(
                                f"Found {len(time_values) - len(unique_times)} duplicate time values in GFS dataset. Removing duplicates."
                            )
                        ...
                        py_dt = datetime(
                            year=int(cftime_obj.year),
                            month=int(cftime_obj.month),
                            day=int(cftime_obj.day),
                            hour=int(cftime_obj.hour),
                            minute=int(cftime_obj.minute),
                            second=int(cftime_obj.second),
                        ...
                        time_difference = abs(
                            selected_time_from_slice_dt - analysis_target_dt_np
                        )
                    else:
                        ...
                        try:
                            ...
                            time_calendar = full_ds.time.attrs.get(
                                "calendar", "standard"
                            )
                            if not time_units:
                                logger.error(
                                    f"Time units attribute missing from undecoded time coordinate for target {target_time}. Cannot select accurately."
                                )
                            ...
                            if len(unique_times) < len(time_values):
                                logger.warning(
                                    f"Found {len(time_values) - len(unique_times)} duplicate numerical time values in GFS dataset. Removing duplicates."
                                )
                                # Keep only the first occurrence of each unique time
                                unique_indices_sorted = np.sort(unique_indices)
                                full_ds = full_ds.isel(time=unique_indices_sorted)
                            ...
                            analysis_slice = full_ds.sel(
                                time=numerical_target_for_sel, method="nearest"
                            )
                            selected_numerical_time_from_slice = (
                                analysis_slice.time.values
                            )
                            numerical_diff = abs(
                                selected_numerical_time_from_slice
                                - numerical_target_for_sel
                            ...
                            time_difference = np.abs(
                                np.datetime64(time_axis_for_diff_conversion[1])
                                - np.datetime64(time_axis_for_diff_conversion[0])
                            )

                        except Exception as e_numerical_sel:
                            logger.error(
                                f"Error during numerical time selection for target {target_time}: {e_numerical_sel}",
                                exc_info=True,
                            )
                            continue
                    ...
                    if (
                        analysis_slice is not None
                        and hasattr(analysis_slice, "time")
                        and hasattr(analysis_slice.time, "values")
                    ):
                        actual_selected_time_for_log = analysis_slice.time.values

                    if time_difference > np.timedelta64(1, "h"):
                        logger.error(
                            f"Nearest time found ({actual_selected_time_for_log}) is too far ({time_difference}) from requested analysis time {target_time}. Skipping."
                        )
                        continue

                    current_surface_vars = (
                        target_surface_vars
                        if target_surface_vars is not None
                        else GFS_SURFACE_VARS
                    )
                    current_atmos_vars = (
                        target_atmos_vars
                        if target_atmos_vars is not None
                    ...
                    vars_to_load_final = [
                        v
                        for v in current_surface_vars + current_atmos_vars
                        if v in full_ds
                    ]

                    if not vars_to_load_final:
                        logger.warning(
                            f"No targeted GFS variables found in dataset for cycle {target_time}. Surface tried: {current_surface_vars}, Atmos tried: {current_atmos_vars}. Skipping analysis for this time."
                        )
                        continue
                    logger.info(
                        f"For cycle {target_time}, attempting to load variables: {vars_to_load_final}"
                    )
                    ...
                    logger.info(
                        f"Loaded analysis slice for original target {target_time}"
                    )
                    ...
                    logger.error(
                        f"Failed to fetch/process analysis for original target {target_time}: {e}",
                        exc_info=True,
                    )
                finally:
                    # MEMORY LEAK FIX: Always cleanup the full dataset to prevent accumulation
                    if (
                        "dataset_to_cleanup" in locals()
                        and dataset_to_cleanup is not None
                    ):
                        try:
                            ...
                            logger.debug(
                                f"Error during dataset cleanup for {target_time}: {cleanup_err}"
                            )
                    ...
                    if idx % 5 == 0:  # Every 5 iterations
                        ...
                        logger.debug(
                            f"GC collected {collected} objects after processing {idx+1} time steps"
                        )
        ...
        try:
            ...
            collected = gc.collect()
            logger.debug(
                f"Post-concat cleanup: freed {len_analysis_slices} slices, GC collected {collected} objects"
            )
        ...
        vars_to_rename = {
            k: v for k, v in var_mapping.items() if k in combined_ds.data_vars
        }
        processed_ds = combined_ds.rename(vars_to_rename)
        logger.debug(f"Renamed variables: {vars_to_rename}")

        coord_rename = {}
        if "lev" in processed_ds.coords:
            coord_rename["lev"] = "pressure_level"
        ...
        if "z_height" in processed_ds:
            logger.info(
                "Converting Geopotential Height (z_height) to Geopotential (z)..."
            )
            try:
                ...
                processed_ds["z"].attrs[
                    "comment"
                ] = f"Calculated as g * z_height, with g={G} m/s^2"
        ...
        if "pressure_level" in processed_ds.coords:
            current_units = (
                processed_ds["pressure_level"].attrs.get("units", "").lower()
            )
            if current_units == "pa" or not current_units:
                ...
                logger.info(
                    f"Pressure level units ('{current_units}') are already hPa equivalent. Ensuring units attribute is 'hPa'."
                )
                processed_ds["pressure_level"].attrs["units"] = "hPa"
                processed_ds["pressure_level"].attrs["long_name"] = "pressure"
            else:
                logger.warning(
                    f"Pressure level units '{current_units}' are not Pa, hPa, or millibar. Proceeding without conversion."
                )

            try:
                pressure_levels_for_selection = (
                    target_pressure_levels_hpa
                    if target_pressure_levels_hpa is not None
                    else AURORA_PRESSURE_LEVELS_HPA
                )
                processed_ds = processed_ds.sel(
                    pressure_level=pressure_levels_for_selection
                )
                logger.info(
                    f"Selected pressure levels: {pressure_levels_for_selection}"
                )
            except Exception as e_sel_p:
                logger.warning(
                    f"Could not select pressure levels after unit processing: {e_sel_p}"
                )
        ...
        if (
            "lat" in processed_ds.coords
            and len(processed_ds.lat) > 1
            and processed_ds.lat.values[0] < processed_ds.lat.values[-1]
        ):
            logger.info("Reversing latitude coordinate.")
            processed_ds = processed_ds.reindex(
                lat=processed_ds.lat.values[::-1].copy()
            )
        ...
        try:
            logger.info(
                f"Saving processed GFS analysis data to cache: {cache_filename}"
            )
            cache_dir.mkdir(parents=True, exist_ok=True)
            ...
            if not saved_successfully:
                ...
                pickle_filename = cache_filename.with_suffix(".pkl")
                logger.warning(
                    f"All netCDF engines failed, falling back to pickle format: {pickle_filename}"
                )
            ...
            logger.error(
                f"Failed to save GFS analysis to cache {cache_filename}: {e_cache}"
            )
            if isinstance(e_cache, PermissionError):
                logger.error(
                    f"** PERMISSION DENIED saving to {cache_dir}. Please check directory permissions. **"
                )
    ...
    try:
        result_dataset, final_progress_info = await asyncio.to_thread(
            _sync_fetch_and_process_analysis
        )
        ...
        if progress_callback and final_progress_info:
            if final_progress_info.get("completed"):
                ...
                if result_dataset is not None:
                    file_size = (
                        cache_filename.stat().st_size if cache_filename.exists() else 0
                    )
                    await progress_callback(
                        {
                            "operation": "gfs_download",
...
async def _test_fetch_analysis():
    ...
    test_times = [datetime(2024, 6, 15, 0, 0), datetime(2024, 6, 15, 12, 0)]
