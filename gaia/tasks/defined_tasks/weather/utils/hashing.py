...
from .gfs_api import fetch_gfs_analysis_data
...
def generate_manifest_and_signature(
    ...
    try:
        for item_path in sorted(zarr_store_path.rglob("*")):
            if item_path.is_file():
                ...
                if not include_zarr_metadata_in_manifest and (
                    relative_path_str.startswith(".")
                    or relative_path_str.endswith(
                ...
                path_plus_content = (
                    relative_path_str.encode("utf-8") + b"\0" + chunk_content
                )
                if chunk_hash_algo_name == "xxh64":
                    chunk_hashes[relative_path_str] = xxhash.xxh64(
                        path_plus_content
                    ).hexdigest()
                elif chunk_hash_algo_name == "sha256":
                    chunk_hashes[relative_path_str] = hashlib.sha256(
                        path_plus_content
                    ).hexdigest()
                else:
                    logger.error(
                        f"Unsupported chunk_hash_algo_name: {chunk_hash_algo_name}"
                    )
                    return None
        ...
        manifest_json_bytes = json.dumps(
            manifest_data_to_sign, sort_keys=True, indent=4
        ).encode("utf-8")
        ...
        logger.info(
            f"Generated manifest for {miner_hotkey_keypair.ss58_address} at {zarr_store_path}"
        )
        return manifest_data_to_sign, signature_bytes, manifest_content_sha256_hash
    except Exception as e:
        logger.error(
            f"Error generating manifest for {zarr_store_path}: {e}", exc_info=True
        )
        return None
...
def compute_verification_hash(
    ...
    logger.info(
        f"Miner ({miner_hotkey_keypair.ss58_address if miner_hotkey_keypair else 'UnknownKey'}): Starting {PROFILE_STRUCTURE_VERSION} (chunk_algo: {chunk_hash_algo}) hash generation for asset: {asset_base_name}"
    )
    ...
    try:
        ...
        level_dim_name = next(
            (
                d
                for d in data_xr.dims
                if d.lower() in ("level", "pressure_level", "isobaricinhpa", "plev")
            ),
            None,
        )
        if level_dim_name:
            chunks[level_dim_name] = 1
        lat_dim_name = next(
            (d for d in data_xr.dims if d.lower() in ("lat", "latitude")), None
        )
        if lat_dim_name:
            chunks[lat_dim_name] = data_xr.sizes[lat_dim_name]
        lon_dim_name = next(
            (d for d in data_xr.dims if d.lower() in ("lon", "longitude")), None
        )
        if lon_dim_name:
            chunks[lon_dim_name] = data_xr.sizes[lon_dim_name]
        if custom_chunks:
            for dim, size in custom_chunks.items():
                actual_dim = dim
                if actual_dim in data_xr.dims:
                    chunks[actual_dim] = (
                        data_xr.sizes[actual_dim] if size == -1 else size
                    )
        ...
        encoding = {
            var_name: {"compressor": compressor} for var_name in data_xr.data_vars
        }
        data_xr.chunk(chunks).to_zarr(
            zarr_store_full_path, encoding=encoding, consolidated=True, compute=True
        )
        ...
        if manifest_result is None:
            logger.error(
                f"Failed to generate manifest and signature for {zarr_store_full_path}"
            )
            return None
        _manifest_dict, _signature_bytes, manifest_content_sha256_hash_hex = (
            manifest_result
        )
        elapsed_time = time.time() - start_time
        logger.info(
            f"Miner {PROFILE_STRUCTURE_VERSION} manifest hash for {asset_base_name} generated in {elapsed_time:.3f}s. Manifest SHA256 Hash: {manifest_content_sha256_hash_hex}"
        )
        return manifest_content_sha256_hash_hex
    except Exception as e:
        logger.error(
            f"Error in miner compute_verification_hash for {asset_base_name}: {e}",
            exc_info=True,
        )
        return None


def _compute_analysis_profile(
    current_ds: xr.Dataset,
    current_metadata: Dict,
    current_variables: List[str],
    _claimed_hash_unused: str,
    current_job_id: str,
    zarr_url_for_profile: str,
) -> Dict:
    ...
    profile = {
        ...
        "dataset_info": {
            ...
            "coords": {
                str(k): list(map(str, v.values))[:5]
                + (["..."] if len(v.values) > 5 else [])
                for k, v in current_ds.coords.items()
            },  # Show first 5 coord values
        },
        "metadata_param": {
            "date": current_metadata.get("time")[0].strftime("%Y-%m-%d")
            if current_metadata.get("time")
            else "N/A",
    ...
    time_dim = next(
        (d for d in current_ds.dims if d.lower() in ["time", "forecast_time", "step"]),
        "time",
    )
    lat_dim = next(
        (d for d in current_ds.dims if d.lower() in ["lat", "latitude", "y"]), "lat"
    )
    lon_dim = next(
        (d for d in current_ds.dims if d.lower() in ["lon", "longitude", "x"]), "lon"
    )
    level_dim_候補 = [
        d
        for d in current_ds.dims
        if d.lower() in ("level", "pressure_level", "isobaricinhpa", "plev")
    ]
    ...
    if time_dim not in current_ds.dims:
        ...
        profile["notes"] = (
            f"Dataset has no time dimension '{time_dim}'. Found: {list(current_ds.dims)}"
        )
    ...
    if max_t == 0:
        profile["status"] = "test_error"
        profile["notes"] = f"Dataset time dimension '{time_dim}' has size 0."
        profile["total_computation_time_seconds"] = time.time() - inner_start_time
        return profile

    profile_timesteps_indices = sorted(
        list(set([0, max_t // 2, max_t - 1 if max_t > 0 else 0]))
    )
    profile_timesteps_indices = [
        min(idx, max_t - 1) for idx in profile_timesteps_indices
    ]  # ensure valid
    ...
    for canonical_name, possible_names in standard_mapping.items():
        for pn in possible_names:
            if pn in current_ds.data_vars:
                var_mapping[canonical_name] = pn
                break
            if pn.lower() in ds_vars_lower:
                var_mapping[canonical_name] = ds_vars_lower[pn.lower()]
                break

    case1_results = {
        "case_name": "Single Full 2D Slice",
        "status": "skipped",
        "details": {},
    }
    ...
    for var_c1_canonical in vars_to_try_c1:
        ...
        if actual_var_name_c1 and actual_var_name_c1 in current_ds:
            ...
            selection_c1 = None
            selection_details_c1 = {
                time_dim: profile_timesteps_indices[len(profile_timesteps_indices) // 2]
            }  # Mid timestep
            ...
            if is_3d_var:
                if data_array_c1.sizes[level_dim] > 0:
                    ...
                    case1_results["details"]["slice_type"] = (
                        f"3D var ({actual_var_name_c1}), mid-level"
                    )
                else:
                    case1_results["details"]["slice_type"] = (
                        f"3D var ({actual_var_name_c1}), level dim size 0"
                    )
                    continue
            else:
                case1_results["details"]["slice_type"] = (
                    f"2D var ({actual_var_name_c1}), surface"
                )
            ...
            try:
                ...
                case1_results["details"]["selection_indices"] = {
                    k: int(v) for k, v in selection_details_c1.items()
                }
                ...
                case1_results["error_message"] = (
                    f"Error loading slice for {actual_var_name_c1}: {str(e_c1_load)}"
                )
                logger.warning(
                    f"Job {current_job_id}: Case 1 Error - {case1_results['error_message']}"
                )
                executed_case1 = True
                break
    if not executed_case1:
        case1_results["notes"] = (
            f"No suitable variable found or tested from options: {vars_to_try_c1}"
        )
    ...
    case2_results = {
        ...
        "details": {
            "variables_targeted": [],
            "timesteps_used_indices": [int(t) for t in profile_timesteps_indices],
            "levels_used_indices": [],
            "total_slices_loaded": 0,
        },
    }
    ...
    if level_dim and current_ds.sizes.get(level_dim, 0) > 0:
        ...
        levels_to_iterate_c2_indices = sorted(
            list(set([0, max_l // 2, max_l - 1 if max_l > 0 else 0]))
        )
        levels_to_iterate_c2_indices = [
            min(idx, max_l - 1) for idx in levels_to_iterate_c2_indices
        ]  # ensure valid
        case2_results["details"]["levels_used_indices"] = [
            int(l) for l in levels_to_iterate_c2_indices
        ]
    ...
    try:
        for var_c2_canonical in vars_for_case2_canonical:
            ...
            if not actual_var_name_c2 or actual_var_name_c2 not in current_ds:
                logger.warning(
                    f"Job {current_job_id}: Case 2 - Variable {var_c2_canonical} (mapped to {actual_var_name_c2}) not found. Skipping for this test."
                )
                continue
            ...
            is_3d_var_c2 = (
                level_dim
                and level_dim in data_array_c2.dims
                and data_array_c2.sizes[level_dim] > 0
            )
        ...
        if not actual_vars_tested_c2:
            ...
            case2_results["notes"] = (
                f"No suitable variables found from target list: {vars_for_case2_canonical}"
            )
        ...
        case2_results["error_message"] = (
            f"Error during Case 2 data loading: {str(e_c2_load)}"
        )
        logger.error(
            f"Job {current_job_id}: Case 2 Error - {case2_results['error_message']}",
            exc_info=True,
        )
        c2_ops_successful = False
    ...
    if c2_ops_successful and actual_vars_tested_c2:
        ...
        if "notes" not in case2_results and "error_message" not in case2_results:
            case2_results["notes"] = (
                "Test did not complete successfully or no variables were processed."
            )
    ...
    profile["status"] = (
        "test_complete"
        if all(p.get("status") == "success" for p in profile["performance_tests"])
        else "test_completed_with_issues"
    )
    ...
    try:
        ...
        log_filename = os.path.join(
            ANALYSIS_LOG_DIR, f"job_{current_job_id}_perf_profile_{ts}.json"
        )
        with open(log_filename, "w") as f_log:
            json.dump(profile, f_log, indent=4, default=str)
        logger.info(
            f"Job {current_job_id}: Performance profile saved to {log_filename}"
        )
    except Exception as e_log_save:
        logger.error(
            f"Job {current_job_id}: Failed to save performance profile JSON: {e_log_save}"
        )
...
async def verify_manifest_and_get_trusted_store(
    ...
    logger.info(
        f"Starting {PROFILE_STRUCTURE_VERSION} Manifest Verification for job {job_id} (Miner: {miner_hotkey_ss58}) for store: {zarr_store_url_cleaned}"
    )

    verification_details_log = {
        "job_id": job_id,
        "zarr_store_url_checked": zarr_store_url_cleaned,
        ...
        "steps": [],
        "final_status": "pending_preparation",
    }
    ...
    try:
        ...
        verification_details_log["steps"].append(
            {"name": "InitializeFilesystem", "status": "success"}
        )
    except Exception as e_fs_init:
        logger.error(
            f"Job {job_id}: Failed to initialize fsspec filesystem for {protocol}: {e_fs_init}"
        )
        verification_details_log["steps"].append(
            {
                "name": "InitializeFilesystem",
                "status": "failed",
                "reason": str(e_fs_init),
            }
        )
        ...
        try:
            ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
            log_filename = os.path.join(
                VERIFICATION_LOG_DIR, f"job_{job_id}_manifest_verify_log_{ts}.json"
            )
            verification_details_log["overall_manifest_verification_passed"] = False
            verification_details_log["total_verification_time_seconds"] = (
                time.time() - start_time_total_verify
            )
            with open(log_filename, "w") as f:
                json.dump(verification_details_log, f, indent=4)
        except Exception as e_save_err:
            logger.error(f"Failed to save error log for job {job_id}: {e_save_err}")
        return None
    ...
    def _fetch_and_verify_manifest_sync_inner():
        ...
        try:
            ...
            if computed_m_hash != claimed_manifest_content_hash:
                raise ValueError(
                    f"Manifest content hash mismatch! Claimed: {claimed_manifest_content_hash}, Computed: {computed_m_hash}"
                )
            verification_details_log["steps"].append(
                {"name": "VerifyManifestContentHash", "status": "success"}
            )

            trusted_manifest_dict_inner = json.loads(
                manifest_json_bytes.decode("utf-8")
            )
            ...
            if not manifest_signer or manifest_signer != miner_hotkey_ss58:
                raise ValueError(
                    f"Manifest signer hotkey mismatch. Expected: {miner_hotkey_ss58}, Got: {manifest_signer}"
                )
            verification_details_log["steps"].append(
                {"name": "VerifyManifestSigner", "status": "success"}
            )

            verifier_kp = Keypair(ss58_address=miner_hotkey_ss58)
            if not verifier_kp.verify(manifest_json_bytes, signature_bytes):
                raise InvalidSignature(
                    "Manifest signature verification failed (Keypair.verify)"
                )
            verification_details_log["steps"].append(
                {"name": "VerifyManifestSignature", "status": "success"}
            )
            ...
            if "VerifyManifestContentHash" in str(e_verify):
                verification_details_log["final_status"] = (
                    "failed_manifest_hash_mismatch"
                )
            elif "VerifyManifestSigner" in str(e_verify):
                verification_details_log["final_status"] = (
                    "failed_manifest_signer_mismatch"
                )
            elif "VerifyManifestSignature" in str(e_verify):
                verification_details_log["final_status"] = "failed_manifest_signature"
            else:
                verification_details_log["final_status"] = (
                    "failed_manifest_verification_other"
                )

            if (
                not any(
                    s["name"] == "VerifyManifestContentHash" and s["status"] == "failed"
                    for s in verification_details_log["steps"]
                )
                and not any(
                    s["name"] == "VerifyManifestSigner" and s["status"] == "failed"
                    for s in verification_details_log["steps"]
                )
                and not any(
                    s["name"] == "VerifyManifestSignature" and s["status"] == "failed"
                    for s in verification_details_log["steps"]
                )
            ):
                verification_details_log["steps"].append(
                    {
                        "name": "ManifestVerificationFailure",
                        "status": "failed",
                        "reason": str(e_verify),
                    }
                )
            ...
            logger.error(
                f"Job {job_id}: Unexpected error in _fetch_and_verify_manifest_sync_inner: {e_inner_sync}",
                exc_info=True,
            )
            verification_details_log["steps"].append(
                {
                    "name": "InternalSyncVerification",
                    "status": "error",
                    "reason": str(e_inner_sync),
                }
            )
    ...
    try:
        ...
        trusted_manifest_final = await loop.run_in_executor(
            None, _fetch_and_verify_manifest_sync_inner
        )
    except Exception as e_executor_call:
        logger.error(
            f"Job {job_id}: Exception calling executor for manifest fetch/verify: {e_executor_call}",
            exc_info=True,
        )
        verification_details_log["steps"].append(
            {
                "name": "ExecutorCallForManifest",
                "status": "failed",
                "reason": str(e_executor_call),
            }
        )
    ...
    try:
        ...
        verification_details_log["overall_manifest_verification_passed"] = (
            log_overall_success
        )
        verification_details_log["total_verification_time_seconds"] = (
            time.time() - start_time_total_verify
        ...
        with open(log_filename, "w") as f:
            json.dump(verification_details_log, f, indent=4)
        logger.info(
            f"Saved manifest verification log for job {job_id} to: {log_filename}"
        )
    except Exception as e_save:
        logger.error(f"Failed to save manifest log for job {job_id}: {e_save}")

    if log_overall_success:
        logger.info(
            f"Job {job_id}: Manifest successfully verified. Returning VerifyingChunkMapper."
        )
        ...
        logger.error(
            f"Job {job_id}: Manifest verification failed. Cannot provide trusted store. Final status: {verification_details_log.get('final_status')}"
        )
        return None


def get_forecast_summary(zarr_store_url: str, variables: List[str]) -> Dict[str, Any]:
    """
...
def _get_canonical_bytes(
    ds_t0: xr.Dataset, ds_t_minus_6: xr.Dataset
) -> Optional[bytes]:
    """
    ...
    if not isinstance(ds_t0, xr.Dataset) or not isinstance(ds_t_minus_6, xr.Dataset):
        logger.error(
            "Invalid input: Both ds_t0 and ds_t_minus_6 must be xarray Datasets."
        )
        return None
    ...
    if len(ds_t0.time) != 1 or len(ds_t_minus_6.time) != 1:
        logger.error(
            "Invalid input: Datasets should contain only one time step for analysis."
        )
        return None
    ...
    for var in CANONICAL_VARS_FOR_HASHING:
        if var in ds_t_minus_6_squeezed:
            ...
            logger.debug(
                f"Added T-6h var '{var}' (shape: {data_array.shape}, dtype: {data_array.dtype}) to canonical bytes."
            )
        else:
            logger.warning(
                f"Required variable '{var}' missing in T-6h data for hashing."
            )
            pass
    ...
    for var in CANONICAL_VARS_FOR_HASHING:
        if var in ds_t0_squeezed:
            ...
            all_bytes_list.append(data_array.tobytes())
            logger.debug(
                f"Added T=0h var '{var}' (shape: {data_array.shape}, dtype: {data_array.dtype}) to canonical bytes."
            )
        else:
            logger.warning(
                f"Required variable '{var}' missing in T=0h data for hashing."
            )
            pass
    ...
    logger.info(
        f"Generated canonical byte representation: {len(canonical_bytes)} bytes."
    )
...
async def compute_input_data_hash(
    ...
    """
    logger.info(
        f"Computing input data hash for T0={t0_run_time}, T-6={t_minus_6_run_time}"
    )
    ds_t0 = None
    ds_t_minus_6 = None
    try:
        ...
        if ds_t_minus_6_override is not None:
            ...
            logger.info(
                f"Fetching GFS T-6 data ({t_minus_6_run_time}) for hash computation."
            )
            ds_t_minus_6 = await fetch_gfs_analysis_data(
                [t_minus_6_run_time], cache_dir=cache_dir
            )
        ...
        canonical_bytes = await asyncio.to_thread(
            _get_canonical_bytes, ds_t0.copy(deep=True), ds_t_minus_6.copy(deep=True)
        )
        ...
        if ds_t0_override is None and ds_t0 is not None and hasattr(ds_t0, "close"):
            try:
                ...
                pass
        if (
            ds_t_minus_6_override is None
            and ds_t_minus_6 is not None
    ...
    for var in variables_to_include:
        if var in rev_mapping:
            ...
            if ds_var in ds:
                ...
                logger.debug(
                    f"Found {var} via mapping as {ds_var} with shape {ds[ds_var].shape}"
                )
                continue
        ...
        variations = [var]
        if var == "10u":
            variations.append("u10")
        if var == "10v":
            variations.append("v10")
        if var == "2t":
            variations.append("t2m")
        ...
        if not found:
            logger.warning(
                f"Variable '{var}' not found in dataset by any name. Cannot determine shape for hashing."
            )

    logger.info(
        f"_get_data_shape_from_xarray_dataset: Populated data_shape: {data_shape}"
    )
    return data_shape
...
def _efficient_canonical_serialization(
    ...
    logger.info(
        f"SERIALIZATION: Starting serialization for {len(sample_indices)} sample indices."
    )
    ...
    for aurora_name in CANONICAL_VARS_FOR_HASHING:
        if aurora_name not in var_map:
            logger.warning(
                f"SERIALIZATION: Could not find any matching variable for {aurora_name}"
            )
    ...
    for i, idx_info in enumerate(sorted_indices):
        ...
        try:
            ...
            if idx_info["timestep"] >= dataset.sizes[time_dim]:
                logger.warning(
                    f"{log_prefix} - Time index {idx_info['timestep']} out of bounds (max: {dataset.sizes[time_dim] - 1}). Using NaN."
                )
                serialized.extend(serialize_float(float("nan")))
                continue
            ...
            if idx_info["lat"] >= dataset.sizes[lat_dim]:
                logger.warning(
                    f"{log_prefix} - Lat index {idx_info['lat']} out of bounds (max: {dataset.sizes[lat_dim] - 1}). Using NaN."
                )
                serialized.extend(serialize_float(float("nan")))
                continue
            ...
            if idx_info["lon"] >= dataset.sizes[lon_dim]:
                logger.warning(
                    f"{log_prefix} - Lon index {idx_info['lon']} out of bounds (max: {dataset.sizes[lon_dim] - 1}). Using NaN."
                )
                serialized.extend(serialize_float(float("nan")))
                continue
            ...
            if (
                category == "atmos_vars"
                and level_dim in dataset.sizes
                and idx_info["level"] >= dataset.sizes[level_dim]
            ):
                logger.warning(
                    f"{log_prefix} - Level index {idx_info['level']} out of bounds (max: {dataset.sizes[level_dim] - 1}). Using NaN."
                )
                serialized.extend(serialize_float(float("nan")))
                continue
            ...
            logger.debug(
                f"{log_prefix} - Attempting .isel() with kwargs: {isel_kwargs}"
            )
            ...
            if hasattr(data_array, "values"):
                ...
                if hasattr(point_value, "item") and point_value.size == 1:
                    ...
                    logger.warning(
                        f"{log_prefix} - Got non-scalar array of shape {point_value.shape}. Using first element."
                    )
            ...
            try:
                if isinstance(value, (float, int)):
                    ...
                    value_str = str(value)
                logger.debug(
                    f"{log_prefix} - Successfully fetched point. Value: {value_str}. Time: {point_fetch_duration:.3f}s"
                )
            except Exception:
                logger.debug(
                    f"{log_prefix} - Successfully fetched point. Value type: {type(value)}. Time: {point_fetch_duration:.3f}s"
                )
            ...
            logger.warning(
                f"{log_prefix} - Error serializing point: {e_serial}. Using NaN.",
                exc_info=True,
            )
            serialized.extend(serialize_float(float("nan")))
    ...
    logger.info(
        f"SERIALIZATION: Finished. Total bytes: {len(serialized)}. Total time: {total_serialization_duration:.3f}s"
    )
    return bytes(serialized)
...
class VerifyingChunkMapper(fsspec.mapping.FSMap):
    def __init__(
        ...
        self.chunk_hash_algorithm = trusted_manifest.get(
...
def _verify_manifest_integrity_sync(
    ...
    verification_details_log = {
        "job_id": job_id,
        "zarr_store_url_checked": zarr_root_on_server,
        "claimed_manifest_content_hash": claimed_manifest_content_hash,
        "expected_signer_hotkey": expected_signer_hotkey,
        "verification_timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "steps": [],
        "final_status": "pending_sync_verification",
    }
    ...
    try:
        ...
        manifest_json_bytes = fs.cat(manifest_path)
        logger.info(
            f"Job {job_id}: Fetched manifest.json ({len(manifest_json_bytes)} bytes)"
        )

        logger.info(f"Job {job_id}: Fetching signature from {sig_path}")
        signature_bytes = fs.cat(sig_path)
        logger.info(
            f"Job {job_id}: Fetched manifest.sig ({len(signature_bytes)} bytes)"
        )
        ...
        if computed_m_hash != claimed_manifest_content_hash:
            raise ValueError(
                f"Manifest content hash mismatch! Claimed: {claimed_manifest_content_hash[:10]}..., Computed: {computed_m_hash[:10]}..."
            )
        ...
        if not manifest_signer or manifest_signer != expected_signer_hotkey:
            raise ValueError(
                f"Manifest signer hotkey mismatch. Expected: {expected_signer_hotkey}, Got: {manifest_signer}"
            )

        logger.info(
            f"Job {job_id}: Manifest signer hotkey in manifest matches expected: {manifest_signer}"
        )
        verification_details_log["steps"].append(
            {"name": "VerifyManifestSigner", "status": "success"}
        )
        ...
        logger.info(
            f"Job {job_id}: Manifest signature verified successfully for hotkey {expected_signer_hotkey}."
        )
        verification_details_log["steps"].append(
            {"name": "VerifyManifestSignature", "status": "success"}
        )
        ...
        logger.error(
            f"Job {job_id}: File not found (manifest/sig) in _verify_manifest_integrity_sync: {e_fnf}"
        )
        verification_details_log["steps"].append(
            {"name": "FetchManifestAndSig", "status": "failed", "reason": str(e_fnf)}
        )
        verification_details_log["final_status"] = "error_file_not_found"
        ...
        if "Manifest content hash mismatch" in str(e_verify):
            step_name = "VerifyManifestContentHash"
        elif "Manifest signer hotkey mismatch" in str(e_verify):
            step_name = "VerifyManifestSigner"
        elif "Manifest signature verification failed" in str(e_verify):
            step_name = "VerifyManifestSignature"
        verification_details_log["steps"].append(
            {"name": step_name, "status": "failed", "reason": str(e_verify)}
        )
        verification_details_log["final_status"] = (
            f"failed_{step_name.lower().replace('verify', '')}"
        )

    except Exception as e_inner_sync:
        logger.error(
            f"Job {job_id}: Unexpected error in _verify_manifest_integrity_sync: {e_inner_sync}",
            exc_info=True,
        )
        verification_details_log["steps"].append(
            {
...
async def get_trusted_manifest(
    ...
    try:
        ...
        logger.error(
            f"Job {job_id}: Failed to initialize fsspec for manifest: {e_fs_init}"
        )
        verification_log = {
            "job_id": job_id,
            "final_status": "error_fs_init",
            "reason": str(e_fs_init),
        }
    if fs:
        try:
            loop = asyncio.get_running_loop()
            (
                trusted_manifest_dict,
                verification_log_from_executor,
            ) = await loop.run_in_executor(
                None,
                _verify_manifest_integrity_sync,
                fs,
                zarr_store_url_cleaned,
            ...
            logger.error(
                f"Job {job_id}: Executor call for manifest verification failed: {e_exec}",
                exc_info=True,
            )
            verification_log = verification_log or {"job_id": job_id, "steps": []}
            verification_log["steps"].append(
                {"name": "ExecutorCall", "status": "error", "reason": str(e_exec)}
            )
            verification_log["final_status"] = "error_executor_call"
            trusted_manifest_dict = None

    log_overall_success = trusted_manifest_dict is not None
    if not isinstance(verification_log, dict):
        verification_log = {}
    for key_to_ensure in [
        "job_id",
        "zarr_store_url_checked",
        "claimed_manifest_content_hash",
        "expected_signer_hotkey",
        "verification_timestamp_utc",
        "steps",
        "final_status",
    ]:
        if key_to_ensure not in verification_log:
            if key_to_ensure == "job_id":
                verification_log[key_to_ensure] = job_id
            elif key_to_ensure == "zarr_store_url_checked":
                verification_log[key_to_ensure] = zarr_store_url_cleaned
            elif key_to_ensure == "claimed_manifest_content_hash":
                verification_log[key_to_ensure] = claimed_manifest_content_hash
            elif key_to_ensure == "expected_signer_hotkey":
                verification_log[key_to_ensure] = miner_hotkey_ss58
            elif key_to_ensure == "verification_timestamp_utc":
                verification_log[key_to_ensure] = datetime.now(timezone.utc).isoformat()
            elif key_to_ensure == "steps":
                verification_log[key_to_ensure] = []
            elif key_to_ensure == "final_status" and log_overall_success:
                verification_log[key_to_ensure] = "manifest_verified_ok"
            elif (
                key_to_ensure == "final_status"
                and not log_overall_success
                and not verification_log.get("final_status")
            ):
                verification_log[key_to_ensure] = "failed_unknown"
    ...
    if log_overall_success:
        ...
        logger.error(
            f"Job {job_id}: Manifest integrity FAILED. Final status: {verification_log.get('final_status')}"
        )
        return None
