...
class WeatherMultiprocessingConfig:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        env_mappings = {
            ...
            "WEATHER_MP_ENABLE_PERF_LOGGING": (
        ...
        for env_var, (config_key, converter) in env_mappings.items():
            ...
            if env_value is not None:
                try:
                    ...
                    logger.warning(
                        f"Invalid value for {env_var}='{env_value}': {e}, using default"
                    )

        # Validation
        config["max_workers"] = max(1, min(config["max_workers"], mp.cpu_count()))
        config["min_workers"] = max(
            1, min(config["min_workers"], config["max_workers"])
        )
        config["memory_threshold_mb"] = max(
            1000, config["memory_threshold_mb"]
        )  # At least 1GB
    ...
    def _log_config(self):
        ...
        logger.info(
            f"   Max Workers: {self.config['max_workers']}/{mp.cpu_count()} CPUs"
        )
        ...
        logger.info(
            f"   Performance Logging: {self.config['enable_performance_logging']}"
        )

        if self.config["enable_performance_logging"]:
            logger.info("   Multiprocessing Thresholds:")
            logger.info(
                f"     Climatology: ≥{self.config['climatology_mp_threshold']} operations"
            )
            logger.info(
                f"     Metrics: ≥{self.config['metrics_mp_threshold']} array elements"
            )
            logger.info(
                f"     Interpolation: ≥{self.config['interpolation_mp_threshold']} array elements"
            )
...
def log_optimization_summary():
    ...
    if config.get("enable_performance_logging"):
        logger.info("")
        logger.info(
            "   📊 Performance logging is ENABLED - you'll see detailed speedup metrics"
        )
    else:
        logger.info("")
        logger.info(
            "   📊 Performance logging is DISABLED - set WEATHER_MP_ENABLE_PERF_LOGGING=true to enable"
        )
