...
class PerMinerMemoryManager:
    ...
    def cleanup_between_miners(self, miner_hotkey: str, run_id: int, force_aggressive: bool = False) -> Dict[str, Any]:
        """
        ...
        logger.debug(f"[{self.worker_name}] Run {run_id}: Starting per-miner cleanup after {miner_hotkey}")
        ...
        try:
            ...
            if self.cleanup_count % 10 == 0:  # Every 10th miner
                if hasattr(sys, '_clear_type_cache'):
                    ...
                    logger.debug(f"[{self.worker_name}] Run {run_id}: Cleared Python type cache")
            
            logger.debug(f"[{self.worker_name}] Run {run_id}: Per-miner cleanup for {miner_hotkey}: "
            ...
            logger.debug(f"[{self.worker_name}] Run {run_id}: Error in per-miner cleanup for {miner_hotkey}: {cleanup_err}")
    ...
    def _clear_targeted_caches(self, aggressive: bool = False) -> Dict[str, int]:
        ...
        priority_modules = [
            'xarray', 'numpy', 'scipy', 'pandas', 'netCDF4', 'zarr', 'dask',
            'xskillscore', 'matplotlib', 'numba', 'sklearn',
        ...
        if aggressive:
            priority_modules.extend([
                'gaia.validator', 'gaia.miner', 'torch', 'tensorflow',
                'cfgrib', 'metpy', 'cartopy', 'pint'
            ])
                            ...
                            for attr_name in list(module.__dict__.keys()):
                                try:
                                    ...
                                    # Clear cache containers/objects
                                           ...
                                           ['cache', '_cached', '_memo', '_buffer', 'registry']):
...
def apply_global_lru_limits():
    ...
    try:
        ...
        weather_modules = [mod for mod in sys.modules.keys() if 'gaia.tasks.defined_tasks.weather' in mod]
...
def _apply_lru_limit_to_module_functions(module, module_name: str):
    ...
    for attr_name in dir(module):
        try:
            ...
            if (hasattr(attr, 'cache_info') and 
                hasattr(attr, 'cache_clear') and
                callable(attr.cache_info)):
                ...
                if cache_info.maxsize is None:
                    ...
                    if hasattr(attr, '__wrapped__'):
                        ...
                        limited_func = functools.lru_cache(maxsize=DEFAULT_LRU_MAXSIZE)(original_func)
    ...
    if limited_count > 0:
        logger.debug(f"Applied LRU limits to {limited_count} functions in {module_name}")
...
def create_memory_aware_lru_cache(maxsize: Optional[int] = None):
    ...
    try:
        ...
        if memory_percent > 85:  # High memory usage
            ...
            logger.debug(f"High memory usage ({memory_percent}%), reducing LRU cache size to {maxsize}")
