...
def _synchronous_zarr_open_unverified(
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                "SYNC_ZARR_OPEN_UNVERIFIED: Blosc codec test successful in executor thread"
            )
    ...
    try:
        ...
        if consolidated_flag:
            ...
            logger.warning(
                "SYNC_ZARR_OPEN_UNVERIFIED: .zmetadata not found. Attempting non-consolidated."
            )
            ...
            if not fs.exists(zgroup_path):
                ...
                return None
            logger.info(
                "SYNC_ZARR_OPEN_UNVERIFIED: Found .zgroup. Will attempt to open as non-consolidated store."
            )
...
def _synchronous_open_with_verifying_mapper(
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"Job {verifying_mapper.job_id_for_logging}: Blosc codec test successful in executor thread"
            )
        except Exception as codec_err:
            logger.warning(
                f"Job {verifying_mapper.job_id_for_logging}: Codec test failed: {codec_err}"
            )
