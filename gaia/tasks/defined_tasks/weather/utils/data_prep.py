...
def download_static_pickle(
    download_dir: str = ".", resolution: str = "0.25", force_download: bool = False
) -> str:
    """
    ...
    if resolution not in static_urls:
        raise ValueError(
            f"Unsupported resolution: {resolution}. Choose from {list(static_urls.keys())}"
        )
    ...
    if os.path.exists(pickle_file) and force_download:
        logger.info(
            f"Forcing re-download of static pickle file. Deleting {pickle_file}."
        )
        try:
    ...
    if not os.path.exists(pickle_file):
        logger.info(
            f"Downloading Aurora static variables pickle for {resolution}° resolution"
        )
        ...
        if response.status_code == 200:
            ...
            raise RuntimeError(
                f"Failed to download static variables: HTTP {response.status_code}"
            )
    else:
    ...
    try:
        ...
        for var in required_vars:
            if var not in static_data:
                raise KeyError(
                    f"Required static variable '{var}' not found in pickle file"
                )
...
def prepare_aurora_batch(
    ...
    for var_name in ["t", "u", "v", "q", "z"]:
        if var_name in gfs_data and "lev" in gfs_data[var_name].dims:
            ...
            gfs_data[var_name] = gfs_data[var_name].sel(
                lev=aurora_pressure_levels, method="nearest"
            )

    if "time" in gfs_data.dims and len(gfs_data.time) < history_steps:
        raise ValueError(
            f"Need at least {history_steps} time steps, but only have {len(gfs_data.time)}"
        )
    ...
    for var_name in atmos_var_names:
        if var_name in gfs_data and "pressure_level" in gfs_data[var_name].dims:
            ...
            logger.warning(
                f"Atmospheric variable {var_name} missing expected 'pressure_level' dimension"
            )
        else:
...
def create_aurora_batch_from_gfs(
    ...
    if static_pickle_path is None:
        static_pickle_path = download_static_pickle(
            download_dir, resolution, force_download=force_download_static
        )
