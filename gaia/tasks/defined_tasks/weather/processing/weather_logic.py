            ...
            if ensemble_details and (
                ...
                try:
                    ...
                    if local_ground_truth_ds:
                        ...
                        logger.info(
                            "[build_score_row] Calling calculate_era5_ensemble_score..."
                        )
                        ...
                        if ensemble_score is not None:
                            ...
                            logger.warning(
                                "[build_score_row] Ensemble scoring failed or returned None."
                            )
                    else:
                        logger.warning(
                            "[build_score_row] Could not retrieve/receive ground truth data. Cannot score ensemble."
                        )
...
async def get_job_by_gfs_init_time(
    task_instance: "WeatherTask", gfs_init_time_utc: datetime
    ...
    try:
        ...
        job = await task_instance.db_manager.fetch_one(
            query, {"gfs_init_time": gfs_init_time_utc}
        )
        return job
    except Exception as e:
        logger.error(
            f"Error checking for existing job with GFS init time {gfs_init_time_utc}: {e}"
        )
        return None
...
async def verify_miner_response(
    ...
    try:
        ...
        if token_data_tuple is None:
            ...
            logger.warning(
                f"[VerifyLogic, Resp {response_id}] Miner {miner_hotkey} job {job_id} not ready for verification (still processing or failed). Skipping."
            )
            await task_instance.db_manager.execute(
                """
...
async def calculate_era5_miner_score(
    ...
    try:
        ...
        if token_data_tuple is None:
            raise ValueError(
                f"Failed to get fresh access token for {miner_hotkey} job {job_id}. Cannot ensure Zarr URL is current."
            )
        ...
        if miner_forecast_ds is None:
            raise ConnectionError(
                f"Failed to open verified Zarr dataset for miner {miner_hotkey}"
            )

        gfs_operational_fcst_ds = None
        if gfs_init_time_of_run:
            try:
                unique_lead_hours_for_gfs_fetch = sorted(
                    list(
                        set(
                            [
                                int((vt - gfs_init_time_of_run).total_seconds() / 3600)
                                for vt in target_datetimes
                            ]
                        )
                    )
                )

                gfs_vars_to_request = list(
                    set(
                        AURORA_TO_GFS_VAR_MAP.get(vc["name"])
                        for vc in final_scoring_config["variables_levels_to_score"]
                        if AURORA_TO_GFS_VAR_MAP.get(vc["name"])
                    )
                )  # Get GFS names

                gfs_surface_vars_req = [
                    v for v in gfs_vars_to_request if v in GFS_SURFACE_VARS
                ]
                gfs_atmos_vars_req = [
                    v for v in gfs_vars_to_request if v in GFS_ATMOS_VARS
                ]
                pressure_levels_req = list(
                    set(
                        vc.get("level")
                        for vc in final_scoring_config["variables_levels_to_score"]
                ...
                logger.info(
                    f"[FinalScore] Fetching GFS operational reference: Init={gfs_init_time_of_run}, Leads={unique_lead_hours_for_gfs_fetch}, PSurf={gfs_surface_vars_req}, PAtm={gfs_atmos_vars_req}, PLevels={pressure_levels_req}"
                )
                gfs_operational_fcst_ds = await fetch_gfs_data(
                    run_time=gfs_init_time_of_run,
                    lead_hours=unique_lead_hours_for_gfs_fetch,
                    target_surface_vars=gfs_surface_vars_req
                    if gfs_surface_vars_req
                    else None,
                    target_atmos_vars=gfs_atmos_vars_req
                    if gfs_atmos_vars_req
                    else None,
                    target_pressure_levels_hpa=pressure_levels_req,
                )
            except Exception as e_fetch_gfs_ref:
                logger.error(
                    f"[FinalScore] Miner {miner_hotkey}: Error fetching GFS operational reference forecast: {e_fetch_gfs_ref}",
                    exc_info=True,
                )
                gfs_operational_fcst_ds = None
        ...
        if gfs_operational_fcst_ds is None:
            logger.warning(
                f"[FinalScore] Miner {miner_hotkey}: Failed to fetch/process GFS operational reference forecast. Skill score vs GFS will not be available."
            )
                ...
                try:
                    ...
                    if var_name == "z" and var_level == 500:
                        ...
                        if (
                            ...
                            logger.info(
                                "[FinalScore] Unit check OK: Both miner and truth z500 appear to be geopotential (m²/s²)"
                            )
                    ...
                    if var_name == "z" and miner_mean < 10000 and truth_mean > 40000:
                        logger.warning(
                            "[FinalScore] AUTOMATIC UNIT CONVERSION: Converting miner z from geopotential height (m) to geopotential (m²/s²)"
                        )
                    ...
                    # Check for pressure unit conversions (hPa to Pa)
                        ...
                        logger.warning(
                            "[FinalScore] AUTOMATIC UNIT CONVERSION: Converting miner msl from hPa to Pa"
                        )
                    ...
                    if gfs_op_lead_slice is not None:
                        ...
                        logger.warning(
                            f"[FinalScore] UID {miner_uid} - No GFS forecast data available for skill score calculation"
                        )
...
async def _calculate_and_store_aggregated_era5_score(
        ...
        for var_config in vars_levels_scored:
            ...
            if (
                ...
                if (
                    ...
                    logger.info(
                        f"[AggFinalScore] UID {miner_uid} - Using CLIM-based skill for {var_key} L{lead_h}h: {skill_score_val:.4f} (GFS-based not found/valid)"
                    )
                else:
                    logger.warning(
                        f"[AggFinalScore] UID {miner_uid} - No valid GFS or CLIM skill score found for {var_key} L{lead_h}h."
                    )
    ...
    avg_skill_error_score = (
        sum(skill_error_component_scores) / len(skill_error_component_scores)
        if skill_error_component_scores
        else 0.0
    )
    avg_acc_score = (
        sum(acc_component_scores) / len(acc_component_scores)
        if acc_component_scores
        else 0.0
    )
    ...
    logger.info(
        f"[AggFinalScore] UID {miner_uid}, Run {run_id}: AvgNormSkill/Error={avg_skill_error_score:.4f} (from {len(skill_error_component_scores)} items), AvgNormACC={avg_acc_score:.4f} (from {len(acc_component_scores)} items) => Final Composite Score: {final_score_val:.4f}"
    )
