...
def _prepare_http_payload_sync(prepared_batch_for_http: Batch) -> bytes:
    logger.debug("SYNC: Serializing Aurora Batch for HTTP service...")
...
async def run_inference_background(task_instance: "WeatherTask", job_id: str):
    ...
    try:
        ...
        if gfs_init_time:
            ...
            """
            duplicate_job = await task_instance.db_manager.fetch_one(
                duplicate_check_query,
            ...
            if duplicate_job:
                logger.warning(
                    f"[InferenceTask Job {job_id}] Found existing job {duplicate_job['id']} for same timestep {gfs_init_time} with status '{duplicate_job['status']}'. Aborting duplicate inference."
                )
    ...
    try:
        ...
        if current_inference_type == "http_service":
            ...
            logger.info(
                f"[InferenceTask Job {job_id}]   - ds_t0 dims: {dict(ds_t0.dims)}"
            )
            logger.info(
                f"[InferenceTask Job {job_id}]   - ds_t_minus_6 variables: {list(ds_t_minus_6.data_vars.keys())}"
            )
            logger.info(
                f"[InferenceTask Job {job_id}]   - ds_t_minus_6 dims: {dict(ds_t_minus_6.dims)}"
            )
            ...
            logger.info(
                f"[InferenceTask Job {job_id}]   - Combined dims: {dict(gfs_concat_data_for_batch_prep.dims)}"
            )
            logger.info(
            ...
            try:
                ...
                try:
                    ...
                    precomputed_climatology = await precompute_climatology_cache(
                        ...
                        else sample_var,  # Remove time dimension for template
                    )
                                    ...
                                    if any(
                                        ...
                                        try:
                                            ...
                                            if hasattr(cache_obj, "clear"):
                                                cache_obj.clear()
                                                logger.debug(
                                                    f"[Day1ScoringWorker] Run {run_id}: Cleared {module_name}.{attr_name}"
                                                )
                ...
                for i, result in enumerate(evaluation_results):
                    ...
                    if result and isinstance(result, dict):
                        ...
                        lead_scores_json = await asyncio.to_thread(
                            json.dumps, result.get("lead_time_scores"), default=str
                        )
                        ...
                        if resp_id is not None:
                            db_params = {
                                ...
                                "valid_time_utc_val": run_record["gfs_init_time_utc"]
                                if run_record
                                else None,
                            }
                ...
                if db_update_tasks:
                    try:
                        ...
                        logger.error(
                            f"[Day1ScoringWorker] Run {run_id}: Error storing Day-1 QC scores to DB: {db_store_err}",
                            exc_info=True,
                        )
    ...
    try:
        while self.final_scoring_worker_running:
            ...
            try:
                ...
                max_final_lead_hour = (
                    max(sparse_lead_hours_config) if sparse_lead_hours_config else 0
                )

                if self.test_mode:
                    logger.info(
                        "[FinalizeWorker] TEST MODE: Ignoring ERA5 delay, checking all runs for final scoring"
                    )
                    runs_to_score_query = """
                    ...
                    LIMIT 10 
                    """
                    ready_runs = await self.db_manager.fetch_all(
                        runs_to_score_query, {}
                    )
                else:
                    init_time_cutoff = (
                        ...
                        - timedelta(hours=max_final_lead_hour)
                ...
                for run in ready_runs:
                    ...
                    target_datetimes_for_run = [
                        gfs_init_time + timedelta(hours=h)
                        for h in sparse_lead_hours_config
                    ]

                    logger.info(
                        f"[FinalizeWorker] Run {run_id}: Fetching ERA5 analysis for final scoring at lead hours: {sparse_lead_hours_config}."
                    )
                    era5_cache = Path(self.config.get("era5_cache_dir", "./era5_cache"))
                    era5_ds_for_run = await fetch_era5_data(
                        target_times=target_datetimes_for_run, cache_dir=era5_cache
                    )  # Ensure fetch_era5_data is efficient

                    if era5_ds_for_run is None:
                        logger.error(
                            f"[FinalizeWorker] Run {run_id}: Failed to fetch ERA5 data. Aborting final scoring for this run."
                        )
                        await _update_run_status(
                            self,
                            run_id,
                            "final_scoring_failed",
                            error_message="ERA5 fetch failed",
                        )
                    ...
                    miner_scoring_results = await asyncio.gather(
                        *scoring_execution_tasks
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            f"[FinalizeWorker] Run {run_id}: Cleared {cache_objects_cleared} cache objects from {modules_cleared} modules"
                        )
                    ...
                    for i_resp, miner_score_task_succeeded in enumerate(
                        ...
                        if miner_score_task_succeeded:
                            ...
                            logger.warning(
                                f"[FinalizeWorker] Run {run_id}: Skipping aggregated score for UID {resp_rec_inner['miner_uid']} (detailed scoring failed)."
                            )
                        if len(verified_responses_for_run) > 10 and i_resp % 10 == 9:
                            await asyncio.sleep(0)

                    logger.info(
                        f"[FinalizeWorker] Run {run_id}: Completed final scoring for {successful_final_scores_count}/{len(verified_responses_for_run)} miners."
                    )

                    if successful_final_scores_count > 0:
                        logger.info(
                            f"[FinalizeWorker] Run {run_id}: Marked as 'scored'."
                        )
            ...
            try:
                ...
                try:
                    ...
                    if result and hasattr(result, "rowcount"):
                        ...
                        logger.info(
                            "[CleanupWorker] Executed old run deletion query (actual rowcount might not be available from driver)."
                        )
