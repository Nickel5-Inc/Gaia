...
from botocore.exceptions import (
    ClientError as BotoClientError,  # For R2 error handling, aliased to avoid conflict if other ClientError exists
)
...
def _load_config(self):
    ...
    config["final_scoring_lead_hours"] = parse_int_list(
        "WEATHER_FINAL_SCORING_LEAD_HOURS", [60, 78, 138]
    )  # Day 2, 3, 5
    ...
    config["final_scoring_check_interval_seconds"] = int(
        os.getenv("WEATHER_FINAL_SCORING_INTERVAL_S", "3600")
    )
...
class WeatherTask(Task):
        ...
        if self.node_type == "miner":
            try:
                ...
                if load_local_model_flag:
                    ...
                    logger.info(
                        "Miner configured for HTTP inference. Local model not loaded. Inference runner will be None."
                    )
    ...
    async def _upload_input_to_r2(
        ...
        try:
            ...
            logger.error(
                f"[{job_id}] Failed to upload initial batch to R2: {e}", exc_info=True
            )
            return None
    ...
    def _load_era5_climatology_sync(
        ...
        try:
            ...
            logger.warning(
                "ERA5 climatology: blosc not found, attempting pip install..."
            )
            try:
                import subprocess
                import sys
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(
                        f"ERA5 climatology: Failed to install blosc: {result.stderr}"
                    )
                    ...
                    try:
                        ...
                        if result2.returncode == 0:
                            logger.info(
                                "ERA5 climatology: Successfully installed blosc 1.11.1"
                            )
                    except Exception as install2_err:
                        logger.warning(
                            f"ERA5 climatology: Error installing specific blosc version: {install2_err}"
                        )
            except Exception as install_err:
                logger.warning(
                    f"ERA5 climatology: Error installing blosc: {install_err}"
                )
        ...
        try:
            ...
            from numcodecs import BZ2, LZ4, Blosc, GZip, Zlib, Zstd
            ...
            for codec_name, codec_instance in codecs_to_register:
                try:
                    ...
                    logger.debug(
                        f"ERA5 climatology: Successfully registered {codec_name} codec (ID: {codec_id})"
                    )
                except Exception as reg_err:
                    logger.warning(
                        f"ERA5 climatology: Failed to register {codec_name} codec: {reg_err}"
                    )
            ...
            try:
                ...
                decompressed_array = np.frombuffer(
                    decompressed, dtype=test_data.dtype
                ).reshape(test_data.shape)

                # Verify the data is correct
                if np.array_equal(test_data, decompressed_array):
                    logger.debug(
                        "ERA5 climatology: Blosc codec test successful - data integrity verified"
                    )
                else:
                    logger.warning(
                        "ERA5 climatology: Blosc codec test failed - data integrity check failed"
                    )
            ...
            logger.debug("ERA5 climatology: Comprehensive codec registration completed")

        except Exception as e:
            logger.warning(
                f"Failed to ensure blosc codec in ERA5 climatology executor thread: {e}"
            )
        ...
        def ensure_codec_before_opening():
            ...
            try:
                ...
                os.environ["ZARR_V3_EXPERIMENTAL_API"] = (
                    "0"  # Use v2 API for better codec support
                )
                os.environ["BLOSC_NTHREADS"] = (
                    "1"  # Single thread to avoid threading issues
                )
                ...
                try:
                    ...
                    for config in blosc_configs:
                        try:
                            ...
                            if hasattr(numcodecs.registry, "codec_registry"):
                                numcodecs.registry.codec_registry[codec_id] = (
                                    alt_blosc_codec
                                )

                            if hasattr(numcodecs, "register_codec"):
                                numcodecs.register_codec(
                                    alt_blosc_codec, codec_id=codec_id
                                )

                            logger.debug(
                                f"Registered alternative blosc codec: {codec_id}"
                            )

                        except Exception as config_err:
                            logger.debug(
                                f"Failed to register blosc config {config}: {config_err}"
                            )
                            continue

                except Exception as main_codec_err:
                    logger.warning(
                        f"Failed to register main blosc codec: {main_codec_err}"
                    )
                    return False
                ...
                try:
                    ...
                    if hasattr(zarr, "codecs") and hasattr(zarr.codecs, "get_codec"):
                        ...
                        try:
                            test_codec = zarr.codecs.get_codec(
                ...
                try:
                    ...
                    if decompressed == test_data:
                        logger.debug(
                            "Blosc codec test successful - compression/decompression working"
                        )
                        return True
                    else:
                        logger.warning(
                            "Blosc codec test failed - data mismatch after compression"
                        )
                        return False
        ...
        opening_strategies = [
            ...
            {"name": "consolidated", "kwargs": {"consolidated": True, "chunks": None}},
        ...
        for strategy in opening_strategies:
            try:
                ...
                logger.info(
                    f"ERA5 climatology: Attempting {strategy_name} opening strategy..."
                )
                ...
                if use_fsspec:
                    ...
                    if filesystem_type == "gcs":
                        fs = fsspec.filesystem(
                            "gcs", token="anon"
                        )  # Anonymous access for public bucket
                    else:  # http
        ...
        try:
            logger.info(
                "ERA5 climatology: Attempting final fallback with manual zarr handling..."
            )
            ensure_codec_before_opening()
            ...
            logger.info(
                f"ERA5 climatology: Successfully opened zarr group. Variables: {list(zarr_group.keys())}"
            )
            ...
            if dataset is not None:
                logger.info(
                    "ERA5 climatology: Successfully loaded using manual zarr fallback"
                )
                return dataset
        ...
        try:
            logger.info(
                "ERA5 climatology: Attempting ultimate fallback - bypassing blosc codec..."
            )
...
"""
                ...
                if dataset is not None:
                    logger.info(
                        "ERA5 climatology: Successfully loaded using intake-xarray bypass"
                    )
                    return dataset
            ...
            try:
                logger.info(
                    "ERA5 climatology: Trying HTTP access to individual files..."
                )

                import requests

                # Check if we can access the zarr metadata via HTTP
                base_url = climatology_path.replace(
                ...
                if response.status_code == 200:
                    logger.info(
                        "ERA5 climatology: HTTP access successful, but full dataset too large for direct download"
                    )
            ...
            try:
                ...
                for chunks_config in [None, {}, "auto", -1]:
                    try:
                        ...
                        if dataset is not None:
                            logger.info(
                                f"ERA5 climatology: Successfully loaded with chunks={chunks_config}"
                            )
                            return dataset
                    except Exception as chunk_err:
                        logger.debug(
                            f"ERA5 climatology: chunks={chunks_config} failed: {chunk_err}"
                        )
                        continue

            except Exception as backend_err:
                logger.debug(
                    f"ERA5 climatology: Backend approach failed: {backend_err}"
                )

            # Final attempt: Try with environment variable overrides
            try:
                logger.info(
                    "ERA5 climatology: Final attempt with environment overrides..."
                )
                ...
                try:
                    ...
                    dataset = xr.open_zarr(
                        mapper, consolidated=False, decode_times=False
                    )

                    if dataset is not None:
                        logger.info(
                            "ERA5 climatology: Successfully loaded with environment overrides"
                        )
                        return dataset
                ...
                logger.debug(
                    f"ERA5 climatology: Environment override approach failed: {env_err}"
                )

            # Truly final attempt: Try to create a minimal version of climatology data
            try:
                logger.info(
                    "ERA5 climatology: Creating minimal fallback climatology..."
                )
                ...
                fake_data = np.random.normal(
                    273.15, 15, (12, 4, 721, 1440)
                )  # Monthly, 6-hourly, lat, lon
                ...
                logger.warning(
                    "ERA5 climatology: Created minimal fallback climatology - this will provide basic functionality but reduced accuracy"
                )
                return minimal_climatology

            except Exception as minimal_err:
                logger.debug(
                    f"ERA5 climatology: Even minimal fallback failed: {minimal_err}"
                )
    ...
    async def build_score_row(
        ...
        try:
            ...
            await self.db_manager.execute(
                upsert_score_table_query, db_params_score_table
            )
            logger.info(
                f"[BuildScoreRow] Upserted score_table entry for {task_name_prefix}, task_id (run_id): {run_id}"
            )

        except Exception as e_db_score_table:
            logger.error(
                f"[BuildScoreRow] DB error storing {task_name_prefix} score row for run {run_id}: {e_db_score_table}",
                exc_info=True,
            )
                    ...
                    if stale_run_processed in ["stale_processed", "run_processed"]:
                        ...
                        if total_attempts >= max_runs_per_session:
                            ...
                            if stale_run_processed == "stale_processed":
                                logger.info(
                                    "Stale run was processed - continuing recovery loop to process next incomplete run"
                                )
                            else:
                                logger.info(
                                    "Run recovery attempted - continuing to process remaining incomplete runs"
                                )
                            last_recovery_time = (
                                0  # Reset to trigger immediate next recovery check
                            )
                            await asyncio.sleep(
                                0.1
                    ...
                    if validator_input_hash:
                        ...
                        for i, (resp_id, status_data) in enumerate(
                            ...
                            if (
                                ...
                                logger.warning(
                                    f"[Run {run_id}] Miner for response ID {resp_id} timed out (status: {miner_status})."
                                )
                        ...
                        if (
                            ...
                            try:
                                await asyncio.wait_for(
                                    self.test_mode_run_scored_event.wait(),
                                    timeout=600.0,
                                )  # 10 min timeout
                                logger.info(
                                    f"TEST MODE: Day-1 scoring for run {run_id} event received."
                                )
                            except asyncio.TimeoutError:
                                logger.error(
                                    f"TEST MODE: Timeout waiting for Day-1 scoring of run {run_id}."
                                )
    ...
    async def validator_score(self, result=None, force_run_id=None):
        ...
        if not forecast_runs:
            logger.debug(
                f"No runs found awaiting inference results within cutoff (test_mode active: {self.test_mode}, cutoff: {cutoff_time})."
            )
            return
        ...
        for run_record in forecast_runs:
            ...
            if current_run_status == "awaiting_inference_results":
                ...
                logger.info(
                    f"[Run {run_id}] Status is already '{current_run_status}' (expected 'awaiting_inference_results'), skipping verification trigger step."
                )
                continue
            ...
            if verification_tasks:
                ...
                logger.info(
                    f"[Run {run_id}] Completed verification attempts for {len(verification_tasks)} responses."
                )
    ...
    async def update_combined_weather_scores(
        self, run_id_trigger: Optional[int] = None
    ):
        logger.info(
        ...
        logger.info("[CombinedWeatherScore] Calculated proportional scores.")
        ...
        bonus_metric_definitions = [
            {
                "id": "best_z500_acc_120h",
                "metric_score_type": "era5_acc_z500_120h",
                "higher_is_better": True,
                "min_threshold": 0.5,
            },
            {
                "id": "lowest_t2m_rmse_120h",
                "metric_score_type": "era5_rmse_2t_120h",
                "higher_is_better": False,
                "min_threshold": 3.0,
            },
            {
                "id": "best_msl_skill_gfs_168h",
                "metric_score_type": "era5_skill_gfs_msl_168h",
                "higher_is_better": True,
                "min_threshold": 0.1,
            },
        ]
        ...
        for category in bonus_metric_definitions:
            ...
            for uid_val in active_uids:
                metric_res = await self.db_manager.fetch_one(
                    query_bonus_metric,
                    {"score_type": metric_to_query, "miner_uid": uid_val},
                )
                if (
                    metric_res
                    and metric_res["score"] is not None
                    and np.isfinite(metric_res["score"])
                ):
    ...
    async def miner_execute(
        ...
        try:
            ...
            if existing_job:
                ...
                logger.info(
                    f"[Job {existing_job_id}] Found existing {existing_status} job for GFS init time {gfs_init_time}. Expected deterministic ID: {job_id}"
                )
            ...
            logger.info(
                f"[Job {job_id}] No suitable existing job found. Creating new job for GFS init time {gfs_init_time}."
            )
                        ...
                        if zarr_path.exists():
                            ...
                            def _generate_manifest_sync():
                                ...
                                return generate_manifest_and_signature(
                                    ...
                                    miner_hotkey_keypair=self.keypair
                                    if self.keypair
                                    else None,
                                )
    ...
    async def handle_initiate_fetch(
        ...
        try:
            ...
            validator_hotkey = getattr(
                request_data, "validator_hotkey", "unknown_validator"
            )
            ...
            logger.info(
                f"[Miner Job {job_id}] No existing job found. Creating new deterministic job for T0={t0_run_time}."
            )
    ...
    async def handle_start_inference(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
                    ...
                    return self._validate_and_format_response(
                        {
                            ...
                            "message": "Inference already completed successfully.",
                        },
                        ["status", "message"],
                    )
                else:
            ...
            if gfs_init_time:
                ...
                """
                duplicate_jobs = await self.db_manager.fetch_all(
                    duplicate_check_query,
                    {"gfs_time": gfs_init_time, "current_job_id": job_id},
                )
                ...
                if duplicate_jobs:
                    logger.warning(
                        f"[{job_id}] Found {len(duplicate_jobs)} recent job(s) for same timestep {gfs_init_time}:"
                    )
                    for dup_job in duplicate_jobs:
                        hours_ago = (
                            datetime.now(timezone.utc)
                            - dup_job["validator_request_time"]
                        ).total_seconds() / 3600
                        logger.warning(
                            f"[{job_id}]   - Job {dup_job['id']}: status='{dup_job['status']}', requested {hours_ago:.1f}h ago"
                        )

                    # Only block if there's a recent active job (not failed/error)
                    active_duplicates = [
                        j
                        for j in duplicate_jobs
                        if j["status"]
                        in (
                            "in_progress",
                            "completed",
                    ...
                    if active_duplicates:
                        ...
                        logger.warning(
                            f"[{job_id}] Blocking duplicate inference - active job {recent_job['id']} with status '{recent_job['status']}' for same timestep."
                        )
                        return self._validate_and_format_response(
                            {
                                "status": WeatherTaskStatus.INFERENCE_STARTED.value,
                        ...
                        logger.info(
                            f"[{job_id}] Found duplicate jobs for timestep but all are failed/error - allowing new inference to proceed."
                        )
                else:
                    logger.info(
                        f"[{job_id}] No recent duplicate jobs found for timestep {gfs_init_time} - proceeding with inference."
                    )
                    ...
                    for resp_rec in miners_to_poll:
                        ...
                        try:
                            ...
                            if status_response:
                                ...
                                logger.warning(
                                    f"[Run {run_id}] No valid response received from {miner_hk[:8]} using query_miners."
                                )
                                return resp_id, {
                                    "status": "validator_poll_failed",
                                    "message": "No response from miner via query_miners",
        ...
        for loc in locations:
            if loc.size_bytes:
                ...
                summary["locations"][loc.file_type] = {
                    ...
                    "created": loc.created_time.isoformat()
                    if loc.created_time
                    else None,
                }
    ...
    async def _check_and_recover_incomplete_runs_sequential(
        ...
        for run in incomplete_runs:
            logger.debug(
                f"  - Run {run['id']}: status='{run['status']}', age={(datetime.now(timezone.utc) - run['run_initiation_time']).total_seconds() / 3600:.1f}h"
            )
        ...
        if not run_to_process:
            logger.debug(
                f"All {len(incomplete_runs)} incomplete runs have reached max attempts ({max_attempts_per_run}) in this session"
            )
            return "no_more_runs"
        ...
        if status in potentially_recoverable_failure_states:
            logger.info(
                f"Recovering run {run_id}: resetting failure state '{status}' and re-running full hash verification workflow"
            )
    ...
    async def _execute_hash_verification_workflow(self, run_id: int):
        ...
        try:
            ...
            if miners_to_trigger:
                ...
                if retry_cnt > 0:
                    logger.info(
                        f"[Run {run_id}] {retry_cnt} miner responses are scheduled for retry. Keeping run in 'verifying_miner_forecasts'."
                    )
                    await _update_run_status(self, run_id, "verifying_miner_forecasts")
                else:
                    await _update_run_status(self, run_id, "no_matching_hashes")
    ...
    async def _recover_incomplete_scoring_jobs(self):
        ...
        try:
            ...
            logger.info(
                f"Found {len(incomplete_jobs)} incomplete scoring job(s) to recover:"
            )
            ...
            for job in incomplete_jobs:
                ...
                logger.info(
                    f"  - Run {run_id}: {score_type} scoring (started: {started_at})"
                )
