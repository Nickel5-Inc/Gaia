...
class WeatherInputPayload(BaseModel):
    ...
    compressed_data: bytes = Field(
        ..., description="Compressed binary data containing GFS inputs"
    )
    reference_time: datetime = Field(..., description="Reference time of forecast")
    forecast_id: str = Field(
        ..., description="Unique identifier for this forecast task"
    )
    grid_dimensions: Dict[str, int] = Field(
        ..., description="Grid dimensions (lat, lon)"
    )
...
class WeatherRequestPayload(BaseModel):
    ...
    miner_id: Optional[str] = Field(
        None, description="Specific miner to request from (if None, request from all)"
    )
    variables: Optional[List[str]] = Field(
        None, description="Specific variables to request (if None, request all)"
    )
    lead_times: Optional[List[int]] = Field(
        None,
        description="Specific lead times to request in hours (if None, request all)",
    )
    pressure_levels: Optional[List[int]] = Field(
        None, description="Specific pressure levels to request (if None, request all)"
    )
    region: Optional[Dict[str, float]] = Field(
        None, description="Region bounds to request (if None, request global)"
    )
...
class WeatherInputs(Inputs):
    ...
    def validate_inputs(self, inputs: Dict[str, Any]) -> bool:
        ...
        if "validator_input" in inputs:
            ...
            assert isinstance(config.get("reference_time"), datetime), (
                "reference_time must be a datetime"
            )
            assert isinstance(config.get("forecast_horizon"), int), (
                "forecast_horizon must be an integer"
            )
            assert isinstance(config.get("forecast_id"), str), (
    ...
    def prepare_miner_input(
        self, gfs_data: xr.Dataset, forecast_id: str
    ) -> WeatherInputPayload:
        """
...
class WeatherInputData(BaseModel):
    ...
    forecast_start_time: datetime = Field(
        ...,
        description="ISO 8601 timestamp for the forecast initialization time (T=0h).",
    )
    gfs_source_reference_1: str = Field(
        ...,
        description="Reference (e.g., URL) to the source GFS data for the T-6h timestep.",
    )
    gfs_source_reference_2: str = Field(
        ...,
        description="Reference (e.g., URL) to the source GFS data for the T=0h timestep.",
    )
    input_data_hash: str = Field(
        ...,
        description="Hex digest of the SHA-256 hash of the specific input data subset the miner should derive from the sources.",
    )
...
class WeatherKerchunkRequestData(BaseModel):
    ...
    job_id: str = Field(
        ...,
        description="The unique job ID returned by the miner during the initial forecast request acceptance.",
    )
...
class WeatherInitiateFetchData(BaseModel):
    ...
    forecast_start_time: datetime = Field(
        ..., description="ISO 8601 timestamp for the T=0h GFS analysis run."
    )
    previous_step_time: datetime = Field(
        ..., description="ISO 8601 timestamp for the T=-6h GFS analysis run."
    )
...
class WeatherStartInferenceData(BaseModel):
    ...
    job_id: str = Field(
        ..., description="The unique job ID for which inference should start."
    )
