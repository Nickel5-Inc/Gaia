...
from utils.metrics import calculate_rmse, normalize_rmse
...
def create_variable_level_ensemble(
...
def optimize_ensemble_weights(
    ...
    for iteration in range(max_iter):
        ensemble = create_weighted_ensemble(forecasts, weights)
        
        current_error = 0.0
        for var in variables:
            if var in ensemble and var in truth:
                rmse = normalize_rmse(ensemble[var], truth[var], var)
                current_error += rmse
        ...
        for miner in all_miners:
            ...
            for var in variables:
                if var in forecasts[miner] and var in truth:
                    rmse = normalize_rmse(forecasts[miner][var], truth[var], var)
                    miner_error += rmse
...
def ensemble_forecast_pipeline(
    ...
    """
    Complete ensemble forecast pipeline with preprocessing, weight optimization and ensemble creation.
    
    Args:
        miner_data: Dictionary of raw miner forecasts
        historical_weights: Dictionary of historical weights by miner
        lead_time: Target lead time to ensemble
    ...
    processed_data = preprocess_forecasts(miner_data, variables, lead_time)
    ...
    ensemble_forecast = create_weighted_ensemble(processed_data, optimized_weights)
