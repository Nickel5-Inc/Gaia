...
def subdivide_if_urban_present(base_hex, urban_cells_set, lakes_cells_set, target_resolution=5, max_resolution=8):
    ...
    if base_hex in urban_cells_set:
        ...
        for hex_id in subdivisions:
            ...
            if (hex_bounds[2] - hex_bounds[0] < 1.0 or hex_bounds[3] - hex_bounds[1] < 1.0) and target_resolution < max_resolution:
                filtered_cells.extend(subdivide_if_urban_present(hex_id, urban_cells_set, lakes_cells_set, target_resolution + 1, max_resolution))
            else:
...
def largest_inscribed_square(hex_polygon):
    ...
    while square_size > 0:
        inscribed_square = box(center_x - square_size / 2, center_y - square_size / 2,
...
def select_random_region(base_cells, urban_cells_set, lakes_cells_set, min_lat=-56, max_lat=60):
    ...
    filtered_cells = subdivide_if_urban_present(base_cell['index'], urban_cells_set, lakes_cells_set, target_resolution=5)
