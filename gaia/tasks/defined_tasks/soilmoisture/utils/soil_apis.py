...
async def fetch_hls_b4_b8(bbox, datetime_obj, download_dir=None):
    ...
    async def try_month(search_date):
        ...
        headers = {
            'Authorization': f'Bearer {os.getenv("EARTHDATA_API_KEY")}'
        }
        params = {
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    print(f"Unauthorized access for {tile_name}: {response.status} {response.reason}")
                    return None
                else:
                    print(f"Failed to download {tile_name}: {response.status} {response.reason}")
                    return None
...
async def fetch_srtm(bbox, sentinel_bounds=None, sentinel_crs=None, sentinel_shape=None):
    ...
    try:
        ...
        await asyncio.to_thread(reproject,
            source=mosaic[0],
            destination=output,
...
async def fetch_ifs_forecast(bbox, datetime_obj, sentinel_bounds=None, sentinel_crs=None, sentinel_shape=None):
    ...
    try:
        ...
        for cache_path in [cache_file, prev_cache]:
            if os.path.exists(cache_path):
                ...
                data = await extract_ifs_variables(ds, bbox, 
        ...
        if grib_paths:
            ...
            if success:
                ...
                return await extract_ifs_variables(ds, bbox, 
                                              sentinel_bounds=sentinel_bounds,
                                              sentinel_crs=sentinel_crs,
                                              sentinel_shape=sentinel_shape)
            ...
            for var_name, param_id in ecmwf_params.items():
                try:
                    ds = await asyncio.to_thread(
                        xr.open_dataset, grib_path, engine="cfgrib",
...
async def extract_ifs_variables(ds, bbox, sentinel_bounds=None, sentinel_crs=None, sentinel_shape=None):
    ...
    try:
        ...
        for var_name, data in variables_to_process:
            upsampled = await asyncio.to_thread(resize, data, sentinel_shape,
                                 order=0,
                                 preserve_range=True,
                                 anti_aliasing=False)
...
async def get_soil_data(bbox, datetime_obj=None):
    ...
    try:
        ...
        ifs_data = await fetch_ifs_forecast(bbox, datetime_obj,
                                        sentinel_bounds=sentinel_bounds,
                                        sentinel_crs=sentinel_crs,
                                        sentinel_shape=(222, 222))
...
async def combine_tiffs(sentinel_data, ifs_data, srtm_data_tuple, bbox, date_str, profile):
    try:
        srtm_array, srtm_file = srtm_data_tuple
        target_shape = (profile['height'], profile['width'])
        srtm_resized = await asyncio.to_thread(resize, srtm_array, target_shape, 
                                preserve_range=True, 
                                order=1,
                                mode='constant',
                                cval=-9999)
...
def get_data_dir():
    ...
    current_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))))))
