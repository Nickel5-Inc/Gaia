...
def fetch_hls_b4_b8(bbox, datetime_obj, download_dir="/tmp"):
    ...
    def try_month(search_date):
        ...
        headers = {"Authorization": os.getenv("CDS_API_KEY")}
        params = {
        ...
        download_session = SessionWithHeaderRedirection(
                ...
                try:
                    ...
                    if b4_response.status_code == 200:
                        ...
                        with open(b4_path, "wb") as f:
                            for chunk in b4_response.iter_content(
                    ...
                    if b8_response.status_code == 200:
                        ...
                        with open(b8_path, "wb") as f:
                            for chunk in b8_response.iter_content(
                                chunk_size=1024 * 1024
                            ):
                                f.write(chunk)
    ...
    try:
        ...
        for grib_path, step in zip(grib_paths, timesteps):
            ...
            for var_name, param_id in ecmwf_params.items():
                try:
                    ds = xr.open_dataset(
                        grib_path, engine="cfgrib", filter_by_keys={"paramId": param_id}
                    )
...
def fetch_ifs_forecast(
    ...
    try:
        ...
        target_date = (
            current_utc - timedelta(days=1) if current_utc.hour < 2 else current_utc
        )
...
def extract_ifs_variables(
    ...
    try:
        ...
        for var_name, data in variables_to_process:
            upsampled = resize(
                data, sentinel_shape, order=0, preserve_range=True, anti_aliasing=False
            )
