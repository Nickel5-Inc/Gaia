...
class SoilScoringMechanism(ScoringMechanism):
    ...
    executor: concurrent.futures.ThreadPoolExecutor = Field(
        default_factory=concurrent.futures.ThreadPoolExecutor, exclude=True
    )

    def __init__(
        self,
        baseline_rmse: float = 50,
        alpha: float = 10,
        beta: float = 0.1,
        db_manager=None,
        task=None,
    ...
    def compute_final_score(self, metrics: Dict) -> float:
        ...
        try:
            ...
            rootzone_rmse = metrics["validation_metrics"].get(
                "rootzone_rmse", self.beta
            )
            ...
            logger.debug(
                f"Input metrics - Surface RMSE: {surface_rmse}, Rootzone RMSE: {rootzone_rmse}, Surface SSIM: {surface_ssim}, Rootzone SSIM: {rootzone_ssim}"
            )
            ...
            if (
                ...
                logger.warning(
                    f"Invalid surface_ssim value: {surface_ssim}, setting to 0"
                )
                surface_ssim = 0
            ...
            if (
                ...
                logger.warning(
                    f"Invalid rootzone_ssim value: {rootzone_ssim}, setting to 0"
                )
                rootzone_ssim = 0

            # Compute scores with validation
            surface_score_tensor = self.sigmoid_rmse(
                torch.tensor(surface_rmse, dtype=torch.float32)
            )
            rootzone_score_tensor = self.sigmoid_rmse(
                torch.tensor(rootzone_rmse, dtype=torch.float32)
            )
            ...
            logger.debug(
                f"Computed scores - Surface: {surface_score}, Rootzone: {rootzone_score}"
            )

            if math.isnan(surface_score) or math.isinf(surface_score):
                logger.error(
                    f"Invalid surface score: {surface_score} from RMSE: {surface_rmse}"
                )
                return float("nan")

            if math.isnan(rootzone_score) or math.isinf(rootzone_score):
                logger.error(
                    f"Invalid rootzone score: {rootzone_score} from RMSE: {rootzone_rmse}"
                )
                return float("nan")
            ...
            logger.debug(
                f"Final score calculation: 0.6 * {surface_score} + 0.4 * {rootzone_score} = {final_score}"
            )
        ...
        try:
            ...
            if isinstance(pred_data, dict):
                ...
                model_predictions = await loop.run_in_executor(
                    self.executor, self._create_prediction_tensor_sync, pred_data
                )
            else:
    ...
    async def score(self, predictions: Dict) -> Dict[str, float]:
        ...
        try:
            ...
            if pred_info and pred_info.get("status") == "retry_scheduled":
                logger.info(
                    f"Processing retry for timestamp {predictions['target_time']} - retry time has passed"
                )
            elif pred_info and pred_info.get("status") == "sent_to_miner":
                logger.info(
                    f"Processing new prediction for timestamp {predictions['target_time']}"
                )
            else:
                logger.info(
                    f"Processing prediction for timestamp {predictions['target_time']}"
                )
            ...
            if isinstance(metrics, dict) and metrics.get("status") == "retry_scheduled":
                logger.info(
                    f"SMAP data unavailable, retry scheduled: {metrics.get('message')}"
                )
                return None
    ...
    async def compute_smap_score_metrics(
        ...
        try:
            if self.task and self.task.test_mode:
                ...
                logger.info(
                    f"TEST MODE: Scoring with SMAP data from 7 days prior: {target_date}"
                )

            logger.info(
                f"Computing SMAP score metrics for miner {miner_id}, target_date {target_date}"
            )
            ...
            if smap_file_path and os.path.exists(smap_file_path):
                ...
                if await loop.run_in_executor(
                    self.executor, potential_cache_path.exists
                ):
                    ...
                    logger.info(
                        f"Downloading SMAP data from {smap_url} to {temp_smap_filename}"
                    )
                    download_success = await download_smap_data(
                        smap_url, temp_smap_filename
                    )

                    if not download_success:
                        logger.error(f"Failed to download SMAP data from {smap_url}")
                        # Schedule retry for the miner
                        await self.schedule_retry_for_miner(
                            miner_id, target_date, "SMAP download failed"
                        )
                        return {
                            "status": "retry_scheduled",
                            "message": "SMAP download failed",
                        }

            if not temp_smap_filename or not os.path.exists(temp_smap_filename):
                logger.error(
                    f"SMAP data file {temp_smap_filename} not found or not accessible after download/cache check."
                )
                await self.schedule_retry_for_miner(
                    miner_id, target_date, "SMAP file not found post-download"
                )
                return {
                    "status": "retry_scheduled",
                    "message": "SMAP file not found post-download",
                }

            logger.info(
                f"Using SMAP file: {temp_smap_filename} for bounds: {bounds}, CRS: {crs}"
            )
            ...
            if not smap_data:
                logger.error(
                    f"Failed to process SMAP data for bounds {bounds}, CRS {crs}"
                )
                await self.schedule_retry_for_miner(
                    miner_id, target_date, "SMAP processing failed"
                )
                return {
                    "status": "retry_scheduled",
                    "message": "SMAP processing failed",
                }
            ...
            if not isinstance(smap_data, dict) or not all(
                k in smap_data for k in ["surface_sm", "rootzone_sm"]
            ):
                logger.error(
                    f"SMAP data is not in the expected dictionary format or missing keys. Got: {type(smap_data)}"
                )
                return None
            ...
            def _calculate_soil_metrics_sync(preds, truth):
                ...
                try:
                    ...
                    if pred_nan_count > 0 or pred_inf_count > 0:
                        logger.error(
                            f"Predictions contain invalid values: nan_count={pred_nan_count}, inf_count={pred_inf_count}"
                        )
                        return None
                    ...
                    if truth_nan_count > 0:
                        logger.info(
                            f"SMAP ground truth contains {truth_nan_count} NaN values (normal due to satellite data gaps)"
                        )
                        ...
                        if nan_percentage > 90:
                            logger.warning(
                                f"SMAP data has {nan_percentage:.1f}% NaN values - insufficient valid data for scoring"
                            )
                            return None

                        logger.info(
                            f"SMAP data quality: {nan_percentage:.1f}% NaN values, proceeding with valid pixels only"
                        )

                    if truth_inf_count > 0:
                        logger.error(
                            f"Ground truth contains inf values: inf_count={truth_inf_count}"
                        )
                        return None
                    ...
                    if preds_np.ndim == 4 and truth_np.ndim == 4:
                        ...
                        logger.error(
                            f"Unexpected tensor shapes: preds {preds_np.shape}, truth {truth_np.shape}"
                        )
                        return None
                    ...
                    def calculate_rmse_with_valid_pixels(pred, truth):
                        ...
                        logger.debug(
                            f"RMSE calculated from {valid_pixel_count}/{total_pixels} valid pixels ({valid_pixel_count / total_pixels * 100:.1f}%)"
                        )
                        return rmse

                    # Calculate RMSE for surface and rootzone using valid pixels
                    surface_rmse = calculate_rmse_with_valid_pixels(
                        surface_pred, surface_truth
                    )
                    rootzone_rmse = calculate_rmse_with_valid_pixels(
                        rootzone_pred, rootzone_truth
                    )
                    ...
                    def calculate_ssim_with_valid_pixels(pred, truth):
                        ...
                        try:
                            ...
                            if (
                                np.sum(valid_mask) < 100
                            ):  # Need minimum pixels for meaningful SSIM
                                logger.warning(
                                    f"Insufficient valid pixels ({np.sum(valid_mask)}) for SSIM calculation"
                                )
                                return 0.0
                            ...
                            def normalize_for_ssim(data):
                                ...
                                if data_max > data_min and not (
                                    np.isnan(data_min) or np.isnan(data_max)
                                ):
                            ...
                            if np.isnan(ssim_value) or np.isinf(ssim_value):
                                logger.warning(
                                    f"Invalid SSIM calculated: {ssim_value}, setting to 0"
                                )
                                return 0.0
                    ...
                    surface_ssim = calculate_ssim_with_valid_pixels(
                        surface_pred, surface_truth
                    )
                    rootzone_ssim = calculate_ssim_with_valid_pixels(
                        rootzone_pred, rootzone_truth
                    )
            ...
            if not soil_metrics:
                logger.error(
                    f"Failed to compute soil metrics for miner {miner_id} at {target_date}"
                )
                return None
            ...
            if (
                smap_file_path is None or temp_smap_filename != smap_file_path
            ) and Path(temp_smap_filename).name.startswith("tmp"):
                ...
                if os.path.exists(temp_smap_filename):
                    try:
                        await loop.run_in_executor(
                            self.executor, os.unlink, temp_smap_filename
                        )
                        logger.info(
                            f"Successfully deleted temporary SMAP file: {temp_smap_filename}"
                        )
                    except Exception as e_unlink:
                        logger.error(
                            f"Error deleting temporary SMAP file {temp_smap_filename}: {e_unlink}"
                        )
            elif temp_smap_filename and Path(temp_smap_filename).name.startswith(
                "SMAP_L4_SM_gph_"
            ):
            ...
            return {"validation_metrics": soil_metrics, "ground_truth": smap_data}

        except FileNotFoundError as e_fnf:
            logger.error(
                f"SMAP file not found during metric computation for miner {miner_id}, date {target_date}: {e_fnf}"
            )
            await self.schedule_retry_for_miner(
                miner_id, target_date, f"SMAP file not found: {e_fnf.filename}"
            )
            return {
                "status": "retry_scheduled",
                "message": f"SMAP file not found: {e_fnf.filename}",
            }
        except Exception as e:
            logger.error(
                f"Error computing SMAP score metrics for miner {miner_id}, date {target_date}: {str(e)}"
            )
            logger.error(traceback.format_exc())
            # If a generic error occurs, also schedule a retry
            await self.schedule_retry_for_miner(
                miner_id, target_date, "General error during SMAP metrics"
            )
            return {
                "status": "retry_scheduled",
                "message": "General error during SMAP metrics",
            }
            ...
            if process:
                ...
                logger.info(
                    f"Memory usage for compute_smap_score_metrics: {(mem_after - mem_before):.2f} MB (Start: {mem_before:.2f}, End: {mem_after:.2f})"
                )

            gc.collect()  # Explicit garbage collection

    async def cleanup_invalid_prediction(
        self, bounds, target_time: datetime, miner_id: str
    ):
        try:
    ...
    def prepare_soil_history_records(
        self, miner_id: str, miner_hotkey: str, metrics: Dict, target_time: datetime
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not all(
                isinstance(x, (int, float)) for x in [surface_rmse, rootzone_rmse]
            ):
    ...
    async def schedule_retry_for_miner(
        self, miner_id: str, target_date: datetime, error_message: str
    ):
        """
        ...
        try:
            ...
            if (
                "SMAP download failed" in error_message
                or "SMAP file not found" in error_message
            ):
                ...
                "SMAP processing failed" in error_message
                or "_FillValue" in error_message
            ):
                ...
                "General error" in error_message
                or "Failed to calculate score" in error_message
            ):
            ...
            logger.info(
                f"Scheduled retry for miner {miner_id} at {next_retry_time} ({retry_reason}). Error: {error_message}"
            )
            return result
