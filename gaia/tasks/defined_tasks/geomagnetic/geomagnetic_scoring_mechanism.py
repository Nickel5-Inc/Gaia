...
class GeomagneticScoringMechanism(ScoringMechanism):
    ...
    db_manager: Any = Field(
        None, description="Database manager for the scoring mechanism"
    )
    ...
    def calculate_score(self, predicted_value, actual_value):
        ...
        try:
            ...
            if not (-5 <= predicted_value <= 5):
                logger.warning(
                    f"Out-of-range prediction: {predicted_value}. Expected range: (-5, 5)."
                )
            ...
            logger.debug(
                f"Error: {error:.8f}, Raw score: {raw_score:.8f}, Power-transformed: {powered_score:.8f}, Final: {high_precision_score:.10f}"
            )
        ...
        try:
            ...
            for prediction in predictions:
                score_value = self.calculate_score(
                    prediction["predicted_value"], ground_truth
                )
                miner_scores.append(
                    {
                        "miner_uid": prediction["miner_uid"],
                        "miner_hotkey": prediction["miner_hotkey"],
                        "query_time": prediction.get(
