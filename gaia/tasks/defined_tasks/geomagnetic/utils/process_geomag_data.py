...
def debug_raw_data(data):
    ...
    non_dst_lines = [
        line for line in lines if not line.startswith("DST") and line.strip()
    ]
    if non_dst_lines:
...
def parse_data(data):
    ...
    def parse_line(line):
        ...
        try:
            ...
            logger.error(
                f"Skipping line due to invalid date format: {line} - Error: {e}"
            )
            return
        ...
        for value in raw_values:
            if (value.startswith("-") and "9999" in value and len(value) > 5) or (
                ...
                for split_pos in range(
                    start_pos, min(6, len(value))
                ):  # Check positions for X, XX  or -X, -XX, -XXX
                    ...
                    try:
                        ...
                        if (
                            len(remainder) > 0
                            and len(remainder) % 4 == 0
                            and all(
                                remainder[i : i + 4] == "9999"
                            ...
                            logger.debug(
                                f"Split squished value '{value}' into ['{potential_value}'] + {num_9999s} × '9999'"
                            )
                            found_split = True
                            break
                    except ValueError:
                ...
                if not found_split:
                    ...
                    values.append(value)
                    logger.warning(
                        f"Could not parse squished value '{value}', adding as-is"
                    )
            else:
                values.append(value)
        ...
        hourly_values_only = (
            values[:-1] if len(values) > 24 else values
        )  # Exclude daily mean (last value)
        ...
        for i in range(min(24, len(hourly_values_only))):
            ...
            if i < 23:
                ...
                try:
                    ...
                    logger.debug("Skipping hour 24 due to date calculation error")
                    continue

            # Skip placeholder values (9999, 999, etc.)
            if value_str in ["9999", "999", "99999", PLACEHOLDER_VALUE]:
                logger.debug(
                    f"Skipping placeholder value at position {i} (hour {hour}): '{value_str}'"
                )
                continue
            ...
            try:
                ...
                if i < 23:
                    ...
                    timestamp = datetime(
                        target_year, target_month, target_day, hour, tzinfo=timezone.utc
                    )
                ...
                if timestamp < datetime.now(timezone.utc):
                    ...
                    logger.debug(
                        f"Added data point: {timestamp} -> {value} (position {i})"
                    )
                else:
    ...
    logger.info(
        f"Parsed {len(dates)} data points from {len([l for l in data.splitlines() if l.startswith('DST')])} DST lines"
    )
...
async def get_geomag_data_for_hour(
    target_hour, include_historical=False, max_wait_minutes=30
):
    """
    ...
    logger.info(
        f"Fetching geomagnetic data for target hour: {target_hour_aligned} (will wait up to {max_wait_minutes} minutes)"
    )
        ...
        try:
            ...
            if not cleaned_df.empty:
                ...
                if not target_data.empty:
                    ...
                    logger.info(
                        f"✅ Found data for target hour {target_hour_aligned}: {dst_value}"
                    )
                    ...
                    if include_historical:
                        ...
                        historical_data = cleaned_df[
                            cleaned_df["timestamp"] >= start_of_month
                        ]
                    ...
                    wait_time_left = (
                        max_wait_time - datetime.now(timezone.utc)
                    ).total_seconds()

                    if (
                        wait_time_left > 30
                    ):  # Only wait if we have more than 30 seconds left
                        logger.info(
                            f"⏳ Target hour {target_hour_aligned} not available yet (latest: {latest_timestamp}), waiting..."
                        )
                        ...
                        logger.warning(
                            f"❌ Target hour {target_hour_aligned} not available within {max_wait_minutes} minutes. Skipping cycle."
                        )
                        break
            else:
    ...
    logger.error(
        f"❌ Failed to fetch data for target hour {target_hour_aligned} within {max_wait_minutes} minutes. Cycle will be skipped."
    )
    if include_historical:
        return "N/A", "N/A", None
    else:
