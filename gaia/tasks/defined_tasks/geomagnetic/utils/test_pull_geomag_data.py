...
REAL_URL_FOR_TESTING = (
    "https://wdc.kugi.kyoto-u.ac.jp/dst_realtime/202505/dst2505.for.request"
)


class TestFetchGeomagData(unittest.IsolatedAsyncioTestCase):
    @patch(
        "gaia.tasks.defined_tasks.geomagnetic.utils.pull_geomag_data.asyncio.sleep",
        new_callable=AsyncMock,
    )
    @patch(
        "gaia.tasks.defined_tasks.geomagnetic.utils.pull_geomag_data.httpx.AsyncClient"
    )
    ...
    async def test_fetch_data_failure_after_retries(
        self, mock_async_client_constructor, mock_sleep
    ):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            f"Error fetching data after {max_retries} retries: Connection timed out",
        ):
    ...
    async def test_fetch_data_retry_then_succeed(
        self, mock_async_client_constructor, mock_sleep
    ):
    ...
    async def test_fetch_data_initial_wait(
        self, mock_async_client_constructor, mock_sleep, mock_datetime
    ):
        ...
        mock_refreshed_now_utc.year = (
            2025  # Consistent with REAL_URL_FOR_TESTING year/month
        )
        mock_refreshed_now_utc.month = 5

        # If datetime.now() is called more times, provide a consistent mock
        mock_datetime.now.side_effect = [
            mock_initial_now_utc,
            mock_refreshed_now_utc,
            mock_refreshed_now_utc,
        ]
