...
class GeomagneticTask(Task):
    ...
    def __init__(self, node_type: str, db_manager, test_mode: bool = False, **data):
        ...
        if self.node_type == "miner":
            try:
                ...
                custom_model_path = (
                    "gaia/models/custom_models/custom_geomagnetic_model.py"
                )
            ...
            while True:
                try:
                    ...
                    if not self.test_mode:
                        ...
                        window_start = current_hour + datetime.timedelta(
                            minutes=2
                        )  # T:02
                        window_end = current_hour + datetime.timedelta(
                            minutes=15
                        )  # T:15
                        ...
                        if last_executed_hour == current_hour:
                            ...
                            next_window_start = next_hour + datetime.timedelta(
                                minutes=2
                            )
                            sleep_duration = (
                            ...
                            logger.info(
                                f"🔒 EXECUTION COMPLETE: Already executed for hour {current_hour.strftime('%H:%M')}, waiting for next window at {next_window_start.strftime('%H:%M')} (in {sleep_duration:.0f} seconds)"
                            )
                        ...
                        # Check if we're in the current hour's execution window
                            ...
                            logger.info(
                                f"🔒 FLEXIBLE TIMING: Executing within window at {current_time.strftime('%H:%M')} (T:02-T:15 window)"
                            )
                            logger.info(
                                f"🔒 Using {query_hour.strftime('%H:%M')} data to query for {(query_hour + datetime.timedelta(hours=1)).strftime('%H:%M')} predictions"
                            )
                            ...
                            logger.info(
                                f"🔒 SECURE TIMING: Waiting for execution window to open at {window_start.strftime('%H:%M')} (in {sleep_duration:.0f} seconds)"
                            )
                            logger.info(
                                "🔒 Execution window: T:02-T:15 allows flexibility for async delays"
                            )
                            await validator.update_task_status("geomagnetic", "idle")
                            await asyncio.sleep(sleep_duration)
                            query_hour = current_hour
                            ...
                            logger.info(
                                f"⏰ MISSED WINDOW: Past T:15, waiting for next execution window at {next_window_start.strftime('%H:%M')} (in {sleep_duration:.0f} seconds)"
                            )
                            await validator.update_task_status("geomagnetic", "idle")
                            await asyncio.sleep(sleep_duration)
                            query_hour = next_hour
                    else:
                        query_hour = current_time.replace(
                            minute=0, second=0, microsecond=0
                        )
                        logger.info(
                            "Test mode: Running immediately, will sleep for 5 minutes after completion"
                        )

                    logger.info(
                        f"🔒 Starting GeomagneticTask execution at T:02 for hour {query_hour}"
                    )

                    # Step 2: Fetch Geomagnetic Data for the current query hour
                    await validator.update_task_status(
                        "geomagnetic", "processing", "data_fetch"
                    )
                    ...
                    if timestamp == "N/A" or dst_value == "N/A":
                        logger.warning(
                            f"❌ Could not obtain geomagnetic data for target hour {query_hour}. Skipping this cycle."
                        )
                        await validator.update_task_status(
                            "geomagnetic", "idle", "data_unavailable_for_target_hour"
                        )
                    ...
                    if (
                        ...
                        logger.error(
                            f"❌ Expected data for {query_hour} but got {timestamp}. This should not happen with the new logic."
                        )
                        await validator.update_task_status(
                            "geomagnetic", "error", "incorrect_hour_data"
                        )
                        continue

                    logger.info(
                        f"✅ Successfully obtained geomagnetic data for target hour {query_hour}: {dst_value}"
                    )

                    # Step 3: Query Miners for predictions
                    await validator.update_task_status(
                        "geomagnetic", "processing", "miner_query"
                    )
                    # Use 1-hour prediction window from data timestamp
                    target_prediction_hour = timestamp + datetime.timedelta(hours=1)
                    logger.info(
                        f"Querying miners to predict {target_prediction_hour} using data from {timestamp}"
                    )
                    logger.info(
                        f"🔍 Prediction window: {timestamp} → {target_prediction_hour} (1 hour ahead)"
                    )
                    await self._query_miners(
                        ...
                        historical_data,
                        query_hour,
                        target_prediction_hour,
                    )
                    logger.info(f"Collected predictions for {target_prediction_hour}")

                    # Step 4: Score predictions with SECURE ground truth fetching
                    score_target_hour = (
                        query_hour  # Score predictions that targeted this hour
                    )
                    scoring_query_hour = query_hour - datetime.timedelta(
                        hours=1
                    )  # Made 1 hour ago
                    await validator.update_task_status(
                        "geomagnetic", "processing", "scoring"
                    )
                    logger.info(
                        f"🔒 SECURITY: Scoring predictions that targeted {score_target_hour}"
                    )
                    logger.info(
                        f"🔒 SECURITY: These predictions were made at {scoring_query_hour} using aged data"
                    )
                    logger.info(
                        f"🔒 SECURITY: Ground truth fetching will enforce minimum {90 if not self.test_mode else 30}-minute delay"
                    )
                    await self._process_scores(
                        validator, scoring_query_hour, score_target_hour
                    )

                    # Mark this hour as executed to prevent re-execution
                    last_executed_hour = query_hour
                    logger.info(
                        f"🔒 EXECUTION CYCLE COMPLETE: Marked hour {query_hour.strftime('%H:%M')} as executed"
                    )
                    ...
                    if self.test_mode:
                        logger.info(
                            "Test mode: Sleeping for 5 minutes before next execution"
                        )
                        await asyncio.sleep(300)
                        last_executed_hour = (
                            None  # Reset in test mode to allow re-execution
                        )
                    else:
                        # Every 6 hours, run a more comprehensive cleanup of old pending tasks
                        if query_hour.hour % 6 == 0:
                            logger.info(
                                "Running comprehensive cleanup of old pending tasks"
                            )
    ...
    async def _query_miners(
        ...
        logger.info(
            f"Using current data from {timestamp} to ask miners to predict {target_prediction_hour}"
        )
        logger.info(
            f"Running DST basemodel for scoring with task_id: {task_id} (timestamp: {current_hour_start})"
        )

        validator.basemodel_evaluator.test_mode = self.test_mode

        await validator.basemodel_evaluator.predict_geo_and_store(
            historical_data, task_id
        )
        ...
        def _prepare_historical_records_sync(hist_data):
            ...
            if hist_data is not None:
                for (
                    _,
                    rec_row,
                ) in hist_data.iterrows():  # Changed variable name to avoid conflict
                    records.append(
                        {
                            "timestamp": rec_row["timestamp"].isoformat(),
                            "Dst": rec_row["Dst"],
                        }
                    )
            return records
        ...
        if historical_data is not None:
            ...
            historical_records = await loop.run_in_executor(
                None, _prepare_historical_records_sync, historical_data
            )
        else:
            historical_records = []
    ...
    async def _process_scores(self, validator, query_hour, target_hour=None):
        ...
        try:
            ...
            if self.test_mode:
                ...
                logger.info(
                    f"TEST MODE: Using recent time window for immediate scoring: {start_time} to {end_time}"
                )
            else:
            ...
            tasks = await self.get_tasks_for_hour(
                start_time, end_time, validator=validator
            )
            ...
            if all_processed:
                logger.info(
                    f"All tasks already scored for query hour {query_hour.isoformat()}"
                )
                return [], current_time
            ...
            if not await self._check_ground_truth_availability(target_hour):
                logger.info(
                    f"Ground truth not yet available for target hour {target_hour.isoformat()}, tasks will remain pending"
                )
                return [], current_time

            # Get ground truth data for the TARGET hour that predictions were aiming for
            ground_truth_value = await self._fetch_ground_truth_for_time(
                target_hour, max_attempts=3
            )
            if ground_truth_value is None:
                logger.warning(
                    f"Could not fetch ground truth value for target hour {target_hour.isoformat()}, tasks will remain pending"
                )
                return [], current_time
            ...
            for task in tasks:
                ...
                try:
                    ...
                    if validator and hasattr(validator, "basemodel_evaluator"):
                        try:
                            task_timestamp = task.get(
                                "timestamp", task.get("query_time")
                            )
                            if task_timestamp:
                                if isinstance(task_timestamp, datetime.datetime):
                                    if task_timestamp.tzinfo is not None:
                                        task_timestamp_utc = task_timestamp.astimezone(
                                            datetime.timezone.utc
                                        )
                                    else:
                                        task_timestamp_utc = task_timestamp.replace(
                                ...
                                logger.debug(
                                    f"Looking up baseline prediction with task_id: {task_id}"
                                )
                                validator.basemodel_evaluator.test_mode = self.test_mode

                                baseline_score = await validator.basemodel_evaluator.score_geo_baseline(
                                    task_id=task_id, ground_truth=ground_truth_value
                                )
                                if baseline_score is not None:
                                    logger.info(
                                        f"Retrieved baseline score for task_id {task_id}: {baseline_score:.4f}"
                                    )
                                else:
                                    logger.info(
                                        f"No baseline score available for task_id {task_id}"
                                    )
                            else:
                                logger.warning(
                                    "Task has no timestamp, cannot retrieve baseline prediction"
                                )
                        except Exception as e:
                    ...
                    if baseline_score is not None:
                        logger.info(
                            f"Geomagnetic Task - Miner: {task_id} - Miner score: {score:.4f}, Baseline: {baseline_score:.4f}, Diff: {score - baseline_score:.4f}"
                        )
                        ...
                        if baseline_score > theoretical_max - 0.10:
                            epsilon = 0.002
                            logger.debug(
                                f"Using reduced epsilon: {epsilon:.4f} (baseline near theoretical max)"
                            )
                        else:
                        ...
                        if score <= baseline_score + epsilon:
                            if score < baseline_score:
                                logger.info(
                                    f"Score zeroed - Below baseline: {score:.4f} < {baseline_score:.4f}"
                                )
                            else:
                                logger.info(
                                    f"Score zeroed - Insufficient improvement: {score:.4f} vs baseline {baseline_score:.4f} (needed > {baseline_score + epsilon:.4f})"
                                )
                            score = 0
                        else:
                            logger.info(
                                f"Score valid - Exceeds baseline by {score - baseline_score:.4f}"
                            )
                    else:
                        logger.info(
                            f"No baseline comparison available - using raw score: {score:.4f}"
                        )
            ...
            if scored_tasks:
                logger.info(
                    f"Building score row for {len(scored_tasks)} scored tasks targeting {target_hour}"
                )
    ...
    async def get_tasks_for_hour(self, start_time, end_time, validator=None):
        ...
        try:
            ...
            logger.info("Querying tasks with:")
        ...
        try:
            ...
            if data and data.get("data"):
                ...
                input_data = pd.DataFrame(
                    {
                        "timestamp": [
                            pd.to_datetime(data["data"]["timestamp"], utc=True)
                        ],
            ...
            for hotkey, response in responses.items():
                try:
                    ...
                    predicted_value = await loop.run_in_executor(
                        None, self._extract_prediction_sync, response
                    )

                    if predicted_value is None:
                        logger.error(
                            f"No valid prediction found in response from {hotkey}"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.error(
                            f"Invalid prediction from {hotkey}: {predicted_value}"
                        )
                        continue

                    logger.info("=" * 50)
                    logger.info("Received prediction from miner:")
                    ...
                    logger.info(
                        f"Adding prediction to queue for {hotkey} with value {predicted_value}"
                    )
        ...
        try:
            ...
            for task in historical_tasks:
                ...
                if historical_miner_uid is None or historical_miner_hotkey is None:
                    logger.warning(
                        f"Skipping historical task due to missing UID/Hotkey: {task}"
                    )
                    continue
                ...
                if (
                    ...
                    if historical_miner_hotkey in self.validator.metagraph.nodes:
                        ...
                        if hasattr(node_in_metagraph, "node_id") and str(
                            ...
                            logger.warning(
                                f"Metagraph UID mismatch for {historical_miner_hotkey} (Historical Task): Prediction UID {historical_miner_uid}, Metagraph Node UID {metagraph_uid_str}. Skipping score."
                            )
                    else:
                        logger.warning(
                            f"Miner hotkey {historical_miner_hotkey} (Historical Task) not found in current metagraph. Prediction UID {historical_miner_uid}. Skipping score."
                        )
                else:
                    logger.warning(
                        "Validator or metagraph not available for validation (historical_tasks). Skipping."
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to assign score for historical task, UID: {historical_miner_uid}, Score: {task.get('score')}, Error: {e}"
                    )
            ...
            if recent_tasks:
                for task in recent_tasks:
                    ...
                    if recent_miner_uid is None or recent_miner_hotkey is None:
                        logger.warning(
                            f"Skipping recent task due to missing UID/Hotkey: {task}"
                        )
                        continue
                    ...
                    if (
                        ...
                        if recent_miner_hotkey in self.validator.metagraph.nodes:
                            logger.info(
                                f"Recent hotkey {recent_miner_hotkey} found in metagraph"
                            )
                            # Retrieve the Node object from the metagraph
                            node_in_metagraph = self.validator.metagraph.nodes[
                                recent_miner_hotkey
                            ...
                            if hasattr(node_in_metagraph, "node_id") and str(
                                ...
                                logger.warning(
                                    f"Metagraph UID mismatch for {recent_miner_hotkey} (Recent Task): Prediction UID {recent_miner_uid}, Metagraph Node UID {metagraph_uid_str}. Skipping score."
                                )
                        else:
                            logger.warning(
                                f"Miner hotkey {recent_miner_hotkey} (Recent Task) not found in current metagraph. Prediction UID {recent_miner_uid}. Skipping score."
                            )
                    else:
                        logger.warning(
                            "Validator or metagraph not available for validation (recent_tasks). Skipping."
                        )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to assign score for recent task, UID: {recent_miner_uid}, Score: {task.get('score')}, Error: {e}"
                        )
    ...
    async def recalculate_recent_scores(self, uids: List[int]) -> None:
        ...
        try:
            ...
            logger.info(
                f"Recalculating scores for UIDs {uids} from {history_window} to {current_time}"
            )
            ...
            logger.info("Successfully deleted scores for time window")
            ...
            if not history_results:
                logger.warning(
                    f"No historical data found for UIDs {uids} in window {history_window} to {current_time}"
                )
                return
            ...
            if len(history_results) > 1000:
                logger.warning(
                    f"Large geomagnetic history dataset: {len(history_results)} records for UIDs {uids}"
                )
            ...
            hotkey_to_uid: Dict[str, int] = {
                row["hotkey"]: row["uid"] for row in miner_mappings
            }
            ...
            try:
                ...
                if processed_count > 1000:
                    ...
                    logger.info(
                        f"Geomagnetic history processing: GC collected {collected} objects after processing {processed_count} records"
                    )
                ...
                try:
                    ...
                    if processed_count > 500:  # Only for moderate+ datasets
                        logger.info(
                            f"Clearing hourly_records dict with {len(hourly_records)} hour keys"
                        )
                        hourly_records.clear()
                except Exception as hourly_cleanup_err:
                    logger.debug(
                        f"Error during hourly records cleanup: {hourly_cleanup_err}"
                    )
                ...
                try:
                    ...
                    for record in records:
                        try:
                            ...
                            logger.error(
                                f"Error processing record for miner {record.get('miner_hotkey')}: {e}"
                            )
                            continue
    ...
    async def _fetch_ground_truth_with_retry(self):
        ...
        try:
            ...
            if self.test_mode:
                logger.info(
                    f"TEST MODE: Fetching ground truth for current UTC hour: {current_time.hour}"
                )
            else:
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    if len(result) == 2:
                        ...
                        logger.error(
                            f"Unexpected number of values returned from get_latest_geomag_data: {len(result)} (expected 2)"
                        )
                    ...
                    if timestamp == "N/A" or dst_value == "N/A":
                        logger.warning(
                            "No ground truth data available for the current hour."
                        )
                        continue
            ...
            logger.warning(
                "All ground truth fetching attempts failed. No ground truth value returned."
            )
            return None
    ...
    async def _retry_pending_tasks(self, validator, current_time):
        ...
        try:
            ...
            logger.info(
                f"Checking for pending tasks from {start_time} to {current_time}"
            )
            ...
            for hour_key, tasks in hourly_pending_tasks.items():
                try:
                    logger.info(
                        f"Attempting to score {len(tasks)} pending tasks from hour {hour_key}"
                    )
                    ...
                    if not await self._check_ground_truth_availability(hour_key):
                        logger.debug(
                            f"🔒 SECURITY: Ground truth not yet available for retry tasks from hour {hour_key}, will retry later"
                        )
                        continue

                    # SECURITY FIX: Use time-specific ground truth fetching instead of latest data
                    ground_truth_value = await self._fetch_ground_truth_for_time(
                        hour_key, max_attempts=2
                    )
                    if ground_truth_value is None:
                        logger.warning(
                            f"Could not fetch time-specific ground truth for pending tasks from hour {hour_key}, will retry later"
                        )
                        continue

                    logger.info(
                        f"✅ SECURITY: Successfully got time-specific ground truth value {ground_truth_value} for pending tasks from hour {hour_key}"
                    )
                    ...
                    for task in tasks:
                        try:
                            ...
                            if not await self._should_retry_task(task):
                                logger.debug(
                                    f"Skipping task {task['id']} - retry conditions not met"
                                )
                                continue
                            ...
                            if (
                                validator
                                and hasattr(validator, "metagraph")
                                and validator.metagraph is not None
                            ):
                                if (
                                    task["miner_hotkey"]
                                    not in validator.metagraph.nodes
                                ):
                                    logger.warning(
                                        f"Miner {task['miner_hotkey']} no longer in metagraph, deleting task {task['id']}"
                                    )
                                    try:
                                        ...
                                        logger.info(
                                            f"Successfully deleted orphaned task {task['id']} for removed miner {task['miner_hotkey']}"
                                        )
                                    except Exception as e:
                                        logger.error(
                                            f"Failed to delete orphaned task {task['id']}: {e}"
                                        )
                                    continue
                            ...
                            if validator and hasattr(validator, "basemodel_evaluator"):
                                try:
                                    ...
                                    if task_timestamp:
                                        if isinstance(
                                            task_timestamp, datetime.datetime
                                        ):
                                            if task_timestamp.tzinfo is not None:
                                                task_timestamp_utc = (
                                                    task_timestamp.astimezone(
                                                        datetime.timezone.utc
                                                    )
                                                )
                                            else:
                                                task_timestamp_utc = (
                                                    task_timestamp.replace(
                                                        tzinfo=datetime.timezone.utc
                                                    )
                                                )

                                            task_id = str(
                                                task_timestamp_utc.timestamp()
                                            )
                                        else:
                                            task_id = str(task_timestamp)

                                        validator.basemodel_evaluator.test_mode = (
                                            self.test_mode
                                        )
                                        baseline_score = await validator.basemodel_evaluator.score_geo_baseline(
                                            task_id=task_id,
                                        ...
                                        if baseline_score is not None:
                                            logger.info(
                                                f"Retrieved baseline score for retry task_id {task_id}: {baseline_score:.4f}"
                                            )
                                except Exception as e:
                                    logger.warning(
                                        f"Error retrieving baseline score for retry task: {e}"
                                    )
                            ...
                            if baseline_score is not None:
                                ...
                                if score <= baseline_score + epsilon:
                                    logger.info(
                                        f"Retry task score zeroed - insufficient improvement: {score:.4f} vs baseline {baseline_score:.4f}"
                                    )
                                    score = 0
                                else:
                                    logger.info(
                                        f"Retry task score valid - exceeds baseline by {score - baseline_score:.4f}"
                                    )
                            ...
                            scored_tasks.append(task)
                            logger.info(
                                f"Successfully retried and scored task {task['id']} with score {score}"
                            )

                        except Exception as e:
                            logger.error(
                                f"Error processing retry task {task['id']}: {e}"
                            )
                            continue
                    ...
                    if scored_tasks:
                        ...
                        logger.info(
                            f"Built score row for {len(scored_tasks)} retried tasks from hour {hour_key}"
                        )
                        scored_any_tasks = True

                except Exception as e:
                    logger.error(
                        f"Error processing pending tasks for hour {hour_key}: {e}"
                    )
                    logger.error(traceback.format_exc())
                    continue
            ...
            if scored_any_tasks:
                logger.info(
                    "Successfully scored some pending tasks from previous hours"
                )
            else:
        ...
        try:
            ...
            for row in pending_tasks_query:
                ...
                try:
                    ...
                    task_hour = task["query_time"].replace(
                        minute=0, second=0, microsecond=0
                    )
                    ...
                    if not await self._check_ground_truth_availability(task_hour):
                        ...
                        logger.info(
                            f"🔒 SECURITY: Ground truth not yet available for cleanup task {task['id']} from {task_hour}, keeping task pending until security window passes"
                        )
                        skipped_too_young += 1
                        ...
                        ground_truth_value = await self._fetch_ground_truth_for_time(
                            task_hour, max_attempts=1
                        )

                        if ground_truth_value is None:
                            # If ground truth isn't available due to data issues, assign a default low score
                            logger.warning(
                                f"No ground truth data available for cleanup task {task['id']} from {task_hour}, assigning default score of 0"
                            )
                            ground_truth_value = (
                                0.0  # Default fallback for data unavailability
                            )
                            score = (
                                0.0  # Default score when no ground truth data available
                            )
                        else:
                            # Use validator's basemodel_evaluator to get the baseline score for comparison
                            baseline_score = None
                            if validator and hasattr(validator, "basemodel_evaluator"):
                                try:
                                    ...
                                    if task_timestamp:
                                        ...
                                        if baseline_score is not None:
                                            logger.info(
                                                f"Retrieved baseline score for cleanup task_id {task_id}: {baseline_score:.4f}"
                                            )
                                except Exception as e:
                                    logger.warning(
                                        f"Error retrieving baseline score for cleanup task: {e}"
                                    )

                            # Calculate score using actual ground truth
                            score = self.scoring_mechanism.calculate_score(
                                task["predicted_values"],
                                ground_truth_value,  # FIXED: Use actual ground truth
                            )
                            ...
                            if baseline_score is not None:
                                ...
                                if score <= baseline_score + epsilon:
                                    logger.debug(
                                        f"Cleanup task score zeroed - insufficient improvement: {score:.4f} vs baseline {baseline_score:.4f}"
                                    )
                                    score = 0
                                else:
                                    logger.debug(
                                        f"Cleanup task score valid - exceeds baseline by {score - baseline_score:.4f}"
                                    )
            ...
            # Log cleanup summary
            logger.info(
                f"Completed comprehensive cleanup: {scored_ready} tasks scored, {skipped_too_young} tasks kept pending (too young), {deleted_orphaned} orphaned tasks deleted"
            )

            if skipped_too_young > 0:
                logger.info(
                    f"🔒 SECURITY: {skipped_too_young} tasks kept pending until 90-minute security window passes"
                )
            if scored_ready > 0:
                logger.info(
                    f"✅ Successfully scored {scored_ready} tasks with available ground truth"
                )
            if deleted_orphaned > 0:
                logger.info(
                    f"🧹 Cleaned up {deleted_orphaned} orphaned tasks from miners no longer in metagraph"
                )
    ...
    async def _check_ground_truth_availability(
        ...
        try:
            ...
            if time_since_target.total_seconds() < min_delay_seconds:
                logger.debug(
                    f"🔒 SECURITY: Ground truth not available for {target_hour} - only {time_since_target.total_seconds() / 60:.1f} minutes have passed, need {min_delay_seconds / 60:.0f} minutes ({security_message})"
                )
                return False
            ...
            if time_since_target.total_seconds() < 0:
                logger.debug(
                    f"🔒 SECURITY: Cannot check ground truth availability for future time {target_hour}"
                )
                return False
            ...
            if time_since_target.total_seconds() > max_lookback_hours * 3600:
                logger.debug(
                    f"🔒 SECURITY: Target hour {target_hour} is too old ({time_since_target.total_seconds() / 3600:.1f} hours ago, max {max_lookback_hours} hours)"
                )
                return False
            ...
            if time_since_target.total_seconds() > 21600:  # 6 hours
                logger.debug(
                    f"🔒 SECURITY: Ground truth should definitely be available for {target_hour} (data is {time_since_target.total_seconds() / 3600:.1f} hours old)"
                )
                return True
            ...
            try:
                ...
                if len(result) == 3:
                    ...
                    if (
                        ...
                        and latest_dst_value != "N/A"
                        and historical_data is not None
                    ):
                        # Check if historical data contains our target hour
                        target_data = historical_data[
                            historical_data["timestamp"] == target_hour
                        ]
                        if not target_data.empty:
                            logger.debug(
                                f"✅ SECURITY: Ground truth data found for {target_hour} in historical data"
                            )
                            return True
                        else:
                            logger.debug(
                                f"🔒 SECURITY: Target hour {target_hour} not found in historical data range: {historical_data['timestamp'].min()} to {historical_data['timestamp'].max()}"
                            )
                            return False

                logger.debug(
                    f"🔒 SECURITY: Ground truth check returned N/A values for {target_hour}"
                )
                return False

            except Exception as e:
                logger.debug(
                    f"🔒 SECURITY: Error during ground truth availability check for {target_hour}: {e}"
                )
    ...
    async def _fetch_ground_truth_for_time(
        self, target_time: datetime.datetime, max_attempts: int = 3
    ):
        """
        ...
        try:
            ...
            if time_diff.total_seconds() < min_delay_seconds:
                logger.warning(
                    f"🔒 SECURITY: Refusing to fetch ground truth for {target_hour}"
                )
                logger.warning(
                    f"🔒 SECURITY: Only {time_diff.total_seconds() / 60:.1f} minutes have passed, need {min_delay_seconds / 60:.0f} minutes ({security_message})"
                )
                logger.warning(
                    "🔒 SECURITY: This prevents miners from accessing ground truth data during prediction windows"
                )
                return None
            ...
            if time_diff.total_seconds() < 0:
                logger.error(
                    f"🔒 SECURITY: Cannot fetch ground truth for future time {target_hour} (current time: {current_time})"
                )
                return None
            ...
            if time_diff.total_seconds() > max_lookback_hours * 3600:
                logger.warning(
                    f"🔒 SECURITY: Target hour {target_hour} is too old ({time_diff.total_seconds() / 3600:.1f} hours ago, max {max_lookback_hours} hours)"
                )
                return None

            logger.info(
                f"🔒 SECURITY: Fetching ground truth for {target_hour} (safe: {time_diff.total_seconds() / 3600:.1f} hours ago, {time_diff.total_seconds() / 60:.0f} minutes)"
            )
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if len(result) == 3:
                        ...
                        logger.warning(
                            f"Unexpected number of values returned from get_latest_geomag_data: {len(result)} (expected 3 with historical)"
                        )
                    ...
                    if (
                        latest_timestamp == "N/A"
                        or latest_dst_value == "N/A"
                        or historical_data is None
                    ):
                        logger.warning(
                            f"No historical geomagnetic data available (attempt {attempt}/{max_attempts})"
                        )
                        if attempt < max_attempts:
                            await asyncio.sleep(attempt * 5)
                        continue
                    ...
                    if historical_data_max_time < target_hour:
                        logger.warning(
                            f"🔒 SECURITY: Historical data ends at {historical_data_max_time}, cannot provide ground truth for {target_hour}"
                        )
                        if attempt < max_attempts:
                    ...
                    if target_data.empty:
                        logger.warning(
                            f"No ground truth data found for specific hour {target_hour} (attempt {attempt}/{max_attempts})"
                        )
                        logger.info(
                            f"Available data range: {historical_data['timestamp'].min()} to {historical_data['timestamp'].max()}"
                        )
                        if attempt < max_attempts:
                            await asyncio.sleep(attempt * 5)
                        continue
                    ...
                    if (
                        abs(ground_truth_value) > 1000
                    ):  # Sanity check for reasonable DST values
                        logger.warning(
                            f"🔒 SECURITY: Ground truth value {ground_truth_value} seems unreasonable for DST, skipping"
                        )
                        if attempt < max_attempts:
                            await asyncio.sleep(attempt * 5)
                        continue
                    ...
                    logger.info(
                        f"✅ SECURITY: Successfully fetched time-specific ground truth for {target_hour}: {ground_truth_value}"
                    )
                    logger.info(
                        f"✅ SECURITY: Ground truth is {time_diff.total_seconds() / 3600:.1f} hours old, ensuring proper temporal separation"
                    )
                    return ground_truth_value

                except Exception as e:
                    logger.warning(
                        f"Error fetching time-specific ground truth for {target_hour}, attempt {attempt}/{max_attempts}: {e}"
                    )
                    if attempt < max_attempts:
                        await asyncio.sleep(attempt * 5)

            logger.warning(
                f"❌ SECURITY: All attempts failed to fetch ground truth for specific hour {target_hour}"
            )
            logger.warning("❌ SECURITY: This ensures no compromised scoring occurs")
            return None

        except Exception as e:
            logger.error(
                f"Error fetching time-specific ground truth for {target_time}: {e}"
            )
            logger.error(f"{traceback.format_exc()}")
            return None
    ...
    async def _start_pending_retry_worker(self, validator):
        ...
        self.pending_retry_worker_task = asyncio.create_task(
            self._pending_retry_worker_loop(validator)
        )
    ...
    async def _pending_retry_worker_loop(self, validator):
        ...
        """
        retry_interval = (
            600 if not self.test_mode else 60
        )  # 10 minutes in production, 1 minute in test mode
        ...
        while self.pending_retry_worker_running:
            try:
                ...
                logger.info(
                    "Pending retry worker: Checking for tasks that can now be scored..."
                )
    ...
    async def _intelligent_retry_pending_tasks(self, validator):
        ...
        try:
            ...
            logger.info(
                f"Found {len(pending_tasks_query)} pending tasks to check for retry"
            )
            ...
            for hour_key, tasks in hourly_pending_tasks.items():
                try:
                    ...
                    if not await self._check_ground_truth_availability(hour_key):
                        logger.debug(
                            f"Ground truth not yet available for hour {hour_key}, skipping {len(tasks)} tasks"
                        )
                        continue

                    logger.info(
                        f"Ground truth appears available for hour {hour_key}, attempting to score {len(tasks)} tasks"
                    )
                    ...
                    if ground_truth_value is None:
                        logger.warning(
                            f"Could not fetch ground truth for hour {hour_key} despite availability check"
                        )
                        continue

                    logger.info(
                        f"Successfully got ground truth value {ground_truth_value} for hour {hour_key}"
                    )
                    ...
                    for task in tasks:
                        try:
                            ...
                            if validator and hasattr(validator, "basemodel_evaluator"):
                                try:
                                    ...
                                    if task_timestamp:
                                        ...
                                        if baseline_score is not None:
                                            logger.debug(
                                                f"Retrieved baseline score for intelligent retry task_id {task_id}: {baseline_score:.4f}"
                                            )
                                except Exception as e:
                                    logger.debug(
                                        f"Error retrieving baseline score for intelligent retry task: {e}"
                                    )
                            ...
                            if baseline_score is not None:
                                ...
                                if score <= baseline_score + epsilon:
                                    logger.debug(
                                        f"Intelligent retry task score zeroed - insufficient improvement: {score:.4f} vs baseline {baseline_score:.4f}"
                                    )
                                    score = 0
                                else:
                                    logger.debug(
                                        f"Intelligent retry task score valid - exceeds baseline by {score - baseline_score:.4f}"
                                    )
                            ...
                            scored_tasks.append(task)
                            logger.info(
                                f"Intelligent retry: Successfully scored task {task['id']} with score {score}"
                            )

                        except Exception as e:
                            logger.error(
                                f"Error processing intelligent retry task {task['id']}: {e}"
                            )
                            continue
                    ...
                    if scored_tasks:
                        await self.build_score_row(hour_key, scored_tasks)
                        logger.info(
                            f"Built score row for {len(scored_tasks)} intelligently retried tasks from hour {hour_key}"
                        )
                        scored_any_tasks = True

                except Exception as e:
                    logger.error(
                        f"Error processing intelligent retry for hour {hour_key}: {e}"
                    )
                    logger.error(traceback.format_exc())
                    continue
            ...
            if scored_any_tasks:
                ...
                logger.debug(
                    "Intelligent retry: No pending tasks could be scored in this attempt"
                )
    ...
    async def _should_retry_task(self, task: dict, max_retries: int = 10) -> bool:
        ...
        try:
            ...
            if retry_count >= max_retries:
                logger.debug(
                    f"Task {task['id']} has exceeded max retries ({retry_count}/{max_retries})"
                )
                return False
            ...
            if last_retry:
                ...
                if isinstance(last_retry, datetime.datetime):
                    ...
                    if time_since_retry.total_seconds() < 1800:  # 30 minutes
                        logger.debug(
                            f"Task {task['id']} was retried recently ({time_since_retry.total_seconds() / 60:.1f} minutes ago)"
                        )
                        return False
            ...
            if task_time:
                if isinstance(task_time, datetime.datetime):
                    ...
                    if task_age.total_seconds() > 604800:  # 7 days
                        logger.debug(
                            f"Task {task['id']} is too old ({task_age.total_seconds() / 86400:.1f} days)"
                        )
                        return False
            ...
            logger.warning(
                f"Error checking if task {task['id']} should be retried: {e}"
            )
