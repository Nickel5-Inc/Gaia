...
class MinerPerformanceCalculator:
    ...
    def _integrate_pathway_data(self, performance: MinerPerformance, miner_uid: str):
        ...
        try:
            ...
            if (
                hasattr(self, "_current_pathway_data")
                and self._current_pathway_data
                and int(miner_uid) in self._current_pathway_data
            ):
                ...
                performance.raw_calculated_weight = pathway_data.get(
                ...
                performance.weather_weight_contribution = pathway_data.get(
                    "weather_weight_contribution"
                )
                performance.geomagnetic_weight_contribution = pathway_data.get(
                    "geomagnetic_weight_contribution"
                )
                performance.soil_weight_contribution = pathway_data.get(
                ...
                if (
                    pathway_data.get("pathway_details")
                    and "percentile_ranks" in pathway_data["pathway_details"]
                ):
                    percentile_ranks = pathway_data["pathway_details"][
                        "percentile_ranks"
                    ]
                    performance.percentile_rank_weather = percentile_ranks.get(
                        "weather"
                    )
                    performance.percentile_rank_geomagnetic = percentile_ranks.get(
                        "geomagnetic"
                    )
                ...
                if (
                    pathway_data.get("pathway_details")
                    and "excellence_qualified_tasks" in pathway_data["pathway_details"]
                ):
                    performance.excellence_qualified_tasks = pathway_data[
                        "pathway_details"
                    ]["excellence_qualified_tasks"]

                logger.debug(
                    f"Integrated pathway data for miner {miner_uid}: pathway={performance.scoring_pathway}, weight={performance.submitted_weight}"
                )

            # === NEW: Integrate consensus data if available ===
            if (
                hasattr(self, "_current_consensus_data")
                and self._current_consensus_data
                and int(miner_uid) in self._current_consensus_data
            ):
                ...
                if not performance.validator_hotkey and consensus_data.get(
                    "validator_hotkey"
                ):
                    performance.validator_hotkey = consensus_data.get(
                        "validator_hotkey"
                    )

                logger.debug(
                    f"Integrated consensus data for miner {miner_uid}: incentive={performance.incentive}, rank={performance.consensus_rank}"
                )
    ...
    async def calculate_period_stats(
        ...
        """
        logger.info(
            f"Calculating {period_type} performance stats for period {period_start} to {period_end}"
        )
        ...
        for miner_uid in miner_uids:
            try:
                ...
                logger.error(
                    f"Error calculating performance for miner {miner_uid}: {e}"
                )
                continue
    ...
    async def _get_active_miners(self, start: datetime, end: datetime) -> List[str]:
        ...
        weather_miners = (
            sa.select(weather_miner_responses_table.c.miner_uid.distinct())
            .select_from(
        ...
        soil_miners = sa.select(
            soil_moisture_history_table.c.miner_uid.distinct()
        ).where(soil_moisture_history_table.c.scored_at.between(start, end))
        ...
        logger.info(
            f"Found {len(active_registered)} active registered miners out of {len(all_active_miners)} total active miners"
        )
    ...
    async def _calculate_weather_metrics(
        ...
        query = (
            ...
            .select_from(
                weather_miner_responses_table.join(
                    weather_forecast_runs_table
            ...
            .where(
                sa.and_(
                    ...
                    weather_forecast_runs_table.c.run_initiation_time.between(
                        start, end
                    ),
                )
            )
        ...
        metrics.completed = status_counts.get("completed", 0) + status_counts.get(
            "verified", 0
        )
        ...
        if calc_times is not None and len(valid_run_times) > 0:
            ...
            for i, (run_time, calc_time) in enumerate(
                zip(valid_run_times, calc_times[valid_score_mask])
            ):
                if calc_time is not None and run_time is not None:
                    delay_days = (
                        (calc_time - run_time).days if calc_time > run_time else 0
                    )
                    if (
                        delay_days > 5
                    ):  # Scores calculated more than 5 days after forecast
                        delayed_scores += 1
        ...
        metrics.additional_metrics = {
            ...
            "delayed_scores_ratio": (
                delayed_scores / max(len(valid_scores), 1)
                if delayed_scores > 0
                else 0.0
            ),
        }
    ...
    async def _calculate_soil_moisture_metrics(
        ...
        metrics.additional_metrics = {
            "avg_surface_rmse": (
                float(np.mean(valid_surf_rmses)) if len(valid_surf_rmses) > 0 else None
            ),
            "avg_rootzone_rmse": (
                float(np.mean(valid_root_rmses)) if len(valid_root_rmses) > 0 else None
            ),
            "avg_surface_structure": (
                float(np.mean(valid_surf_scores))
                if len(valid_surf_scores) > 0
                else None
            ),
            "avg_rootzone_structure": (
                float(np.mean(valid_root_scores))
                if len(valid_root_scores) > 0
                else None
            ),
    ...
    async def _calculate_geomagnetic_metrics(
        ...
        metrics.additional_metrics = {
            "avg_prediction_error": (
                float(np.mean(valid_errors)) if len(valid_errors) > 0 else None
            ),
            "total_predictions": len(row_data),
    ...
    def _calculate_overall_metrics(self, performance: MinerPerformance):
        ...
        for task_metrics in [
            performance.weather,
            performance.soil_moisture,
            performance.geomagnetic,
        ]:
            if (
                task_metrics
                and task_metrics.avg_score is not None
                and task_metrics.scored > 0
            ):
        ...
        if task_scores:
            ...
            performance.overall_avg_score = float(
                np.average(scores_array, weights=weights_array)
            )
    ...
    async def _calculate_trends_and_status(self, performance: MinerPerformance):
        ...
        if trend_data and len(trend_data) >= 3:  # Need at least 3 points for trend
            ...
            time_numeric = np.array(
                [(t - trend_start).days for t in times], dtype=np.float64
            )
    ...
    async def _get_trend_data(
        self, miner_uid: str, start_time: datetime, end_time: datetime
    ) -> List[Dict]:
    ...
    def _calculate_detailed_metrics(self, performance: MinerPerformance):
        ...
        if all_scores:
            ...
            performance.score_distribution = {
                ...
                "overall_std": (
                    float(np.std(scores_only)) if len(scores_only) > 1 else 0.0
                ),
    ...
    def _calculate_ranks(self, performances: List[MinerPerformance]):
        ...
        scored_performances = [
            p for p in performances if p.overall_avg_score is not None
        ]
        if scored_performances:
            scores = np.array(
                [p.overall_avg_score for p in scored_performances], dtype=np.float64
            )
    ...
    async def save_performance_stats(self, performances: List[MinerPerformance]):
        ...
        for perf in performances:
            row_data = {
                ...
                "weather_tasks_attempted": (
                    perf.weather.attempted if perf.weather else 0
                ),
                "weather_tasks_completed": (
                    perf.weather.completed if perf.weather else 0
                ),
                ...
                "weather_success_rate": (
                    perf.weather.success_rate if perf.weather else None
                ),
                ...
                "weather_latest_score": (
                    perf.weather.latest_score if perf.weather else None
                ),
                "soil_moisture_tasks_attempted": (
                    perf.soil_moisture.attempted if perf.soil_moisture else 0
                ),
                "soil_moisture_tasks_completed": (
                    perf.soil_moisture.completed if perf.soil_moisture else 0
                ),
                "soil_moisture_tasks_scored": (
                    perf.soil_moisture.scored if perf.soil_moisture else 0
                ),
                "soil_moisture_avg_score": (
                    perf.soil_moisture.avg_score if perf.soil_moisture else None
                ),
                "soil_moisture_success_rate": (
                    perf.soil_moisture.success_rate if perf.soil_moisture else None
                ),
                "soil_moisture_rank": (
                    perf.soil_moisture.rank if perf.soil_moisture else None
                ),
                "soil_moisture_surface_rmse_avg": (
                    perf.soil_moisture.additional_metrics.get("avg_surface_rmse")
                    if perf.soil_moisture and perf.soil_moisture.additional_metrics
                    else None
                ),
                ...
                "soil_moisture_best_score": (
                    perf.soil_moisture.best_score if perf.soil_moisture else None
                ),
                "soil_moisture_latest_score": (
                    perf.soil_moisture.latest_score if perf.soil_moisture else None
                ),
                "geomagnetic_tasks_attempted": (
                    perf.geomagnetic.attempted if perf.geomagnetic else 0
                ),
                "geomagnetic_tasks_completed": (
                    perf.geomagnetic.completed if perf.geomagnetic else 0
                ),
                "geomagnetic_tasks_scored": (
                    perf.geomagnetic.scored if perf.geomagnetic else 0
                ),
                "geomagnetic_avg_score": (
                    perf.geomagnetic.avg_score if perf.geomagnetic else None
                ),
                "geomagnetic_success_rate": (
                    perf.geomagnetic.success_rate if perf.geomagnetic else None
                ),
                "geomagnetic_rank": perf.geomagnetic.rank if perf.geomagnetic else None,
                "geomagnetic_best_score": (
                    perf.geomagnetic.best_score if perf.geomagnetic else None
                ),
                "geomagnetic_latest_score": (
                    perf.geomagnetic.latest_score if perf.geomagnetic else None
                ),
                "geomagnetic_avg_error": (
                    perf.geomagnetic.additional_metrics.get("avg_prediction_error")
                    if perf.geomagnetic and perf.geomagnetic.additional_metrics
                    else None
                ),
                # === NEW WEIGHT CALCULATION PIPELINE FIELDS ===
                ...
                "pathway_details": (
                    json.dumps(perf.pathway_details) if perf.pathway_details else None
                ),
                # === NEW CHAIN CONSENSUS INTEGRATION FIELDS ===
                ...
                "detailed_metrics": (
                    json.dumps(perf.detailed_metrics) if perf.detailed_metrics else None
                ),
                "score_distribution": (
                    json.dumps(perf.score_distribution)
                    if perf.score_distribution
                    else None
                ),
        ...
        logger.info(
            f"Successfully saved performance stats for {len(performances)} miners"
        )
    ...
    async def cleanup_deregistered_miners(self) -> int:
        ...
        try:
            ...
            if not registered_uids:
                logger.warning(
                    "No registered miners found - skipping performance stats cleanup"
                )
                return 0

            # Get all miners with performance stats
            stats_miners_query = sa.select(
                miner_performance_stats_table.c.miner_uid.distinct()
            )
            ...
            logger.info(
                f"Cleaned up performance stats for {deleted_count} deregistered miners: {list(deregistered_uids)[:10]}..."
            )
    ...
    async def cleanup_specific_miner(
        self, miner_uid: int, miner_hotkey: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            if miner_hotkey:
                conditions.append(
                    miner_performance_stats_table.c.miner_hotkey == miner_hotkey
                )
            ...
            if deleted_count > 0:
                logger.info(
                    f"Cleaned up {deleted_count} performance records for miner UID {miner_uid}"
                )
    ...
    async def calculate_bulk_rankings(
        self, performances: List[MinerPerformance]
    ) -> Dict[str, np.ndarray]:
        """
        ...
        weather_scores = np.array(
            [
                (
                    p.weather.avg_score
                    if p.weather and p.weather.avg_score is not None
                    else 0.0
                )
                for p in performances
            ],
        ...
        soil_scores = np.array(
            [
                (
                    p.soil_moisture.avg_score
                    if p.soil_moisture and p.soil_moisture.avg_score is not None
                    else 0.0
                )
                for p in performances
            ],
        ...
        geo_scores = np.array(
            [
                (
                    p.geomagnetic.avg_score
                    if p.geomagnetic and p.geomagnetic.avg_score is not None
                    else 0.0
                )
                for p in performances
            ],
        ...
        # Overall rankings
        overall_percentiles = np.percentile(
            overall_scores, np.arange(0, 101, 10)
        )  # Decile rankings
        ...
        overall_quantiles = np.quantile(
            overall_scores[overall_scores > 0], [0.25, 0.5, 0.75, 0.9]
        )
        rankings["performance_tiers"] = np.digitize(overall_scores, overall_quantiles)

        return rankings

    def batch_calculate_score_distributions(
        self, all_scores: List[np.ndarray]
    ) -> Dict[str, float]:
        """
        ...
        combined_scores = np.concatenate(
            [scores for scores in all_scores if len(scores) > 0]
        )
        ...
        distribution = {
            ...
            "iqr": float(
                np.percentile(combined_scores, 75) - np.percentile(combined_scores, 25)
            ),
...
async def calculate_daily_stats(db_manager, target_date: datetime):
    ...
    performances = await calculator.calculate_period_stats(
        period_type="daily", period_start=period_start, period_end=period_end
    )
...
async def calculate_weekly_stats(db_manager, week_start: datetime):
    ...
    performances = await calculator.calculate_period_stats(
        period_type="weekly", period_start=period_start, period_end=period_end
    )
...
async def calculate_monthly_stats(db_manager, month_start: datetime):
    ...
    performances = await calculator.calculate_period_stats(
        period_type="monthly", period_start=period_start, period_end=period_end
    )
