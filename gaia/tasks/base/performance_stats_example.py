...
async def example_query_performance_stats():
    ...
    async with engine.begin() as conn:
        ...
        query = (
            ...
            .order_by(miner_performance_stats_table.c.overall_rank.asc())
            .limit(10)
        )
        ...
        for row in top_performers:
            print(
                f"Rank {row.overall_rank:2d}: {row.miner_hotkey[:12]}... "
...
async def example_get_task_specific_rankings():
    ...
    async with engine.begin() as conn:
        ...
        weather_query = (
            ...
            .order_by(miner_performance_stats_table.c.weather_rank.asc())
            .limit(5)
        )
        ...
        for row in weather_leaders:
            print(
                f"Rank {row.weather_rank}: {row.miner_hotkey[:12]}... "
...
async def example_bulk_analysis():
    ...
    async with engine.begin() as conn:
        ...
        performances = await calculator.calculate_period_stats(
            period_type="weekly", period_start=start_time, period_end=end_time
        )
        ...
        if performances:
            ...
            print(
                f"Overall score percentiles: {rankings.get('overall_percentiles', [])}"
            )
            ...
            if all_scores:
                distribution = calculator.batch_calculate_score_distributions(
                    all_scores
                )
...
async def example_monitoring_dashboard_data():
    ...
    async with engine.begin() as conn:
        ...
        query = (
            ...
            .where(miner_performance_stats_table.c.period_type == "daily")
            .order_by(
        ...
        if dashboard_data:
            ...
            active_miners = [
                row for row in dashboard_data if row.total_tasks_attempted > 0
            ]

            print(f"Active miners: {len(active_miners)}")
            print(
                f"Avg success rate: {np.mean([row.overall_success_rate or 0 for row in active_miners]):.1%}"
            )
