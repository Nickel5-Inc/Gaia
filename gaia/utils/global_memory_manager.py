...
class GlobalMemoryManager:
    ...
    def register_cleanup_callback(
        ...
        with self._lock:
            ...
            logger.debug(
                f"Registered cleanup callback '{name}' for thread {thread.name}"
            )
    ...
    def unregister_cleanup_callback(self, name: str) -> bool:
        ...
        with self._lock:
            if name in self._cleanup_callbacks:
                ...
                self._cleanup_stats["registered_callbacks"] = len(
    ...
    def trigger_coordinated_cleanup(self, context: str = "global") -> Dict[str, any]:
        ...
        if stats["callbacks_attempted"] > 0:
            logger.info(
                f"Global memory cleanup ({context}): {stats['callbacks_succeeded']}/{stats['callbacks_attempted']} "
                f"callbacks succeeded, GC collected {stats['gc_collected']} objects in {duration:.3f}s"
            )

        if stats["callbacks_failed"] > 0:
            logger.warning(
                f"Global cleanup had {stats['callbacks_failed']} failures: {stats['errors']}"
            )
...
class ThreadLocalCleanup:
    ...
    def register_cache_cleanup(
        self, name: str, cache_obj, clear_method: str = "clear"
    ) -> None:
        """
    ...
    def register_custom_cleanup(
        self, name: str, cleanup_func: Callable[[], None]
    ) -> None:
