...
class LiveProfiler:
    ...
    def __init__(self):
        ...
        self.profile_mode = os.getenv(
            "PROFILE_MODE", "hotspots"
        ).lower()  # hotspots, detailed, memory
        ...
        if self.enabled:
            logger.info(
                f"🔍 Live profiler enabled - mode: {self.profile_mode}, interval: {self.profile_interval}s"
            )
                ...
                try:
                    ...
                    with self.lock:
                        ...
                        if execution_time > threshold_seconds:
                            ...
                            logger.warning(
                                f"🐌 Slow function detected: {function_name} took {execution_time:.3f}s"
                            )
                ...
                try:
                    ...
                    with self.lock:
                        ...
                        if execution_time > threshold_seconds:
                            self.slow_functions[function_name] += 1
                            logger.warning(
                                f"🐌 Slow async function detected: {function_name} took {execution_time:.3f}s"
                            )
        ...
        try:
            ...
            with self.lock:
                ...
                if execution_time > threshold_seconds:
                    ...
                    logger.warning(
                        f"🐌 Slow code block: {block_name} took {execution_time:.3f}s"
                    )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for func_name, count in sorted(
                self.slow_functions.items(), key=lambda x: x[1], reverse=True
            )[:10]:
    ...
    def print_performance_report(self):
        ...
        sorted_by_avg = sorted(
            summary["average_times"].items(), key=lambda x: x[1]["avg"], reverse=True
        )
        for func_name, data in sorted_by_avg[:5]:
            logger.info(
                f"  {data['avg']:.3f}s avg ({data['max']:.3f}s max, {data['calls']} calls): {func_name}"
            )
    ...
    def save_cprofile_snapshot(self, duration_seconds=30):
        ...
        logger.info(
            f"📊 Human-readable report: {profile_file.replace('.prof', '.txt')}"
        )
    ...
    async def start_continuous_monitoring(self):
        ...
        logger.info(
            f"🔍 Starting continuous performance monitoring (interval: {self.profile_interval}s)"
        )
...
class HotspotDetector:
    ...
    def track_execution(
        ...
        logger.warning(
            f"🔥 HOTSPOT DETECTED: {function_name} took {execution_time:.3f}s"
        )
    ...
    def print_hotspot_summary(self):
        ...
        logger.info(
            f"🔥 HOTSPOT SUMMARY (last 10 minutes): {len(recent_hotspots)} hotspots detected"
        )
        ...
        for func_name, times in sorted(
            ...
            max_time = max(times)
            logger.warning(
                f"  {len(times):2d}x {func_name}: avg {avg_time:.3f}s, max {max_time:.3f}s"
            )
...
def track_hotspot(
    function_name: str, execution_time: float, call_stack: Optional[str] = None
):
