...
try:
    ...
    print(
        "psutil not found, memory logging will be skipped."
    )  # Use print for early feedback
...
def patched_query_substrate(
    substrate, module: str, storage_function: str, params=None, block_hash=None
):
    """Patched version of query_substrate that handles ProcessIsolatedSubstrate results."""
    try:
        result = _original_query_substrate(
            substrate, module, storage_function, params, block_hash
        )
        return result
    except AttributeError as e:
        if "'int' object has no attribute 'value'" in str(
            e
        ) or "'list' object has no attribute 'value'" in str(e):
            # The error is because substrate.query returned a raw value instead of an object with .value
            logger.debug(
                "query_substrate compatibility fix: handling .value attribute error"
            )
            ...
            class CompatibleResult:
                ...
                def __iter__(self):
                    return (
                        ...
                        else iter([self.value])
                    )

            # Get the actual result by calling substrate.query directly (this should return the raw value)
            try:
                direct_result = substrate.query(
                    module, storage_function, params, block_hash
                )
                logger.debug(
                    f"query_substrate compatibility fix: wrapped {type(direct_result).__name__} as CompatibleResult"
                )
...
async def perform_handshake_with_retry(
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            logger.debug(
                f"Handshake attempt {attempt + 1}/{max_retries + 1} with timeout {current_timeout:.1f}s"
            )
            ...
            if attempt < max_retries:
                ...
                logger.warning(
                    f"Handshake timeout on attempt {attempt + 1}, retrying in {wait_time:.1f}s..."
                )
                await asyncio.sleep(wait_time)
                continue
            else:
                logger.error(
                    f"Handshake failed after {max_retries + 1} attempts due to timeout"
                )
                break
        except Exception as e:
            last_exception = e
            if attempt < max_retries:
                ...
                logger.warning(
                    f"Handshake error on attempt {attempt + 1}: {type(e).__name__} - {e}, retrying in {wait_time:.1f}s..."
                )
                await asyncio.sleep(wait_time)
                continue
            else:
                logger.error(
                    f"Handshake failed after {max_retries + 1} attempts due to error: {type(e).__name__} - {e}"
                )
                break
...
class GaiaValidator:
        ...
        try:
            ...
            for path in sys.path:
                if "site-packages" in path:
                    ...
                    potential_asyncio_dist = os.path.join(
                        path, "asyncio-3.4.3.dist-info"
                    )

                    if os.path.exists(potential_asyncio_path) and os.path.exists(
                        potential_asyncio_dist
                    ):
                        print(
                            f"[STARTUP] ⚠️ Detected problematic asyncio package at: {potential_asyncio_path}"
                        )
            ...
            try:
                ...
                if "site-packages" in asyncio_file and "asyncio" in asyncio_file:
                    print(
                        f"[STARTUP] ❌ Asyncio importing from site-packages: {asyncio_file}"
                    )
                    ...
                    asyncio_dist_dir = os.path.join(
                        asyncio_package_path, "asyncio-3.4.3.dist-info"
                    )

                    print(
                        "[STARTUP] 🔧 Attempting to remove problematic asyncio package..."
                    )
                    ...
                    try:
                        ...
                        if "/usr/lib/python" in new_asyncio_file:
                            print(
                                f"[STARTUP] ✅ Successfully fixed asyncio! Now using: {new_asyncio_file}"
                            )
                            ...
                            if test_result.returncode == 0:
                                print(
                                    "[STARTUP] ✅ Asyncio multiprocessing compatibility verified"
                                )
                            else:
                                print(
                                    f"[STARTUP] ⚠️ Asyncio test failed: {test_result.stderr}"
                                )

                        else:
                            print(
                                f"[STARTUP] ⚠️ Asyncio still not using built-in version: {new_asyncio_file}"
                            )
                    ...
                    print(
                        f"[STARTUP] ✅ Asyncio using built-in version: {asyncio_file}"
                    )
    ...
    def _clear_pycache_files(self):
        ...
        try:
            ...
            result_pyc = subprocess.run(
                cmd_pyc, shell=True, capture_output=True, text=True
            )

            # Clear __pycache__ directories
            cmd_pycache = f"find {repo_root} -name '__pycache__' -type d -exec rm -rf {{}} + 2>/dev/null || true"
            result_pycache = subprocess.run(
                cmd_pycache, shell=True, capture_output=True, text=True
            )
    ...
    def __init__(self, args):
        ...
        if os.getenv("VALIDATOR_CLEAR_PYCACHE_ON_STARTUP", "true").lower() in [
            "true",
            "1",
            "yes",
        ]:
        ...
        self.geomagnetic_task = GeomagneticTask(
            node_type="validator", db_manager=self.database_manager, test_mode=args.test
        )
        ...
        self.miner_client = httpx.AsyncClient(
            timeout=httpx.Timeout(
                connect=5.0, read=30.0, write=15.0, pool=3.0
            ),  # REDUCED: 120s->30s read, 10s->5s connect
        ...
        self.memory_monitor_enabled = os.getenv(
            "VALIDATOR_MEMORY_MONITORING_ENABLED", "true"
        ).lower() in ["true", "1", "yes"]
        ...
        self.memory_warning_threshold_mb = int(
            os.getenv("VALIDATOR_MEMORY_WARNING_THRESHOLD_MB", "8000")
        )  # 8GB (reduced from 10GB)
        self.memory_emergency_threshold_mb = int(
            os.getenv("VALIDATOR_MEMORY_EMERGENCY_THRESHOLD_MB", "10000")
        )  # 10GB (reduced from 12GB)
        self.memory_critical_threshold_mb = int(
            os.getenv("VALIDATOR_MEMORY_CRITICAL_THRESHOLD_MB", "12000")
        )  # 12GB (reduced from 15GB)
        ...
        self.emergency_gc_cooldown = (
            60  # Minimum 60 seconds between emergency GC attempts
        )
        ...
        self.is_source_validator_for_db_sync = (
        ...
        if self.args.test:
            ...
            logger.info(
                f"Test mode enabled: DB sync interval set to {self.db_sync_interval_hours} hours (15 minutes)."
            )
        else:
            ...
            if self.db_sync_interval_hours <= 0:
                logger.warning(
                    f"DB_SYNC_INTERVAL_HOURS ('{os.getenv('DB_SYNC_INTERVAL_HOURS')}') is invalid ({self.db_sync_interval_hours}). Defaulting to 1 hour."
                )
        ...
        self.DB_MONITOR_HISTORY_MAX_SIZE = (
            120  # e.g., 2 hours of data if monitor runs every minute
        )
        ...
        self.tracemalloc_snapshot1: Optional[tracemalloc.Snapshot] = (
        ...
        for dt_thresh, weights_dict in self.task_weight_schedule:
            if not math.isclose(sum(weights_dict.values()), 1.0):
                logger.error(
                    f"Task weights for threshold {dt_thresh.isoformat()} do not sum to 1.0! Sum: {sum(weights_dict.values())}. Fix configuration."
                )
    ...
    async def _initiate_shutdown(self):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to create cleanup completion file: {e_cleanup_file}"
                )
            ...
            for task_name in [
                ...
                try:
                    ...
                    if (
                        task_name in self.task_health
                        or hasattr(self, f"{task_name}_task")
                        or (task_name.startswith("db_sync") and self.auto_sync_manager)
                    ):
                        await self.update_task_status(task_name, "stopping")
                    else:
                        logger.debug(
                            f"Skipping status update for non-existent/inactive task: {task_name}"
                        )
                except Exception as e_status_update:
                    logger.error(
                        f"Error updating {task_name} task status during shutdown: {e_status_update}"
                    )
            ...
            try:
                ...
                logger.warning(
                    "Resource cleanup timed out after 30 seconds, proceeding with shutdown"
                )
            ...
            logger.error(
                f"Error during main shutdown sequence: {e_shutdown_main}", exc_info=True
            )
            # Ensure cleanup_done is set even if part of the main shutdown fails, to prevent re-entry
            self._cleanup_done = True
            logger.warning(
                "Graceful shutdown sequence partially completed due to error."
            )
            ...
            try:
                ...
                logger.info(
                    f"Created cleanup completion file (with errors): {cleanup_file}"
                )
            except Exception as e_cleanup_file:
                logger.error(
                    f"Failed to create cleanup completion file after error: {e_cleanup_file}"
                )
    ...
    def _get_substrate_interface(self):
        ...
        try:
            ...
            logger.warning(
                f"Failed to create process-isolated substrate interface: {e}"
            )
        ...
        try:
            ...
            def query_wrapper(self, module, storage_function, params, block_hash=None):
                result = original_query(
                    self, module, storage_function, params, block_hash
                )
                if hasattr(result, "value"):
                    if isinstance(result.value, list):
                        result.value = [
                            int(x) if hasattr(x, "__int__") else x for x in result.value
                        ]
            ...
            def blocks_since_wrapper(substrate, netuid, node_id):
                ...
                last_updated_value = substrate.query(
                    "SubtensorModule", "LastUpdate", [netuid]
                )
            ...
            try:
                ...
                self.substrate_connection_created_at = (
                    time.time()
                )  # Track initial connection age
                logger.info(
                    "🛡️  Created isolated substrate interface for memory leak prevention"
                )
            except Exception as e_sub_init:
                logger.error(
                    f"CRITICAL: Failed to initialize isolated substrate interface with endpoint {self.subtensor_chain_endpoint}: {e_sub_init}",
                    exc_info=True,
                )
                return False
            ...
            try:
                ...
                logger.error(
                    f"CRITICAL: Failed to initialize Metagraph: {e_meta_init}",
                    exc_info=True,
                )
                return False
            ...
            try:
                ...
                logger.info(
                    f"Successfully synced {len(self.metagraph.nodes) if self.metagraph.nodes else '0'} nodes from the network."
                )
            except Exception as e_meta_sync:
                logger.error(
                    f"CRITICAL: Metagraph sync_nodes() FAILED: {e_meta_sync}",
                    exc_info=True,
                )
    ...
    async def query_miners(
        self, payload: Dict, endpoint: str, hotkeys: Optional[List[str]] = None
    ) -> Dict:
        """Query miners with the given payload in parallel with batch retry logic."""
        try:
            logger.info(
                f"Querying miners for endpoint {endpoint} with payload size: {len(str(payload))} bytes. Specified hotkeys: {hotkeys if hotkeys else 'All/Default'}"
            )
            ...
            try:
                ...
                if payload_size_mb > 50:  # Log large payloads early
                    logger.warning(
                        f"Large payload detected: {payload_size_mb:.1f}MB - will reuse same reference for all miners"
                    )
            except Exception:
                pass
            ...
            if (
                ...
                or current_time - self.last_metagraph_sync
                > self.metagraph_sync_interval
            ):
                logger.info(
                    f"Metagraph not initialized or sync interval ({self.metagraph_sync_interval}s) exceeded. Syncing metagraph before querying miners. Last sync: {current_time - self.last_metagraph_sync if self.metagraph else 'Never'}s ago."
                )
                try:
                    await asyncio.wait_for(self._sync_metagraph(), timeout=60.0)
                except asyncio.TimeoutError:
                    logger.error(
                        "Metagraph sync timed out within query_miners. Proceeding with potentially stale metagraph."
                    )
                except Exception as e_sync:
                    logger.error(
                        f"Error during metagraph sync in query_miners: {e_sync}. Proceeding with potentially stale metagraph."
                    )
            else:
                logger.debug(
                    f"Metagraph recently synced. Skipping sync for this query_miners call. Last sync: {current_time - self.last_metagraph_sync:.2f}s ago."
                )

            if not self.metagraph or not self.metagraph.nodes:
                logger.error(
                    "Metagraph not available or no nodes in metagraph after sync attempt. Cannot query miners."
                )
                return {}
            ...
            if hotkeys:
                ...
                for hk in hotkeys:
                    if hk in nodes_to_consider:
                        ...
                        logger.warning(
                            f"Specified hotkey {hk} not found in current metagraph. Skipping."
                        )
                if not miners_to_query:
                    logger.warning(
                        f"No specified hotkeys found in metagraph. Querying will be empty for endpoint: {endpoint}"
                    )
                    return {}
            else:
                ...
                if self.args.test and len(miners_to_query) > 10:
                    ...
                    miners_to_query = {
                        k: miners_to_query[k] for k in selected_hotkeys_for_test
                    }
                    logger.info(
                        f"Test mode: Selected the last {len(miners_to_query)} miners to query for endpoint: {endpoint} (no specific hotkeys provided)."
                    )
                elif not self.args.test:
                    logger.info(
                        f"Querying all {len(miners_to_query)} available miners for endpoint: {endpoint} (no specific hotkeys provided)."
                    )

            if not miners_to_query:
                logger.warning(
                    f"No miners to query for endpoint {endpoint} after filtering. Hotkeys: {hotkeys}"
                )
                return {}
            ...
            chunk_concurrency = (
                6  # REDUCED: 15->6 to prevent thread/connection exhaustion
            )
            chunks = []
            ...
            logger.info(
                f"Processing {len(miners_to_query)} miners in {len(chunks)} chunks of {chunk_size} (concurrency: {chunk_concurrency} per chunk)"
            )
            ...
            async def query_single_miner_with_retries(
                miner_hotkey: str, node, semaphore: asyncio.Semaphore
            ) -> Optional[Dict]:
                ...
                async with (
                    semaphore
                ):  # Acquire semaphore before starting attempts for a miner
                    for attempt in range(max_retries_per_miner):
                        ...
                        try:
                            logger.debug(
                                f"Miner {miner_hotkey} attempt {attempt + 1}/{max_retries_per_miner}"
                            )
                            ...
                            try:
                                ...
                                if not success:
                                    raise Exception(
                                        "Handshake failed: server returned unsuccessful status"
                                    )

                                symmetric_key_str = base64.b64encode(
                                    symmetric_key
                                ).decode()

                            except Exception as hs_err:
                                logger.debug(
                                    f"Handshake failed for miner {miner_hotkey} attempt {attempt + 1}: {type(hs_err).__name__}"
                                )
                                ...
                                return {
                                    ...
                                    "details": f"{type(hs_err).__name__}",
                                }

                            handshake_duration = time.time() - handshake_start_time
                            logger.debug(
                                f"Handshake with {miner_hotkey} completed in {handshake_duration:.2f}s (attempt {attempt + 1})"
                            )

                            if process:
                                logger.debug(
                                    f"Memory after handshake ({miner_hotkey}): {process.memory_info().rss / (1024 * 1024):.2f} MB"
                                )
                            ...
                            try:
                                logger.debug(
                                    f"Making request to {miner_hotkey} (attempt {attempt + 1})"
                                )
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        and len(resp.content) > 1024 * 1024
                                    ):  # > 1MB
                                        logger.debug(
                                            f"Clearing large response content for {miner_hotkey}: {len(resp.content)} bytes"
                                        )

                                except Exception as cleanup_err:
                                    logger.debug(
                                        f"Non-critical cleanup error for {miner_hotkey}: {cleanup_err}"
                                    )
                                ...
                                if process:
                                    ...
                                    logger.debug(
                                        f"Memory after request ({miner_hotkey}): {mem_after:.2f} MB"
                                    )
                                ...
                                if resp and resp.status_code < 400:
                                    ...
                                    logger.info(
                                        f"SUCCESS: {miner_hotkey} responded in {request_duration:.2f}s (attempt {attempt + 1})"
                                    )
                                    return response_data  # Success, return immediately
                                else:
                                    logger.debug(
                                        f"Bad response from {miner_hotkey} attempt {attempt + 1}: status {resp.status_code if resp else 'None'}"
                                    )
                                    if attempt < max_retries_per_miner - 1:
                                    ...
                                    return {
                                        ...
                                        "status": "failed",
                                        "reason": "Bad Response",
                                        "details": f"Status code {resp.status_code if resp else 'N/A'}",
                                    }
                                ...
                                return {
                                    ...
                                    "details": "Timeout during non-streamed POST",
                                }
                                ...
                                logger.debug(
                                    f"Request error for {miner_hotkey} attempt {attempt + 1}: {type(request_error).__name__}"
                                )
                                if attempt < max_retries_per_miner - 1:
                                    await asyncio.sleep(0.5 * (attempt + 1))
                                    continue  # Go to next attempt for this miner
                                return {
                                    ...
                                    "details": f"{type(request_error).__name__}",
                                }

                        except Exception as outer_error:  # Catch errors within the attempt loop but outside handshake/request
                            logger.debug(
                                f"Outer error for {miner_hotkey} attempt {attempt + 1}: {type(outer_error).__name__} - {outer_error}"
                            )
                            if attempt < max_retries_per_miner - 1:
                                await asyncio.sleep(0.5 * (attempt + 1))
                                continue  # Go to next attempt for this miner
                            return {
                                ...
                                "status": "failed",
                                "reason": "Outer Error",
                                "details": f"{type(outer_error).__name__}",
                            }

                    # If loop finishes, all attempts for this miner failed (should be caught by returns above but as a fallback)
                    logger.debug(
                        f"All {max_retries_per_miner} attempts failed for {miner_hotkey} (fallback)."
                    )
                    return {
                        "hotkey": miner_hotkey,
                        "status": "failed",
                        "reason": "All Attempts Failed",
                        "details": "Fell through retry loop",
                    }
            ...
            for chunk_idx, chunk_miners in enumerate(chunks):
                ...
                logger.info(
                    f"Processing chunk {chunk_idx + 1}/{len(chunks)} with {len(chunk_miners)} miners..."
                )
                ...
                for hotkey, node in chunk_miners.items():
                    if node.ip and node.port:
                        task = asyncio.create_task(
                            query_single_miner_with_retries(
                                hotkey, node, chunk_semaphore
                            ),
                ...
                if just_tasks:  # Only process if we have tasks
                    for completed_task in asyncio.as_completed(just_tasks):
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Exception processing response in chunk {chunk_idx + 1}: {e}"
                            )

                chunk_time = time.time() - chunk_start_time
                chunk_successful = len(
                    [
                        r
                        for r in successful_responses.values()
                        if r.get("hotkey") in chunk_miners
                    ]
                )
                chunk_failed = len(chunk_miners) - chunk_successful
                logger.info(
                    f"Chunk {chunk_idx + 1}/{len(chunks)} completed: {chunk_successful} successful, {chunk_failed} failed in {chunk_time:.2f}s"
                )
                ...
                try:
                    ...
                    if chunk_idx % 3 == 0:  # Every 3 chunks
                        ...
                        if collected > 20:
                            logger.debug(
                                f"GC collected {collected} objects after chunk {chunk_idx + 1}"
                            )
                except Exception as cleanup_err:
            ...
            success_rate = (
                success_count / total_queries * 100 if total_queries > 0 else 0
            )
            avg_rate = total_queries / total_time if total_time > 0 else 0

            logger.info(
                f"Query Summary - Total: {total_queries}, Success: {success_count} ({success_rate:.1f}%), "
            ...
            try:
                ...
                if collected > 100:  # Log significant cleanup
                    logger.info(
                        f"Final cleanup: GC collected {collected} objects after miner queries"
                    )
    ...
    async def _cleanup_idle_connections(self):
        ...
        try:
            if hasattr(self, "miner_client") and not self.miner_client.is_closed:
                ...
                if hasattr(self.miner_client, "_transport") and hasattr(
                    self.miner_client._transport, "_pool"
                ):
                    ...
                    if hasattr(pool, "_connections"):
                        ...
                        if hasattr(connections, "items"):  # Dict-like
                            ...
                            for key, conn in connection_items:
                                ...
                                if (
                                    hasattr(conn, "is_idle")
                                    and conn.is_idle()
                                    and hasattr(conn, "_idle_time")
                                    and getattr(conn, "_idle_time", 0) > 60
                                ):  # 60 seconds idle
                                    try:
                                        await conn.aclose()
                                        del connections[key]
                                        closed_count += 1
                                        logger.debug(
                                            f"Closed stale idle connection to {key}"
                                        )
                                    except Exception as e:
                                        logger.debug(
                                            f"Error closing idle connection: {e}"
                                        )
                        else:  # List-like
                            for conn in list(connections):
                                if (
                                    ...
                                    try:
                                        ...
                                        closed_count += 1
                                        logger.debug("Closed stale idle connection")
                                    except Exception as e:
                                        logger.debug(
                                            f"Error closing idle connection: {e}"
                                        )
                        ...
                        if closed_count > 0:
                            logger.info(
                                f"Cleaned up {closed_count} stale idle connections"
                            )
                        else:
            ...
            if hasattr(self, "miner_client") and not self.miner_client.is_closed:
                ...
                if hasattr(self.miner_client, "_transport") and hasattr(
                    ...
                    if hasattr(pool, "_connections"):
                            ...
                            for key, conn in connection_items:
                                ...
                                if hasattr(conn, "is_idle") and conn.is_idle():
                                    try:
                                        ...
                                        closed_count += 1
                                        logger.debug(
                                            f"Aggressively closed idle connection to {key}"
                                        )
                                    except Exception as e:
                            ...
                            for conn in list(connections):
                                if hasattr(conn, "is_idle") and conn.is_idle():
                                    try:
                                        ...
                                        closed_count += 1
                                        logger.debug(
                                            "Aggressively closed idle connection"
                                        )
                                    except Exception as e:
                                        logger.debug(f"Error closing connection: {e}")
                        ...
                        if closed_count > 0:
                            logger.info(
                                f"Aggressively cleaned up {closed_count} idle connections"
                            )
    ...
    def _log_memory_usage(self, context: str, threshold_mb: float = 100.0):
        ...
        try:
            ...
            if abs(memory_change) > threshold_mb or context in [
                "calc_weights_start",
                "calc_weights_after_cleanup",
            ]:
                ...
                try:
                    ...
                    if memory_change > 200 and hasattr(
                        self, "last_metagraph_sync"
                    ):  # Only after validator is fully running
                        logger.warning(
                            f"Large memory increase detected ({memory_change:.1f}MB), forcing comprehensive cleanup"
                        )
                        memory_freed = self._comprehensive_memory_cleanup(
                            f"emergency_{context}"
                        )
                        ...
                        logger.info(
                            f"Emergency comprehensive cleanup freed {memory_freed:.1f}MB (total reduction: {total_savings:.1f}MB)"
                        )
                    elif memory_change > 200:
                        logger.info(
                            f"Large memory increase detected during startup ({memory_change:.1f}MB) - skipping cleanup until fully initialized"
                        )

                except Exception as e:
                    logger.info(
                        f"Memory usage [{context}]: {current_memory:.1f}MB ({'+' if memory_change > 0 else ''}{memory_change:.1f}MB) - detailed info error: {e}"
                    )
            else:
                logger.debug(
                    f"Memory usage [{context}]: {current_memory:.1f}MB ({'+' if memory_change > 0 else ''}{memory_change:.1f}MB)"
                )
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if "500" in str(e):
                        logger.warning(
                            f"GitHub temporarily unavailable (500 error): {e}"
                        )
                    else:
    ...
    async def update_task_status(
        self, task_name: str, status: str, operation: Optional[str] = None
    ):
            ...
            if operation:
                if operation != health.get("current_operation"):
                    ...
                    try:
                        ...
                        health["resources"]["memory_peak"] = health["resources"][
                            "memory_start"
                        ]
                ...
                if health.get("current_operation"):
                    ...
                    try:
                        ...
                        memory_change = (
                            current_memory - health["resources"]["memory_start"]
                        )
                        peak_memory = max(
                            current_memory, health["resources"].get("memory_peak", 0)
                        )
                        logger.info(
        ...
        if not self.watchdog_running:
            ...
            if self.memory_monitor_enabled:
                try:
                    ...
                    logger.info("🔍 Validator memory monitoring enabled:")
                    logger.info(
                        f"  System memory: {system_memory.total / (1024**3):.1f} GB total, {system_memory.available / (1024**3):.1f} GB available"
                    )
                    logger.info(
                        f"  Memory thresholds: Warning={self.memory_warning_threshold_mb}MB, Emergency={self.memory_emergency_threshold_mb}MB, Critical={self.memory_critical_threshold_mb}MB"
                    )
                    ...
                    if self.pm2_restart_enabled:
                        ...
                        pm2_id = (
                            os.getenv("pm_id")
                            or os.getenv("NODE_APP_INSTANCE")
                            or os.getenv("PM2_INSTANCE_ID")
                            or "not detected"
                        )
                        ...
                        try:
                            ...
                            if process_name:
                                ...
                                logger.info("  PM2 process name: not detected")
                        except Exception as e:
                            logger.debug(
                                f"Error getting PM2 process name during startup: {e}"
                            )
                except ImportError:
                    logger.warning(
                        "psutil not available - memory monitoring will be disabled"
                    )
    ...
    async def _check_memory_usage(self, current_time):
        ...
        try:
            ...
            if current_time - self.last_memory_log_time > self.memory_log_interval:
                logger.info(
                    f"🔍 Validator memory status: {memory_mb:.1f} MB RSS ({system_percent:.1f}% system memory)"
                )
            ...
            if memory_mb > self.memory_critical_threshold_mb:
                logger.error(
                    f"💀 CRITICAL MEMORY: {memory_mb:.1f} MB - OOM imminent! (threshold: {self.memory_critical_threshold_mb} MB)"
                )

                if critical_operations_active:
                    logger.warning(
                        f"⚠️  Critical operations active: {critical_operations_active}. Will wait maximum 30 seconds before forced restart."
                    )
                    ...
                    if (
                        current_time - self.last_emergency_gc_time > 15
                    ):  # Reduced cooldown to 15 seconds
                        logger.error(
                            "Attempting emergency garbage collection while critical operations are active..."
                        )
                        try:
                            collected = gc.collect()
                            ...
                            immediate_post_gc_memory = process.memory_info().rss / (
                                1024 * 1024
                            )
                            if immediate_post_gc_memory > (
                                self.memory_critical_threshold_mb * 0.95
                            ):  # Still >95% of critical
                                logger.error(
                                    f"🚨 GC INEFFECTIVE: Memory still {immediate_post_gc_memory:.1f} MB after emergency GC"
                                )
                                if self.pm2_restart_enabled:
                                    logger.error(
                                        "🔄 FORCED RESTART: Cannot wait for critical operations - OOM imminent!"
                                    )
                                    await self._trigger_pm2_restart(
                                        "Forced restart - GC ineffective and OOM imminent"
                                    )
                                    return

                        except Exception as gc_err:
                            logger.error(
                                f"Emergency GC failed during critical operation: {gc_err}"
                            )
                            if self.pm2_restart_enabled:
                                logger.error(
                                    "🔄 FORCED RESTART: GC failed and OOM imminent!"
                                )
                                await self._trigger_pm2_restart(
                                    "Forced restart - GC failed during critical operation"
                                )
                                return
                else:
                    # NO critical operations - restart immediately after quick GC attempt
                    logger.error(
                        "NO CRITICAL OPERATIONS - immediate restart after emergency GC..."
                    )
                    try:
                        collected = gc.collect()
                        logger.error(
                            f"Emergency GC freed {collected} objects before restart"
                        )
                        ...
                        if post_gc_memory > (
                            self.memory_critical_threshold_mb * 0.85
                        ):  # Restart if still >85% of critical
                            if self.pm2_restart_enabled:
                                logger.error(
                                    f"🔄 TRIGGERING IMMEDIATE PM2 RESTART: Memory still high after GC ({post_gc_memory:.1f} MB)"
                                )
                                await self._trigger_pm2_restart(
                                    "Immediate restart - memory still critical after GC"
                                )
                                return  # Exit the monitoring
                            else:
                                logger.error(
                                    f"💀 MEMORY CRITICAL BUT PM2 RESTART DISABLED: {post_gc_memory:.1f} MB - system will likely be killed by OOM"
                                )
                        else:
                            logger.info(
                                f"✅ Memory reduced to {post_gc_memory:.1f} MB after GC - continuing with monitoring"
                            )
                    except Exception as gc_err:
                        logger.error(f"Emergency GC failed: {gc_err}")
                        if self.pm2_restart_enabled:
                            logger.error(
                                "🔄 IMMEDIATE RESTART: GC failed and memory critical"
                            )
                            await self._trigger_pm2_restart(
                                "Immediate restart - GC failed and memory critical"
                            )
                            ...
                            logger.error(
                                "💀 GC FAILED AND PM2 RESTART DISABLED - system will likely crash"
                            )

            elif memory_mb > self.memory_emergency_threshold_mb:
                logger.warning(
                    f"🚨 EMERGENCY MEMORY PRESSURE: {memory_mb:.1f} MB - OOM risk HIGH! (threshold: {self.memory_emergency_threshold_mb} MB)"
                )
                ...
                if not hasattr(self, "_emergency_memory_start_time"):
                    ...
                    logger.warning(
                        "🕐 Emergency memory pressure started - will force restart if sustained for 60 seconds"
                    )
                ...
                if (
                    current_time - self.last_emergency_gc_time > 30
                ):  # More frequent GC at emergency level
                    collected = gc.collect()
                    ...
                    if post_emergency_gc_memory < self.memory_emergency_threshold_mb:
                        logger.info(
                            f"✅ Emergency GC successful - memory reduced to {post_emergency_gc_memory:.1f} MB"
                        )
                        ...
                        if critical_operations_active:
                            logger.error(
                                f"🔄 EMERGENCY TIMEOUT: {emergency_duration:.0f}s of emergency memory pressure - forcing restart despite critical operations"
                            )
                        else:
                            logger.error(
                                f"🔄 EMERGENCY TIMEOUT: {emergency_duration:.0f}s of emergency memory pressure - forcing restart"
                            )

                        if self.pm2_restart_enabled:
                            await self._trigger_pm2_restart(
                                f"Emergency memory pressure sustained for {emergency_duration:.0f} seconds"
                            )
                            return
                        else:
                            logger.error(
                                "💀 EMERGENCY TIMEOUT BUT PM2 RESTART DISABLED - system at high OOM risk"
                            )

            elif memory_mb > self.memory_warning_threshold_mb:
                # Only log warning once per interval to avoid spam
                if current_time - self.last_memory_log_time > (
                    self.memory_log_interval / 2
                ):  # Half interval for warnings
                    logger.warning(
                        f"🟡 HIGH MEMORY: Validator process using {memory_mb:.1f} MB ({system_percent:.1f}% of system) (threshold: {self.memory_warning_threshold_mb} MB)"
                    )
            else:
    ...
    def _check_critical_operations_active(self):
        ...
        for task_name, health in self.task_health.items():
            if health["status"] in ["processing"] and health.get("current_operation"):
                ...
                if any(
                    pattern in current_op for pattern in critical_operation_patterns
                ):
    ...
    async def _trigger_pm2_restart(self, reason: str):
        ...
        try:
            ...
            if process_name:
                logger.info(
                    f"Running under PM2 process '{process_name}' - triggering restart..."
                )
                ...
                logger.warning(
                    "Not running under PM2 or process not found - triggering system exit"
                )
    ...
    async def _check_resource_usage(self, current_time):
        ...
        for task_name, health in self.task_health.items():
            if health["status"] != "idle":
                ...
                if (
                    current_memory > health["resources"]["memory_peak"] * 1.5
                ):  # 50% increase
                    logger.warning(
    ...
    async def _check_task_health(self, current_time):
        ...
        for task_name, health in self.task_health.items():
            ...
            if (
                ...
                and current_time - health["operation_start"] > timeout
            ):
    ...
    async def _fetch_nodes_managed(self, netuid, force_fresh=False):
        ...
        try:
            logger.debug(
                f"Fetching nodes for netuid {netuid} using ultra-aggressive memory management"
            )

            # First, try to use cached nodes from metagraph if available and recent (unless force_fresh is True)
            if (
                not force_fresh
                and hasattr(self, "metagraph")
                and self.metagraph
                and hasattr(self.metagraph, "nodes")
                and self.metagraph.nodes
                and hasattr(self, "last_metagraph_sync")
                and time.time() - self.last_metagraph_sync < 300
            ):  # Use cache if less than 5 minutes old
                logger.info(
                    f"✅ Using cached metagraph nodes ({len(self.metagraph.nodes)} nodes) - NO substrate calls needed"
                )
            ...
            if force_fresh:
                ...
                logger.warning(
                    "No cached nodes available - making substrate call (potential memory leak)"
                )
            ...
            original_fetch_get_substrate = getattr(
                fetch_nodes_module, "get_substrate", None
            )
            ...
            try:
                ...
                logger.info(
                    "🔄 Created fresh connection for node fetching using substrate manager"
                )
                ...
                logger.info(
                    f"🛡️ Fetched {len(nodes) if nodes else 0} nodes with process-isolated substrate (ABC leak prevented)"
                )
                return nodes
            finally:
            ...
            logger.error(
                "CRITICAL: All node fetching approaches failed - using process-isolated fallback"
            )
    ...
    async def _fetch_nodes_process_isolated(self, netuid: int):
        ...
        try:
            logger.debug(
                f"🛡️ Fetching nodes for netuid {netuid} using process-isolated substrate"
            )
            ...
            timeout = 120.0
            logger.debug(
                f"🛡️ Making {13} substrate queries with {timeout}s timeout each"
            )

            hotkeys = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Hotkeys", [netuid], timeout=timeout
            )
            coldkeys = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Coldkeys", [netuid], timeout=timeout
            )
            uids = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Uids", [netuid], timeout=timeout
            )
            stakes = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Stake", [netuid], timeout=timeout
            )
            trust = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Trust", [netuid], timeout=timeout
            )
            vtrust = substrate._run_substrate_operation(
                "query", "SubtensorModule", "ValidatorTrust", [netuid], timeout=timeout
            )
            incentive = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Incentive", [netuid], timeout=timeout
            )
            emission = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Emission", [netuid], timeout=timeout
            )
            consensus = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Consensus", [netuid], timeout=timeout
            )
            dividends = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Dividends", [netuid], timeout=timeout
            )
            last_update = substrate._run_substrate_operation(
                "query", "SubtensorModule", "LastUpdate", [netuid], timeout=timeout
            )
            validator_permit = substrate._run_substrate_operation(
                "query", "SubtensorModule", "ValidatorPermit", [netuid], timeout=timeout
            )

            # Get axon info with extended timeout
            axons = substrate._run_substrate_operation(
                "query", "SubtensorModule", "Axons", [netuid], timeout=timeout
            )
            ...
            if hotkeys and len(hotkeys) > 0:
                for i, hotkey in enumerate(hotkeys):
                    try:
                        ...
                        node.trust = (
                            float(trust[i]) if trust and i < len(trust) else 0.0
                        )
                        node.vtrust = (
                            float(vtrust[i]) if vtrust and i < len(vtrust) else 0.0
                        )
                        node.incentive = (
                            float(incentive[i])
                            if incentive and i < len(incentive)
                            else 0.0
                        )
                        node.emission = (
                            float(emission[i])
                            if emission and i < len(emission)
                            else 0.0
                        )
                        node.consensus = (
                            float(consensus[i])
                            if consensus and i < len(consensus)
                            else 0.0
                        )
                        node.dividends = (
                            float(dividends[i])
                            if dividends and i < len(dividends)
                            else 0.0
                        )
                        node.last_update = (
                            int(last_update[i])
                            if last_update and i < len(last_update)
                            else 0
                        )
            ...
            logger.info(
                f"🛡️ Process-isolated node fetch completed: {len(nodes)} nodes (NO ABC memory leak)"
            )
            return nodes
    ...
    async def _sync_metagraph(self):
        ...
        async with self.metagraph_sync_lock:
            ...
            if (
                ...
                and current_time - self.last_metagraph_sync < 30
            ):
                logger.debug(
                    f"Metagraph recently synced {current_time - self.last_metagraph_sync:.1f}s ago, skipping"
                )
                return
            ...
            try:
                ...
                needs_fresh_connection = (
                    not hasattr(self, "substrate")
                    or self.substrate is None
                    or connection_age
                    > 900  # Increased from 600s (10min) to 900s (15min)
                )
                ...
                if needs_fresh_connection:
                    ...
                    logger.info(
                        f"🔄 Created fresh substrate connection (age: {connection_age:.1f}s, limit: 900s)"
                    )
                else:
                    logger.debug(
                        f"♻️ Reusing substrate connection (age: {connection_age:.1f}s, limit: 900s)"
                    )
                ...
                if hasattr(self, "metagraph") and self.metagraph:
                    ...
                    logger.debug(
                        f"Using standard fiber sync_nodes() with {connection_type} substrate connection"
                    )
                    await asyncio.to_thread(self.metagraph.sync_nodes)

                    node_count = (
                        len(self.metagraph.nodes) if self.metagraph.nodes else 0
                    )
                    logger.info(
                        f"✅ Metagraph sync completed: {node_count} nodes using {connection_type} substrate connection"
                    )
                else:
    ...
    def _track_background_task(self, task: asyncio.Task, task_name: str = "unnamed"):
        ...
        def cleanup_task(completed_task):
            ...
            if completed_task.cancelled():
                ...
                logger.warning(
                    f"Background task '{task_name}' failed: {completed_task.exception()}"
                )
            else:
        ...
        logger.debug(
            f"Tracking background task '{task_name}' (total: {len(self._background_tasks)})"
        )
        return task
    ...
    def _get_current_block_cached(self, force_refresh: bool = False) -> int:
        ...
        should_refresh = (
            force_refresh
            or cache["block_number"] is None
            or current_time - cache["last_update_time"] > cache["cache_duration"]
        )
        ...
        if should_refresh:
            try:
                ...
                logger.debug(
                    f"Refreshed shared block cache: {block_number} (age will be 0s)"
                )
                return block_number
                ...
                if cache["block_number"] is not None:
                    logger.warning(
                        f"Using stale cached block number: {cache['block_number']}"
                    )
            ...
            logger.debug(
                f"Using cached block number: {cache['block_number']} (age: {cached_age:.1f}s)"
            )
    ...
    def _aggressive_substrate_cleanup(self, context: str = "substrate_cleanup"):
        ...
        try:
            ...
            for module_name in list(sys.modules.keys()):
                if any(
                    pattern in module_name.lower() for pattern in substrate_patterns
                ):
            ...
            if substrate_modules_cleared > 0:
                logger.info(
                    f"Substrate cleanup ({context}): cleared {cache_objects_cleared} cache objects "
                    f"from {substrate_modules_cleared} substrate modules, GC collected {collected}"
                )
                return cache_objects_cleared
            else:
                logger.debug(
                    f"Substrate cleanup ({context}): no substrate caches found to clear"
                )
                return 0
    ...
    async def _cleanup_background_tasks(self):
        ...
        async with self._task_cleanup_lock:
            ...
            logger.info(
                f"Cleaning up {len(self._background_tasks)} background tasks..."
            )
    ...
    async def cleanup_resources(self):
        ...
        try:
            ...
            if (
                hasattr(self, "miner_client")
                and self.miner_client
                and not self.miner_client.is_closed
            ):
                ...
                try:
                    ...
                    logger.debug(
                        f"Error cleaning up connections before client close: {e}"
                    )
            ...
            if (
                hasattr(self, "api_client")
                and self.api_client
                and not self.api_client.is_closed
            ):
            ...
            try:
                ...
                warnings.filterwarnings(
                    "ignore",
                    category=RuntimeWarning,
                    message=".*Non-thread-safe operation.*",
                )
            ...
            logger.info(
                "Continuing shutdown despite cleanup errors to allow PM2 restart"
            )
    ...
    async def recover_task(self, task_name: str):
        ...
        try:
            ...
            if task_name == "soil":
                ...
                self.substrate_connection_created_at = (
                    time.time()
                )  # Track connection age for reuse
                logger.info(
                    "🔄 Force reconnect - created fresh substrate connection using substrate manager"
                )
                await self._sync_metagraph()  # Use fresh connection
            elif task_name == "deregistration":
                await (
                    self._sync_metagraph()
                )  # Use managed connection instead of direct sync
                self.nodes = {}
    ...
    async def cleanup_stale_history_on_startup(self):
        ...
        """
        logger.info(
            "Starting cleanup of stale miner history based on current metagraph..."
        )
        try:
            if not self.metagraph:
                logger.warning(
                    "Metagraph not initialized, cannot perform stale history cleanup."
                )
                return
            if not self.database_manager:
                logger.warning(
                    "Database manager not initialized, cannot perform stale history cleanup."
                )
                return
            ...
            try:
                active_nodes_list = await self._fetch_nodes_managed(
                    self.metagraph.netuid
                )
                if active_nodes_list is None:
                    active_nodes_list = []  # Ensure it's an iterable if None is returned
                    logger.warning(
                        "Managed node fetching returned None, proceeding with empty list for stale history check."
                    )
            except Exception as e_fetch_nodes:
                logger.error(
                    f"Failed to fetch nodes for stale history check: {e_fetch_nodes}",
                    exc_info=True,
                )
            ...
            logger.info(
                f"Built current_nodes_info with {len(current_nodes_info)} active UIDs for stale history check."
            )
            ...
            try:
                ...
                all_historical_pairs.update(
                    (row["miner_uid"], row["miner_hotkey"]) for row in geo_results
                )
                logger.info(
                    f"Found {len(geo_results)} distinct (miner_uid, hotkey) pairs in geomagnetic_history."
                )
            except Exception as e:
                logger.warning(
                    f"Could not query geomagnetic_history (may not exist yet): {e}"
                )

            try:
                soil_results = await self.database_manager.fetch_all(soil_history_query)
                all_historical_pairs.update(
                    (row["miner_uid"], row["miner_hotkey"]) for row in soil_results
                )
                logger.info(
                    f"Found {len(soil_results)} distinct (miner_uid, hotkey) pairs in soil_moisture_history."
                )
            except Exception as e:
                logger.warning(
                    f"Could not query soil_moisture_history (may not exist yet): {e}"
                )

            if not all_historical_pairs:
                logger.info(
                    "No historical data found to check. Skipping stale history cleanup."
                )
                return

            logger.info(
                f"Found {len(all_historical_pairs)} total distinct historical (miner_uid, hotkey) pairs to check."
            )

            # 3. Identify Mismatches
            uids_to_cleanup = defaultdict(
                set
            )  # uid -> {stale_historical_hotkey1, stale_historical_hotkey2, ...}

            # --- DIAGNOSTIC LOGGING START ---
            current_node_keys = list(current_nodes_info.keys())
            logger.info(
                f"Diagnostic: current_nodes_info has {len(current_node_keys)} keys. Sample keys: {current_node_keys[:5]} (Type: {type(current_node_keys[0]) if current_node_keys else 'N/A'})"
            )
            ...
            for hist_uid_str, hist_hotkey in all_historical_pairs:
                try:
                    ...
                    logger.warning(
                        f"Could not convert historical UID '{hist_uid_str}' to int. Skipping."
                    )
                    continue

                # --- DIAGNOSTIC LOGGING START ---
                logger.info(
                    f"Diagnostic: Checking hist_uid: {hist_uid} (Type: {type(hist_uid)}), hist_hotkey: {hist_hotkey}"
                )
                # --- DIAGNOSTIC LOGGING END ---
                ...
                if current_node is None:
                    ...
                    logger.warning(
                        f"Found historical entry for miner_uid {hist_uid} (Hotkey: {hist_hotkey}), but node is unexpectedly None in current metagraph sync. Skipping direct cleanup based on this, hotkey mismatch check will handle if applicable."
                    )
                ...
                if current_node.hotkey != hist_hotkey:
                    ...
                    logger.warning(
                        f"Mismatch found: miner_uid {hist_uid} historical hotkey {hist_hotkey} != current metagraph hotkey {current_node.hotkey}. Marking for cleanup."
                    )
            ...
            logger.info(
                f"Identified {len(uids_to_cleanup)} UIDs with stale history/scores."
            )

            # 4. Perform Cleanup
            # Get relevant task names for score zeroing
            distinct_task_names_rows = await self.database_manager.fetch_all(
                "SELECT DISTINCT task_name FROM score_table"
            )
            all_task_names_in_scores = [
                row["task_name"] for row in distinct_task_names_rows if row["task_name"]
            ]
            tasks_for_score_cleanup = [
                name
                for name in all_task_names_in_scores
                if name == "geomagnetic" or name.startswith("soil_moisture")
            ]
            if not tasks_for_score_cleanup:
                logger.warning(
                    "No relevant task names (geomagnetic, soil_moisture*) found in score_table for cleanup."
                )
                # Proceed with history deletion and node_table update anyway

            async with (
                self.miner_table_lock
            ):  # Use lock to coordinate with deregistration loop
                for uid, stale_hotkeys in uids_to_cleanup.items():
                    logger.info(
                        f"Cleaning up miner_uid {uid} associated with stale historical hotkeys: {stale_hotkeys}"
                    )
                    ...
                    for stale_hk in stale_hotkeys:
                        logger.info(
                            f"Processing stale hotkey {stale_hk} for miner_uid {uid}."
                        )
                        ...
                        for table, ts_col in history_tables_and_ts_cols.items():
                            try:
                                ...
                                if (
                                    result
                                    and result["min_ts"] is not None
                                    and result["max_ts"] is not None
                                ):
                                    all_min_ts.append(result["min_ts"])
                                    all_max_ts.append(result["max_ts"])
                                    timestamps_found = True
                                    logger.info(
                                        f"  Found time range in {table} for ({uid}, {stale_hk}): {result['min_ts']} -> {result['max_ts']}"
                                    )

                            except Exception as e_ts:
                                logger.warning(
                                    f"Could not query timestamps from {table} for miner_uid {uid}, Hotkey {stale_hk}: {e_ts}"
                                )
                        ...
                        logger.info(
                            f"  Deleting history entries for ({uid}, {stale_hk})"
                        )
                        try:
                            await self.database_manager.execute(
                            ...
                            logger.warning(
                                f"  Could not delete from geomagnetic_history for miner_uid {uid}, Hotkey {stale_hk}: {e_del_geo}"
                            )
                        try:
                            await self.database_manager.execute(
                            ...
                            logger.warning(
                                f"  Could not delete from soil_moisture_history for miner_uid {uid}, Hotkey {stale_hk}: {e_del_soil}"
                            )

                        # 4.3 Zero out scores in score_table *only for the determined time window*
                        if (
                            tasks_for_score_cleanup
                            and timestamps_found
                            and min_ts
                            and max_ts
                        ):
                            logger.info(
                                f"  Zeroing scores for miner_uid {uid} in tasks {tasks_for_score_cleanup} within window {min_ts} -> {max_ts}"
                            )
                            ...
                            logger.warning(
                                f"  Skipping score zeroing for miner_uid {uid}, Hotkey {stale_hk} - could not determine time window from history tables."
                            )
                        elif not tasks_for_score_cleanup:
                            logger.info(
                                f"  Skipping score zeroing for miner_uid {uid}, Hotkey {stale_hk} - no relevant task names found in score_table."
                            )
                        else:  # Should not happen if timestamps_found is true, but defensive check
                            logger.warning(
                                f"  Skipping score zeroing for miner_uid {uid}, Hotkey {stale_hk} due to missing min/max timestamps."
                            )
                    ...
                    if current_node_for_update:
                        logger.info(
                            f"Updating node_table info for miner_uid {uid} to match current metagraph hotkey {current_node_for_update.hotkey}."
                        )
                        try:
                            await self.database_manager.update_miner_info(
                                index=uid,
                                hotkey=current_node_for_update.hotkey,
                                coldkey=current_node_for_update.coldkey,
                                ip=current_node_for_update.ip,
                                ip_type=str(current_node_for_update.ip_type),
                                port=current_node_for_update.port,
                                incentive=float(current_node_for_update.incentive),
                                stake=float(current_node_for_update.stake),
                                trust=float(current_node_for_update.trust),
                                vtrust=float(current_node_for_update.vtrust),
                                protocol=str(current_node_for_update.protocol),
                            )
                        except Exception as e_update:
                            logger.error(
                                f"Failed to update node_table for miner_uid {uid}: {e_update}"
                            )
                    else:
                        # This case should now be extremely unlikely given the check adjustments above.
                        logger.error(
                            f"Critical inconsistency: Attempted cleanup for miner_uid {uid}, but node became None before final update. Skipping node_table update."
                        )
    ...
    async def main(self):
        ...
        async def run_validator_logic():
            ...
            def custom_excepthook(exc_type, exc_value, exc_traceback):
                ...
                if (
                    exc_type == RuntimeWarning
                    and (
                        "coroutine" in str(exc_value)
                        and "never awaited" in str(exc_value)
                    )
                    or ("Non-thread-safe operation" in str(exc_value))
                ):
            ...
            try:
                ...
                if self.database_manager._storage_locked:
                    logger.warning(
                        "Database storage is locked - no data will be stored until manually unlocked"
                    )
                ...
                auto_updater_task = self.create_tracked_task(
                ...
                if self.auto_sync_manager:
                    logger.info(
                        "AutoSyncManager is active - Starting setup and scheduling..."
                    )
                    logger.info(
                        f"DB Sync Configuration: Primary={self.is_source_validator_for_db_sync}"
                    )

                    # Setup AutoSyncManager (includes system configuration AND scheduling)
                    try:
                        logger.info(
                            "🚀 Setting up AutoSyncManager (includes system config and scheduling)..."
                        )
                        ...
                        if setup_success:
                            ...
                            try:
                                ...
                                logger.info(
                                    "✅ AutoSyncManager fallback scheduling started successfully!"
                                )
                            except Exception as fallback_e:
                                logger.error(
                                    f"❌ AutoSyncManager fallback scheduling also failed: {fallback_e}"
                                )
                                self.auto_sync_manager = None
                    except Exception as e:
                        logger.error(
                            f"❌ AutoSyncManager setup failed with exception: {e}"
                        )
                        logger.info(
                            "🔄 Attempting fallback scheduling for basic monitoring..."
                        )
                        ...
                        try:
                            ...
                            logger.error(
                                "🚫 AutoSyncManager will be completely disabled"
                            )
                            self.auto_sync_manager = None
                else:
                    logger.info(
                ...
                shutdown_waiter = (
                    None  # Define here for access in except CancelledError
                )
                try:
                    logger.info(f"Creating {len(tasks_lambdas)} main service tasks...")
                    active_service_tasks = [
                        self.create_tracked_task(t(), f"service_task_{i}")
                        for i, t in enumerate(tasks_lambdas)
                    ]
                    logger.info(
                        f"All {len(active_service_tasks)} main service tasks created."
                    )

                    shutdown_waiter = self.create_tracked_task(
                        self._shutdown_event.wait(), "shutdown_waiter"
                    )
                    ...
                    while not self._shutdown_event.is_set():
                        ...
                        current_wait_list = [
                            t for t in all_tasks_being_monitored if not t.done()
                        ]
                        ...
                        if not current_wait_list:
                            ...
                            if not self._shutdown_event.is_set():
                                logger.warning(
                                    "All tasks completed but shutdown event was not explicitly set. Setting it now to ensure proper cleanup."
                                )
                                self._shutdown_event.set()  # Ensure shutdown is triggered
                            break  # Exit the while loop

                        done, pending = await asyncio.wait(
                            current_wait_list, return_when=asyncio.FIRST_COMPLETED
                        )
                        ...
                        if self._shutdown_event.is_set() or shutdown_waiter.done():
                            logger.info(
                                "Shutdown signaled or shutdown_waiter completed. Breaking main monitoring loop."
                            )
                            break
                        ...
                        for task in done:
                            if (
                                task in active_service_tasks
                            ):  # Check if it's one of our main service tasks
                                try:
                                    result = (
                                        task.result()
                                    )  # Access result to raise exception if task failed
                                    logger.warning(
                                        f"Main service task {task.get_name()} completed unexpectedly with result: {result}. It will not be automatically restarted by this loop."
                                    )
                                except asyncio.CancelledError:
                                    logger.info(
                                        f"Main service task {task.get_name()} was cancelled."
                                    )
                                except Exception as e:
                                    logger.error(
                                        f"Main service task {task.get_name()} failed with exception: {e}",
                                        exc_info=True,
                                    )

                    # --- After the while loop (either by break or _shutdown_event being set before loop start) ---
                    logger.info(
                        "Main monitoring loop finished. Initiating cancellation of any remaining active tasks for shutdown."
                    )
                    ...
                    for task_to_cancel in active_service_tasks:
                        if not task_to_cancel.done():
                            logger.info(
                                f"Cancelling service task: {task_to_cancel.get_name()}"
                            )
                    ...
                    await asyncio.gather(
                        *(
                            active_service_tasks
                            + ([shutdown_waiter] if shutdown_waiter else [])
                        ),
                        return_exceptions=True,
                    )
                    logger.info(
                        "All main service tasks and the shutdown waiter have been processed (awaited/cancelled)."
                    )

                except asyncio.CancelledError:
                    logger.info(
                        "Main task execution block was cancelled. Ensuring child tasks are also cancelled."
                    )
                    # This block handles if validator.main() itself is cancelled from outside.
                    tasks_to_ensure_cancelled = []
                    if (
                        "active_service_tasks" in locals()
                    ):  # Check if list was initialized
                        tasks_to_ensure_cancelled.extend(active_service_tasks)
                    if (
                        "shutdown_waiter" in locals() and shutdown_waiter
                    ):  # Check if waiter was initialized
                        tasks_to_ensure_cancelled.append(shutdown_waiter)

                    for task_to_cancel in tasks_to_ensure_cancelled:
                        if task_to_cancel and not task_to_cancel.done():
                            logger.info(
                                f"Cancelling task due to main cancellation: {task_to_cancel.get_name()}"
                            )
                            task_to_cancel.cancel()
                    await asyncio.gather(
                        *tasks_to_ensure_cancelled, return_exceptions=True
                    )
                    logger.info(
                        "Child tasks cancellation process completed due to main cancellation."
                    )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        for module_name in list(sys.modules.keys()):
                            if (
                                "scalecodec" in module_name.lower()
                                or "substrate" in module_name.lower()
                            ):
                                module = sys.modules.get(module_name)
                                if hasattr(module, "__dict__"):
                                    for attr_name in list(module.__dict__.keys()):
                                        if (
                                            "cache" in attr_name.lower()
                                            or "registry" in attr_name.lower()
                                        ):
                                            try:
                                                cache_obj = getattr(module, attr_name)
                                                if hasattr(
                                                    cache_obj, "clear"
                                                ) and callable(cache_obj.clear):
                                                    cache_obj.clear()
                                                    cleared_count += 1
                                                elif isinstance(
                                                    cache_obj, (dict, list, set)
                                                ):
                                                    cache_obj.clear()
                                                    cleared_count += 1
                                            except Exception:
                        ...
                        if cleared_count > 0:
                            collected = gc.collect()
                            logger.debug(
                                f"Substrate reconnect cleanup: cleared {cleared_count} cache objects, GC collected {collected}"
                            )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "🔄 Created fresh connection for status logger using substrate manager"
                        )
                        ...
                        try:
                            ...
                            if cleared_count > 0:
                                collected = gc.collect()
                                logger.debug(
                                    f"Status logger substrate cleanup: cleared {cleared_count} cache objects, GC collected {collected}"
                                )
                ...
                try:
                    ...
                    if stats["has_active_connection"]:
                        age_info = (
                            f"age: {stats['connection_age_seconds']:.1f}s"
                            if stats["connection_age_seconds"]
                            else "new"
                        )
    ...
    async def handle_miner_deregistration_loop(self) -> None:
        logger.info(
            "Starting miner state synchronization loop (handles hotkey changes, new miners, and info updates)"
        )
        ...
        while True:
            ...
            try:
                ...
                if not self.metagraph:
                    logger.warning(
                        "Metagraph object not initialized, cannot sync miner state."
                    )
                ...
                await (
                    self._sync_metagraph()
                )  # Use fresh connection instead of direct sync
                if not self.metagraph.nodes:
                    logger.warning(
                        "Metagraph empty after sync, skipping miner state update."
                    )
                    await asyncio.sleep(600)  # Sleep before retrying
                    continue
                ...
                async with self.miner_table_lock:
                    logger.info(
                        "Performing miner hotkey change check and info update..."
                    )
                    ...
                    try:
                        ...
                        if active_nodes_list is None:
                            ...
                            logger.warning(
                                "Managed node fetching returned None in handle_miner_deregistration_loop."
                            )
                    except Exception as e_fetch_nodes_dereg:
                        logger.error(
                            f"Failed to fetch nodes in handle_miner_deregistration_loop: {e_fetch_nodes_dereg}",
                            exc_info=True,
                        )
                        active_nodes_list = []
                    ...
                    chain_nodes_info = {
                        node.node_id: node for node in active_nodes_list
                    }
                    ...
                    db_miners_rows = await self.database_manager.fetch_all(
                        db_miner_query
                    )
                    db_miners_info = {
                        row["uid"]: row["hotkey"] for row in db_miners_rows
                    }
                    ...
                    for db_uid, db_hotkey in db_miners_info.items():
                        ...
                        if chain_node_for_uid is None:
                            ...
                            logger.warning(
                                f"UID {db_uid} (DB hotkey: {db_hotkey}) not found in current metagraph sync. Potential deregistration missed? Skipping."
                            )
                        ...
                        if chain_node_for_uid.hotkey != db_hotkey:
                            ...
                            logger.info(
                                f"UID {db_uid} hotkey changed. DB: {db_hotkey}, Chain: {chain_node_for_uid.hotkey}. Marking for data cleanup and update."
                            )
                    ...
                    if uids_to_clear_and_update:
                        logger.info(
                            f"Cleaning old data and updating hotkeys for UIDs: {list(uids_to_clear_and_update.keys())}"
                        )
                        for (
                            uid_to_process,
                            new_chain_node_data,
                        ) in uids_to_clear_and_update.items():
                            original_hotkey = db_miners_info.get(
                                uid_to_process
                            )  # Get the old hotkey from DB cache
                            if not original_hotkey:
                                logger.warning(
                                    f"Could not find original hotkey in DB cache for UID {uid_to_process}. Skipping cleanup for this UID."
                                )
                                continue

                            logger.info(
                                f"Processing hotkey change for UID {uid_to_process}: Old={original_hotkey}, New={new_chain_node_data.hotkey}"
                            )
                            try:
                                # 1. Delete from prediction tables by UID
                                prediction_tables_by_uid = [
                                    "geomagnetic_predictions",
                                    "soil_moisture_predictions",
                                ]
                                for table_name in prediction_tables_by_uid:
                                    try:
                                        table_exists_res = await self.database_manager.fetch_one(
                                            f"SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = '{table_name}')"
                                        )
                                        if (
                                            table_exists_res
                                            and table_exists_res["exists"]
                                        ):
                                            ...
                                            logger.info(
                                                f"  Deleted from {table_name} for old hotkey {original_hotkey} (UID {uid_to_process}) due to hotkey change."
                                            )
                                        # No else needed, if table doesn't exist, we just skip
                                    except Exception as e_pred_del:
                                        logger.warning(
                                            f"  Could not clear {table_name} for UID {uid_to_process}: {e_pred_del}"
                                        )

                                # 2. Delete from history tables by OLD hotkey
                                history_tables_by_hotkey = [
                                    "geomagnetic_history",
                                ...
                                distinct_task_names_rows = (
                                    await self.database_manager.fetch_all(
                                        "SELECT DISTINCT task_name FROM score_table"
                                    )
                                )
                                all_task_names_in_scores = [
                                    row["task_name"]
                                    for row in distinct_task_names_rows
                                    if row["task_name"]
                                ]
                                if all_task_names_in_scores:
                                    logger.info(
                                        f"  Zeroing all scores in score_table for UID {uid_to_process} across tasks: {all_task_names_in_scores}"
                                    )
                                    await self.database_manager.remove_miner_from_score_tables(
                                        uids=[uid_to_process],
                                    ...
                                    logger.info(
                                        f"  No task names found in score_table to zero-out for UID {uid_to_process}."
                                    )

                                # 4. Update node_table with NEW info
                                logger.info(
                                    f"  Updating node_table info for UID {uid_to_process} with new hotkey {new_chain_node_data.hotkey}."
                                )
                                await self.database_manager.update_miner_info(
                                ...
                                self.nodes[uid_to_process] = {
                                    "hotkey": new_chain_node_data.hotkey,
                                    "uid": uid_to_process,
                                }
                                logger.info(
                                    f"Successfully processed hotkey change for UID {uid_to_process}."
                                )
                    ...
                    if uids_to_update_info:
                        logger.info(
                            f"Updating potentially changed info (stake, IP, etc.) for {len(uids_to_update_info)} existing UIDs..."
                        )
                        ...
                        if batch_updates:
                            try:
                                await self.database_manager.batch_update_miners(
                                    batch_updates
                                )
                                logger.info(
                                    f"Successfully batch updated {len(batch_updates)} existing miners"
                                )
                            except Exception as e:
                                logger.error(
                                    f"Error in batch update of existing miners: {str(e)}"
                                )
                                for update_data in batch_updates:
                                    try:
                                        ...
                                        logger.debug(
                                            f"Successfully updated info for existing UID {uid} (fallback)"
                                        )
                                    except Exception as individual_e:
                                        logger.error(
                                            f"Error updating info for existing UID {uid}: {str(individual_e)}",
                                            exc_info=True,
                                        )
                    ...
                    for chain_uid, chain_node in chain_nodes_info.items():
                        if chain_uid not in processed_uids:
                            logger.info(
                                f"New miner detected on chain: UID {chain_uid}, Hotkey {chain_node.hotkey}. Adding to DB."
                            )
                    ...
                    if new_miner_updates:
                        try:
                            ...
                            logger.info(
                                f"Successfully batch added {new_miners_detected} new miners to the database"
                            )
                        except Exception as e:
                            logger.error(
                                f"Error in batch update of new miners: {str(e)}"
                            )
                            successful_adds = 0
                            for update_data in new_miner_updates:
                                try:
                                    ...
                                    logger.error(
                                        f"Error adding new miner UID {uid} (Hotkey: {update_data['hotkey']}): {str(individual_e)}",
                                        exc_info=True,
                                    )
                ...
                logger.error(
                    f"Error in miner state synchronization loop: {e}", exc_info=True
                )
    ...
    def _perform_weight_calculations_sync(
        self,
        weather_results,
        geomagnetic_results,
        soil_results,
        ...
        try:
            ...
            if weather_results:
                for result in weather_results:
                    ...
                    for uid in range(256):
                        if (
                            not isinstance(scores[uid], str)
                            and not np.isnan(scores[uid])
                            and scores[uid] != 0.0
                        ):
            ...
            if soil_results:
                ...
                logger.info(
                    f"Processing {len(soil_results)} soil records with enhanced memory management"
                )
                zero_soil_scores = 0
                ...
                scores_matrix = np.full(
                    (256, num_results), np.nan, dtype=np.float32
                ...
                for result_idx, result in enumerate(soil_results):
                    age_days = (now - result["created_at"]).total_seconds() / (
                        24 * 3600
                    )
                    ...
                    if not isinstance(scores, list) or len(scores) != 256:
                        scores = [np.nan] * 256  # Defensive
                        logger.warning(
                            f"Applied defensive fallback for soil scores in result {result_idx}"
                        )

                    # Enhanced logging for first result
                    if result_idx == 0:
                        logger.info(
                            f"SOIL DEBUG: First result - created_at={result['created_at']}, age_days={age_days:.3f}, decay={decay:.6f}"
                        )
                        logger.info(
                            f"SOIL DEBUG: Scores type={type(scores)}, len={len(scores)}"
                        )
                        logger.info(f"SOIL DEBUG: First 10 scores: {scores[:10]}")
                        logger.info(
                            f"SOIL DEBUG: Score types: {[type(x) for x in scores[:5]]}"
                        )
                    ...
                    for uid in range(256):
                        ...
                        try:
                            ...
                            if not sample_logged:
                                logger.error(
                                    f"SOIL DEBUG: np.isnan() failed for UID {uid}: {e}, score_val={score_val}, type={type(score_val)}"
                                )
                                sample_logged = True
                        ...
                        if is_string or is_nan:
                            ...
                            if is_string:
                                logger.warning(
                                    f"SOIL DEBUG: String score at UID {uid}: '{original_val}'"
                                )
                            elif nan_check_failed:
                                logger.warning(
                                    f"SOIL DEBUG: NaN check failed for UID {uid}: {original_val}"
                                )
                            else:
                                total_nan_scores += 1
                        else:
                ...
                logger.info(
                    f"SOIL DEBUG: Processed {total_scores_processed} total scores"
                )
                logger.info(
                    f"SOIL DEBUG: Valid scores: {total_valid_scores}, NaN scores: {total_nan_scores}"
                )
                logger.info(f"SOIL DEBUG: Zero scores counted: {zero_soil_scores}")
                logger.info(
                    f"SOIL DEBUG: UIDs with non-zero counts: {np.sum(soil_counts > 0)}"
                )

                # Log concerning conditions
                if zero_soil_scores == 256:
                    logger.error(
                        "SOIL DEBUG: ALL 256 SCORES CONVERTED TO ZERO! This indicates a systematic conversion issue."
                    )
                elif zero_soil_scores > 200:
                    logger.warning(
                        f"SOIL DEBUG: High zero score count ({zero_soil_scores}/256) - possible data quality issue"
                    )
                    ...
                    if np.any(non_zero_mask):
                        ...
                        if weight_sum > 0:
                            ...
                            expected_predictions = len(
                                soil_results
                            )  # Number of available scoring periods
                            completeness_ratio = num_predictions / max(
                                expected_predictions, 1
                            )

                            # Consistent threshold with geomagnetic task
                            completeness_threshold = (
                                0.30  # 30% threshold for fair new miner treatment
                            )
                            if completeness_ratio >= completeness_threshold:
                                completeness_factor = (
                                    1.0  # Full weight for adequate participation
                                )
                            else:
                                # Gradual scaling below threshold, not linear penalty
                                completeness_factor = (
                                    completeness_ratio / completeness_threshold
                                ) ** 0.5  # Square root for gentler penalty
                            ...
                            if num_predictions > 0:
                                logger.debug(
                                    f"Soil UID {uid}: {num_predictions}/{expected_predictions} predictions "
                ...
                final_valid_soil_scores = np.sum(
                    ~np.isnan(soil_scores) & (soil_scores != 0.0)
                )
                logger.info(
                    f"SOIL DEBUG: Final result - {final_valid_soil_scores} UIDs have valid soil scores"
                )
                ...
                logger.info(
                    f"Processed {len(soil_results)} soil records with {zero_soil_scores} zero scores (memory-optimized)"
                )
            ...
            logger.info(
                "Processing geomagnetic scores with 3-day lookback for sybil attack protection..."
            )
            ...
            if geomagnetic_results:
                ...
                logger.info(
                    f"Geomagnetic data spans {len(daily_scores)} days: {sorted(daily_scores.keys())}"
                )
                ...
                for uid in range(256):
                    ...
                    if num_days_available >= 3:
                        ...
                        if days_with_nonzero_scores >= required_days_for_full:
                            ...
                            completeness_factor = (
                                days_with_nonzero_scores / required_days_for_full
                            ) ** 0.5
                    ...
                    if base_score > 0 or days_with_nonzero_scores > 0:
                        logger.debug(
                            f"Geo UID {uid}: {days_with_nonzero_scores}/{num_days_available} days with scores, "
                            f"factor={completeness_factor:.3f}, base={base_score:.4f}, final={geomagnetic_scores[uid]:.4f}"
                        )

                    logger.info(
                        f"Processed 3-day geomagnetic scoring for sybil attack protection: "
                        f"{len(daily_scores)} days of data, "
                        f"{len([s for s in geomagnetic_scores if s > 0])} UIDs with positive scores"
                    )
            else:
                logger.info(
                    "No geomagnetic results in last 3 days - all scores set to 0"
                )
                geomagnetic_scores.fill(0.0)

            logger.info(
                "Aggregate scores calculated. Implementing hybrid excellence/diversity pathway system..."
            )
            ...
            for task_name, scores_array in [
                ("weather", weather_scores),
                ("geomagnetic", geomagnetic_scores),
                ("soil", soil_scores),
            ]:
                valid_scores = scores_array[
                    ~np.isnan(scores_array) & (scores_array != 0.0)
                ]
                ...
                if len(valid_scores) >= 5:  # Minimum sample for percentiles
                    ...
                    percentiles = np.percentile(
                        valid_scores, [15, 25, 35, 50, 65, 75, 85, 95]
                    )
                    task_percentiles[task_name] = {
                        "valid_count": len(valid_scores),
                        "percentile_85": percentiles[
                            6
                        ],  # Excellence threshold (top 15%)
                        "sorted_scores": np.sort(
                            valid_scores
                        ),  # For efficient rank calculation
                    }
                    logger.info(
                        f"{task_name.capitalize()} task: {len(valid_scores)} vallid scores, 85th percentile: {percentiles[6]:.4f}"
                    )
                else:
                    task_percentiles[task_name] = {"valid_count": 0}
                    logger.info(
                        f"{task_name.capitalize()} task: insufficient scores ({len(valid_scores)}) for percentile calculation"
                    )
            ...
            if len(valid_geo_scores) > 0:
                ...
                logger.info(
                    f"Dynamic sigmoid for geo: x0={sigmoid_x0:.4f} (mean), k={sigmoid_k:.1f}, std={geo_std:.4f}"
                )
            else:
                sigmoid_x0 = 0.90
                sigmoid_k = 15
                logger.info(
                    f"No valid geo scores, using fallback sigmoid: x0={sigmoid_x0}, k={sigmoid_k}"
                )
            ...
            def diversity_sigmoid(percentile_rank):
                ...
                sigmoid_val = 1 / (
                    1 + math.exp(-steepness * (percentile_rank - center))
                )
            ...
            def get_percentile_rank(score, task_name):
                ...
                if (
                    task_name not in task_percentiles
                    or task_percentiles[task_name]["valid_count"] == 0
                ):
            ...
            for idx in range(256):
                ...
                for task_name, score in [
                    ...
                    if not np.isnan(score) and task_name in task_percentiles:
                        ...
                        if (
                            task_data["valid_count"] >= 10
                            and score >= task_data["percentile_85"]
                        ):
                ...
                if num_tasks > 0:
                    ...
                    diversity_weight = (
                        sum(diversity_contributions.values()) * multi_task_bonus
                    )
                ...
                if weights_final[idx] > 0.1:
                    pathway = (
                        "excellence"
                        if excellence_weight > diversity_weight
                        else "diversity"
                    )
                    node_obj = (
                        validator_nodes_by_uid_list[idx]
                        if idx < len(validator_nodes_by_uid_list)
                        else None
                    )
                    hk_chain = node_obj.get("hotkey", "N/A")[:8] if node_obj else "N/A"
                    logger.debug(
                        f"UID {idx} ({hk_chain}): {pathway} pathway, "
            ...
            logger.info(
                f"Pathway usage - Excellence: {total_excellence} miners "
                f"(W:{excellence_count['weather']}, G:{excellence_count['geomagnetic']}, S:{excellence_count['soil']})"
            )
            logger.info(
                f"Diversity: {total_diversity} miners "
                f"(1-task:{diversity_count['1_task']}, 2-task:{diversity_count['2_task']}, 3-task:{diversity_count['3_task']})"
            )
            logger.info(
                f"Weights before normalization: Min={np.min(weights_final):.4f}, Max={np.max(weights_final):.4f}, Mean={np.mean(weights_final):.4f}"
            )
            ...
            for idx in nz_indices:
                sig_p = a + (k - a) / np.power(
                    1 + Q * np.exp(-b * (norm_weights[idx] - M)), 1 / v
                )
            ...
            if trans_nz.any():
                logger.info(
                    f"Transformed weights: Min={np.min(trans_nz):.4f}, Max={np.max(trans_nz):.4f}, Mean={np.mean(trans_nz):.4f}"
                )
            else:
                logger.warning(
                    "No positive weights after sigmoid transformation!"
            ...
            if final_sum > 0:
                ...
                if final_nz_vals.any():
                    logger.info(
                        f"Final Norm Weights: Count={len(final_nz_vals)}, Min={np.min(final_nz_vals):.4f}, Max={np.max(final_nz_vals):.4f}, Std={np.std(final_nz_vals):.4f}"
                    )
                    if len(np.unique(final_nz_vals)) < len(final_nz_vals) / 2:
                        logger.warning(
                            f"Low unique weights! {len(np.unique(final_nz_vals))}/{len(final_nz_vals)}"
                        )
                    if final_nz_vals.max() > 0.90:
                        logger.warning(
                            f"Max weight {final_nz_vals.max():.4f} is very high!"
                        )
                final_weights_list = transformed_w.tolist()
                logger.info("Final normalized weights calculated.")
            else:
                logger.warning(
                    "Sum of weights is zero, cannot normalize! Returning None."
                )
                final_weights_list = None
            ...
            try:
                ...
                if "nz_weights" in locals():
                    del nz_weights
                if "norm_weights" in locals():
                    del norm_weights
                if "positives" in locals():
                    del positives
                if "trans_nz" in locals():
                    del trans_nz
                if "sorted_indices" in locals():
                    del sorted_indices
            except Exception as cleanup_e:
                logger.warning(
                    f"Error during sync weight calculation cleanup: {cleanup_e}"
                )
    ...
    async def _calc_task_weights(self):
        ...
        try:
            ...
            geo_params = {
                "geo_start_time": one_day_ago
            }  # For geomagnetic 24-hour lookback
            ...
            try:
                ...
                if geomagnetic_results and not isinstance(
                    geomagnetic_results, Exception
                ):
                    logger.info(
                        f"Geomagnetic dataset: {len(geomagnetic_results)} records"
                    )
                if soil_results and not isinstance(soil_results, Exception):
                    ...
                    if soil_results:
                        ...
                        if "score" in sample_soil and isinstance(
                            sample_soil["score"], list
                        ):
                            logger.info(
                                f"Soil score array size per record: {len(sample_soil['score'])} elements"
                            )
                if validator_nodes_list and not isinstance(
                    validator_nodes_list, Exception
                ):
                    logger.info(
                        f"Validator nodes dataset: {len(validator_nodes_list)} records"
                    )
                ...
                if not validator_nodes_list:
                    logger.error(
                        "Failed to fetch validator nodes - cannot calculate weights"
                    )
                    return None

                logger.info(
                    f"Fetched scores: Weather={len(weather_results)}, Geo={len(geomagnetic_results)}, Soil={len(soil_results)}"
                )
                ...
                try:
                    ...
                    collected = gc.collect()
                    logger.debug(
                        f"Pre-sync GC: collected {collected} objects before weight calculation"
                    )

                    self._log_memory_usage("calc_weights_pre_sync_gc")
                except Exception as pre_sync_cleanup_err:
                    logger.debug(
                        f"Error during pre-sync cleanup: {pre_sync_cleanup_err}"
                    )
                ...
                try:
                    ...
                    collected = gc.collect()
                    logger.info(
                        f"Weight calculation cleanup: collected {collected} objects after data deletion"
                    )
                    ...
                    if hasattr(self, "last_metagraph_sync"):
                        ...
                        logger.info(
                            f"Weight calculation comprehensive cleanup: freed {memory_freed:.1f}MB"
                        )
                    else:
                        ...
                        for _ in range(2):  # Multiple GC passes
                            collected += gc.collect()
                        logger.info(
                            f"Basic GC cleanup during startup: collected {collected} objects"
                        )
                        memory_freed = 0
                    ...
                    logger.warning(
                        f"Error during weight calculation cleanup: {cleanup_err}"
                    )
                ...
                logger.error(
                    f"Error during database queries for weight calculation: {query_error}"
                )
                return None
    ...
    async def manage_earthdata_token(self):
        ...
        logger.info(
            "🌍 Earthdata token management task started - running initial check immediately..."
        )
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if token:
                    logger.info(
                        f"✅ Earthdata token check successful. Current token (first 10 chars): {token[:10]}..."
                    )
                else:
                    logger.warning(
                        "⚠️ Earthdata token check failed or no token available."
                    )

                logger.info(
                    "⏰ Earthdata token check complete. Sleeping for 24 hours until next check..."
                )
                ...
                logger.error(
                    f"❌ Error in Earthdata token management task: {e}", exc_info=True
                )
                logger.info(
                    "🔄 Retrying Earthdata token check in 1 hour due to error..."
                )
    ...
    async def _initialize_db_sync_components(self):
        ...
        db_sync_enabled_str = os.getenv(
            "DB_SYNC_ENABLED", "True"
        )  # Default to True if not set
        if db_sync_enabled_str.lower() != "true":
            logger.info(
                "DB_SYNC_ENABLED is not 'true'. Database synchronization feature will be disabled."
            )
            self.auto_sync_manager = None
            return
        ...
        try:
            ...
            self.auto_sync_manager = await get_auto_sync_manager(
                test_mode=self.args.test
            )
            if self.auto_sync_manager:
                logger.info("✅ AutoSyncManager initialized successfully")
                logger.info(
                    "🔧 AutoSyncManager provides automated setup and application-controlled scheduling"
                )
                logger.info(
                    "📝 To set up database sync, run: python gaia/validator/sync/setup_auto_sync.py --primary (or --replica)"
                )
                return
            else:
                logger.warning(
                    "AutoSyncManager failed to initialize - check environment variables"
                )
        except Exception as e:
            logger.warning(f"AutoSyncManager initialization failed: {e}")
            logger.info(
                "💡 To enable DB sync, configure PGBACKREST_R2_* environment variables"
            )
    ...
    async def database_monitor(self):
        ...
        while not self._shutdown_event.is_set():
            ...
            try:
                ...
                try:
                    activity_snapshot = await self.database_manager.fetch_all(
                        activity_query, timeout=45.0
                    )
                except DatabaseTimeout:
                    collected_stats["error"] = (
                        "Timeout fetching pg_stat_activity snapshot."
                    )
                    logger.warning(f"[DB Monitor] {collected_stats['error']}")
                except Exception as e:
                    collected_stats["error"] = (
                        f"Error fetching pg_stat_activity snapshot: {type(e).__name__}"
                    )
                ...
                # 3. Fetch other stats
                try:
                    collected_stats["session_manager_stats"] = (
                        self.database_manager.get_session_stats()
                    )
                except Exception as e:
                    collected_stats["session_manager_stats"] = (
                        f"[Error fetching session manager stats: {type(e).__name__}]"
                    )
                ...
                try:
                    collected_stats[
                        ...
                        lock_details_query, timeout=45.0
                    )
                except DatabaseTimeout:
                    collected_stats["lock_details"] = "[Query Timed Out]"
                except Exception as e:
                    collected_stats["lock_details"] = (
                        f"[Query Error: {type(e).__name__}]"
                    )

            except Exception as e_outer:
                collected_stats["error"] = (
                    f"Outer error in database_monitor: {str(e_outer)}"
                )
    ...
    def _process_activity_snapshot(self, activity_snapshot):
        ...
        for row_proxy in activity_snapshot:
            ...
            if state is None:
                ...
                if row.get("query_start"):
                    row["query_age"] = datetime.now(timezone.utc) - row.get(
                        "query_start"
                    )
                else:
        ...
        idle_in_transaction_details.sort(
            key=lambda r: r.get("state_change")
            or datetime.min.replace(tzinfo=timezone.utc)
        )
        active_query_details.sort(
            key=lambda r: r.get("query_start")
            or datetime.min.replace(tzinfo=timezone.utc),
            reverse=True,
        )

        summary_list = [
            {"state": s if s is not None else "null", "count": c}
            ...
            try:
                ...
                if isinstance(value, list):
                    ...
                    try:
                        ...
                        log_output += (
                            f"  {title}: {json.dumps(value, indent=2, default=str)}\n"
                        )
    ...
    def _generate_and_save_plot_sync(self, history_copy):
        ...
        if not history_copy or len(history_copy) < 2:
            logger.info(
                "Not enough data in history_copy for sync plot generation. Skipping."
            )
            return
        ...
        for record in history_copy:
            try:
                ...
                if isinstance(session_stats, dict):
                    avg_session_times.append(
                        session_stats.get("avg_session_time_ms", float("nan"))
                    )
                    min_session_times.append(
                        session_stats.get("min_session_time_ms", float("nan"))
                    )
                    max_session_times.append(
                        session_stats.get("max_session_time_ms", float("nan"))
                    )
                else:
                ...
                if isinstance(connection_summary, list):
                    for conn_info in connection_summary:
                        if (
                            ...
                            and "count" in conn_info
                            and isinstance(conn_info["count"], (int, float))
                        ):
                            current_total_connections += conn_info["count"]
                total_connections_list.append(current_total_connections)
            except Exception as e:
                logger.warning(
                    f"Skipping record in sync plot generation due to parsing error: {e} - Record: {record}"
                )
                continue

        if not timestamps or len(timestamps) < 2:
            logger.info(
                "Not enough valid data points for sync plot generation after parsing. Skipping."
            )
            return
        ...
        while not self._shutdown_event.is_set():
            ...
            async with self.db_monitor_history_lock:
                if not self.db_monitor_history or len(self.db_monitor_history) < 2:
                    logger.info(
                        "Not enough data in db_monitor_history to generate plots. Skipping this cycle."
                    )
                    continue
            ...
            if not history_copy_for_plot:
                logger.info(
                    "History copy for plotting is empty, skipping plot generation."
                )  # Should be caught by above check too
                continue

            try:
                # Offload the plotting to the synchronous helper method in an executor thread
                await asyncio.to_thread(
                    self._generate_and_save_plot_sync, history_copy_for_plot
                )
            except Exception as e:
                logger.error(
                    f"Error occurred when calling plot generation in executor: {e}"
                )
                logger.error(traceback.format_exc())
    ...
    def get_current_task_weights(self) -> Dict[str, float]:
        ...
        return (
            active_weights.copy()
        )  # Return a copy to prevent modification of the schedule

    async def monitor_client_health(self):
        """Monitor HTTP client connection pool health."""
        while not self._shutdown_event.is_set():
            try:
                if hasattr(self, "miner_client") and hasattr(
                    self.miner_client, "_transport"
                ):
                    ...
                    if hasattr(transport, "_pool"):
                        ...
                        if hasattr(pool, "_connections"):
                            ...
                            for conn in connection_items:
                                ...
                                if (
                                    hasattr(conn, "_keepalive_expiry")
                                    and conn._keepalive_expiry
                                ):
                                ...
                                elif hasattr(conn, "_socket") and hasattr(
                            ...
                            if hasattr(self.miner_client, "_limits"):
                                if hasattr(
                                    self.miner_client._limits, "max_connections"
                                ):
                                    pool_limit = (
                                        self.miner_client._limits.max_connections
                                    )
                                if hasattr(
                                    self.miner_client._limits,
                            ...
                            log_level = (
                                logger.info if total_connections > 75 else logger.debug
                            )
                            log_level(
                                f"HTTP Client Pool Health - "
                            ...
                            if total_connections > 80 and unique_hosts:
                                logger.info(
                                    f"Connection pool has {total_connections} connections to {len(unique_hosts)} unique hosts"
                                )
                                if (
                                    len(unique_hosts) <= 15
                                ):  # Only log if manageable number
                                    logger.debug(
                                        f"Connected hosts: {list(unique_hosts)[:15]}"
                                    )

                            # Only trigger cleanup if we have excessive idle connections (not just any idle)
                            if idle_connections > 30:
                                logger.info(
                                    f"High idle connection count ({idle_connections}), triggering cleanup"
                                )
                                try:
                                    await self._cleanup_idle_connections()
                                except Exception as e:
                                    logger.warning(
                                        f"Error during automatic connection cleanup: {e}"
                                    )
    ...
    async def periodic_substrate_cleanup(self):
        ...
        logger.info(
            "🛡️  Periodic substrate cleanup is disabled - using isolated substrate interface for memory leak prevention"
        )
        ...
        while True:
            ...
            logger.debug(
                "🛡️  Isolated substrate interface prevents memory leaks - no cleanup needed"
            )
            ...
            try:
                ...
                if PSUTIL_AVAILABLE:
                    ...
                    if cleanup_level == "aggressive":
                        logger.warning(
                            f"HIGH MEMORY PRESSURE: {memory_before:.1f}MB - performing aggressive cleanup"
                        )

                        # Force comprehensive cleanup first
                        if hasattr(self, "last_metagraph_sync"):
                            comp_memory_freed = self._comprehensive_memory_cleanup(
                                "pressure_aggressive"
                            )
                        ...
                        try:
                            ...
                            if task_count > 50:  # Too many background tasks
                                logger.warning(
                                    f"High background task count: {task_count} - cleaning up completed tasks"
                                )
                                completed_tasks = [
                                    t for t in self._background_tasks if t.done()
                                ]
                                for task in completed_tasks:
                                    self._background_tasks.discard(task)
                                logger.info(
                                    f"Removed {len(completed_tasks)} completed background tasks"
                                )
                        except Exception:
                            pass
                        ...
                        logger.info(
                            f"MODERATE MEMORY PRESSURE: {memory_before:.1f}MB - performing moderate cleanup"
                        )
                    ...
                    if cleanup_level == "aggressive" or memory_freed > 50:
                        logger.info(
                            f"Memory cleanup ({cleanup_level}): {memory_before:.1f}MB → {memory_after:.1f}MB "
                            f"(freed {memory_freed:.1f}MB, GC collected {collected} objects)"
                        )
                    elif collected > 50 or comp_memory_freed > 10:
                        logger.info(
                            f"Memory cleanup ({cleanup_level}): GC collected {collected} objects, "
                            f"comprehensive freed {comp_memory_freed:.1f}MB"
                        )
                    else:
                        logger.debug(
                            f"Memory cleanup ({cleanup_level}): minimal cleanup needed"
                        )
    ...
    def _comprehensive_memory_cleanup(self, context: str = "general"):
        ...
        try:
            ...
            try:
                ...
                for module_name in list(sys.modules.keys()):
                    if any(
                        pattern in module_name.lower() for pattern in target_modules
                    ):
                        module = sys.modules.get(module_name)
                        if hasattr(module, "__dict__"):
                            module_cleared = False
                ...
                if modules_cleared > 0:
                    logger.info(
                        f"Module cache cleanup: cleared {cache_objects_cleared} cache objects from {modules_cleared} modules"
                    )
            ...
            try:
                ...
                try:
                    ...
                    if hasattr(fsspec, "config") and hasattr(fsspec.config, "conf"):
                        fsspec.config.conf.clear()
                    if hasattr(fsspec, "filesystem") and hasattr(
                        fsspec.filesystem, "_cache"
                    ):
                        fsspec.filesystem._cache.clear()
                    if hasattr(fsspec, "registry") and hasattr(
                        fsspec.registry, "registry"
                    ):
            ...
            try:
                if (
                    ...
                    if hasattr(self.miner_client, "_transport"):
                        transport = self.miner_client._transport
                        if hasattr(transport, "_pool") and hasattr(
            ...
            try:
                if hasattr(self, "database_manager") and getattr(
                    self, "database_manager", None
                ):
                    # Force database manager to clear any cached connections/results
                    if (
                        hasattr(self.database_manager, "_engine")
                        and self.database_manager._engine
                    ):
            ...
            try:
                ...
                if memory_freed > 10:  # Only log significant memory freeing
                    logger.info(
                        f"Comprehensive cleanup ({context}): freed {memory_freed:.1f}MB, GC collected {collected_total} objects"
                    )
    ...
    async def run_comprehensive_database_setup():
        try:
            ...
            setup_success = await setup_comprehensive_database(
                test_mode=args.test, config=db_config
            )

            if not setup_success:
                logger.error(
                    "❌ Comprehensive database setup failed - validator cannot start safely"
                )
            ...
            logger.error(
                f"❌ Critical error in comprehensive database setup: {e}", exc_info=True
            )
