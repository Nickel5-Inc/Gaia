...
class AlembicCorruptionRepair:
    ...
    def _construct_db_url_from_env(self) -> str:
        ...
        tcp_url = (
            f"postgresql+psycopg2://postgres:{db_password}@localhost:5432/validator_db"
        )
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                if not table_check.scalar():
                    logger.info(
                        "✅ No alembic_version table found - no corruption possible"
                    )
                    return True
                ...
                if not current_version:
                    logger.warning(
                        "⚠️ Empty alembic_version table - this is unusual but not corruption"
                    )
                    return True
                ...
                if current_version in self.known_bad_revisions:
                    logger.warning(
                        f"⚠️ Known stale/bad revision detected: '{current_version}'"
                    )
                    target_version = self.known_bad_revisions[current_version]
                    logger.info(
                        f"🔧 Will replace with correct revision: '{target_version}'"
                    )
                else:
                    # Unknown corruption detected!
                    logger.error(
                        f"❌ CORRUPTION DETECTED: Invalid migration ID '{current_version}'"
                    )
                ...
                # Apply the fix
                logger.info(
                    f"🔧 Updating alembic_version from '{current_version}' to '{target_version}'"
                )
                conn.execute(
                ...
                if new_version == target_version:
                    logger.info(
                        f"✅ Corruption repaired successfully! Version is now: {new_version}"
                    )
                    return True
                else:
                    logger.error(
                        f"❌ Repair verification failed. Expected {target_version}, got {new_version}"
                    )
                    return False
    ...
    def _determine_correct_version(self, conn) -> Optional[str]:
        ...
        try:
            ...
            weight_column_check = conn.execute(
                text(
                    """
                SELECT EXISTS (
            ...
            if weight_column_check.scalar():
                logger.info(
                    "✅ miner_performance_stats weight columns detected - setting version to 8d832ba6c04d"
                )
                return "8d832ba6c04d"

            # Check for miner_performance_stats table
            mps_table_check = conn.execute(
                text(
                    """
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
            ...
            if mps_table_check.scalar():
                logger.info(
                    "✅ miner_performance_stats table detected - setting version to 7cc0a0cb963c"
                )
                return "7cc0a0cb963c"

            # Check for weather_scoring_jobs table
            wsj_table_check = conn.execute(
                text(
                    """
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
            ...
            if wsj_table_check.scalar():
                logger.info(
                    "✅ weather_scoring_jobs table detected - setting version to 7819e940673d"
                )
                return "7819e940673d"

            # Check for geomagnetic_predictions optimization indexes
            geo_index_check = conn.execute(
                text(
                    """
                SELECT EXISTS (
            ...
            if geo_index_check.scalar():
                logger.info(
                    "✅ geomagnetic_predictions optimization indexes detected - setting version to dc4f1a1ad6db"
                )
                return "dc4f1a1ad6db"
            ...
            if constraint_check.scalar():
                logger.info(
                    "✅ Unique constraint detected - setting version to a1b2c3d4e5f6"
                )
                return "a1b2c3d4e5f6"
            ...
            if table_check.scalar():
                logger.info(
                    "✅ soil_moisture_history table exists but no constraint - setting version to 9184456655c8"
                )
                return "9184456655c8"
            else:
                logger.info(
                    "⚠️ soil_moisture_history table missing - using initial migration"
                )
                return "f75e4f7343a1"
    ...
    def _add_missing_constraint(self, conn) -> bool:
        ...
        try:
            logger.info(
                "🔧 Adding missing unique constraint to soil_moisture_history..."
            )
            ...
            if duplicate_count > 0:
                logger.info(
                    f"🧹 Removing {duplicate_count} sets of duplicate records..."
                )
                conn.execute(
                    text(
                        """
