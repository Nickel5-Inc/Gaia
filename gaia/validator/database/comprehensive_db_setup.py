...
class ComprehensiveDatabaseSetup:
    ...
    def __init__(
        self, config: Optional[DatabaseConfig] = None, test_mode: bool = False
    ):
    ...
    async def setup_complete_database_system(self) -> bool:
        ...
        if not await self._ensure_postgresql_running():
            logger.error(
                "❌ Failed to start or repair the PostgreSQL service. Cannot proceed."
            )
            return False
        ...
        if not await self._restart_postgresql_service():
            logger.error(
                "❌ Failed to restart PostgreSQL after configuration. The service might be unstable."
            )
    ...
    async def _ensure_postgresql_installed(self) -> bool:
        ...
        try:
            if self.system_info["os"] in ["ubuntu", "debian"]:
                ...
                logger.warning(
                    f"Unsupported OS: {self.system_info['os']}, attempting generic installation"
                )
    ...
    async def _install_postgresql_debian(self) -> bool:
        ...
        commands = [
            ...
            [
                ...
                "-y",
                f"postgresql-{self.config.postgres_version}",
                f"postgresql-client-{self.config.postgres_version}",
                "postgresql-contrib",
            ],
        ]
    ...
    async def _install_postgresql_rhel(self) -> bool:
        ...
        commands = [
            [
                ...
                "install",
                "-y",
                f"postgresql{self.config.postgres_version}-server",
                f"postgresql{self.config.postgres_version}",
                f"postgresql{self.config.postgres_version}-contrib",
            ]
        ]
        ...
        init_cmd = [
            "sudo",
            f"postgresql-{self.config.postgres_version}-setup",
            "initdb",
        ]
    ...
    async def _install_postgresql_arch(self) -> bool:
        ...
        commands = [["sudo", "pacman", "-Sy", "--noconfirm", "postgresql"]]
    ...
    async def _install_postgresql_generic(self) -> bool:
        ...
        logger.warning(
            "⚠️ Using generic installation method - may not work on all systems"
        )
    ...
    async def _detect_and_repair_corruption(self) -> bool:
        ...
        if not data_dir.exists() or not data_dir.is_dir():
            logger.warning(
                f"⚠️ Data directory not found at {data_dir}, initializing fresh cluster."
            )
        ...
        if not config_dir.exists() or not config_dir.is_dir():
            logger.warning(
                f"⚠️ Config directory not found at {config_dir}, will repair."
            )
    ...
    async def _initialize_fresh_cluster(self) -> bool:
        ...
        try:
            ...
            if not success:
                logger.error(
                    f"Failed to create cluster with pg_createcluster: {stderr}"
                )
                return False
    ...
    async def _configure_postgresql_system(self) -> bool:
        ...
        try:
            ...
            await self._run_command(
                ["sudo", "chown", "-R", "postgres:postgres", str(config_dir)]
            )
    ...
    async def _configure_postgresql_conf(self) -> bool:
        ...
        config_settings = {
            ...
            "checkpoint_completion_target": str(
                self.config.checkpoint_completion_target
            ),
        ...
        try:
            ...
            if process.returncode == 0:
                logger.info(
                    f"✅ Custom PostgreSQL configuration written to: {custom_config_file}"
                )
                return True
            else:
            ...
            logger.error(
                f"Error configuring postgresql.conf via conf.d: {e}", exc_info=True
            )
            return False
    ...
    async def _ensure_postgresql_running(self) -> bool:
        ...
        is_active, _, _ = await self._run_command(
        ...
        status_success, status_stdout, status_stderr = await self._run_command(
        ...
        start_success, start_stdout, start_stderr = await self._run_command(
        ...
        if start_success:
            ...
            is_active_after_start, _, _ = await self._run_command(
                ["sudo", "systemctl", "is-active", service_name]
            )
            if is_active_after_start:
                ...
                logger.warning(
                    f"Service '{service_name}' start command succeeded but service is not active."
                )
        ...
        if not data_dir.exists():
            ...
            logger.info(
                "🔧 This appears to be a missing cluster, not corruption. Initializing fresh cluster..."
            )
            return await self._initialize_fresh_cluster()
        ...
        if not config_file.exists():
            ...
            logger.info(
                "🔧 This appears to be a missing configuration, not corruption. Initializing fresh cluster..."
            )
            return await self._initialize_fresh_cluster()

        # Check PostgreSQL logs for more specific error information
        log_success, log_output, _ = await self._run_command(
        ...
        if log_success and log_output:
            ...
            if any(
                ...
                logger.warning(
                    "🔍 Detected permission or missing file issues - attempting ownership fix..."
                )

                # Try fixing ownership first
                chown_success, _, _ = await self._run_command(
                    ["sudo", "chown", "-R", "postgres:postgres", str(data_dir)]
                )
                chmod_success, _, _ = await self._run_command(
                    ["sudo", "chmod", "-R", "700", str(data_dir)]
                )

                if chown_success and chmod_success:
                    logger.info(
                        "🔧 Fixed data directory ownership. Retrying service start..."
                    )
                    retry_success, _, _ = await self._run_command(
                        ["sudo", "systemctl", "start", service_name]
                    )
                    if retry_success:
                        await asyncio.sleep(3)
                        is_active_retry, _, _ = await self._run_command(
                            ["sudo", "systemctl", "is-active", service_name]
                        )
                        if is_active_retry:
                            logger.info(
                                "✅ Service started successfully after ownership fix."
                            )
                            return True

        # Only resort to aggressive repair if other methods failed
        logger.error(
            "🚨 Service start failed after diagnostics and simple fixes. Attempting aggressive repair as last resort..."
        )

        repaired = await self._remove_and_recreate_cluster()
        if repaired:
            logger.info(
                "✅ Aggressive repair successful. PostgreSQL should now be running."
            )
            return True
        else:
            logger.error(
                "💥 Aggressive repair FAILED. The database system is in a bad state."
            )
            return False
    ...
    async def _test_database_connection(self, max_fix_attempts: int = 1) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "psql",
                "--cluster",
                f"{self.config.postgres_version}/main",
                "-c",
                "SELECT version();",
            ]
            success, stdout, stderr = await self._run_command(cmd, timeout=10)
    ...
    async def _create_application_database(self) -> bool:
        ...
        try:
            ...
            check_cmd = ["sudo", "-u", "postgres", "psql", "-lqt"]
    ...
    async def _run_alembic_migrations(self) -> bool:
        ...
        try:
            ...
            if not repair_success:
                logger.warning(
                    "⚠️ Alembic corruption repair failed, but continuing with migration attempt..."
                )
            else:
                logger.info("✅ Pre-migration corruption check passed")
        except ImportError:
            logger.warning(
                "⚠️ Alembic corruption repair module not found - skipping corruption check"
            )
        except Exception as e:
            logger.warning(
                f"⚠️ Error during corruption repair: {e} - continuing with migration attempt..."
            )
        ...
        success, stdout, stderr = await self._run_command(
            cmd, timeout=120, cwd=project_root_dir
        )
                ...
                try:
                    ...
                    if repair_success:
                        ...
                        if success:
                            ...
                            logger.error(
                                f"Migration still failed after repair: {stderr}"
                            )
                    else:
    ...
    async def _validate_complete_setup(self) -> bool:
        ...
        try:
            ...
            if not await self._test_database_connection(
                max_fix_attempts=0
            ):  # No more fixes at this stage
                logger.error("❌ Database connection test failed")
                return False

            # Test 3: Application database exists
            check_db_cmd = ["sudo", "-u", "postgres", "psql", "-lqt"]
            ...
            if (
                not success or "t" not in stdout.lower()
    ...
    async def _run_command(
        self, cmd: List[str], timeout: int = 30, cwd: Optional[str] = None
    ) -> Tuple[bool, str, str]:
        ...
        if "sudo" in cmd and "-u" in cmd and "postgres" in cmd and cwd is None:
            effective_cwd = "/tmp"
            logger.debug(
                f"🔧 Overriding cwd to '{effective_cwd}' for postgres user command"
            )
        ...
        try:
            ...
            try:
                ...
                if success:
                    ...
                    if stdout_str:
                        logger.debug(
                            f"   Output: {stdout_str[:200]}{'...' if len(stdout_str) > 200 else ''}"
                        )
                else:
                    logger.warning(
                        f"❌ Command failed (code {process.returncode}): {cmd_str}"
                    )
                    if stderr_str:
                        logger.warning(
                            f"   Error: {stderr_str[:200]}{'...' if len(stderr_str) > 200 else ''}"
                        )
    ...
    async def emergency_repair(self) -> bool:
        ...
        logger.info(
            "🚨 Aggressively removing and recreating cluster for emergency repair..."
        )
        return await self._remove_and_recreate_cluster()
    ...
    async def _emergency_stop_postgresql(self):
        ...
        try:
            ...
            if service_name:
                await self._run_command(
                    ["sudo", "systemctl", "stop", service_name], timeout=30
                )
...
async def setup_comprehensive_database(
    test_mode: bool = False, config: Optional[DatabaseConfig] = None
) -> bool:
    """
    ...
    try:
        ...
        if not success:
            logger.error(
                "💥 Main database setup failed. Attempting emergency repair..."
            )
            repaired = await setup_manager.emergency_repair()

            if repaired:
                logger.info(
                    "✅ Emergency repair successful. Re-running setup on the fresh cluster..."
                )
                # After repair, we MUST run the setup again to configure the new cluster.
                success = await setup_manager.setup_complete_database_system()
            else:
                logger.error(
                    "❌ Emergency repair also failed! The system is in a non-recoverable state."
                )
                success = False
        ...
        logger.error(
            f"💥 Unexpected error in comprehensive database setup: {e}", exc_info=True
        )
        return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--database-name", default="gaia_validator", help="Database name"
    )
    ...
    config = DatabaseConfig(
        database_name=args.database_name, postgres_version=args.postgres_version
    )

    success = asyncio.run(
        setup_comprehensive_database(test_mode=args.test, config=config)
    )
