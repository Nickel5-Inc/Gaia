    ...
    async def remove_miner_from_score_tables(
        ...
        for task_name in task_names:
            try:
                ...
                if not rows:
                    logger.info(
                        f"No '{task_name}' score rows found to update for the given criteria."
                    )
                    continue
                ...
                for row in rows:
                    try:
                        ...
                        if not isinstance(all_scores, list):
                            logger.warning(
                                f"Score field is not a list for score_row with task_id {row['task_id']}"
                            )
                            continue
                        ...
                        for uid in uids:
                            if 0 <= uid < len(all_scores):
                                ...
                                is_nan_or_zero = isinstance(current_score, str) or (
                                    isinstance(current_score, float)
                                    and (
                                        math.isnan(current_score)
                                        or current_score == 0.0
                                    )
                                )
                                logger.debug(
                                    f"Score for UID {uid} in row {row['task_id']}: {current_score} (is_nan_or_zero: {is_nan_or_zero})"
                                )
        ...
        try:
            ...
            if session:
                ...
                async with self.session(
                    operation_name=f"execute_new_session_query_snippet_{query[:30]}"
                ) as new_session:
                    try:
                        ...
                        logger.debug(
                            f"Query executed successfully within session {id(new_session)} for query: {query[:100]}..."
                        )
                        return result
                    except asyncio.CancelledError:
                        logger.warning(
                            f"Execute operation cancelled for session {id(new_session)} query: {query[:100]}..."
                        )
                        ...
                        logger.error(
                            f"Error during query for session {id(new_session)} (query: {query[:100]}...): {e_inner}."
                        )
                        # Rollback will be handled by BaseDatabaseManager.session's except block.
