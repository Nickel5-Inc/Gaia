...
def patched_query_substrate(
    ...
    try:
        ...
        if "'int' object has no attribute 'value'" in str(
            ...
            logger.debug(
                f"query_substrate compatibility fix: handling .value attribute error"
            )
...
class GaiaValidator:
        ...
        try:
            ...
            try:
                ...
                if "site-packages" in asyncio_file and "asyncio" in asyncio_file:
                    ...
                    print(
                        f"[STARTUP] 🔧 Attempting to remove problematic asyncio package..."
                    )
    ...
    def __init__(self, args):
        ...
        self.miner_client = httpx.AsyncClient(
            timeout=httpx.Timeout(
                connect=15.0, read=60.0, write=30.0, pool=10.0
            ),  # INCREASED: More reasonable timeouts for network latency
        ...
        self.task_weight_schedule = [
            (datetime(2025, 5, 28, 0, 0, 0, tzinfo=timezone.utc), {"weather": 1.0}),
            # Consistent weather-only approach since other tasks are disabled
            (datetime(2025, 6, 1, 0, 0, 0, tzinfo=timezone.utc), {"weather": 1.0}),
            # Weather-only continues
            (datetime(2025, 6, 5, 0, 0, 0, tzinfo=timezone.utc), {"weather": 1.0}),
        ]
    ...
    async def query_miners(
        ...
        try:
            ...
            request_concurrency = (
                8  # HIGHER: Can process more requests once handshakes are established
            )
            chunks = []
            ...
            logger.info(
                f"Processing {len(miners_to_query)} miners in {len(chunks)} chunks of {chunk_size}"
            )
            logger.info(
                f"Handshake concurrency: {handshake_concurrency} (LOW to prevent event loop congestion)"
            )
            logger.info(
                f"Request concurrency: {request_concurrency} (HIGHER once handshakes established)"
            )

            # Configuration for immediate retries
            max_retries_per_miner = (
                2  # INCREASED: Allow more retry attempts for better reliability
            )
            base_timeout = (
                20.0  # INCREASED: More reasonable timeout for network latency
            )
                        ...
                        try:
                            ...
                            try:
                                ...
                                async with global_handshake_semaphore:
                                    logger.debug(
                                        f"🤝 Starting handshake for {miner_hotkey} (global handshake #{global_handshake_semaphore._value})"
                                    )
                                    ...
                                    logger.debug(
                                        f"✅ Handshake with {miner_hotkey} completed in {time.time() - handshake_start_time:.2f}s"
                                    )
                            ...
                            Exception
                        ) as outer_error:  # Catch errors within the attempt loop but outside handshake/request
                            logger.debug(
                            ...
                            for conn in list(connections):
                                if hasattr(conn, "is_idle") and conn.is_idle():
                                    try:
                                        ...
                                        closed_count += 1
                                        logger.debug(
                                            f"Aggressively closed idle connection"
                                        )
                                    except Exception as e:
                                        logger.debug(f"Error closing connection: {e}")
                ...
                try:
                    ...
                    if memory_change > 200:
                        if hasattr(self, "last_metagraph_sync"):
                            logger.info(
                                f"Large memory increase detected ({memory_change:.1f}MB) - periodic cleanup will handle if needed"
                            )
                        else:
                            logger.info(
                                f"Large memory increase detected during startup ({memory_change:.1f}MB) - normal for initialization"
                            )
            ...
            if memory_mb > self.memory_critical_threshold_mb:
                ...
                if critical_operations_active:
                    ...
                    if (
                        ...
                        try:
                            ...
                            if immediate_post_gc_memory > (
                                ...
                                if self.pm2_restart_enabled:
                                    logger.error(
                                        f"🔄 FORCED RESTART: Cannot wait for critical operations - OOM imminent!"
                                    )
                            ...
                            if self.pm2_restart_enabled:
                                logger.error(
                                    f"🔄 FORCED RESTART: GC failed and OOM imminent!"
                                )
                                await self._trigger_pm2_restart(
                    ...
                    try:
                        ...
                        if self.pm2_restart_enabled:
                            logger.error(
                                f"🔄 IMMEDIATE RESTART: GC failed and memory critical"
                            )
                            await self._trigger_pm2_restart(
                ...
                if not hasattr(self, "_emergency_memory_start_time"):
                    ...
                    logger.warning(
                        f"🕐 Emergency memory pressure started - will force restart if sustained for 60 seconds"
                    )
        ...
        try:
            ...
            try:
                ...
                if active_nodes_list is None:
                    active_nodes_list = (
                        []
                    )  # Ensure it's an iterable if None is returned
                    logger.warning(
                ...
                active_nodes_list = (
                    []
                )  # Proceed with empty list to avoid further errors here
            ...
            try:
                ...
                logger.info(
                    f"Found {len(weather_results)} distinct (miner_uid, hotkey) pairs in weather score history."
                )
            except Exception as e:
            ...
            if not all_historical_pairs:
                logger.info(
                    "No weather historical data found to check. Skipping stale history cleanup."
                )
                return

            logger.info(
                f"Found {len(all_historical_pairs)} total distinct weather historical (miner_uid, hotkey) pairs to check."
            )
            ...
            tasks_for_score_cleanup = [
                name for name in all_task_names_in_scores if name == "weather"
            ]
                    ...
                    for stale_hk in stale_hotkeys:
                        ...
                        try:
                            result = await self.database_manager.fetch_one(
                                score_table_ts_query, {"uid": uid, "hotkey": stale_hk}
                            )
                            ...
                            if (
                                ...
                                logger.info(
                                    f"  Found time range in weather_miner_scores for UID {uid}, hotkey {stale_hk}: {result['min_ts']} -> {result['max_ts']}"
                                )

                        except Exception as e_ts:
                            logger.warning(
                                f"Could not query timestamps from weather_miner_scores for miner_uid {uid}, hotkey {stale_hk}: {e_ts}"
                            )
                        ...
                        logger.info(
                            f"  Processing weather score cleanup for UID {uid} with stale hotkey {stale_hk}"
                        )
        ...
        async def run_validator_logic():
            ...
            try:
                ...
                if self.auto_sync_manager:
                    logger.info(
                        f"AutoSyncManager is active - Starting setup and scheduling..."
                    )
                    logger.info(
                ...
                active_service_tasks = (
                    []
                )  # Define here for access in except CancelledError
                shutdown_waiter = (
        ...
        while True:
            ...
            try:
                ...
                async with self.miner_table_lock:
                    ...
                    uids_to_update_info = (
                        {}
                    )  # uid: new_chain_node (for existing miners with potentially changed info)
                        ...
                        for (
                            ...
                            try:
                                ...
                                prediction_tables_by_uid = (
                                    []
                                )  # Geomagnetic and soil moisture predictions disabled
                                ...
                                history_tables_by_hotkey = (
                                    []
                                )  # Geomagnetic and soil moisture history disabled
                                ...
                                logger.info(
                                    f"  Cleaning weather-specific tables for UID {uid_to_process}"
                                )
                                try:
                                    ...
                                    logger.info(
                                        f"  Deleted weather_miner_scores for UID {uid_to_process}"
                                    )
                                    ...
                                    logger.info(
                                        f"  Deleted weather_miner_responses for UID {uid_to_process}"
                                    )
                                    ...
                                    logger.info(
                                        f"  Deleted weather_historical_weights for old hotkey {original_hotkey}"
                                    )
                                except Exception as weather_cleanup_err:
                                    logger.error(
                                        f"  Error cleaning weather tables for UID {uid_to_process}: {weather_cleanup_err}"
                                    )
                                ...
                                logger.info(
                                    f"  Zeroing scores in score_table for UID {uid_to_process} for weather task"
                                )
                                await self.database_manager.remove_miner_from_score_tables(
                                    uids=[uid_to_process],
    ...
    def _perform_weight_calculations_sync(
        self, weather_results, now, validator_nodes_by_uid_list
    ):
        """
        ...
        try:
            ...
            if len(valid_scores) >= 5:  # Minimum sample for percentiles
                ...
                logger.info(
                    f"{task_name.capitalize()} task: {len(valid_scores)} valid scores, 85th percentile: {percentiles[6]:.4f}"
                )
            else:
            ...
            for idx in range(256):
                ...
                node_info = (
                    ...
                    if validator_nodes_by_uid_list
                    else None
                )
                if node_info:
                    ip = node_info.get("ip")
                ...
                if "weather" in task_percentiles:
                    ...
                    if (
                        ...
                        and w_s >= task_data["percentile_85"]
                    ):
                        # Excellence threshold: top 15% get excellence bonus
                        weights_final[idx] = (
                            w_s * 1.0
                        )  # Full weight since weather is 100% of scoring
                ...
                qualified_for_excellence = (
                    "weather" in task_percentiles
                    and task_percentiles["weather"]["valid_count"] >= 10
                    and w_s >= task_percentiles["weather"]["percentile_85"]
                )
                ...
                pathway_tracking[idx] = {
                    ...
                    "pathway_details": {
                        ...
                        "percentile_rank": (
                            get_percentile_rank(w_s, "weather")
                            if "weather" in task_percentiles
                            else None
                        ),
                ...
                if weights_final[idx] > 0.1:
                    ...
                    logger.debug(
                        f"UID {idx} ({hk_chain}): weather-only pathway, "
            ...
            logger.info(
                f"Weather-only scoring - Excellence miners: {total_excellence} "
                f"(Weather: {excellence_count['weather']})"
            )
            logger.info(
                f"Total miners with weather scores: {np.sum(weather_scores > 0)}"
            )
            logger.info(
            ...
            if not np.any(non_zero_mask):
                logger.warning(
                    "No non-zero weights to normalize! Setting fallback weights: 100% to UID 252, 0% to all others."
                )
            ...
            if max_w_val == 0:
                logger.warning(
                    "Max weight is 0, cannot normalize. Setting fallback weights: 100% to UID 252, 0% to all others."
                )
                fallback_weights = np.zeros(256)
                fallback_weights[252] = 1.0  # Set 100% weight to UID 252
                return fallback_weights.tolist(), {}
            ...
            if not positives.any():
                logger.warning(
                    "No positive weights after initial normalization! Setting fallback weights: 100% to UID 252, 0% to all others."
                )
                fallback_weights = np.zeros(256)
                fallback_weights[252] = 1.0  # Set 100% weight to UID 252
                return fallback_weights.tolist(), {}
            ...
            if not nz_indices.any():
                logger.warning(
                    "No positive weight indices for transformation! Setting fallback weights: 100% to UID 252, 0% to all others."
                )
                fallback_weights = np.zeros(256)
                fallback_weights[252] = 1.0  # Set 100% weight to UID 252
                return fallback_weights.tolist(), {}
            ...
            if final_sum > 0:
                ...
                logger.warning(
                    "Sum of weights is zero, cannot normalize! Setting fallback weights: 100% to UID 252, 0% to all others."
                )
                fallback_weights = np.zeros(256)
                fallback_weights[252] = 1.0  # Set 100% weight to UID 252
            ...
            if final_weights_list:
                for idx in range(256):
                    if idx in pathway_tracking:
                        pathway_tracking[idx]["submitted_weight"] = final_weights_list[
                            idx
                        ]
            ...
            try:
                ...
                if "excellence_count" in locals():
                    del excellence_count
                if "diversity_count" in locals():
                    del diversity_count
    ...
    async def _calc_task_weights(self):
        ...
        try:
            ...
            weather_params = {
                "weather_start_time": weather_lookback
            }  # For weather 15-day lookback
            ...
            try:
                ...
                if weather_results and not isinstance(weather_results, Exception):
                    logger.info(
                        f"Weather dataset: {len(weather_results)} records (lookback: 15 days for delayed ERA5 scores)"
                    )
                    ...
                    if weather_results:
                        ...
                        logger.info(
                            f"Weather scores age range: {score_age_range} days (newest: {(now - newest_score).days} days old)"
                        )
                    else:
                        logger.warning(
                            "Weather query returned empty results - checking manually..."
                        )
                        ...
                        debug_result = await self.database_manager.fetch_one(
                            debug_weather_query
                        )
                        logger.info(f"Debug weather check: {debug_result}")
                else:
                    logger.warning(
                        f"Weather query failed or returned no results: {type(weather_results)} - {weather_results if isinstance(weather_results, Exception) else 'Empty result'}"
                    )
                ...
                if isinstance(weight_result, tuple) and len(weight_result) == 2:
                    ...
                    logger.info(
                        "No weather scores available - setting fallback weights: 100% to UID 252, 0% to all others"
                    )
                    ...
                    logger.warning(
                        f"Weight calculation returned unexpected format: {type(weight_result)} - using as-is"
                    )
                        ...
                        if hasattr(pool, "_connections"):
                            ...
                            if hasattr(self.miner_client, "_limits"):
                                ...
                                if hasattr(
                                    ...
                                    keepalive_limit = (
                                        self.miner_client._limits.max_keepalive_connections
                                    )
            ...
            try:
                ...
                if PSUTIL_AVAILABLE:
                    ...
                    if cleanup_level == "aggressive":
                        ...
                        logger.info(
                            f"LIGHT MEMORY PRESSURE: {memory_before:.1f}MB - performing minimal cleanup"
                        )
                        ...
                        logger.debug(
                            f"PERFORMANCE MODE: {memory_before:.1f}MB - no cache clearing needed"
                        )
                    ...
                    if cleanup_level != "none":
                        try:
                            ...
                            if global_stats.get("callbacks_attempted", 0) > 0:
                                logger.debug(
                                    f"Global thread cleanup ({cleanup_level}): {global_stats['callbacks_succeeded']}/{global_stats['callbacks_attempted']} callbacks succeeded"
                                )
                        except Exception as e:
    ...
    def _comprehensive_memory_cleanup(self, context: str = "general"):
        ...
        try:
            ...
            skip_module_caches = (
                context == "pressure_light"
            )  # Don't clear module caches in light pressure
            ...
            if not skip_module_caches:
                ...
                logger.debug(
                    f"Skipping module cache cleanup in {context} mode to preserve performance"
                )

            # 3. Context-aware LRU cache cleanup (selective in light pressure mode)
            skip_lru_caches = (
                context == "pressure_light"
            )  # Preserve LRU caches in light pressure
            ...
            if not skip_lru_caches:
                try:
                    ...
                    with warnings.catch_warnings():
                        ...
                        if lru_caches_cleared > 0:
                            logger.info(
                                f"LRU cache cleanup: cleared {lru_caches_cleared} LRU caches from {lru_modules_checked} modules"
                            )
                ...
                logger.debug(
                    f"Skipping LRU cache cleanup in {context} mode to preserve performance"
                )
