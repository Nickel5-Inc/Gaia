...
class RestoreManager:
    ...
    async def _trigger_pgbackrest_command(
        ...
        logger.warning(
            "Ensure this command is run with appropriate permissions (e.g., as 'postgres' user) "
            "and that PostgreSQL is stopped if performing a full restore."
        )
        try:
...
async def get_restore_manager(test_mode: bool = False) -> Optional[RestoreManager]:
    ...
    try:
        ...
        logger.error(
            "pgBackRest command-line tool not found in PATH. RestoreManager will likely fail."
        )
