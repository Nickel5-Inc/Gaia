    ...
    async def _verify_postgresql_configuration(self, postgres_user: str) -> bool:
            ...
            try:
                ...
                if returncode == 0:
                    ...
                    if archive_mode == "on":
                        ...
                        cmd_returncode, cmd_stdout_text, cmd_stderr_text = (
                            await self._run_postgres_command_with_logging(
                                check_cmd,
                                timeout=15,
                                description="Archive command check",
                            )
                        )
        ...
        if await self._emergency_postgresql_restart():
            ...
            try:
                ...
                final_returncode, final_stdout_text, final_stderr_text = (
                    await self._run_postgres_command_with_logging(
                        check_cmd,
                        timeout=10,
...
"""
    ...
    async def _initialize_pgbackrest(self) -> bool:
        ...
        try:
            ...
            if verification_success:
                ...
                logger.info(
                    f"🎉 pgBackRest initialization FULLY COMPLETED with R2 upload verification"
                )
                logger.info(
                ...
                logger.warning(
                    f"⚠️ Initial backup completed but R2 upload verification failed"
                )
                logger.warning(
    ...
    async def _backup_scheduler(self):
        ...
        try:
            ...
            def cleanup_backup_caches():
                ...
                logger.debug(
                    f"[BackupScheduler] Performed cleanup, collected {collected} objects"
                )
    ...
    async def _replica_sync_scheduler(self):
        ...
        try:
            ...
            def cleanup_replica_caches():
                ...
                collected = gc.collect()
                logger.debug(
                    f"[ReplicaScheduler] Performed cleanup, collected {collected} objects"
                )
        ...
        try:
            ...
            try:
                ...
                if restore_success:
                    ...
                    logger.info(
                        f"🎉 REPLICA SYNC COMPLETED: Database completely replaced with primary data"
                    )
                    logger.info(
                ...
                if backup_info and len(backup_info) > 0:
                    ...
                    if "backup" in stanza_info and len(stanza_info["backup"]) > 0:
                        ...
                        try:
                            ...
                            if backup_timestamp != "unknown":
                                ...
                                if backup_time:
                                    ...
                                    if time_diff <= 600:  # Within 10 minutes
                                        ...
                                        if time_diff <= 3600:
                                            logger.info(
                                                f"✅ R2 upload VERIFIED - Backup found within reasonable timeframe"
                                            )
                                            logger.info(
                                                f"🌥️ Backup successfully stored in R2 bucket: {self.config['r2_bucket']}"
                                            )
                                    ...
                                    logger.info(
                                        f"✅ R2 upload assumed VERIFIED - Backup exists in repository"
                                    )
                                    logger.info(
                                        f"🌥️ Backup successfully stored in R2 bucket: {self.config['r2_bucket']}"
                                    )
                            ...
                            logger.info(
                                f"✅ R2 upload VERIFIED - Backup exists in repository"
                            )
                            logger.info(
                                f"🌥️ Backup successfully stored in R2 bucket: {self.config['r2_bucket']}"
                            )
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if (
                    ...
                    logger.warning(
                        f"⚠️ WAL archiving timeout detected (this is often temporary)"
                    )
    ...
    async def restore_from_backup(self, target_time: Optional[str] = None) -> bool:
        ...
        if not success:
            ...
            if has_corruption:
                ...
                info_return_code, info_stdout, info_stderr = (
                    await self._run_postgres_command_with_logging(
                ...
                if info_success and info_stdout:
                    ...
                    if len(backup_lines) > 1:
                        ...
                        if backup_match:
                            ...
                            older_return_code, _, older_stderr = (
                                await self._run_postgres_command_with_logging(
                                    older_restore_cmd,
                                    timeout=7200,
        ...
        if not success:
            ...
            full_return_code, _, full_stderr = (
                await self._run_postgres_command_with_logging(
                    full_restore_cmd,
                    timeout=7200,
                    description="Full pgBackRest restore",
                )
            )
    ...
    def print_current_status(self):
        ...
        if self.is_primary and not self.test_mode:
            ...
            print(
                f"   • Catch-up Window: Every hour at :50-59 minutes (if backup missed)"
            )
            ...
            if not reload_success:
                ...
                if pg_version and pgdata_path:
                    ...
                    if pg_ctl_path.exists():
                        ...
                        return_code_pgctl, _, stderr_pgctl = (
    ...
    async def _ensure_correct_archive_command(self) -> bool:
        ...
        try:
            ...
            if not archive_mode_correct:
                ...
                return_code_mode_set, _, stderr_mode_set = (
