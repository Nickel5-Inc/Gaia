...
class AutoSyncManager:
    ...
    def __init__(self, test_mode: bool = False):
        ...
        logger.info(
            f"MODE: {'PRIMARY DATABASE' if self.is_primary else 'REPLICA DATABASE'}"
        )
        logger.info(
            f"TEST MODE: {'ENABLED (Fast scheduling)' if self.test_mode else 'DISABLED (Production scheduling)'}"
        )
    ...
    def _detect_system_configuration(self) -> Dict:
        ...
        try:
                    ...
                    with open("/etc/os-release") as f:
                        for line in f:
                            if line.startswith("ID="):
                                ...
                                system_info["distribution_version"] = (
                                    line.split("=")[1].strip().strip('"')
                                )
            ...
            try:
                ...
                logger.info(
                    f"Systemd: {'Available' if system_info['systemd_available'] else 'Not available'}"
                )
            except Exception:
                pass
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    for line in lines:
                        if "postgresql@" in line and (
                            "loaded" in line or "active" in line
                        ):
                            ...
                            if parts and parts[0].endswith(".service"):
                                ...
                                logger.info(
                                    f"🐘 Found cluster-specific PostgreSQL service: {cluster_service}"
                                )
                                break
            except Exception as e:
                logger.debug(
                    f"Error detecting cluster-specific PostgreSQL service: {e}"
                )
            ...
            for user in user_variations:
                try:
                    result = subprocess.run(
                        ["id", user], capture_output=True, text=True, timeout=5
                    )
                    if result.returncode == 0:
    ...
    def _find_pgdata_path(self) -> str:
        ...
        try:
            logger.info(
                "🔍 Attempting to discover PostgreSQL data directory from running instance..."
            )
            ...
            if pgdata_path and os.path.exists(pgdata_path):
                logger.info(
                    f"✅ Discovered active PostgreSQL data directory at: {pgdata_path}"
                )
                return pgdata_path
        except subprocess.TimeoutExpired:
            logger.warning(
                "⏱️ PostgreSQL query timed out after 10 seconds - PostgreSQL may not be running"
            )
        except (subprocess.CalledProcessError, FileNotFoundError) as e:
            logger.warning(
                f"❌ Could not get data_directory from running PostgreSQL. Reason: {e}"
            )

        logger.info(
            "🔍 Falling back to checking common PostgreSQL installation paths for pg_control..."
        )
        common_paths = [
        ...
        if pgdata_env:
            logger.warning(
                f"⚠️ Could not find a valid pgdata path. Using PGBACKREST_PGDATA from environment: {pgdata_env}"
            )
            return pgdata_env

        logger.error(
            "❌ Could not determine PostgreSQL data directory through any method"
        )
        raise FileNotFoundError(
            "Could not determine PostgreSQL data directory. Please ensure PostgreSQL is running or set PGBACKREST_PGDATA."
        )
    ...
    def _load_config(self) -> Dict:
        ...
        try:
            ...
            logger.warning(
                f"   🗄️ PostgreSQL: ⚠️ Using fallback directory: {pgdata_path}"
            )
        ...
        if os.getenv("PGBACKREST_STANZA_NAME"):
            ...
            logger.info(
                f"🔧 Using explicit stanza override: {explicit_stanza} (was auto-detected as: {stanza_name})"
            )
        ...
        config = {
            ...
            "pgpassword": os.getenv(
                "PGBACKREST_PGPASSWORD", os.getenv("DB_PASSWORD", "postgres")
            ),  # Optional password with fallback
            "postgres_password": os.getenv(
                "PGBACKREST_PGPASSWORD", os.getenv("DB_PASSWORD", "postgres")
            ),  # Add alias for backward compatibility
            "is_primary": os.getenv("IS_SOURCE_VALIDATOR_FOR_DB_SYNC", "False").lower()
            == "true",
            "replica_discovery_endpoint": os.getenv(
                "REPLICA_DISCOVERY_ENDPOINT"
            ),  # For primary to announce itself
            "primary_discovery_endpoint": os.getenv(
                "PRIMARY_DISCOVERY_ENDPOINT"
            ),  # For replica to find primary
            "network": network_suffix,  # Store network for reference
            "replica_startup_sync": os.getenv("REPLICA_STARTUP_SYNC", "true").lower()
            == "true",  # Enable immediate sync on startup
        }
        ...
        if pg_version:
            ...
            if match:
                ...
                config["config_directory"] = f"/etc/postgresql/{pg_version}/main"
                logger.info(
                    f"Derived pg_version '{pg_version}' from service name for config path."
                )
            else:
                logger.warning(
                    "Could not determine PostgreSQL version for config path. Re-apply may fail."
                )
    ...
    async def _install_dependencies(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"System: {self.system_info.get('os_type', 'unknown')} with {self.system_info.get('package_manager', 'unknown')} package manager"
            )
        ...
        try:
            ...
            commands = [
                ...
                (
                    ...
                    300,
                    "Installing pgBackRest and PostgreSQL client",
                ),
            ]
    ...
    async def _install_dependencies_rhel(self) -> bool:
        ...
        try:
            ...
            commands = [
                (
                    ["sudo", package_cmd, "install", "-y", "epel-release"],
                    120,
                    "Installing EPEL repository",
                ),
                (["sudo", package_cmd, "update", "-y"], 180, "Updating packages"),
                (
                    ["sudo", package_cmd, "install", "-y", "pgbackrest", "postgresql"],
                    300,
                    "Installing pgBackRest and PostgreSQL",
                ),
            ]
            ...
            for cmd, timeout, description in commands:
                ...
                try:
                    ...
                    if process.returncode != 0:
                        ...
                        if (
                            "already installed" in error_output
                            or "nothing to do" in error_output
                        ):
                            logger.info(f"✅ {description} (already installed)")
                        else:
                            logger.warning(
                                f"⚠️ {description} had issues: {stderr.decode()}"
                            )
    ...
    async def _install_dependencies_arch(self) -> bool:
        ...
        try:
            ...
            commands = [
                ...
                (
                    ["sudo", "pacman", "-S", "--noconfirm", "pgbackrest", "postgresql"],
                    300,
                    "Installing pgBackRest and PostgreSQL",
                ),
            ]
    ...
    async def _install_dependencies_fallback(self) -> bool:
        ...
        try:
            logger.warning(
                "⚠️ Unknown package manager, attempting fallback installation..."
            )
            ...
            logger.error(
                "   - From source: https://pgbackrest.org/user-guide.html#installation"
            )
    ...
    async def _configure_postgresql(self) -> bool:
        ...
        try:
            ...
            if not self.is_primary:
                logger.info(
                    "REPLICA MODE: Minimal PostgreSQL configuration (will be overwritten by restore)"
                )
            ...
            try:
                ...
                if process.returncode != 0:
                    logger.warning(
                        f"Failed to detect config file location: {stderr.decode()}"
                    )
                ...
                logger.warning(
                    "Config detection timed out after 30 seconds, using fallback..."
                )
                postgres_conf = await self._fallback_config_detection()
            except Exception as e:
            ...
            if hba_conf.exists():
                ...
                logger.warning(
                    f"⚠️ pg_hba.conf not found at expected location: {hba_conf}"
                )
            ...
            try:
                ...
                return_code, stdout, stderr = await self._run_command_async(
                    alter_cmd, "Set archive_mode via ALTER SYSTEM"
                )
                if return_code == 0:
                    logger.info(
                        "✅ Successfully set archive_mode = on via ALTER SYSTEM"
                    )
                    # archive_mode requires a full restart, not just reload
                    logger.info(
                        "🔄 Restarting PostgreSQL for archive_mode to take effect..."
                    )
                    await self._restart_postgresql_service(service_name)
                else:
                    logger.warning(
                        f"Failed to set archive_mode via ALTER SYSTEM: {stderr}"
                    )
            except Exception as e:
    ...
    async def _restart_postgresql_service(self, service_name: str) -> bool:
        ...
        try:
            ...
            for candidate_service in unique_candidates:
                try:
                    logger.info(
                        f"Attempting to restart PostgreSQL service: {candidate_service}"
                    )
                    ...
                    return_code, _, stderr = await self._run_command_async(
                        restart_cmd, f"Restart PostgreSQL service ({candidate_service})"
                    )

                    if return_code == 0:
                        logger.info(
                            "Waiting for PostgreSQL to initialize after restart..."
                        )
                        ...
                        if await self._verify_service_status(candidate_service):
                            logger.info(
                                f"✅ Successfully restarted PostgreSQL using service: {candidate_service}"
                            )
                            ...
                            logger.warning(
                                f"Service {candidate_service} restart reported success but service is not active"
                            )
                    else:
                        logger.debug(
                            f"Failed to restart PostgreSQL service {candidate_service}: {stderr}"
                        )

                except Exception as e:
                    logger.debug(
                        f"Exception trying to restart PostgreSQL service {candidate_service}: {e}"
                    )
                    continue

            logger.error(
                "❌ Failed to restart PostgreSQL service with any known service name"
            )
            return False
        ...
        for attempt in range(max_retries):
            try:
                ...
                (
                    ...
                    stdout_text,
                    stderr_text,
                ) = await self._run_postgres_command_with_logging(
                    check_cmd,
                    timeout=15,
                    description=f"Archive mode check (attempt {attempt + 1})",
                )
                ...
                if returncode == 0:
                    ...
                    if archive_mode == "on":
                        ...
                        if cmd_returncode == 0:
                            ...
                            logger.info(
                                f"📋 Current archive_command: {current_archive_cmd}"
                            )
                            ...
                            if self.config["stanza_name"] in current_archive_cmd:
                                ...
                                logger.warning(
                                    f"⚠️ Archive command doesn't contain expected stanza: {self.config['stanza_name']}"
                                )

                    else:
                        logger.warning(
                            f"⚠️ archive_mode is '{archive_mode}' instead of 'on' (attempt {attempt + 1}/{max_retries})"
                        )
                        if attempt < max_retries - 1:
                            logger.info(
                                "🔄 Waiting for PostgreSQL configuration to take effect..."
                            )
                            continue
                else:
                    logger.warning(
                        f"⚠️ Failed to check archive_mode (attempt {attempt + 1}/{max_retries}): {stderr_text}"
                    )

            except asyncio.TimeoutError:
                logger.warning(
                    f"⚠️ Timeout checking archive_mode (attempt {attempt + 1}/{max_retries})"
                )

            except Exception as e:
                logger.warning(
                    f"⚠️ Error checking archive_mode (attempt {attempt + 1}/{max_retries}): {e}"
                )

            if attempt < max_retries - 1:
                logger.info(
                    f"⏳ Waiting 3 seconds before retry {attempt + 2}/{max_retries}..."
                )
                await asyncio.sleep(3)

        logger.error(
            "❌ Failed to verify that archive_mode is enabled after PostgreSQL restart"
        )
        logger.error(
            "💡 Attempting emergency PostgreSQL restart to fix hanging issues..."
        )

        # Emergency restart attempt
        if await self._emergency_postgresql_restart():
            logger.info(
                "🔄 Emergency restart completed - trying verification one more time..."
            )
            try:
                check_cmd = [
                    "sudo",
                ...
                (
                    ...
                    check_cmd,
                    timeout=10,
                    description="Final verification after emergency restart",
                )
                ...
                if final_returncode == 0:
                    ...
                    if archive_mode == "on":
                        ...
                        logger.warning(
                            f"⚠️ archive_mode is still '{archive_mode}' after emergency restart"
                        )
                else:
                ...
                logger.error(
                    "❌ Final verification still timed out after emergency restart"
                )
            except Exception as e:
        ...
        try:
            ...
            if self.config["pgpassword"]:
                ...
                if process.returncode == 0:
                    ...
                    logger.warning(
                        f"PostgreSQL authentication test failed: {stderr.decode()}"
                    )
                    logger.warning(
                        "Will attempt to continue - pgBackRest might still work"
                    )

            else:
                logger.info(
                    "No password configured - relying on peer/trust authentication"
                )
                ...
                if process.returncode == 0:
                    logger.info(
                        "✅ PostgreSQL peer/trust authentication test successful"
                    )
                else:
                    logger.error(
                        f"PostgreSQL authentication test failed: {stderr.decode()}"
                    ...
                    logger.error(
                        "You may need to set PGBACKREST_PGPASSWORD or configure peer authentication"
                    )
                    return False
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if (
                    ...
                    or "already exists" in stderr.decode().lower()
                ):
    ...
    async def _configure_pgbackrest(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(config_file):
                logger.info(
                    "🔍 Checking existing pgBackRest configuration for stanza mismatches..."
                )
                try:
                    ...
                    if existing_stanzas:
                        ...
                        if expected_stanza not in existing_stanzas:
                            ...
                            logger.info(
                                f"✅ Configuration already has correct stanza: [{expected_stanza}]"
                            )

                except Exception as e:
                    logger.warning(
                        f"Could not analyze existing config: {e}. Will recreate."
                    )
...
"""
            ...
            logger.info(
                f"✅ pgBackRest configured successfully with stanza: [{expected_stanza}]"
            )
            return True
    ...
    async def _initialize_pgbackrest(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "🏗️ Initializing pgBackRest stanza with intelligent backup detection..."
            )
            logger.info(
                f"⏱️ Setup started at: {setup_start_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
            )
            ...
            if process.returncode != 0:
                ...
                if "already exists" in stderr.decode().lower():
                    logger.info(
                        f"✅ Stanza already exists (checked in {stanza_duration.total_seconds():.1f}s)"
                    )
                else:
                    logger.error(
                        f"❌ Failed to create stanza after {stanza_duration.total_seconds():.1f}s: {stderr.decode()}"
                    )
                    return False
            else:
                logger.info(
                    f"✅ Stanza created successfully in {stanza_duration.total_seconds():.1f} seconds"
                )
            ...
            if backup_decision["skip_backup"]:
                ...
                logger.info(
                    f"⏱️ Backup analysis and skip decision took: {(datetime.now() - setup_start_time).total_seconds():.1f} seconds"
                )
                return True
            ...
            logger.info(
                f"🚀 Taking {backup_decision['action']} {backup_type.upper()} backup..."
            )
            backup_start_time = datetime.now()
            logger.info(
                f"⏱️ Backup started at: {backup_start_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
            )
            ...
            if process.returncode != 0:
                logger.error(
                    f"❌ Initial {backup_type.upper()} backup FAILED after {backup_duration.total_seconds():.1f} seconds"
                )
            ...
            logger.info(
                f"✅ Initial {backup_type.upper()} backup completed successfully"
            )
            logger.info(
                f"⏱️ Backup duration: {backup_duration.total_seconds():.1f} seconds"
            )
            logger.info(
                f"⏱️ Backup finished at: {backup_end_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
            )
            ...
            verification_success = await self._verify_r2_upload(
                backup_type, backup_end_time
            )

            verification_end_time = datetime.now()
            verification_duration = verification_end_time - verification_start_time
            logger.info(
                f"⏱️ Upload verification took: {verification_duration.total_seconds():.1f} seconds"
            )

            if verification_success:
                total_duration = verification_end_time - setup_start_time
                logger.info(
                    "🎉 pgBackRest initialization FULLY COMPLETED with R2 upload verification"
                )
                logger.info(
                    f"⏱️ Total setup + backup + verification time: {total_duration.total_seconds():.1f} seconds"
                )
                logger.info(
                    f"📊 Breakdown: Stanza: {stanza_duration.total_seconds():.1f}s, Backup: {backup_duration.total_seconds():.1f}s, Verification: {verification_duration.total_seconds():.1f}s"
                )
                return True
            else:
                logger.warning(
                    "⚠️ Initial backup completed but R2 upload verification failed"
                )
                logger.warning(
                    f"📊 Backup took {backup_duration.total_seconds():.1f}s but upload verification failed"
                )
                return False
        ...
        try:
            ...
            try:
                if str(backup_timestamp).isdigit():
                    backup_time = datetime.fromtimestamp(
                        int(backup_timestamp), tz=timezone.utc
                    )
                else:
                    # Try parsing as formatted timestamp
                    backup_time = datetime.strptime(
                        str(backup_timestamp), "%Y-%m-%d %H:%M:%S"
                    )
                ...
                logger.debug(
                    f"Could not parse backup timestamp {backup_timestamp}: {e}"
                )
            ...
            if backup_age_hours is not None:
                if self.test_mode:
                    ...
                    if (
                        backup_type == "full" and backup_age_hours < 24
                    ):  # Recent full backup
                        return {
                            "skip_backup": True,
                            "recommended_type": None,
                        ...
                        backup_type in ["diff", "incr"] and backup_age_hours < 4
                    ):  # Recent diff/incr
                        return {
                            "skip_backup": True,
                            "recommended_type": None,
    ...
    async def start_scheduling(self):
        ...
        if (self.backup_task and not self.backup_task.done()) or (
            self.health_check_task and not self.health_check_task.done()
        ):
            logger.info(
                "🔄 AutoSyncManager scheduling is already running, skipping duplicate start"
            )
            return
        ...
        logger.info(
            "✅ Database setup handled by comprehensive system - starting scheduling"
        )
        ...
        if self.is_primary:
            ...
            if self.test_mode:
                logger.info(
                    "⚡ TEST MODE ACTIVE: Differential backups every 15 minutes, health checks every 5 minutes ⚡"
                )
                print("⚡ TEST MODE: FAST BACKUP SCHEDULE ENABLED FOR TESTING ⚡")
            else:
                logger.info(
                    "🏭 PRODUCTION MODE: Full backups daily at 8:30 AM UTC, differential backups hourly at :24 minutes 🏭"
                )
            ...
            if self.test_mode:
                logger.info(
                    "⚡ TEST MODE REPLICA: Downloads every 30 minutes with 5-minute backup buffer ⚡"
                )
                ...
                logger.info(
                    f"🏭 REPLICA MODE: Downloads hourly at :{sync_minute:02d} minutes ({buffer_minutes}min buffer after primary backup) 🏭"
                )
            ...
            if not self.is_primary:
                ...
                if self.config.get("replica_startup_sync", False):
                    logger.info(
                        "🚀 REPLICA STARTUP: Initial sync requested. Attempting to restore from latest backup..."
                    )
                    restore_success = await self.restore_from_backup()
                    if not restore_success:
                        logger.error(
                            "❌ Initial replica sync FAILED. The validator will continue with existing database."
                        )
                        logger.warning(
                            "⚠️ WARNING: Database may be stale. Replica sync will retry automatically."
                        )
                        logger.warning(
                            "💡 To resolve: Check PostgreSQL status and pgBackRest configuration."
                        )
                        # Continue with scheduling even if initial sync failed - the replica sync scheduler will retry
                    else:
                        logger.info(
                            "✅ Initial replica sync successful. Database is now synchronized."
                        )
                        logger.info("🎉 Continuing with normal operation.")
                else:
                    logger.info(
                        "⏭️ REPLICA STARTUP: Skipping initial sync as per configuration (REPLICA_STARTUP_SYNC is not 'true')."
                    )
    ...
    async def _backup_scheduler(self):
        ...
        logger.info(
            f"⏰ SCHEDULER START TIME: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if (
                    self.backup_schedule["full_backup_time"]
                    and now.strftime("%H:%M")
                    == self.backup_schedule["full_backup_time"]
                    and now.date() > last_full_backup
                ):
                    logger.info(
                        "⏰ Scheduled full backup time reached - triggering backup..."
                    )
                    backup_success = await self._trigger_backup_with_timeout(
                        "full", timeout_minutes=30
                    )
                    if backup_success:
                        last_full_backup = now.date()
                        logger.info(
                            f"✅ Full backup completed, next full backup: tomorrow at {self.backup_schedule['full_backup_time']}"
                        )
                ...
                if self.test_mode:
                    ...
                    if hours_since_diff >= self.backup_schedule["diff_backup_interval"]:
                        ...
                        logger.info(
                            f"🚨 TEST MODE BACKUP TRIGGER: {hours_since_diff:.1f} hours since last diff backup (threshold: {self.backup_schedule['diff_backup_interval']}) - triggering backup... 🚨"
                        )
                        backup_success = await self._trigger_backup_with_timeout(
                            "diff", timeout_minutes=15
                        )
                        if backup_success:
                            last_diff_backup = now
                            next_diff_time = now + timedelta(
                                hours=self.backup_schedule["diff_backup_interval"]
                            )
                            print("✅ DIFFERENTIAL BACKUP COMPLETED SUCCESSFULLY ✅")
                            logger.info(
                                f"✅ Differential backup completed, next diff backup: {next_diff_time.strftime('%H:%M:%S')}"
                            )
                    else:
                        # Log status periodically for visibility (every 10 minutes in test mode)
                        if int(now.minute) % 10 == 0 and now.second < 10:
                            time_until_next = (
                                self.backup_schedule["diff_backup_interval"]
                                - hours_since_diff
                            )
                            print(
                                f"📊 TEST MODE BACKUP STATUS: Next diff backup in {time_until_next:.1f} hours (last: {last_diff_backup.strftime('%H:%M:%S')})"
                            )
                            logger.info(
                                f"📊 Test mode backup scheduler: Next diff backup in {time_until_next:.1f} hours (last: {last_diff_backup.strftime('%H:%M:%S')})"
                            )
                else:
                    ...
                    last_backup_hour = (
                        last_diff_backup.hour
                        if last_diff_backup.date() == now.date()
                        else -1
                    )
                    ...
                    if need_backup_this_hour:
                        ...
                        if (
                            ...
                            and current_minute <= target_minute + 5
                        ):
                        ...
                        # Opportunity 2: Catch-up window (last 10 minutes of hour if backup still missing)
                            ...
                            current_minute >= 50 and hours_since_diff >= 0.9
                        ):  # 54+ minutes since last backup
                            should_trigger_backup = True
                        ...
                        # Opportunity 3: Emergency catch-up (if >75 minutes since last backup)
                            ...
                            should_trigger_backup = True
                            trigger_reason = (
                                f"emergency_catchup_{hours_since_diff:.1f}h_overdue"
                            )

                    # Execute backup if triggered and not already attempted this hour
                    if should_trigger_backup:
                        attempts_this_hour = backup_attempts_this_hour.get(
                            current_hour_key, 0
                        )
                        ...
                        if attempts_this_hour < 3:  # Max 3 attempts per hour
                            ...
                            print(
                                f"🚨 ATTEMPT {attempts_this_hour + 1}/3 FOR HOUR {current_hour:02d} 🚨"
                            )
                            logger.info(
                                f"🚨 RESILIENT BACKUP TRIGGER: {trigger_reason} - attempt {attempts_this_hour + 1}/3 🚨"
                            )

                            backup_attempts_this_hour[current_hour_key] = (
                            ...
                            if backup_success:
                                ...
                                logger.info(
                                    f"✅ Resilient backup completed, next backup: {next_hour:02d}:{target_minute:02d}"
                                )
                                ...
                                logger.warning(
                                    f"❌ Backup attempt {attempts_this_hour + 1}/3 failed for hour {current_hour}"
                                )
                        else:
                            if (
                                int(now.minute) % 15 == 0 and now.second < 10
                            ):  # Log every 15 minutes
                                logger.error(
                                    f"❌ Max backup attempts (3) reached for hour {current_hour}. Will retry next hour."
                                )
                    ...
                    if int(now.minute) % 10 == 0 and now.second < 10:
                        if need_backup_this_hour:
                            ...
                            logger.info(
                                f"📊 Backup needed this hour. Next opportunities: {target_minute:02d}min (preferred) or {50}min (catchup in {minutes_until_catchup}min)"
                            )
                        else:
                            next_hour = (current_hour + 1) % 24
                            logger.info(
                                f"📊 Backup scheduler: Hour {current_hour} complete. Next backup: {next_hour:02d}:{target_minute:02d}"
                            )
                ...
                if minutes_since_check >= self.backup_schedule["check_interval"]:
                    logger.info(
                        f"🔍 {minutes_since_check:.1f} minutes since last check (threshold: {self.backup_schedule['check_interval']}) - running health check..."
                    )
                    check_success = await asyncio.wait_for(
                        self._trigger_check(), timeout=300
                    )  # 5 minute timeout
                    last_check = now
                    if check_success:
                ...
                logger.error(
                    "⚠️ Backup scheduler operation timed out - continuing with next cycle"
                )
                await asyncio.sleep(60)
            except Exception as e:
    ...
    async def _health_monitor(self):
        ...
        print(
            f"💚 CHECKING EVERY {self.backup_schedule['health_check_interval']} SECONDS 💚"
        )
        logger.info("💚 HEALTH MONITOR LOOP ACTIVE 💚")
        logger.info(
            f"💚 HEALTH CHECK INTERVAL: {self.backup_schedule['health_check_interval']} seconds"
        )
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if not status["healthy"]:
                    logger.warning(
                        f"❌ Backup system health check failed: {status.get('error', 'Unknown error')}"
                    )
                    ...
                    if self._health_check_counter % 10 == 0:  # Every 10th check
                        next_check_time = datetime.now() + timedelta(
                            seconds=self.backup_schedule["health_check_interval"]
                        )
                        logger.info(
                            f"💚 Health monitor: System healthy (check #{self._health_check_counter}), next check at {next_check_time.strftime('%H:%M:%S')}"
                        )
    ...
    async def _replica_sync_scheduler(self):
        ...
        logger.info(
            "🔄 Replica sync scheduler starting - will coordinate with primary backup schedule"
        )
        ...
        logger.info(
            f"SYNC SCHEDULER START TIME: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
            ...
            try:
                ...
                if self.test_mode:
                    ...
                    if hours_since_sync >= self.replica_schedule["sync_interval"]:
                        ...
                        print("📥" * 50)
                        logger.info(
                            f"📥 TEST MODE SYNC TRIGGER: {hours_since_sync:.1f} hours since last sync (threshold: {self.replica_schedule['sync_interval']}) - triggering sync... 📥"
                        )
                        if await self._trigger_replica_sync():
                            last_sync = now
                            next_sync_time = now + timedelta(
                                hours=self.replica_schedule["sync_interval"]
                            )
                            print("✅ REPLICA SYNC COMPLETED SUCCESSFULLY ✅")
                            logger.info(
                                f"✅ Replica sync completed, next sync: {next_sync_time.strftime('%H:%M:%S')}"
                            )
                    else:
                        # Log status periodically for visibility (every 10 minutes in test mode)
                        if int(now.minute) % 10 == 0 and now.second < 10:
                            time_until_next = (
                                self.replica_schedule["sync_interval"]
                                - hours_since_sync
                            )
                            print(
                                f"📊 TEST MODE SYNC STATUS: Next replica sync in {time_until_next:.1f} hours (last: {last_sync.strftime('%H:%M:%S')})"
                            )
                            logger.info(
                                f"📊 Test mode replica scheduler: Next sync in {time_until_next:.1f} hours (last: {last_sync.strftime('%H:%M:%S')})"
                            )
                else:
                    ...
                    if (
                        current_minute >= target_minute
                        and current_minute <= target_minute + 2
                        and now.second < 30
                    ):  # Only trigger in first 30 seconds to avoid double triggers
                        # Check if we haven't already done a sync this hour
                        last_sync_hour = (
                            last_sync.hour if last_sync.date() == now.date() else -1
                        )

                        if current_hour != last_sync_hour:
                            backup_minute = 24  # Primary backup minute
                            buffer_minutes = self.replica_schedule[
                                "backup_buffer_minutes"
                            ]
                            print("\n" + "📥" * 50)
                            print("📥 REPLICA SYNC TRIGGERED (SCHEDULED) 📥")
                            print(
                                f"📥 HOURLY SYNC AT {current_hour:02d}:{target_minute:02d} 📥"
                            )
                            print(
                                f"📥 ({buffer_minutes}min buffer after {backup_minute:02d}min backup) 📥"
                            )
                            print("📥" * 50)
                            logger.info(
                                f"📥 SCHEDULED SYNC TRIGGER: Hourly sync at {current_hour:02d}:{target_minute:02d} ({buffer_minutes}min buffer after primary backup) - triggering sync... 📥"
                            )
                            if await self._trigger_replica_sync():
                                last_sync = now
                                next_hour = (current_hour + 1) % 24
                                print("✅ REPLICA SYNC COMPLETED SUCCESSFULLY ✅")
                                logger.info(
                                    f"✅ Replica sync completed, next sync: {next_hour:02d}:{target_minute:02d}"
                                )
                    else:
                        ...
                        if int(now.minute) % 10 == 0 and now.second < 10:
                            next_hour = (
                                ...
                                else (current_hour + 1) % 24
                            )
                            print(
                                f"📊 REPLICA STATUS: Next sync at {next_hour:02d}:{target_minute:02d} (last: {last_sync.strftime('%H:%M:%S')})"
                            )
                            logger.info(
                                f"📊 Replica scheduler active: Next sync at {next_hour:02d}:{target_minute:02d} (last: {last_sync.strftime('%H:%M:%S')})"
                            )
    ...
    async def _trigger_replica_sync(self) -> bool:
        ...
        try:
            ...
            logger.info("🔍 Checking available backups from primary...")
            ...
            try:
                backup_info = json.loads(stdout.decode())
                logger.info("📊 Backup info retrieved successfully")
                ...
                stanza_info = (
                    backup_info[0] if isinstance(backup_info, list) else backup_info
                )
                if "backup" not in stanza_info or len(stanza_info["backup"]) == 0:
                ...
                backup_timestamp = latest_backup.get("timestamp", {}).get(
                    "stop", "unknown"
                )

                logger.info(
                    f"📦 Latest backup found: {backup_type} backup from {backup_timestamp}"
                )
                ...
                logger.info(
                    "🎯 REPLICA STRATEGY: Complete database overwrite with primary data"
                )
                ...
                logger.info(
                    "🔄 Initiating complete database restore from primary backup..."
                )
                restore_success = await self.restore_from_backup()
                ...
                if restore_success:
                    ...
                    logger.info(
                        "🎉 REPLICA SYNC COMPLETED: Database completely replaced with primary data"
                    )
                    logger.info(
                        f"⏱️ Total sync time: {duration.total_seconds():.1f} seconds"
                    )
                    logger.info(
                        f"📊 Restored from: {backup_type} backup (timestamp: {backup_timestamp})"
                    )
    ...
    async def _trigger_backup_with_timeout(
        self, backup_type: str, timeout_minutes: int = 20
    ) -> bool:
        """Trigger a backup with timeout protection to prevent blocking the scheduler."""
        try:
            logger.info(
                f"🚀 Starting {backup_type.upper()} backup with {timeout_minutes}min timeout..."
            )
            timeout_seconds = timeout_minutes * 60

            # Use asyncio.wait_for to add timeout protection
            backup_success = await asyncio.wait_for(
                self._trigger_backup(backup_type), timeout=timeout_seconds
            )
            ...
            if backup_success:
                ...
                logger.warning(
                    f"❌ {backup_type.upper()} backup failed (completed but with errors)"
                )
            ...
            logger.error(
                f"⏰ {backup_type.upper()} backup TIMED OUT after {timeout_minutes} minutes"
            )
            logger.error(
                "🚨 Backup process may still be running in background - scheduler continues"
            )
            return False
        except Exception as e:
    ...
    async def _trigger_backup(self, backup_type: str) -> bool:
        ...
        try:
            ...
            if process.returncode == 0:
                logger.info(
                    f"✅ {backup_type.upper()} backup process completed successfully"
                )
                logger.info(
                    f"⏱️ Backup duration: {backup_duration.total_seconds():.1f} seconds"
                )
                ...
                verification_duration = (
                    upload_verification_end - upload_verification_start
                )
                logger.info(
                    f"⏱️ Upload verification took: {verification_duration.total_seconds():.1f} seconds"
                )
                ...
                if verification_success:
                    ...
                    logger.info(
                        f"🎉 {backup_type.upper()} backup FULLY COMPLETED with R2 upload verification"
                    )
                    logger.info(
                        f"⏱️ Total backup + verification time: {total_duration.total_seconds():.1f} seconds"
                    )
                    return True
                else:
                    logger.warning(
                        f"⚠️ {backup_type.upper()} backup completed but R2 upload verification failed"
                    )
                    return False

            else:
                logger.error(
                    f"❌ {backup_type.upper()} backup FAILED after {backup_duration.total_seconds():.1f} seconds"
                )
                logger.error(f"Error output: {stderr.decode()}")
                if stdout:
                    logger.debug(f"Backup stdout: {stdout.decode()}")
    ...
    async def _verify_r2_upload(
        self, backup_type: str, backup_completion_time: datetime
    ) -> bool:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.error(
                    f"❌ Failed to get backup info for R2 verification: {stderr.decode()}"
                )
                return False
            ...
            try:
                ...
                if backup_info and len(backup_info) > 0:
                    ...
                    if "backup" in stanza_info and len(stanza_info["backup"]) > 0:
                        ...
                        backup_timestamp = recent_backup.get("timestamp", {}).get(
                            "stop", "unknown"
                        )
                        backup_size = recent_backup.get("info", {}).get("size", 0)
                        backup_size_mb = (
                            backup_size / (1024 * 1024) if backup_size else 0
                        )
                        backup_repo_size = (
                            recent_backup.get("info", {})
                            .get("repository", {})
                            .get("size", 0)
                        )
                        backup_repo_size_mb = (
                            backup_repo_size / (1024 * 1024) if backup_repo_size else 0
                        )
                        compression_ratio = (
                            (1 - backup_repo_size / backup_size) * 100
                            if backup_size > 0
                            else 0
                        )

                        logger.info("📊 Latest backup in R2:")
                        logger.info(f"   📋 Label: {backup_label}")
                        logger.info(f"   📅 Timestamp: {backup_timestamp}")
                        logger.info(f"   📦 Original size: {backup_size_mb:.1f} MB")
                        logger.info(
                            f"   🗜️ Compressed size: {backup_repo_size_mb:.1f} MB"
                        )
                        logger.info(
                        ...
                        try:
                            ...
                            if backup_timestamp != "unknown":
                                ...
                                try:
                                    ...
                                    if str(backup_timestamp).isdigit():
                                        ...
                                        logger.debug(
                                            f"📅 Parsed Unix timestamp {backup_timestamp} as {backup_time}"
                                        )
                                    else:
                                        ...
                                        for fmt in formats_to_try:
                                            try:
                                                ...
                                                logger.debug(
                                                    f"📅 Parsed timestamp {backup_timestamp} using format {fmt}"
                                                )
                                                break
                                            except ValueError:
                                                continue
                                    ...
                                    logger.debug(
                                        f"Error parsing timestamp {backup_timestamp}: {e}"
                                    )
                                ...
                                if backup_time:
                                    ...
                                    if backup_completion_time.tzinfo is None:
                                        backup_completion_time = (
                                            backup_completion_time.replace(
                                                tzinfo=timezone.utc
                                            )
                                        )
                                    ...
                                    time_diff = abs(
                                        (
                                            backup_completion_time - backup_time
                                        ).total_seconds()
                                    )
                                    logger.info(
                                        f"⏰ Backup timestamp: {backup_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
                                    )
                                    logger.info(
                                        f"⏰ Completion time: {backup_completion_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
                                    )
                                    logger.info(
                                    ...
                                    if time_diff <= 600:  # Within 10 minutes
                                        logger.info(
                                            f"✅ R2 upload VERIFIED - Recent backup found (time diff: {time_diff:.1f}s)"
                                        )
                                        logger.info(
                                            f"🌥️ Backup successfully stored in R2 bucket: {self.config['r2_bucket']}"
                                        )
                                        return True
                                    else:
                                        logger.warning(
                                            f"⚠️ Latest backup is older than expected (time diff: {time_diff:.1f}s)"
                                        )
                                        # Still consider it successful if within reasonable range (1 hour)
                                        if time_diff <= 3600:
                                            logger.info(
                                                "✅ R2 upload VERIFIED - Backup found within reasonable timeframe"
                                            )
                                            logger.info(
                                                f"🌥️ Backup successfully stored in R2 bucket: {self.config['r2_bucket']}"
                                            )
                                    ...
                                    logger.warning(
                                        f"⚠️ Could not parse backup timestamp: {backup_timestamp}"
                                    )
                                    # If we can't parse timestamp but backup exists, assume success
                                    logger.info(
                                        "✅ R2 upload assumed VERIFIED - Backup exists in repository"
                                    )
                                    logger.info(
                                        f"🌥️ Backup successfully stored in R2 bucket: {self.config['r2_bucket']}"
                                    )
                            ...
                            logger.info(
                                "✅ R2 upload VERIFIED - Backup exists in repository"
                            )
                            logger.info(
                                f"🌥️ Backup successfully stored in R2 bucket: {self.config['r2_bucket']}"
                            )
    ...
    async def _trigger_check(self) -> bool:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if (
                    process.returncode == 82
                    and "was not archived before the" in error_msg
                    and "timeout" in error_msg
                ):
                    logger.warning(
                        "⚠️ WAL archiving timeout detected (this is often temporary)"
                    )
                    ...
                    if await self._check_recent_archive_activity():
                        logger.info(
                            "✅ Recent WAL archive activity detected - treating timeout as temporary issue"
                        )
                        return True  # Treat as success since archiving is generally working
                    else:
                        logger.warning(
                            "❌ No recent WAL archive activity - this may indicate a real problem"
                        )
                        return False

                logger.warning(
                    f"❌ pgBackRest check failed with return code {process.returncode}"
                )
                if stderr:
                ...
                if (
                    ...
                    and "backup and archive info files exist but do not match the database"
                    in error_msg
                ):
                    logger.warning(
                        "🔄 Detected stanza mismatch after replica sync - attempting repair..."
                    )
    ...
    async def _check_recent_archive_activity(self) -> bool:
        ...
        try:
            ...
            log_paths = ["/var/lib/postgresql/14/main/log", "/var/log/postgresql"]
                    ...
                    for log_file in sorted(
                        ...
                        try:
                            with open(log_file) as f:
                                ...
                                for line in lines:
                                    if (
                                        "archive-push command end: completed successfully"
                                        in line
                                        or "pushed WAL file" in line
                                    ):
                                        ...
                                        if self._is_log_line_recent(line, minutes=30):
                                            ...
                                            logger.debug(
                                                f"Found recent archive activity: {line.strip()}"
                                            )
                                            break
    ...
    async def _handle_stanza_mismatch_after_sync(self) -> bool:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.warning(
                    f"Stanza delete warning (expected): {stderr.decode() if stderr else 'unknown'}"
                )
            ...
            if process.returncode != 0:
                logger.error(
                    f"Failed to create fresh stanza: {stderr.decode() if stderr else 'unknown'}"
                )
                return False
            ...
            if process.returncode == 0:
                ...
                logger.error(
                    f"Stanza repair verification failed: {stderr.decode() if stderr else 'unknown'}"
                )
                return False
    ...
    async def restore_from_backup(self, target_time: Optional[str] = None) -> bool:
        ...
        logger.info(
            f"   - Running delta restore command: {' '.join(delta_restore_cmd)}"
        )
        ...
        try:
            ...
            if (
                ...
                and stderr
                and (
                    "FileMissingError" in stderr
                    or "unable to open missing file" in stderr
                )
            ):
                logger.error(
                    "❌ Restore reported success but detected missing files in backup - treating as failure"
                )
                success = False
        ...
        if success:
            has_permission_warnings = stderr and (
                "unknown user 'root'" in stderr or "unknown group 'root'" in stderr
            )
            if has_permission_warnings:
                logger.warning(
                    "⚠️ Delta restore completed successfully with permission warnings (normal when running as root)"
                )
        ...
        if not success:
            ...
            has_corruption = (
                "FileMissingError" in stderr or "unable to open missing file" in stderr
            )
            has_permission_warnings = (
                "unknown user 'root'" in stderr or "unknown group 'root'" in stderr
            )

            if has_corruption:
                logger.error(
                    "❌ Delta restore failed due to missing files in backup - backup appears corrupted."
                )
                logger.warning(
                    "⚠️ This indicates the backup in R2 storage is incomplete or corrupted."
                )
                ...
                if info_success and info_stdout:
                    ...
                    if len(backup_lines) > 1:
                        ...
                        backup_match = re.search(
                            r"(\d{8}-\d{6}F(?:_\d{8}-\d{6}[DI])?)", backup_lines[1]
                        )
                        if backup_match:
                            older_backup = backup_match.group(1)
                            logger.info(
                                f"🎯 Attempting restore from older backup: {older_backup}"
                            )
                            ...
                            (
                                ...
                                older_restore_cmd,
                                timeout=7200,
                                description="pgBackRest older backup restore",
                            )

                            # Convert return code to boolean (0 = success, non-zero = failure)
                            older_success = older_return_code == 0

                            if older_success and not (
                                "FileMissingError" in older_stderr
                                or "unable to open missing file" in older_stderr
                            ):
                                logger.warning(
                                    "⚠️ Successfully restored from older backup due to corruption in latest backup"
                                )
                                logger.warning(
                                    "💡 Consider investigating backup integrity on primary node"
                                )
                                # Skip the full restore fallback since we succeeded with older backup
                                success = True
                            else:
                                logger.error(
                                    f"❌ Older backup restore also failed: {older_stderr}"
                                )

                if not success:
                    logger.error(
                        "❌ All available backups appear corrupted. Attempting full clean restore as last resort..."
                    )
                ...
                logger.warning(
                    "⚠️ Delta restore completed with permission warnings (this is normal when running as root)"
                )
                logger.info("🔧 Will fix ownership after restore completes")
                success = True
            else:
                logger.error(
                    "❌ Delta restore failed. This may indicate a corrupted local directory."
                )
        ...
        if not success:
            ...
            try:
                ...
                logger.info(
                    f"   - Wiping data directory for clean restore: {data_path}"
                )
                ...
                logger.info(
                    f"   - Successfully wiped and recreated data directory: {data_path}"
                )
            except Exception as e:
                logger.critical(
                    f"💥 Failed to wipe directory for full restore fallback: {e}. Manual intervention required."
                )
                return False
            ...
            logger.info(
                f"   - Running full restore command: {' '.join(full_restore_cmd)}"
            )
            ...
            if not full_success:
                logger.critical(
                    "💥 FALLBACK RESTORE FAILED. The database is in an inconsistent state and requires manual intervention."
                )
                if full_stderr:
                    try:
                        ...
                        if (
                            "Permission denied" in stderr_str
                            and "postmaster.pid" in stderr_str
                        ):
                            logger.error(
                                "🔍 DIAGNOSIS: Permission denied on postmaster.pid - this indicates ownership issues"
                            )
                            logger.error(
                                "💡 SOLUTION: The PostgreSQL data directory needs to be owned by 'postgres' user"
                            )
                            logger.error(
                                "🔧 MANUAL FIX: sudo chown -R postgres:postgres /var/lib/postgresql/"
                            )

                    except:
                        logger.error(f"   - Stderr: {full_stderr}")
                return False
            else:
        ...
        if not success:
            logger.error(
                "❌ All restore attempts failed. Database is in an inconsistent state."
            )
            return False
        ...
        if not await self._reapply_local_configuration_post_restore():
            logger.error(
                "❌ Failed to re-apply local configurations. The database may not be accessible."
            )
            return False

        logger.info(
            "📋 Step 6: Restarting PostgreSQL to load new configuration and initiate WAL replay..."
        )
        service_name = self.system_info.get("postgresql_service", "postgresql")
        if not await self._restart_postgresql_service(service_name):
            logger.error(
                "❌ Failed to restart PostgreSQL after re-applying local configuration."
            )
            if not await self._emergency_postgresql_restart():
                logger.critical(
                    "💥 Emergency restart also failed. The database is in an inconsistent state."
                )
                return False
        ...
        if not await self._test_database_connection():
            ...
            try:
                ...
                logger.info(
                    "🔄 Attempting final PostgreSQL restart after permission fix..."
                )
                if await self._restart_postgresql_service("postgresql"):
                    logger.info(
                        "⏳ Waiting 10 seconds for PostgreSQL to fully initialize..."
                    )
                    ...
                    if await self._test_database_connection():
                        logger.info(
                            "🎉 Database connectivity restored after permission fix!"
                        )
                        return True
                    else:
                        logger.error(
                            "❌ Database still not accessible after permission fix"
                        )
                else:
            ...
            logger.critical(
                "   2. Verify data directory permissions: ls -la /var/lib/postgresql/"
            )
            logger.critical(
                "   3. Check if PostgreSQL is running: systemctl status postgresql"
            )
            logger.critical(
                "   4. Try manual restart: sudo systemctl restart postgresql"
            )
        ...
        if not await self._promote_database_to_standalone():
            logger.error(
                "❌ Failed to promote database to standalone mode - database will remain read-only"
            )
            logger.warning(
                "💡 The database is still in recovery mode and cannot accept write operations"
            )
            logger.warning(
                "💡 You may need to manually promote it: SELECT pg_promote();"
            )
            return False
    ...
    def print_current_status(self):
        ...
        print(
            f"🔄 BACKUP TASK RUNNING: {self.backup_task is not None and not self.backup_task.done()}"
        )
        print(
            f"💚 HEALTH TASK RUNNING: {self.health_check_task is not None and not self.health_check_task.done()}"
        )
        ...
        if self.is_primary and not self.test_mode:
            ...
            print(
                f"   • Preferred Window: Every hour at :{target_minute:02d}-{target_minute + 5:02d} minutes"
            )
            print(
                "   • Catch-up Window: Every hour at :50-59 minutes (if backup missed)"
            )
            print("   • Emergency Catch-up: Any time if >75 minutes overdue")
            print("   • Max Attempts: 3 per hour")
            print("   • Timeout Protection: 15 minutes per backup attempt")
            print("   • Multiple Opportunities: No more missed backups due to timing!")
    ...
    async def get_resilient_backup_stats(self) -> Dict:
        ...
        try:
                    ...
                    if backup_info and len(backup_info) > 0:
                        ...
                        if "backup" in stanza_info and len(stanza_info["backup"]) > 0:
                            ...
                            if backup_timestamp and str(backup_timestamp).isdigit():
                                ...
                                stats["last_backup_time"] = backup_time.strftime(
                                    "%Y-%m-%d %H:%M:%S UTC"
                                )
                                ...
                                stats["backup_within_schedule"] = (
                                    hours_since <= 1.5
                                )  # Within 90 minutes is good
            ...
            if self.is_primary and not self.test_mode:
                ...
                if current_minute < target_minute:
                    next_preferred = current_time.replace(
                        minute=target_minute, second=0, microsecond=0
                    )
                else:
                    next_hour = (current_hour + 1) % 24
                    next_preferred = current_time.replace(
                        hour=next_hour, minute=target_minute, second=0, microsecond=0
                    )
                ...
                if current_minute < 50:
                    next_catchup = current_time.replace(
                        minute=50, second=0, microsecond=0
                    )
                else:
                    next_hour = (current_hour + 1) % 24
                    next_catchup = current_time.replace(
                        hour=next_hour, minute=50, second=0, microsecond=0
                    )
                    if next_hour == 0:  # Next day
    ...
    async def _detect_stanza_conflicts(self):
        ...
        try:
            expected_stanza = self.config["stanza_name"]
            logger.info(
                f"🔍 Checking for stanza conflicts. Expected stanza: [{expected_stanza}]"
            )
            ...
            if process.returncode == 0 and stdout:
                ...
                if existing_stanzas:
                    ...
                    if expected_stanza not in existing_stanzas:
                        logger.warning("🔧 Stanza mismatch detected in pgBackRest!")
                        logger.warning(f"   Expected: [{expected_stanza}]")
                        logger.warning(f"   Found: {existing_stanzas}")

                        # Automatically clean up old stanzas and recreate with correct name
                        await self._fix_stanza_mismatch(
                            existing_stanzas, expected_stanza
                        )
                    else:
                        logger.info(
                            f"✅ Correct stanza already exists: [{expected_stanza}]"
                        )
                else:
                    logger.info(
                        f"📋 No existing stanzas found. Will create: [{expected_stanza}]"
                    )
            else:
                logger.info(
                    f"📋 pgBackRest info command failed (expected for new setup). Will create: [{expected_stanza}]"
                )

        except Exception as e:
            logger.debug(f"Stanza conflict detection failed: {e}")

    async def _fix_stanza_mismatch(
        self, existing_stanzas: List[str], expected_stanza: str
    ):
        ...
        try:
            ...
            for old_stanza in existing_stanzas:
                if old_stanza != expected_stanza:
                    ...
                    if process.returncode == 0:
                        logger.info(
                            f"✅ Successfully removed old stanza: [{old_stanza}]"
                        )
                    else:
                        logger.warning(
                            f"⚠️ Warning during stanza deletion (may be expected): {stderr.decode() if stderr else 'unknown'}"
                        )
            ...
            if process.returncode == 0:
                logger.info(
                    f"✅ Successfully created correct stanza: [{expected_stanza}]"
                )
            else:
                # Check if stanza already exists
                if "already exists" in stderr.decode().lower():
                    ...
                    logger.error(
                        f"❌ Failed to create stanza: {stderr.decode() if stderr else 'unknown'}"
                    )
                    return False
            ...
            if process.returncode == 0:
                ...
                logger.warning(
                    f"⚠️ Stanza fix verification had issues: {stderr.decode() if stderr else 'unknown'}"
                )
                logger.info(
                    "🔄 Will proceed with setup - issues may be resolved during initialization"
                )
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if current_cmd and current_cmd != expected_cmd:
                    logger.info("🔍 Archive command mismatch detected:")
    ...
    async def _fix_failing_archiver(self):
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                *ps_cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await asyncio.wait_for(process.communicate(), timeout=10)
            logger.debug(
                f"🔍 ps command completed with return code: {process.returncode}"
            )
            ...
            if process.returncode == 0:
                ...
                logger.info(
                    f"🔍 Found {len(postgres_lines)} PostgreSQL-related processes"
                )

                archiver_failed_lines = [
                    line for line in postgres_lines if "archiver failed" in line
                ]
                if archiver_failed_lines:
                    logger.warning(
                        "⚠️ Detected failing PostgreSQL archiver - this can cause hangs"
                    )
                    for line in archiver_failed_lines:
                        logger.warning(f"⚠️ Failing archiver process: {line.strip()}")

                    logger.info(
                        "🔧 Temporarily disabling archive_mode to fix archiver..."
                    )

                    # First, try to connect and see if PostgreSQL responds
                    logger.info(
                        "🔍 Testing PostgreSQL connectivity before disabling archiver..."
                    )
                    ...
                    try:
                        ...
                        logger.debug(
                            f"🔍 Test command completed with return code: {test_process.returncode}"
                        )
                        ...
                        if test_process.returncode == 0:
                            ...
                            logger.warning(
                                f"⚠️ PostgreSQL test query failed: {test_stderr_text}"
                            )
                    except asyncio.TimeoutError:
                        logger.warning(
                            "⚠️ PostgreSQL test query timed out - confirming archiver is causing hangs"
                        )
                    ...
                    logger.debug(
                        f"🔍 Disable command completed with return code: {process.returncode}"
                    )
                    ...
                    if process.returncode == 0:
                        ...
                        logger.info(
                            "🔧 Re-enabling archiving with correct configuration..."
                        )
                        if await self._ensure_correct_archive_command():
                            logger.info(
                                "✅ Fixed failing archiver and restored proper archiving configuration"
                            )
                        else:
                            logger.warning(
                                "⚠️ Fixed failing archiver but could not restore archiving configuration"
                            )
                    else:
                        logger.warning(
                            f"⚠️ Failed to disable archive_mode: {stderr.decode()}"
                        )
                else:
                    ...
                    test_cmd = ["sudo", "-u", "postgres", "psql", "-c", "SELECT 1;"]
                    logger.debug(
                        f"🔍 Running responsiveness test: {' '.join(test_cmd)}"
                    )
                    ...
                    try:
                        ...
                        logger.debug(
                            f"🔍 Responsiveness test completed with return code: {test_process.returncode}"
                        )
                        ...
                        if test_process.returncode == 0:
                            ...
                            logger.warning(
                                f"⚠️ PostgreSQL responsiveness test failed: {test_stderr.decode()}"
                            )
                    except asyncio.TimeoutError:
                        logger.warning("⚠️ PostgreSQL responsiveness test timed out")
            else:
                logger.warning(
                    f"⚠️ Could not check for failing archiver: {stderr.decode()}"
                )
    ...
    async def _detect_and_fix_postgresql_corruption(self) -> bool:
        ...
        try:
            ...
            if not data_dir.exists():
                logger.warning(
                    f"⚠️ PostgreSQL data directory does not exist: {data_dir}"
                )
            ...
            if missing_files:
                logger.warning(
                    f"⚠️ PostgreSQL data directory is corrupted - missing files: {missing_files}"
                )
    ...
    async def _initialize_fresh_postgresql_cluster(self) -> bool:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                logger.error(
                    f"❌ Failed to initialize PostgreSQL cluster: {stderr.decode()}"
                )
                return False
    ...
    async def _setup_fresh_cluster_auth(self):
        ...
        try:
            ...
            if hba_conf_path.exists():
                ...
                logger.info(
                    f"✅ Updated pg_hba.conf for initial setup: {hba_conf_path}"
                )
    ...
    async def _update_hba_to_md5(self):
        ...
        try:
            ...
            config_locations = [
                ...
                ),  # Debian/Ubuntu config directory (preferred)
                Path("/etc/postgresql/15/main"),
                Path("/etc/postgresql/16/main"),
            ...
            if hba_conf_path and hba_conf_path.exists():
                ...
                logger.info(
                    f"✅ Updated pg_hba.conf to use md5 authentication: {hba_conf_path}"
                )
            else:
    ...
    async def _run_postgres_command_with_logging(
        self, cmd: list, timeout: int = 15, description: str = "PostgreSQL command"
    ) -> tuple:
        ...
        try:
            ...
            logger.debug(
                f"🔍 {description} completed in {elapsed:.2f} seconds with return code: {process.returncode}"
            )
    ...
    async def _setup_early_authentication(self, postgres_user: str):
        ...
        if self.config["postgres_password"]:
            logger.info(
                f"🔑 Setting up early authentication for user '{self.current_user}'..."
            )
            await self._create_pgpass_file(self.current_user)
        else:
            logger.info(
                "🔑 No postgres password set, assuming TRUST authentication is sufficient for setup."
            )
    ...
    async def _create_pgpass_file(self, for_user: str):
        ...
        try:
            ...
            if for_user != self.current_user:
                logger.warning(
                    f"Cannot create .pgpass for another user '{for_user}'. Skipping."
                )
                return
            ...
            logger.info("✅ Successfully created and secured .pgpass file.")

        except Exception as e:
            logger.error(
                f"Failed to create .pgpass file for user '{for_user}': {e}",
                exc_info=True,
            )
    ...
    async def _setup_temporary_trust_auth(self):
        ...
        """
        if (
            not self.config["hba_file_path"]
            or not self.config["hba_file_path"].exists()
        ):
            logger.error(
                "HBA file path not configured or file does not exist. Cannot set up trust auth."
            )
            return
        ...
        try:
            async with aiofiles.open(self.config["hba_file_path"]) as f:
            ...
            if (
                f"local   all             {self.config['postgres_user']}                                     trust"
                in original_hba_content
            ):
    ...
    async def _emergency_postgresql_restart(self) -> bool:
        ...
        try:
            ...
            for service_name in service_candidates:
                try:
                    logger.info(
                        f"Attempting emergency start of PostgreSQL service: {service_name}"
                    )
                    start_cmd = ["sudo", "systemctl", "start", service_name]
                    return_code, _, stderr = await self._run_command_async(
                        start_cmd, f"Emergency start PostgreSQL ({service_name})"
                    )

                    if return_code == 0:
                        logger.info(
                            "Waiting for PostgreSQL to stabilize after emergency restart..."
                        )
                        await asyncio.sleep(3)
                        ...
                        if await self._verify_service_status(service_name):
                            logger.info(
                                f"✅ Emergency restart successful using service: {service_name}"
                            )
                            # Update the detected service name for future use
                            ...
                            logger.warning(
                                f"Emergency start of {service_name} reported success but service is not active"
                            )
                    else:
                        logger.debug(
                            f"Failed to emergency start PostgreSQL service {service_name}: {stderr}"
                        )

                except Exception as e:
                    logger.debug(
                        f"Exception during emergency start of PostgreSQL service {service_name}: {e}"
                    )
                    continue

            logger.error(
                "❌ Failed to emergency start PostgreSQL service with any known service name"
            )
            return False

        except Exception as e:
            logger.error(
                f"Exception during emergency PostgreSQL restart: {e}", exc_info=True
            )
            return False
    ...
    async def _stop_postgresql_service(self):
        ...
        try:
            ...
            return_code, _, stderr = await self._run_command_async(
                stop_cmd, f"Stop PostgreSQL service ({running_service})"
            )
            ...
            if return_code == 0:
                ...
                # Verify it's stopped
                if not await self._verify_service_status(
                    running_service, should_be_active=False
                ):
                    logger.info("✅ PostgreSQL service stopped successfully.")
                else:
                    logger.warning(
                        f"Service {running_service} may not have stopped completely."
                    )
            else:
                logger.warning(
                    f"Failed to stop PostgreSQL service {running_service}: {stderr}"
                )
        ...
        try:
            ...
            if shutil.which("systemctl"):
                for service_name in service_candidates:
                    try:
                        ...
                        return_code, _, stderr = await self._run_command_async(
                            reload_cmd,
                            f"Reload PostgreSQL config (systemd: {service_name})",
                        )
                        if return_code == 0:
                            logger.info(
                                f"✅ Successfully reloaded config using systemctl with service: {service_name}"
                            )
                            reload_success = True
                            break
                        else:
                            logger.debug(
                                f"Failed to reload via systemctl with {service_name}: {stderr}"
                            )
                    except Exception as e:
                        logger.debug(
                            f"Exception trying systemctl reload with {service_name}: {e}"
                        )
                        continue
                ...
                if pg_version and pgdata_path:
                    ...
                    if pg_ctl_path.exists():
                        ...
                        (
                            ...
                            reload_cmd_pgctl, "Reload PostgreSQL config (pg_ctl)"
                        )
                        if return_code_pgctl == 0:
                            logger.info(
                                "✅ Successfully reloaded config using pg_ctl with explicit PGDATA."
                            )
                            reload_success = True
                        else:
                            logger.warning(
                                f"Failed to reload config with pg_ctl: {stderr_pgctl}"
                            )
                    else:
                        logger.warning(f"Could not find pg_ctl at {pg_ctl_path}")
                else:
                    logger.warning(
                        f"Missing PostgreSQL version ({pg_version}) or PGDATA path ({pgdata_path}) for pg_ctl reload"
                    )
            ...
            if not reload_success:
                ...
                try:
                    ...
                    return_code_sql, _, stderr_sql = await self._run_command_async(
                        sql_reload_cmd, "Reload PostgreSQL config (SQL)"
                    )
                    if return_code_sql == 0:
                        logger.info(
                            "✅ Successfully reloaded config using SQL pg_reload_conf()."
                        )
                        reload_success = True
                    else:
            ...
            logger.error(
                f"Exception while reloading PostgreSQL configuration: {e}",
                exc_info=True,
            )
    ...
    async def _ensure_postgresql_running(self):
        ...
        for service_name in service_candidates:
            ...
            if is_running:
                logger.info(
                    f"✅ PostgreSQL service is already running via: {service_name}"
                )
                return
        ...
        for service_name in service_candidates:
            try:
                ...
                start_cmd = ["sudo", "systemctl", "start", service_name]
                return_code, _, stderr = await self._run_command_async(
                    start_cmd, f"Start PostgreSQL service ({service_name})"
                )
                ...
                if return_code == 0:
                    ...
                    if await self._verify_service_status(service_name):
                        logger.info(
                            f"✅ Successfully started PostgreSQL service: {service_name}"
                        )
                        return
                    else:
                        logger.warning(
                            f"Service {service_name} reported start but is not active."
                        )

                else:
                    logger.debug(
                        f"Failed to start PostgreSQL service {service_name}: {stderr}"
                    )

            except Exception as e:
                logger.debug(
                    f"Exception trying to start PostgreSQL service {service_name}: {e}"
                )
                continue

        logger.error(
            "❌ Failed to start PostgreSQL service with any known service name"
        )
        logger.error(
            "💡 You may need to manually start PostgreSQL or check the service configuration"
        )
    ...
    async def _ensure_correct_archive_command(self) -> bool:
        ...
        try:
            ...
            return_code_mode, current_mode, stderr_mode = await self._run_command_async(
                get_mode_cmd, "Get current archive_mode"
            )

            if return_code_mode != 0:
                logger.error(f"Failed to get current archive_mode: {stderr_mode}")
                logger.warning(
                    "Could not verify current archive mode, attempting to set it anyway."
                )
            ...
            return_code, current_command, stderr = await self._run_command_async(
                get_cmd, "Get current archive_command"
            )

            if return_code != 0:
                logger.error(f"Failed to get current archive_command: {stderr}")
                logger.warning(
                    "Could not verify current archive command, attempting to set it anyway."
                )
            ...
            if not archive_mode_correct:
                ...
                (
                    return_code_mode_set,
                    _,
                    stderr_mode_set,
                ) = await self._run_command_async(set_mode_cmd, "Set archive_mode")
            ...
            if not archive_command_correct:
                ...
                return_code_set, _, stderr_set = await self._run_command_async(
                    set_cmd, "Set archive_command"
                )
            ...
            if not archive_mode_correct:
                logger.info(
                    "🔄 archive_mode was changed - performing full PostgreSQL restart..."
                )
                service_name = self.system_info.get("postgresql_service", "postgresql")
                if not await self._restart_postgresql_service(service_name):
                    logger.error(
                        "Failed to restart PostgreSQL after setting archive_mode"
                    )
                    return False
                logger.info(
                    "✅ PostgreSQL restarted successfully for archive_mode change"
                )
            else:
            ...
            logger.error(
                f"An error occurred while ensuring correct archive settings: {e}",
                exc_info=True,
            )
            return False
    ...
    async def _intelligent_stanza_setup(self) -> bool:
        ...
        try:
            logger.info(
                "Ensuring stanza is started to clear any previous stop files..."
            )
            start_cmd = [
                "sudo",
                "-u",
            ...
            logger.warning(
                f"Could not run pgbackrest start command, which may be acceptable: {e}"
            )
        ...
        try:
            ...
            if return_code == 0 and stdout:
                ...
                if info_json and info_json[0]["status"]["code"] == 0:
                    ...
                    logger.warning(
                        f"Stanza 'info' command reported an error: {message}"
                    )
            else:
                logger.warning(
                    f"Stanza check via 'info' command failed. Stderr: {stderr}"
                )
        ...
        if not stanza_is_healthy:
            logger.info(
                "🔄 Stanza is not healthy or does not exist. Attempting to recreate."
            )
        ...
        logger.info(
            "🔧 Ensuring archive_mode and archive_command are properly configured..."
        )
        if not await self._ensure_correct_archive_command():
            logger.error(
                "Failed to configure archive_mode and archive_command. Stanza setup cannot proceed."
            )
            return False
        ...
        logger.info(
            "⚙️ Finalizing setup and verifying backups with pgBackRest initialization..."
        )
        if not await self._initialize_pgbackrest():
            logger.error(
                "Failed to initialize pgBackRest and ensure backups are configured."
            )
            return False
    ...
    async def fix_archive_command(self) -> bool:
        ...
        try:
            logger.info(
                "🔧 Fixing PostgreSQL archive_mode and archive_command for network-aware stanza..."
            )
    ...
    async def prepare_for_network_transition(self, force_clean: bool = False) -> bool:
        ...
        try:
            if force_clean:
                logger.warning(
                    "🧹 FORCE CLEAN: Removing existing stanza for fresh start..."
                )
                logger.warning(
                    f"⚠️ This will delete backup history for stanza: {self.config['stanza_name']}"
                )
                ...
                if process.returncode == 0:
                    logger.info(
                        "✅ Stanza deleted successfully - ready for fresh start"
                    )
                else:
                    logger.warning(
                        f"⚠️ Stanza deletion had issues (might not exist): {stderr.decode()}"
                    )
                    logger.info("Continuing with setup anyway...")

            else:
                logger.info(
                    "🔄 Preparing for network transition without force clean..."
                )
                ...
                if process.returncode == 0:
                    logger.info(
                        "📊 Existing stanza found - will integrate with existing backups"
                    )
                else:
    ...
    async def _validate_system_before_sync(self) -> bool:
        ...
        try:
            logger.info(
                "🔍 Pre-flight validation: Checking system readiness for sync operations..."
            )
            ...
            if not data_path.exists():
                logger.error(
                    f"❌ PostgreSQL data directory does not exist: {data_path}"
                )
                return False

            if not os.access(data_path, os.R_OK):
                logger.error(
                    f"❌ PostgreSQL data directory is not readable: {data_path}"
                )
                return False
            ...
            for service_name in service_candidates:
                try:
                    ...
                    if status_stdout.decode().strip() == "active":
                        logger.info(
                            f"✅ PostgreSQL is running via service: {service_name}"
                        )
            ...
            if not active_service:
                ...
                for service_name in service_candidates:
                    try:
                        ...
                        if status_stdout.decode().strip() == "active":
                            logger.info(
                                f"✅ PostgreSQL started successfully via service: {service_name}"
                            )
                            active_service = service_name
                            break
                    except Exception:
                ...
                if not active_service:
                    logger.warning(
                        "⚠️ PostgreSQL still not running - checking for corruption..."
                    )

                    # Check if data directory is corrupted and attempt auto-repair
                    if await self._detect_and_fix_postgresql_corruption():
                        logger.info(
                            "🔧 Attempted PostgreSQL corruption repair - retrying service start..."
                        )
                        await self._ensure_postgresql_running()
                        ...
                        for service_name in service_candidates:
                            try:
                                ...
                                if status_stdout.decode().strip() == "active":
                                    logger.info(
                                        f"✅ PostgreSQL started after corruption repair via service: {service_name}"
                                    )
                                    active_service = service_name
                                    break
                            except Exception:
                    ...
                    if not active_service:
                        logger.error(
                            "❌ Failed to start PostgreSQL even after corruption repair - sync operations unsafe"
                        )
                        return False
            ...
            try:
                ...
                if test_process.returncode != 0:
                    logger.error(
                        f"❌ Database connectivity test failed: {test_stderr.decode()}"
                    )
                    return False
            ...
            try:
                ...
                if check_process.returncode != 0:
                    logger.warning(
                        f"⚠️ pgBackRest check failed: {check_stderr.decode()}"
                    )
                    logger.warning(
                        "⚠️ Sync operations may fail - consider running setup again"
                    )
            ...
            logger.info(
                "✅ Pre-flight validation passed - system ready for sync operations"
            )
            return True
    ...
    async def _reapply_local_configuration_post_restore(self):
        ...
        try:
            ...
            config_lines = [
                f"{key} = {value}" for key, value in config_settings.items()
            ]
            ...
            if hba_conf_path.exists():
                ...
                with open(hba_conf_path, "r+") as f:
                    ...
                    if hba_entry not in content:
                        ...
                        logger.info(
                            "✅ Re-applied local trust authentication to pg_hba.conf"
                        )
            else:
                logger.warning(
                    f"⚠️ Could not find pg_hba.conf at {hba_conf_path} to re-apply settings."
                )
            ...
            logger.error(
                f"❌ Failed to re-apply local configurations: {e}", exc_info=True
            )
            return False
    ...
    async def _fix_postgresql_ownership(self):
        ...
        try:
            ...
            if process.returncode != 0:
                logger.warning(
                    f"⚠️ chown command had non-zero exit code: {process.returncode}"
                )
                if stderr:
            ...
            if process.returncode != 0:
                logger.warning(
                    f"⚠️ chmod command had non-zero exit code: {process.returncode}"
                )
                if stderr:
            ...
            for config_dir in config_dirs:
                if os.path.exists(config_dir):
                    ...
                    if process.returncode == 0:
                        ...
                        logger.debug(
                            f"⚠️ Could not fix ownership for {config_dir} (may not be critical)"
                        )
    ...
    async def _test_database_connection(self):
        ...
        logger.info(
            f"⏳ Waiting for PostgreSQL socket {socket_path} to be available..."
        )
        for attempt in range(max_wait_time // wait_interval):
            if os.path.exists(socket_path):
                logger.info(
                    f"✅ PostgreSQL socket found after {attempt * wait_interval} seconds"
                )
                break
            await asyncio.sleep(wait_interval)
        else:
            logger.error(
                f"❌ PostgreSQL socket {socket_path} not found after {max_wait_time} seconds"
            )
            ...
            try:
                ...
                if status_stderr:
                    logger.warning(
                        f"PostgreSQL service status stderr:\n{status_stderr.decode()}"
                    )
            except Exception as e:
            ...
            for attempt in range(15):  # 30 seconds max
                if os.path.exists(socket_path):
                    logger.info(
                        f"✅ PostgreSQL socket found after restart in {attempt * 2} seconds"
                    )
                    break
                await asyncio.sleep(2)
            else:
            ...
            if test_process.returncode == 0:
                ...
                try:
                    ...
                    if gaia_process.returncode == 0:
                        ...
                        logger.warning(
                            f"⚠️ {db_name} database test failed: {gaia_stderr.decode()}"
                        )
    ...
    async def _run_command_async(
        ...
        try:
            ...
            if process.returncode != 0:
                logger.debug(
                    f"Command '{description}' failed with code {process.returncode}. Stderr: {stderr_str}"
                )
            else:
            ...
            logger.error(
                f"Failed to execute command '{description}': {e}", exc_info=True
            )
            return -1, "", str(e)

    async def _verify_service_status(
        self, service_name: str, should_be_active: bool = True
    ) -> bool:
    ...
    async def _promote_database_to_standalone(self) -> bool:
        ...
        try:
            ...
            return_code, stdout, stderr = await self._run_command_async(
                check_recovery_cmd, "Check recovery status"
            )
            ...
            logger.info(
                f"💡 Database recovery status: {'IN RECOVERY (standby)' if is_in_recovery else 'NOT IN RECOVERY (standalone)'}"
            )

            if not is_in_recovery:
                logger.info(
                    "✅ Database is already in standalone mode - no promotion needed"
                )
                return True
            ...
            return_code, stdout, stderr = await self._run_command_async(
                remove_signal_cmd, "Remove standby.signal file"
            )
            ...
            return_code, stdout, stderr = await self._run_command_async(
                promote_cmd, "Promote database using pg_promote()"
            )
            ...
            if return_code == 0:
                ...
                return_code, stdout, stderr = await self._run_command_async(
                    verify_cmd, "Verify promotion success"
                )
                ...
                if return_code == 0:
                    ...
                    if not is_still_in_recovery:
                        logger.info(
                            "🎉 Database successfully promoted to standalone mode - now accepting writes!"
                        )
                        return True
                    else:
                        logger.error(
                            "❌ Database is still in recovery mode after promotion attempt"
                        )
                        return False
                else:
...
async def get_auto_sync_manager(test_mode: bool = False) -> Optional[AutoSyncManager]:
    ...
    try:
        ...
        print(f"❌ ERROR: {e} ❌")
        logger.error(
            f"❌ AutoSyncManager factory: Failed to create - {e}", exc_info=True
        )
        return None
