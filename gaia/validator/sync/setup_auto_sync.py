...
def load_env_file():
    ...
    env_paths = [Path(".env"), Path("/root/Gaia/.env"), project_root / ".env"]
...
def validate_environment(is_primary: bool):
    ...
    if missing_vars:
        ...
        logger.error(
            "PGBACKREST_R2_ENDPOINT=https://account-id.r2.cloudflarestorage.com"
        )
    ...
    try:
        ...
        if success:
            ...
            if is_primary:
                ...
                logger.info(
                    "2. Backups are scheduled in the application (no cron needed)"
                )
                logger.info(
                    "3. Check backup status with: python -c 'from gaia.validator.sync.auto_sync_manager import *; import asyncio; asyncio.run(check_status())'"
                )
            else:
                logger.info("\n📋 Next steps for REPLICA node:")
                logger.info("1. To restore from backup, run:")
                logger.info(
                    "   python -c 'from gaia.validator.sync.auto_sync_manager import *; import asyncio; asyncio.run(restore_latest())'"
                )
...
async def check_status():
    ...
    if manager:
        ...
        logger.info(
            f"Backup system status: {'✅ Healthy' if status['healthy'] else '❌ Unhealthy'}"
        )
        if status.get("info"):
...
def main():
    ...
    mode_group.add_argument(
        "--replica", action="store_true", help="Set up as replica node (restore target)"
    )
