...
class BackupManager:
    def __init__(self, test_mode: bool = False):  # Removed storage_manager
        ...
        logger.info(
            f"BackupManager initialized. Stanza: {self.pgbackrest_stanza}, Test_mode: {self.test_mode}. "
    ...
    async def _trigger_pgbackrest_command(
        ...
        if not any(
            arg.startswith("--stanza=") or arg == self.pgbackrest_stanza
            for arg in command_args
        ):
            # Add stanza if it's a common command like 'backup', 'info', 'check', 'archive-push'
            if operation_name not in [
                "Stanza Create"
            ]:  # Stanza Create takes stanza as a positional arg
                ...
                if not is_stanza_command and not any(
                    arg.startswith(f"--stanza={self.pgbackrest_stanza}")
                    for arg in command_args
                ):
                    ...
                    if (
                        command_args[0] != "stanza-create"
                    ):  # stanza-create uses positional stanza
                        command_args = [
        ...
        try:
            ...
            logger.error(
                "pgBackRest command not found. Ensure it's installed and in PATH."
            )
            return False
    ...
    async def trigger_backup(self, backup_type: str = "full") -> bool:
        ...
        if backup_type not in ["full", "diff", "incr"]:
            logger.error(
                f"Invalid backup type: {backup_type}. Must be 'full', 'diff', or 'incr'."
            )
            return False
        ...
        if self.test_mode:
            ...
            logger.info(
                f"[Test Mode] Ad-hoc pgBackRest Backup ({backup_type}) with test options."
            )

        return await self._trigger_pgbackrest_command(
            args, f"Ad-hoc Backup ({backup_type})"
        )
...
async def get_backup_manager(
...
) -> Optional[BackupManager]:  # Removed storage_manager argument
    """
    ...
    try:
        ...
        logger.error(
            "pgBackRest command-line tool not found in PATH. BackupManager will likely fail."
        )
        # return None # Decide if this should be a fatal error for the manager
