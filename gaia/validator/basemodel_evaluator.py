...
class BaseModelEvaluator:
    ...
    def __init__(self, db_manager=None, test_mode: bool = False):
        ...
        self.geo_scoring = (
            GeomagneticScoringMechanism(db_manager=db_manager) if db_manager else None
        )
        self.soil_scoring = (
            SoilScoringMechanism(db_manager=db_manager) if db_manager else None
        )
    ...
    def _soilmodel_inference_sync(self, sentinel_tensor, era5_tensor, elev_ndvi_tensor):
        ...
        with torch.no_grad():
            return self.soil_model(
                sentinel=sentinel_tensor, era5=era5_tensor, elev_ndvi=elev_ndvi_tensor
            )
    ...
    async def initialize_models(self):
        ...
        try:
            await asyncio.gather(
                self.initialize_geo_model(), self.initialize_soil_model()
            )
    ...
    async def initialize_soil_model(self):
        ...
        try:
            ...
            logger.info(
                f"Soil model loaded successfully with {param_count:,} parameters"
            )
            logger.info(
                f"Soil model device: {next(self.soil_model.parameters()).device}"
            )
    ...
    async def predict_geo_and_store(
        self, data: pd.DataFrame, task_id: str
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if (
                "timestamp" not in processed_df.columns
                or "value" not in processed_df.columns
            ):
                logger.error(
                    f"GEO BASEMODEL: Missing required columns. Found: {list(processed_df.columns)}"
                )
                return None
            ...
            if isinstance(prediction_timestamp, pd.Timestamp):
                ...
                if prediction_timestamp.tzinfo is None:
                    prediction_timestamp = prediction_timestamp.replace(
                        tzinfo=timezone.utc
                    )
            ...
            processed_data = await loop.run_in_executor(
                None, self._geoprep_process_miner_data_sync, processed_df
            )
            ...
            logger.info("GEO BASEMODEL: Running model prediction")

            if hasattr(self.geo_model, "run_inference"):
                logger.info(
                    "GEO BASEMODEL: Using custom geomagnetic model for inference (direct call)"
                )
                ...
                raw_prediction = await loop.run_in_executor(
                    None, self._geomodel_predict_sync, processed_data
                )

                if np.isnan(raw_prediction) or np.isinf(raw_prediction):
                    logger.warning(
                        "GEO BASEMODEL: Model returned NaN/Inf, using fallback value"
                    )
            ...
            logger.info(
                f"GEO BASEMODEL: Prediction result: {predictions['predicted_value']}"
            )

            # Store the prediction in the database
            if self.db_manager:
                if prediction_timestamp.tzinfo is not None:
                    prediction_timestamp_utc = prediction_timestamp.astimezone(
                        timezone.utc
                    )
                else:
                    prediction_timestamp_utc = prediction_timestamp.replace(
                        tzinfo=timezone.utc
                    )
                ...
                logger.info(
                    f"GEO BASEMODEL: Storage {'successful' if success else 'failed'}"
                )
    ...
    async def predict_soil_and_store(
        self, data: Dict[str, Any], task_id: str, region_id: Union[str, int]
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            logger.info(
                f"SOIL BASEMODEL: Processing data for task_id={task_id}, region={region_id_str}"
            )
                ...
                try:
                    ...
                    try:
                        target_time = datetime.fromtimestamp(
                            float(target_time), tz=timezone.utc
                        )
                    except:
                        logger.warning(
                            f"Could not parse target_time: {target_time}, using current time"
                        )
            ...
            for key, value in data.items():
                if isinstance(value, torch.Tensor):
                    ...
                    logger.info(
                        f"SOIL BASEMODEL: {key} min: {value.min().item():.4f}, max: {value.max().item():.4f}"
                    )

            loop = asyncio.get_event_loop()  # Get event loop
            processed_data = await loop.run_in_executor(
                None, self._soilmodel_prepare_inputs_sync, data
            )
            ...
            sentinel = (
                processed_data["sentinel_ndvi"][:2].unsqueeze(0).to(self.device)
            )  # Keep initial tensor ops on main thread if fast
            ...
            logger.info("SOIL BASEMODEL: Running soil model with inputs:")
            ...
            raw_output = await loop.run_in_executor(
                None, self._soilmodel_inference_sync, sentinel, era5, elev_ndvi
            )
            ...
            if (
                ...
                or "surface" not in predictions
                or "rootzone" not in predictions
            ):
                logger.error(
                    f"SOIL BASEMODEL: Invalid prediction output. Keys: {list(predictions.keys()) if predictions else 'None'}"
                )
                return None
            ...
            if self.db_manager:
                ...
                logger.info(
                    f"SOIL BASEMODEL: Storage {'successful' if success else 'failed'}"
                )
    ...
    async def get_geo_baseline_prediction(self, task_id: str) -> Optional[float]:
        ...
        try:
            if not task_id:
                logger.warning(
                    "No task_id provided, cannot retrieve geomagnetic baseline prediction"
                )
                return None

            logger.info(
                f"Retrieving geomagnetic baseline prediction for task_id: {task_id}"
            )
            result = await self.db_manager.get_baseline_prediction(
                task_name="geomagnetic", task_id=task_id
            )
            ...
            if result and "prediction" in result:
                ...
                if isinstance(prediction_value, (float, int)):
                    logger.info(
                        f"Retrieved geomagnetic baseline prediction for task_id {task_id}: {float(prediction_value)}"
                    )
                    return float(prediction_value)
                elif (
                    isinstance(prediction_value, dict)
                    and "value" in prediction_value
                    and isinstance(prediction_value["value"], (float, int))
                ):
                    logger.warning(
                        f"Retrieved geomagnetic baseline prediction for task_id {task_id} in dict format: {prediction_value['value']}. Consider re-saving in simpler format."
                    )
                    return float(prediction_value["value"])
                else:
                    logger.warning(
                        f"Geomagnetic baseline prediction for task_id {task_id} has unexpected payload type or structure: {type(prediction_value)}, value: {prediction_value}"
                    )
                    return None
            else:
                logger.warning(
                    f"No geomagnetic baseline prediction found or 'prediction' field missing for task_id {task_id}"
                )
                return None
        except Exception as e:
    ...
    async def get_soil_baseline_prediction(
        self, task_id: str, region_id: Union[str, int]
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            logger.warning(
                f"No soil moisture baseline prediction found for task_id {task_id}, region {region_id_str}"
            )
            return None
        except Exception as e:
    ...
    async def _generate_missing_baseline_prediction(
        self, task_id: str, region_id: str
    ) -> Optional[Dict]:
        """
        ...
        if not self.db_manager:
            logger.warning(
                "No database manager available, cannot generate baseline prediction"
            )
            return None
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Invalid region_id format: {region_id} (type: {type(region_id)})"
                )
                return None

            region_result = await self.db_manager.fetch_one(
                region_query, {"region_id": region_id_int}
            )
            ...
            logger.info(
                f"Generating baseline prediction for region {region_id}, target_time {target_time}"
            )
            logger.info(
                f"Combined data size: {len(combined_data) / (1024 * 1024):.2f} MB"
            )
            ...
            try:
                ...
                def _write_and_preprocess_sync(data_bytes):
                    ...
                    try:
                        ...
                        if (
                            t_file_path
                            and ("m_inputs" not in locals() or m_inputs is None)
                ...
                model_inputs, temp_file_path = await loop.run_in_executor(
                    None, _write_and_preprocess_sync, combined_data
                )
                ...
                if model_inputs:
                    ...
                    logger.info(
                        f"Preprocessed data for baseline generation. Keys: {list(model_inputs.keys())}"
                    )

                    # Generate the baseline prediction
                    baseline_prediction = await self.predict_soil_and_store(
                        data=model_inputs, task_id=task_id, region_id=region_id
                    )

                    if baseline_prediction:
                        logger.info(
                            f"Successfully generated and stored baseline prediction for region {region_id}"
                        )
                        return baseline_prediction
                    else:
                        logger.error(
                            f"Failed to generate baseline prediction for region {region_id}"
                        )
                        return None
                else:
                    logger.error(
                        f"Preprocessing failed for baseline generation, region {region_id}"
                    )
                    return None

            except Exception as e:
                logger.error(
                    f"Error preprocessing data for baseline generation: {str(e)}"
                )
                logger.error(traceback.format_exc())
                return None
            finally:
    ...
    async def score_geo_baseline(
        self, task_id: str, ground_truth: float
    ) -> Optional[float]:
        """
        ...
        try:
            if not task_id:
                logger.warning(
                    "No task_id provided, cannot score geomagnetic baseline prediction"
                )
                return None

            logger.info(
                f"Scoring geomagnetic baseline prediction for task_id: {task_id}"
            )

            baseline_prediction = await self.get_geo_baseline_prediction(task_id)
            if baseline_prediction is None:
                logger.warning(
                    f"No geomagnetic baseline prediction found for task_id {task_id}"
                )
                return None

            score = self.geo_scoring.calculate_score(baseline_prediction, ground_truth)
            logger.info(
                f"Geomagnetic baseline score for task_id {task_id}: {score:.4f}"
            )
    ...
    async def score_soil_baseline(
        ...
        try:
            ...
            baseline_prediction = await self.get_soil_baseline_prediction(
                task_id, region_id_str
            )
            if baseline_prediction is None:
                logger.warning(
                    f"No soil baseline prediction found for task_id {task_id}, region {region_id_str}"
                )

                # Try to generate baseline prediction from original region data
                logger.info(
                    f"Attempting to generate missing baseline prediction for task_id {task_id}, region {region_id_str}"
                )
                baseline_prediction = await self._generate_missing_baseline_prediction(
                    task_id, region_id_str
                )

                if baseline_prediction is None:
                    logger.error(
                        f"Failed to generate baseline prediction for task_id {task_id}, region {region_id_str}"
                    )
                    return None
                else:
                    logger.info(
                        f"Successfully generated missing baseline prediction for task_id {task_id}, region {region_id_str}"
                    )

            if (
                "surface_sm" in baseline_prediction
                and "rootzone_sm" in baseline_prediction
            ):
                ...
                logger.info(
                    f"Converted baseline prediction to tensor with shape: {tensor_predictions.shape}"
                )
            else:
                logger.error(
                    f"Baseline prediction missing required fields: {list(baseline_prediction.keys())}"
                )
                return None
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Could not parse target_time string: {target_time}"
                            )
                            return None
            ...
            if metrics is None:
                logger.error(
                    f"Failed to compute metrics for baseline prediction, task_id {task_id}, region {region_id_str}"
                )
                return None
            ...
            logger.info(
                f"Soil baseline score for task_id {task_id}, region {region_id_str}: {score_result:.4f}"
            )
            return score_result
    ...
    async def compare_geo_scores(
        self, task_id: str, ground_truth: float, miner_scores: List[Dict]
    ) -> List[Dict]:
        """
        ...
        if baseline_score is None:
            logger.warning(
                f"No baseline score available for comparison, task {task_id}"
            )
            return miner_scores
    ...
    async def compare_soil_scores(
        ...
        """
        region_id_str = str(region_id)
        baseline_score = await self.score_soil_baseline(
            task_id, region_id_str, ground_truth
        )

        if baseline_score is None:
            logger.warning(
                f"No baseline score available for comparison, task {task_id}, region {region_id_str}"
            )
            return miner_score
