...
class PgBackRestIntegration:
    ...
    def _check_pgbackrest_available(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["which", "pgbackrest"], capture_output=True, text=True
            )
            ...
            if not config_file_exists and not env_config_complete:
                logger.debug(
                    "pgBackRest config file and environment variables both incomplete"
                )
                return False
    ...
    async def should_disable_builtin_sync(self) -> bool:
        ...
        return self.is_available and (
            await self.is_primary_node() or await self.is_replica_node()
        )
    ...
    async def perform_manual_backup(self, backup_type: str = "diff") -> bool:
        ...
        if not self.is_available or not await self.is_primary_node():
            logger.warning(
                "Manual backup not available (not a primary node or pgBackRest not configured)"
            )
            return False
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        ...
        if not self.is_available:
            status["recommendations"].append(
                "pgBackRest not configured - using built-in sync"
            )
            return status
        ...
        if status["node_type"] == "replica":
            ...
            if lag is not None and lag > 300:  # 5 minutes
                status["recommendations"].append(
                    f"High replication lag: {lag:.1f} seconds"
                )
        ...
        if status["node_type"] == "primary" and not status["backup_status"].get(
            "wal_archiving", False
        ):
...
async def initialize_pgbackrest_integration() -> None:
    ...
    if pgbackrest_integration.is_available:
        ...
        logger.info(
            "pgBackRest not available - using built-in database synchronization"
        )
