...
class ProcessIsolatedSubstrate:
    ...
    def __init__(self, subtensor_network: str, chain_endpoint: str = None):
        ...
        logger.debug(
            f"ProcessIsolatedSubstrate created for {subtensor_network}@{chain_endpoint}"
        )
    ...
    def _run_substrate_operation(
        self, operation_type: str, *args, timeout: float = 15.0
    ) -> Any:
        """
        ...
        logger.debug(
            f"Process-isolated {op_desc} #{self._operation_count} (timeout: {timeout}s) called from {caller_info}"
        )
...
"""
            ...
            if result.returncode == 0:
                ...
                if isinstance(data, dict) and "error" in data:
                    raise Exception(
                        f"Substrate {operation_type} failed: {data['error']}"
                    )
                ...
                if isinstance(data, dict) and "wrapped" in data and data["wrapped"]:
                    ...
                    class SubstrateResult:
                        ...
                        def __len__(self):
                            return (
                                len(self.value) if hasattr(self.value, "__len__") else 1
                            )
                    ...
                    logger.debug(
                        f"Process-isolated {operation_desc} #{self._operation_count} completed in {execution_time:.2f}s → wrapped result"
                    )
                    return result_obj
                else:
                    logger.debug(
                        f"Process-isolated {operation_desc} #{self._operation_count} completed in {execution_time:.2f}s → raw result"
                    )
                    return data
            else:
                ...
                raise Exception(
                    f"Subprocess failed (code {result.returncode}){stderr_info}"
                )

        except subprocess.TimeoutExpired:
            execution_time = time.time() - start_time if "start_time" in locals() else 0
            logger.error(
                f"Process-isolated {operation_desc} #{self._operation_count} TIMED OUT after {execution_time:.2f}s"
            )
            logger.error(
                f"Network appears to be unresponsive. Consider checking substrate network status."
            )
            raise Exception(f"Substrate {operation_type} timeout after {timeout}s")
        except Exception as e:
            execution_time = time.time() - start_time if "start_time" in locals() else 0
            logger.error(
                f"Process-isolated {operation_desc} #{self._operation_count} failed after {execution_time:.2f}s: {e}"
            )
            raise
    ...
    def query(
        ...
        return self._run_substrate_operation(
            "query", module, storage_function, params or [], block_hash, timeout=timeout
        )

    def rpc_request(
        self, method: str, params: List = None, timeout: float = 90.0
    ) -> Any:
        """Make an RPC request in an isolated process."""
        return self._run_substrate_operation(
            "rpc_request", method, params or [], timeout=timeout
        )
        ...
        try:
            ...
            if isinstance(result, dict):
                if "block" in result:
                    ...
                    logger.debug(
                        f"Unexpected get_block response structure: {list(result.keys())}"
                    )
                    return result
            else:
    ...
    def close(self):
        ...
        logger.debug(
            f"ProcessIsolatedSubstrate close() called (no-op - {self._operation_count} operations completed)"
        )
...
class SubstrateManager:
    ...
    def __init__(
        ...
        logger.info(
            f"SubstrateManager initialized for {subtensor_network}@{chain_endpoint} ({isolation_mode})"
        )
    ...
    def get_fresh_connection(self) -> SubstrateInterface:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug(
                    f"Creating fresh substrate connection #{self._connection_count}"
                )
                ...
                if self.use_process_isolation:
                    ...
                    logger.debug(
                        f"✅ Fresh regular substrate connection #{self._connection_count} created"
                    )
    ...
    def force_cleanup(self):
        ...
        with self._lock:
            if self._current_connection is not None:
                ...
                logger.debug(
                    "🧹 Force cleanup of scalecodec caches (no active connection)"
                )
                self._clear_scalecodec_caches()
                gc.collect()
...
def get_substrate_manager(
    subtensor_network: str, chain_endpoint: str, use_process_isolation: bool = False
) -> SubstrateManager:
    """
    ...
    with _global_lock:
        ...
        needs_new_manager = (
            ...
            or _global_manager.subtensor_network != subtensor_network
            or _global_manager.use_process_isolation != use_process_isolation
        )
        ...
        if needs_new_manager:
            ...
            if _global_manager is not None:
                old_isolation = (
                    "process-isolated"
                    if _global_manager.use_process_isolation
                    else "regular"
                )
                new_isolation = (
                    "process-isolated" if use_process_isolation else "regular"
                )
                logger.info(
                    f"Manager change: {_global_manager.subtensor_network}({old_isolation}) → {subtensor_network}({new_isolation}), recreating manager"
                )
                _global_manager.shutdown()

            # Create new manager - normalize endpoint
            normalized_endpoint = chain_endpoint.strip() if chain_endpoint else ""
            _global_manager = SubstrateManager(
                subtensor_network, normalized_endpoint, use_process_isolation
            )
            isolation_mode = "process-isolated" if use_process_isolation else "regular"
            logger.info(
                f"🔄 Created new global SubstrateManager for {subtensor_network} ({isolation_mode})"
            )
        else:
            ...
            if _global_manager.chain_endpoint != normalized_endpoint:
                logger.debug(
                    f"Updating endpoint: {_global_manager.chain_endpoint} → {normalized_endpoint}"
                )
...
def get_fresh_substrate_connection(
    subtensor_network: str, chain_endpoint: str, use_process_isolation: bool = False
) -> SubstrateInterface:
    """
    ...
    manager = get_substrate_manager(
        subtensor_network, chain_endpoint, use_process_isolation
    )
    return manager.get_fresh_connection()


def get_process_isolated_substrate(
    subtensor_network: str, chain_endpoint: str
) -> SubstrateInterface:
    """
    ...
    return get_fresh_substrate_connection(
        subtensor_network, chain_endpoint, use_process_isolation=True
    )
...
def get_substrate_manager_stats(
    subtensor_network: str = None, chain_endpoint: str = None
) -> dict:
    """
