...
async def get_tables_to_wipe(
    trigger_path: str = DEFAULT_TRIGGER_PATH,
) -> Optional[List[str]]:
    """
...
async def wipe_database(db_manager, tables: Optional[List[str]] = None) -> bool:
    ...
    try:
        if not tables:
            ...
            tables = [
                row["table_name"] for row in result if row["table_name"] != "node_table"
            ]

            logger.warning("\n" + "#" * 70)
            logger.warning(
                "#" * 20 + " WIPING ALL TABLES - FULL DATABASE RESET " + "#" * 20
            )
            logger.warning("#" * 70)
            ...
            logger.warning("\n" + "#" * 70)
            logger.warning(
                "#" * 20 + f" WIPING SPECIFIC TABLES: {', '.join(tables)} " + "#" * 20
            )
            logger.warning("#" * 70)
            ...
            for table in category_tables:
                if table in tables:
                    try:
                        ...
                        if final_count == 0:
                            logger.warning(
                                f"## SUCCESSFULLY WIPED table: {table} (removed {initial_count} rows) ##"
                            )
                            success_count += 1
                        else:
                            logger.error(
                                f"## FAILED to completely wipe table: {table} (remaining rows: {final_count}) ##"
                            )
                            error_count += 1
        ...
        logger.warning(
            "#" * 20
            + f" DATABASE WIPE SUMMARY: {success_count} tables wiped, {error_count} errors "
            + "#" * 20
        )
        logger.warning("#" * 70)
