...
class Miner:
    ...
    Target Netuid: {self.netuid}
            ...
            try:
                ...
                self.logger.info("=" * 80)
                self.logger.info(
                    f"🔑 Hotkey (self.keypair.ss58_address): '{self.keypair.ss58_address}'"
                )
                self.logger.info(
                    f"📏 Hotkey length: {len(self.keypair.ss58_address)} characters"
                )
                self.logger.info(f"🌐 NetUID (self.netuid): {self.netuid}")
                self.logger.info(
                    f"🔗 Chain endpoint (self.config.chain_endpoint): '{endpoint_to_use}'"
                )
                self.logger.info(f"🧪 Hotkey format check:")
                self.logger.info(
                    f"   - Starts with: '{self.keypair.ss58_address[:10]}...'"
                )
                self.logger.info(
                    f"   - Ends with: '...{self.keypair.ss58_address[-10:]}'"
                )
                self.logger.info(
                    f"   - Has whitespace? {bool(self.keypair.ss58_address.strip() != self.keypair.ss58_address)}"
                )
                self.logger.info("=" * 80)

                substrate_for_check = SubstrateInterface(url=endpoint_to_use)
                self.logger.info(
                    "✅ MINER_SELF_CHECK: Substrate connection established successfully"
                )
                ...
                self.logger.info(
                    f"📋 MINER_SELF_CHECK: Retrieved {len(all_nodes)} nodes from metagraph"
                )

                # DEBUG: Show sample of nodes for comparison
                if all_nodes:
                    self.logger.info(
                        "🔍 MINER_SELF_CHECK: Sample nodes from metagraph (first 3):"
                    )
                    for i, node in enumerate(all_nodes[:3]):
                        self.logger.info(
                            f"   [{i}] Hotkey: '{node.hotkey}' (len: {len(node.hotkey)})"
                        )
                        self.logger.info(
                            f"       IP: {node.ip}, Port: {node.port}, Protocol: {node.protocol}"
                        )
                        # Character-by-character comparison with our hotkey
                        if len(node.hotkey) == len(self.keypair.ss58_address):
                            diff_count = sum(
                                1
                                for a, b in zip(node.hotkey, self.keypair.ss58_address)
                                if a != b
                            )
                            self.logger.info(
                                f"       Character differences with our hotkey: {diff_count}"
                            )
                    ...
                    self.logger.error(
                        "❌ MINER_SELF_CHECK: No nodes found in metagraph!"
                    )
                    self.logger.warning(
                ...
                found_own_node = None
                self.logger.info(
                    f"🔍 MINER_SELF_CHECK: Searching for exact hotkey match among {len(all_nodes)} nodes..."
                )
                exact_matches = 0
                partial_matches = []
                ...
                for node_info_from_list in all_nodes:
                    if node_info_from_list.hotkey == self.keypair.ss58_address:
                        ...
                        exact_matches += 1
                        self.logger.info(
                            f"✅ MINER_SELF_CHECK: EXACT MATCH FOUND! Node hotkey: '{node_info_from_list.hotkey}'"
                        )
                        break
                    # Check for partial matches to help debug
                    elif (
                        node_info_from_list.hotkey.strip()
                        == self.keypair.ss58_address.strip()
                    ):
                        partial_matches.append(
                            ("whitespace_difference", node_info_from_list.hotkey)
                        )
                    elif len(node_info_from_list.hotkey) == len(
                        self.keypair.ss58_address
                    ):
                        diff_count = sum(
                            1
                            for a, b in zip(
                                node_info_from_list.hotkey, self.keypair.ss58_address
                            )
                            if a != b
                        )
                        if diff_count <= 3:  # Very close match
                            partial_matches.append(
                                (
                                    f"{diff_count}_char_difference",
                                    node_info_from_list.hotkey,
                                )
                            )

                self.logger.info(
                    f"🔍 MINER_SELF_CHECK: Search complete - Exact matches: {exact_matches}"
                )

                if not found_own_node and partial_matches:
                    self.logger.warning(
                        f"⚠️ MINER_SELF_CHECK: No exact match found, but found {len(partial_matches)} partial matches:"
                    )
                    for match_type, hotkey in partial_matches[
                        :5
                    ]:  # Show first 5 partial matches
                        self.logger.warning(f"   - {match_type}: '{hotkey}'")
                elif not found_own_node:
                    self.logger.error(
                        f"❌ MINER_SELF_CHECK: No exact or partial matches found among {len(all_nodes)} nodes"
                    )
                    # Show first few hotkeys for debugging
                    self.logger.info(
                        "🔍 MINER_SELF_CHECK: First 5 hotkeys in metagraph for comparison:"
                    )
                        ...
                        if weather_inference_type == "http_service":
                            if not service_url_env:
                                ...
                                if _is_local_url(service_url_env):
                                    ...
                                    if service_ready:
                                        self.logger.info(
                                            f"Local inference service is ready."
                                        )
                                    else:
                                        self.logger.error(
        ...
        try:
                ...
                for fs_type in ["file", "memory", "http", "https", "s3", "gcs"]:
                    try:
                        if hasattr(fsspec, "registry") and fs_type in fsspec.registry:
                            ...
                            if hasattr(fs_cls, "_cache") and hasattr(
                                fs_cls._cache, "clear"
                            ):
                                fs_cls._cache.clear()
                                cleanup_stats["fsspec_caches"] += 1
            ...
            try:
                ...
                try:
                    ...
                    if hasattr(requests, "sessions") and hasattr(
    ...
    def _configure_cache_limits(self):
            ...
            if hasattr(xr.backends, "file_manager"):
                ...
                if hasattr(xr.backends.file_manager, "FILE_CACHE"):
                    ...
                    if hasattr(original_cache, "maxsize"):
                        ...
                        self.logger.info(
                            f"Set xarray FILE_CACHE maxsize to {cache_maxsize}"
                        )
        ...
        try:
            ...
            fsspec_cache_dir = os.getenv(
                "MINER_FSSPEC_CACHE_DIR", "/tmp/miner_fsspec_cache"
            )
            ...
            if hasattr(fsspec, "config"):
                ...
                self.logger.info(
                    f"Configured fsspec cache: dir={fsspec_cache_dir}, size={fsspec_cache_size}"
                )
        ...
        try:
            ...
            try:
                ...
                def miner_memory_cleanup():
                    ...
                    self.logger.debug(
                        f"Miner memory cleanup: cleared {cleanup_stats['total_caches']} caches, "
                        f"freed {cleanup_stats['gc_collected']} objects"
                    )

                cleanup_helper.register_custom_cleanup(
                    "miner_memory_monitor", miner_memory_cleanup
                )
                self.logger.debug(
                    "Registered miner memory monitor thread for global cleanup"
                )
            except Exception as e:
                self.logger.debug(
                    f"Failed to register miner memory monitor cleanup: {e}"
                )
            ...
            while True:
                try:
                    ...
                    if memory_mb > critical_threshold_mb:
                        ...
                        self.logger.error(
                            "Attempting comprehensive emergency cleanup to prevent OOM kill..."
                        )
                        try:
                            cleanup_stats = self._comprehensive_miner_memory_cleanup()
                            self.logger.error(
                                f"Emergency cleanup: cleared {cleanup_stats['total_caches']} caches, "
                                f"freed {cleanup_stats['gc_collected']} objects, "
                        ...
                        self.logger.warning(
                            "Performing emergency cache cleanup to prevent OOM"
                        )
                        # Comprehensive cleanup at emergency level
                        try:
                            cleanup_stats = self._comprehensive_miner_memory_cleanup()
                            if (
                                cleanup_stats["total_caches"] > 0
                                or cleanup_stats["gc_collected"] > 0
                            ):
                                self.logger.info(
                                    f"Emergency cleanup: cleared {cleanup_stats['total_caches']} caches, "
                                    f"freed {cleanup_stats['gc_collected']} objects "
                            ...
                            try:
                                ...
                                if collected > 0:
                                    self.logger.info(
                                        f"Fallback GC collected {collected} objects"
                                    )
                            except Exception:
                                pass
                    ...
                    if current_time - last_log_time >= 300:  # 5 minutes
                        ...
                        if memory_mb > proactive_threshold:
                            self.logger.debug(
                                f"Performing proactive cache cleanup (memory: {memory_mb:.1f}MB > {proactive_threshold:.1f}MB)"
                            )
                            try:
                                cleanup_stats = (
                                    self._comprehensive_miner_memory_cleanup()
                                )
                                if cleanup_stats["total_caches"] > 0:
                                    self.logger.debug(
                                        f"Proactive cleanup: cleared {cleanup_stats['total_caches']} caches, "
                                        f"freed {cleanup_stats['gc_collected']} objects"
                                    )
                            except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if alembic_auto_upgrade:
            print(
                f"[Startup] ALEMBIC_AUTO_UPGRADE is True. Attempting to upgrade database schema to head..."
            )
