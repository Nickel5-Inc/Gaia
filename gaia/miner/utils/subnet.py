...
def factory_router(miner_instance) -> APIRouter:
        ...
        try:
            if decrypted_payload.data:
                ...
                if not hasattr(miner_instance, "geomagnetic_task"):
                    logger.error(
                        "Miner instance is missing the 'geomagnetic_task' attribute."
                    )
                    return JSONResponse(
                        status_code=500,
                        content={"error": "Miner not configured for geomagnetic task"},
                    )

                logger.info("Miner executing geomagnetic prediction ...")
                result = miner_instance.geomagnetic_task.miner_execute(
                    response_data, miner_instance
                )
                    ...
                    if "predicted_values" in result:
                        ...
                        try:
                            ...
                            if np.isnan(pred_value).any() or np.isinf(pred_value).any():
                                logger.warning(
                                    "Invalid prediction value received, setting to 0.0"
                                )
                                result["predicted_values"] = 0.0
                            else:
                                result["predicted_values"] = float(pred_value)
                        except (ValueError, TypeError):
                            logger.warning(
                                "Could not convert prediction to float, setting to 0.0"
                            )
                            result["predicted_values"] = 0.0
                    else:
                        logger.error(
                            "Missing 'predicted_values' in result, setting to default 0.0"
                        )
                        result["predicted_values"] = 0.0

                    if "timestamp" not in result:
                        logger.warning(
                            "Missing timestamp in result, using fallback timestamp"
                        )
    ...
    async def soilmoisture_require(
        ...
        try:
            if not hasattr(miner_instance, "soil_task"):
                ...
                return JSONResponse(
                    status_code=500,
                    content={"error": "Miner not configured for soil moisture task"},
                )
    ...
    async def weather_kerchunk_require(
        ...
        """
        logger.info("Received decrypted weather forecast data request")
        try:
            if (
            ...
            logger.info("Handling weather forecast data request...")
