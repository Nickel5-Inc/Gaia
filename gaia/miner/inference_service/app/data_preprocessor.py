...
def save_dataset_step_to_gzipped_netcdf(
    ...
    try:
        ...
        logger.error(f"[SaveStep {step_index}] Error saving dataset step to {output_file_path}: {e}", exc_info=True)
        # Clean up partially written file if error occurred
        if output_file_path.exists():
            try:
                os.remove(output_file_path)
            except OSError as e_remove:
                logger.error(f"[SaveStep {step_index}] Could not remove partial file {output_file_path} after error: {e_remove}")
        return None 