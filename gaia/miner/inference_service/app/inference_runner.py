...
async def initialize_inference_runner(app_config: Dict[str, Any]):
    ...
    if INFERENCE_RUNNER is None:
        ...
        try:
            ...
            logger.info(
                f"[INIT_RUNNER_DEBUG] Global INFERENCE_RUNNER assigned. Checking model state..."
            )
            print(
                f"[INIT_RUNNER_DEBUG_PRINT] Global INFERENCE_RUNNER assigned. Checking model state...",
                flush=True,
            )
...
async def run_model_inference(
    prepared_batch: BatchType, config: Dict[str, Any]  # Main application config
) -> Optional[List[BatchType]]:
    """
    ...
    try:
        ...
        if not predictions:  # Empty list of predictions
            ...
            return (
                []
            )  # Return empty list rather than None if inference ran but produced nothing
