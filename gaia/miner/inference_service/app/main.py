...
try:
    ...
    if os.getenv("LOG_LEVEL", "DEBUG").upper() in ["DEBUG", "INFO"]:
        _logger.info(
            "🚀 Inference service using orjson for high-performance JSON responses"
        )
    ...
    if os.getenv("LOG_LEVEL", "DEBUG").upper() in ["DEBUG", "INFO"]:
        _logger.info(
            "⚡ Inference service using standard JSON - install orjson for 2-3x performance boost"
        )
...
async def _upload_bytes_to_r2(object_key: str, data: bytes) -> bool:
    ...
    bucket_name = R2_CONFIG["bucket_name"]
    _logger.info(
        f"Attempting to upload {len(data)} bytes to s3://{bucket_name}/{object_key}"
    )
    ...
    async with R2_UPLOAD_SEMAPHORE:
        try:
            ...
            _logger.error(
                f"ClientError during R2 bytes upload to s3://{bucket_name}/{object_key}: {e_ce.response.get('Error', {}).get('Message', str(e_ce))}",
                exc_info=True,
            )
        except Exception as e:
            _logger.error(
                f"Unexpected error during R2 bytes upload to s3://{bucket_name}/{object_key}: {e}",
                exc_info=True,
            )
        return False
    ...
    for i, res in enumerate(results):
        if res is True:
            ...
            if successful_uploads % 10 == 0 or successful_uploads == len(results):
                _logger.info(
                    f"[{job_run_uuid}] Upload progress: {successful_uploads}/{len(upload_tasks)} completed"
                )
        else:
            failed_uploads += 1
            _logger.error(
...
async def _perform_inference_and_upload(
    ...
    try:
        ...
        if not (_AURORA_AVAILABLE and isinstance(initial_batch, Batch)) and not (
            not _AURORA_AVAILABLE and initial_batch is not None
        ):  # type: ignore
            raise ValueError(
    ...
    try:
        ...
        if prediction_steps_batch_list is None:
            ...
            return {
                "output_r2_prefix": f"outputs/{job_run_uuid}",
                "base_time": base_time_for_coords.isoformat()
    ...
    output_manifest = {
        ...
        "job_run_uuid": job_run_uuid,
        "num_steps_uploaded": num_steps,
        "info": f"Upload process finished. See error details for any non-critical issues. Details: {error_msg}"
        if error_msg
...
async def combined_runpod_handler(job: Dict[str, Any]):
    ...
    if not INITIALIZED:
        ...
        try:
            await (
                initialize_app_for_runpod()
            )  # initialize_app_for_runpod is already async
            INITIALIZED = True
            _logger.info(
