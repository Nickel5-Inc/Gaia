...
class MinerDatabaseManager(BaseDatabaseManager):
    ...
    async def initialize_database(self, session):
        ...
        await session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_task_queue_status ON task_queue(status)")
        )
        await session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_task_queue_query_time ON task_queue(query_time)")
        )
    ...
    async def _create_task_table(self, schema: Dict[str, Any]):
        ...
        """
        columns = [f"{name} {definition}" for name, definition in schema["columns"].items()]
        query = f"""
