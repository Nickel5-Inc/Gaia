        ...
        async def wrapper(self: "MinerDatabaseManager", *args, **kwargs):
            try:
                ...
                if duration > self.MINER_QUERY_TIMEOUT / 2:
                    ...
                    logger.warning(
                        f"Long-running {operation_type} operation detected: {duration:.2f}s"
                    )
...
class MinerDatabaseManager(BaseDatabaseManager):
    ...
    def __init__(
        ...
        if not hasattr(self, "_initialized"):
            ...
            connection_type = os.getenv(
                "MINER_DB_CONNECTION_TYPE", os.getenv("DB_CONNECTION_TYPE", "tcp")
            ).lower()
            ...
            if connection_type == "socket":
                ...
                db_host_resolved = os.getenv(
                    "MINER_DB_SOCKET_PATH",
                    os.getenv("MINER_DB_HOST", "/var/run/postgresql"),
                )  # Default socket path
                db_port_resolved = None  # Port is not used for socket connections with SQLAlchemy like this
                logger.info(
                    "MinerDatabaseManager configured for UNIX socket connection."
                )
                logger.info(
                    f"MinerDatabaseManager connecting with: socket_path='{db_host_resolved}', db='{db_name_resolved}', user='{db_user_resolved}'"
                )
            else:  # Default to TCP
                db_host_resolved = os.getenv("MINER_DB_HOST", host)
                db_port_resolved = int(os.getenv("MINER_DB_PORT", str(port)))
                logger.info("MinerDatabaseManager configured for TCP/IP connection.")
                logger.info(
                    f"MinerDatabaseManager connecting with: host='{db_host_resolved}', port={db_port_resolved}, db='{db_name_resolved}', user='{db_user_resolved}'"
                )

            if db_password_resolved and db_password_resolved != password:
                logger.info(
                    "MinerDatabaseManager using a custom password from environment."
                )
            elif not db_password_resolved and password:
                logger.info(
                    "MinerDatabaseManager using default password as MINER_DB_PASSWORD env var is not set."
                )
    ...
    async def initialize_database(self):
        ...
        try:
            ...
            logger.info(
                "Successfully connected to miner database. Schema should be managed by Alembic."
            )
        except Exception as e:
