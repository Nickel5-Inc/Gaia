...
class TestWeightPerturbation:
    ...
    def test_perturbation_preserves_normalization(self):
        ...
        # Check normalization
        assert abs(perturbed.sum() - 1.0) < 1e-9, f"Perturbed weights not normalized: sum={perturbed.sum()}"
    ...
    def test_perturbation_preserves_top_k(self):
        ...
        assert overlap >= k * 0.8, f"Top-{k} not sufficiently preserved: {overlap}/{k} overlap"
    ...
    def test_perturbation_l1_distance(self):
        ...
        assert abs(l1_distance - expected_l1) < 0.01, f"L1 distance {l1_distance} != expected {expected_l1}"
    ...
    def test_deterministic_perturbation(self):
        ...
        # Should be identical
        assert np.allclose(perturbed1, perturbed2), "Same seed did not produce identical perturbation"
    ...
    def test_different_seeds_produce_different_results(self):
        ...
        # Should be different
        assert not np.allclose(perturbed1, perturbed2), "Different seeds produced identical perturbation"
    ...
    def test_tail_boost(self):
        ...
        for idx in tail_indices[:10]:  # Check first 10 tail miners
            assert perturbed[idx] > base_weights[idx], f"Tail miner {idx} did not receive boost"
    ...
    def test_vtrust_estimation(self):
        ...
        assert abs(estimated_vtrust - expected_vtrust) < 0.001, \
            f"V-trust estimation incorrect: {estimated_vtrust} != {expected_vtrust}"
        
        # Check that with default settings, v-trust is below 0.60
        assert estimated_vtrust <= 0.60, f"V-trust {estimated_vtrust} not below target 0.60"
    ...
    def test_disabled_perturbation(self):
        ...
        with patch.dict(os.environ, {"PERTURB_P": "0"}):
            ...
            manager = gaia.validator.weight_perturbation.WeightPerturbationManager(self.mock_db_manager)
