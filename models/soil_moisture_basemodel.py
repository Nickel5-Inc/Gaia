...
class PhysicalConsistencyLoss(nn.Module):
    ...
    def forward(self, predictions, era5_features, era5_raw, prev_predictions=None):
        ...
        physical_loss = bound_loss + 0.1 * precip_loss + 0.1 * temp_loss + 0.1 * evap_loss
        
        if prev_predictions is not None:
            temporal_loss = F.relu(torch.abs(predictions - prev_predictions) - self.max_hourly_change).mean()
...
def meteorological_consistency_loss(predictions, era5, indices, downsample):
    ...
    for scale in scales:
        ...
        for key, idx in indices.items():
            if key in ['tp', 'snowfall', 'snowmelt', 'surface_runoff', 'subsurface_runoff']:
        ...
        scale_loss = 0.0
        scale_loss += F.relu(pred_at_scale * F.relu(era5_at_scale['t2m'] - 298.15)).mean()
        scale_loss += F.relu(pred_at_scale * F.relu(era5_at_scale['soil_temp_1'] - 298.15)).mean()
        scale_loss += F.relu(-(pred_at_scale * F.relu(era5_at_scale['tp']))).mean()
        scale_loss += F.relu(-(pred_at_scale * F.relu(era5_at_scale['snowmelt']))).mean()
        scale_loss += F.relu(pred_at_scale * F.relu(era5_at_scale['total_evap'])).mean()
        scale_loss += F.relu(pred_at_scale * F.relu(era5_at_scale['bare_soil_evap'])).mean()
...
class MultiTemporalFusion(nn.Module):
    def __init__(self, era5_channels=20, sentinel_channels=2, static_channels=2):
        ...
        self.scale_attention = nn.Sequential(
            nn.Conv2d(era5_channels + sentinel_channels + static_channels, 32, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(32, 3, 1),
            nn.Sigmoid()
        )
...
class CrossResolutionAttention(nn.Module):
    ...
    def forward(self, smap_features, hr_features, mid_features, lr_features):
        smap_up = F.interpolate(smap_features, size=hr_features.shape[-2:], mode='bilinear')
        mid_up = F.interpolate(mid_features, size=hr_features.shape[-2:], mode='bilinear')
...
class ResolutionAwareProcessing(nn.Module):
    ...
    def ensure_smap_structure(self, x):
        ...
        if x.shape[-1] != self.smap_size:
            x = F.interpolate(
                x,
                size=(self.smap_size, self.smap_size),
                mode='area'
            )
        return x
...
class ParallelFeatureProcessing(nn.Module):
    ...
    """
    def __init__(self, num_era5_features=20, num_sentinel_features=3, num_static_features=2, smap_size=11):
        super().__init__()
        ...
        self.scale_processors = nn.ModuleDict({
            f'scale_{scale}': ResidualBlock(64)
            for scale in self.intermediate_scales
        })
    ...
    def forward(self, sentinel, era5, elev_ndvi):
        ...
        sentinel_scales = {
            ...
            'mid_res': F.interpolate(
                sentinel_hr,
                size=(self.resolution_handler.input_size // 2, self.resolution_handler.input_size // 2),
                mode='bilinear',
                align_corners=False
            ),
...
class SoilMoistureModel(pl.LightningModule):
    ...
    def forward(self, sentinel, era5, elev_ndvi):      
        ...
        predictions = {}
        for scale in [220, 110, 55, 22, 11]:
            scale_features = F.interpolate(features['fused'], size=(scale, scale), 
    ...
    def training_step(self, batch, batch_idx):  
        ...
        era5 = batch['era5']
        elev_ndvi = torch.cat([batch['elevation'], batch['sentinel_ndvi'][:, 2:3]], dim=1)
        ...
        physical_loss = self.physical_loss(
            outputs['predictions'],
            outputs['features']['era5_features'],
            era5
        )
        ...
        total_loss = mse_loss + 0.1 * physical_loss + 0.1 * spatial_loss + 0.1 * meteo_loss + 0.1 * uncertainty_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for scale in [220, 110, 55, 22, 11]:
            ...
            mse_loss += scale_weight * F.mse_loss(
                outputs['multi_scale_predictions'][str(scale)], 
                target_at_scale
            )
        ...
        self.log_dict({
            'val_loss': mse_loss + 0.1 * physical_loss + 0.1 * spatial_loss + 0.1 * meteo_loss,
            'val_mse': mse_loss,
    ...
    def calculate_r2(self, pred, target):
        ...
        if pred.shape[1] != 2:
            raise ValueError("Predictions should have 2 channels (surface and root zone)")
